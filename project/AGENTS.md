...
- 이 `AGENTS.md` (한국어) — 본 저장소 코딩 에이전트용 진입점. 충돌 시 본 문서가 최신.

## 위치 규약

- 프로토타입 종속 메모/스케치는 `project/` 안에.
- 저장소 전반의 아키텍처/엔진 전략 문서는 `docs/` 로 (참조: `docs/AGENTS.md`).
- 사용자 의도 트랜스크립트는 `chats/` 로 (참조: `chats/AGENTS.md`).
- 실제 React/Electron 구현체는 `app/` 로 (참조: `app/AGENTS.md`). v5 디자인 토큰/19 화면을 `app/` 에 반영하는 작업은 별도 페이즈.
