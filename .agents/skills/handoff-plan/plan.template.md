...
- cleanup/rollback: …
- **다중 저장소 쓰기**(원자적으로 함께 쓸 수 없는 두 곳 이상에 쓰는 동작): 쓰기 지점 나열 → 각 지점 실패/크래시 시 관측 상태 → 허용 불가 조합과 그것을 없애는 설계. 해당 없으면 "해당 없음".
...
> 구현 세부·명백한 엣지 누락은 선조치 가능. 제품 의도·신규 의존성·ACTIVE Decision·AC 변경은 단독 변경 금지.
