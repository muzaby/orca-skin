...
> **고친 프로덕션 hunk를 하나씩 되돌려 스위트를 돌리고 무엇이 실패했는지 적는다**(impl §3).
> 아무것도 실패하지 않으면 그 수정은 잠기지 않은 것이다 — 배선을 지운 회귀가 게이트를 초록으로
> 통과한다. 잠글 수 없는 지점은 `잠금 없음`과 이유를 적되, **검증자가 같은 되돌림을 다시 돌리므로**
> (verify §4) 재현되지 않는 사유는 파생 이슈가 된다 — 이 칸은 측정 불가 항목의 하치장이 아니다.

| 수정 지점 | 되돌린 방법 | 실패한 테스트 / 케이스 수 | 결과 |
|---|---|---|---|
| `파일:줄` | 해당 hunk 제거 | `<케이스명>` 외 N건 | 잠김 / **잠금 없음 — 사유** |
