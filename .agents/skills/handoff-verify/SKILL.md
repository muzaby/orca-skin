...
- 설계자·구현자가 자신이어도 처음 보는 남의 PR처럼 읽는다. **구현자가 자신이면 그것만으로 부족하다** — §4의 자기검증 분모 규칙을 함께 수행한다.
...
- 각 pair에 production path·§10 강제 지점 전수·직접 oracle이 있는가. 구조적 proxy·0건/전수·배선 존재처럼 적대 증거가 필요한 pair는 선택 이유와 변이가 있고, 나머지는 직접 oracle로 충분한 이유가 있는가. 분모가 path의 계약 운반 edge를 자리로 담지 않았어도 path가 그 edge를 명시하면 구현자가 닫을 수 있으므로 `PLAN_GAP`이 아니라 그 pair의 분모로 재측정한다.
- `SUPERSEDED`로 분해된 pair의 AC·선택 적대 증거가 대체 pair로 이관됐는가. 이관도 폐기 근거도 없으면 `PLAN_GAP`이다. 이 요구 전에 쓴 revision은 이관 매핑을 읽기 전용으로 합성하고, 사라진 증거가 현재 판정에 필요할 때만 gap이다.
...
- **등록된 적대 증거와 인용 변이를 검증자가 다시 센다.** 구현자가 무엇을 심었다고 보고했든 관계없이 pair가 선택한 변이, 파생 이슈가 인용한 변이, 이번 라운드가 새로 만든 구조적 proxy·0건/전수·배선 oracle의 민감도 검사를 그대로 재현한다. **자리가 적히지 않은 등록 변이는 이번 변경이 닿은 pair path의 자리마다 심는다** — 구현자·이전 검증자가 고른 한 자리의 red는 다른 자리를 재지 않는다(0238 r2: 컴포넌트 안 M13만 red, `ProviderDetail` 전달은 미측정). 일반 hunk를 자동 분모로 삼지 않고 직접 행동 oracle이 충분한 pair에 새 mutation을 발명하지 않는다.
- **동작을 보존하는 추출·재배치 라운드에서 hunk 되돌림은 아무것도 재지 못한다.** 되돌리면 동작이 같은 이전 코드로 돌아갈 뿐이라 초록이 정답이므로, 그 초록을 `잠금 없음`이나 `잠김`으로 읽지 않는다.
- **이전 라운드가 red로 관측한 변이를 이번 라운드에 다시 실행하고 결과를 나란히 적는다.** `red → green`은 **덮개 회귀**이고 그 자체가 finding이다 — 구현자가 장치를 교체하며 구 장치가 잡던 자리를 잃은 것이라, 새 장치가 무엇을 잡는지만 보면 보이지 않는다. 0209 r6은 r5에서 red였던 배선 삭제 변이를 green으로 되돌렸고, 같은 라운드가 D18을 `closed`로 적었다.
...
- **구현자가 이번 라운드의 검증자와 같은 에이전트면 분모에 구현 보고가 이름을 대지 않은 적대 축을 최소 하나 넣는다.** 보고된 변이를 다시 심는 것은 남이 할 때는 독립 재측정이지만 자신이 할 때는 자기 목록의 재실행이라 자기 사각을 통과시킨다 — 같은 계약을 **다른 지점**에서 깨는 변이, 보고가 세지 않은 **형제 지점**, 또는 분모 자체의 독립 재열거 중 하나를 만든다. 그 축을 만들지 못했으면 `자기 검증 여부`에 그 사실을 적고 PASS로 올리지 않는다.

> 사례(0209 r11). 구현·검증이 같은 에이전트인 첫 라운드였다. 보고된 10행은 전부 red로 재현됐고 그것만 보면 완결이었는데, 보고에 없던 세 변이(같은 계약을 seam 안쪽에서 깨기·형제 진입점 2개)와 §10 분모 독립 재열거에서 **read 라벨을 단 mutation 3건**이 나왔다(D29).
...
plan §10은 pair 안에서 **별도 분모**로 계속 걷는다. `언제 강제`에 열거된 지점을 코드에서 하나씩 다시 세고, `실패 의미`의 적대 상태를 만든다. **구현자가 분모를 코드의 라벨로 세었으면 그 라벨이 참인지 표본으로 확인한다** — 0209 r11의 `readOnly: true` 8건 중 셋은 `stash push`·`commit -a`·`reset --hard` 였다. “다른 게이트가 막는다”는 주장도 필요한 pair의 선택된 변이로 재측정한다. 표 밖 지점이 현재 pair·ACTIVE Decision·AC에 필수면 `PLAN_GAP`, 그 밖이면 `NON_BLOCKING`/`NEXT_HANDOFF`다.
