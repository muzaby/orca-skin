...
- **이번 라운드가 고친 프로덕션 지점을 되돌려 무엇이 실패하는지 직접 본다.** 구현자가 그 수정을 잠갔다고 보고했든 안 했든 검증자가 다시 센다 — 고친 hunk를 하나씩 되돌리고 스위트를 돌려 실패 수를 적는다. **아무것도 실패하지 않는 수정은 배선이 지워져도 게이트가 초록인 자리**이므로 파생 이슈다. 0198 r5는 이번 라운드 수정 5곳(D18 배선 2·D20 정규화 2·신규 key 필터 1)이 전부 미검출이었다.
