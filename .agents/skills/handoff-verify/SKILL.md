...
- `docs/handoff/INDEX.md`: 상태·다음 주체·대상 커밋·PASS archive 이동이 실제 상태와 맞는지 확인한다.
- commit trailer: root `AGENTS.md` / `docs/git-template.md` 허용값을 따른다. 허용되지 않은 `Agent` 값 등을 통과시키지 않는다.
