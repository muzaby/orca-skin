...
- 보존 사유: **사용자 요청이 아니라 review 판단**이다. 1라운드 외부 검토 원문은 이 개편의 rationale 자체이고 `regression-coverage.md` 요약만으로는 "왜 그 지침이 그 형태인가" 를 복원할 수 없다. `handoff-review/SKILL.md §7`의 round 문서 보존 규칙에 따라 **동시에 1개만 유지**하며, 다음 라운드 문서가 생기면 이 파일을 교체한다. 실행 정본이 아니다.
