...
- Composer의 `@` 자동완성은 `ProviderInfo.catalog`가 있고 cached `tools`가 비어 있지 않은 provider를 내장 Plugin 후보로 투영한다. root plain 토큰에서는 Plugin 그룹을 path 그룹과 분리해 먼저 표시하고, slash/quoted 토큰에서는 기존 파일 경로 그룹만 표시한다. 선택값과 decoration은 표시 label이 아닌 `ProviderInfo.id`를 사용하며 인증 상태(`none`·`valid`·`expired`·`unknown`)는 후보 자격을 바꾸지 않는다.
