...
| POP3 메일 수집 | `node-pop3` + 격리 소켓 factory | exact package version in `app/package.json` | **Mail Plugin opt-in** | `UIDL` 증분 수집·`TOP`/`RETR`·TLS. 기본 배포에는 binding을 만들지 않으며, 명령 whitelist와 대량 소실 보호를 적용한다. |
...
- **모델 파싱 규약**(`features/harnesses/claude/model-parser.ts`, 순수 함수): `env.ANTHROPIC_DEFAULT_{SONNET,OPUS,HAIKU}_MODEL` 키가 있으면 그 alias 만 노출(=`isCustom`), 전무하면 sonnet/opus/haiku 3개를 `model:null` 로 노출한다 — **그 3개 폴백은 노출 목록이 끝내 빌 때만** 쓴다(`env.ANTHROPIC_MODEL` 만 있어도 폴백하지 않는다; 목록에 들어가지 않는 top-level `model` 은 폴백을 막지 못한다). `[1m]` 접미사는 분리해 `oneMillionContext` 로 보존하며 **모델 동일성 판정은 `(모델명, 1M)` 한 쌍**이다 — `X` 와 `X[1m]` 은 서로 다른 실행 대상이라 둘 다 노출된다. `env.ANTHROPIC_MODEL` 은 **노출 목록에도 더한다**(중복이면 추가하지 않는다); top-level `model` 은 default 선정에만 쓴다. default 는 그 명시 모델(`env.ANTHROPIC_MODEL`>`model`)·alias 폴백(sonnet→haiku→opus)을 노출 목록 안에서 평가해 **정확히 1개** 부여한다. `model:null` 항목은 SDK 가 bare alias 를 해석한다(모델명 추측 금지). 같은 규칙을 runtime 카탈로그(`runtime-catalog.ts`)가 `runtimeEnv.ANTHROPIC_MODEL` 로 공유한다.
