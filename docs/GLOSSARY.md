...
| **Auth provider** | *(0180 에서 제거)* 인증 절차를 구현하던 빌드 타임 contribution. 계약(`contracts/auth-method.ts`)과 구현이 함께 삭제됐다 — 0181 이 `Provider.auth`(`AuthSpec` 4종: api-key · password · pat · oauth) 한 필드로 대체한다. | *(현재 없음)* | 0157 → **0180 제거** |
| **Connector** | *(0180 에서 제거)* 인증된 요청으로 사내 서비스 기능을 제공하던 contribution. 계약(`contracts/connector.ts`)·런타임(`features/connectors/`)이 함께 삭제됐다 — 0181 이 `Provider{kind:'service'}` 의 `tools` 로 대체한다. | *(현재 없음)* | 0157 → **0180 제거** |
| **Plugin** | 두 레지스터를 구분한다: (A) UI 우산어인 플러그인 카탈로그 탭(`nav.plugins` — 0180 현재 스킬·MCP 2탭), (B) Claude Code 플랫폼 배포 산출물(`ORCA_PLUGIN_NAME`). 0157~0178 의 (C) *provider·connector 를 묶는 빌드타임 패키지* 는 **0180 에서 사라졌다**. 문맥 없이 서로 대체하지 않는다. | `nav.plugins` · `ORCA_PLUGIN_NAME` | 0159 → **0180 개정** |
