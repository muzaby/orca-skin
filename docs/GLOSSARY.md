...
| **Tweaks** | 사용자 환경 설정 — `theme`(white/dark) / `density` / `sidebarCollapsed` / `sidebarWidth` / `spendingLimitUsd` 등. electron-store 로 영속 (전체 17 키 카탈로그는 IPC_CONTRACT §2.4 / persistence.md §1.2). | `Tweaks` | `app/src/renderer/src/shared/hooks/useTweaks.ts` |
