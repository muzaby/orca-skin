...
| **Tweaks** | 사용자 환경 설정 — `theme`(white/dark) / `density` / `sidebarCollapsed` / `sidebarWidth` / `spendingLimitUsd` 등. electron-store 로 영속 (키 카탈로그는 IPC_CONTRACT §2.4 / persistence.md §1.2, 개수는 [생성물](./generated/inventory.md)). | `Tweaks` | `app/src/renderer/src/shared/hooks/useTweaks.ts` |
...
| **Principal** | 인증된 계정을 **표시하기 위한** 식별자(대개 email·아이디). `Grant.principalId` 에 실려 `ProviderInfo.principal` 로 renderer 에 나가고 사이드바 하단에 뜬다. **비밀이 아니다** — vault 를 거치지 않는다. 출처는 방식마다 다르다: `password` 는 입력 아이디, `browser-session` 은 `exchange.principalPath` → `whoami`, `api-key`·`pat` 은 **없다**(값이 계정에 묶이지 않는다). 없는 것이 정상인 경우가 있어 화면은 항상 폴백 라벨을 갖는다. | `Grant.principalId`(`main/contracts/provider.ts`) · `selectGatePrincipal`(`renderer/features/providers/lib/principal.ts`) | 0182 |
| **whoami** | 세션 성립 후 **같은 cookie jar 로 한 번 더** 사내 API 를 불러 principal 을 읽는 선언(`{ path, valuePath }`). `probe` 를 재사용하지 않는 이유는 그쪽이 판정 전용이라 본문을 버리고 리다이렉트 체인의 마지막 홉 응답이기 때문이다. **조회 실패는 로그인 실패가 아니다.** | `SessionLookup`(`main/contracts/provider.ts`) | 0182 |
