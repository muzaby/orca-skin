...
| **Connector** | 인증된 요청으로 외부·사내 서비스 기능을 제공하는 contribution (= "인증이 필요한 내장 도구"). auth provider 와 **역할이 다른 별개 계약** — connector 는 raw credential 을 읽지 않고 `authenticatedFetch(bindingId, …)` 로 위임한다. 한 플러그인 패키지가 provider 와 connector 를 함께 담을 수 있다. | `ConnectorRuntimeV1`(`main/contracts/connector-plugin.ts`) | 0157 |
| **Plugin** | 세 레지스터를 구분한다: (A) UI 우산어인 플러그인 페이지(`nav.plugins`), (B) Claude Code 플랫폼 배포 산출물(`ORCA_PLUGIN_NAME`), (C) provider·connector를 묶는 빌드 타임 플러그인 패키지(`AUTH_PLUGIN_PACKAGES`). 문맥 없이 서로 대체하지 않는다. | `nav.plugins` · `ORCA_PLUGIN_NAME` · `AUTH_PLUGIN_PACKAGES` | 0159 |
| **Connector 인스턴스** | 사용자가 UI 에서 템플릿으로 만든 connector (0161). 코드로 배포된 **정적 connector** 와 대비되며 DTO 의 `source`(`instance` \| `static`) 가 구분한다. `connectorId` 는 host+컨텍스트 경로에서 파생되고 **주소는 생성 후 불변** — 수정은 삭제 후 재생성이다. 설정(`connectorInstances`)에 영속되고 자격증명은 vault 가 따로 소유한다. | `ConnectorTemplate` · `connectorInstances` · `source` | 0161 |
| **Auth target** | 인증 결과의 사용 **대상**. `application`(Orca 앱 로그인) 또는 `connector`(특정 서비스 연결). 둘은 별도 인증 인터페이스가 아니라 **같은 lifecycle 에 이 판별자만 다르게** 넘긴다. ⚠️ `application` 은 **UX 게이트이지 보안 경계가 아니다** — 인증 전에도 main IPC 는 열려 있다. | `AuthTarget`(`shared/ipc.ts`) | 0157 / `guides/closed-network-extensions.md` §5 |
| **Auth binding** | 대상과 인증 결과를 잇는 **불투명 레코드**. 실제 secret·cookie jar·Electron `Session` 을 포함하지 않고 `handleId` 만 갖는다(타입상 표현 불가). 앱 로그인에 종속된 서비스 binding 은 `parentBindingId` 로 연결되어 logout cascade 를 결정한다. 영속하지 않는다 — 매 앱 실행마다 재인증. | `AuthBindingInfo`(`shared/ipc.ts`) | 0157 |
| **Session group** | 여러 binding 이 **같은 Electron cookie jar 를 직접 공유**하도록 지정하는 논리 이름(`persist:auth.<group>` partition). ADFS SSO cookie 가 여러 서비스의 redirect 에서 재사용되는 폐쇄망 구조를 모델링한다. partition 간 cookie 복사는 공유가 아니라 수동 복제이며 사용하지 않는다. | `BrowserSessionStore`(`main/infra/auth/`) | 0157 |
| **Credential presentation** | secret 을 HTTP 요청의 header·cookie·query 중 **어디에 어떤 형식으로** 넣는지에 대한 선언. **credential kind 에서 추론하지 않는다** — 같은 PAT 를 서비스별로 Bearer / Basic password / `PRIVATE-TOKEN` 으로 다르게 붙인다. connector manifest 가 소유한다. | `CredentialPresentation`(`shared/ipc.ts`) | 0157 |
...
- ❌ **"SSO 모듈"** (0130 어휘) → **Auth provider** 로 통일 (0157). 구 `SsoProviderModule` 은 "한 빌드 = 회사 로그인 1개" 였고 서비스 연결 개념이 없었다. 새 계약은 복수 provider 등록 + `AuthTarget` 으로 앱 로그인·서비스 연결을 함께 다룬다. `contracts/sso.ts`·`features/sso/`·`orca:sso:*` 채널은 제거됨.
