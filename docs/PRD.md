...
export type ChatEvent =
  ...
  | { type: 'tool_result'; data: { toolUseId: string; output: unknown; isError: boolean; durationMs?: number } }
...
| 항목 | Claude Code | opencode *(예약 사양 — Phase 1·2 미구현)* |
|---|---|---|
...
> Claude 백엔드 통합 상세 — SDK `query()` / `Options` / SDKMessage 명세는 [`docs/spec/claude/agent-sdk/typescript.md`](./spec/claude/agent-sdk/typescript.md), 채택 범위 표·SDKMessage→ChatEvent 매핑은 [arch/backend/adapters.md](./arch/backend/adapters.md), CLI 시기 외부 계약 (절 번호 anchor 보존) 은 [`claude-code-spec.md`](./claude-code-spec.md) §10·§11·§14 참조.
