...
| 채널 | 방향 | 페이로드 | 응답 | 설명 |
| --- | --- | --- | --- | --- |
| `orca:git:status` | R→M (invoke) | `GitPathRequest` = `{ cwd: string }` | `GitStatus` = `{ isRepo: boolean; branch: string \| null; detached: boolean; dirty: GitDirtyStat \| null }` | 작업 경로의 git 상태. `isRepo:false`(저장소 아님·git 미설치·경로 부재)가 **정상 응답**이고 renderer 는 그때 브랜치 칩을 렌더하지 않는다. `branch` 는 `symbolic-ref --short` 라 커밋 0개인 unborn 브랜치에서도 이름이 나온다. `dirty` 는 `diff HEAD --shortstat` — **추적 파일만** 센다(미추적 파일은 체크아웃을 막지 않는다). 무해 read = zod 실패 시 `isRepo:false` 폴백. |
...
| ------------------------- | --------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `session.updated`         | `patch: { model?; cwd?; agentTools?: string[]; cliVersion?: string }`                                                                                 | 어댑터의 첫 메시지 (SDK `SDKSystemMessage.init`)                                               | `state.sessionId` 저장, `state.cwd` 갱신. `agentTools`·`cliVersion`(0212 R-01)은 **누락 = 무변경**으로 병합한다 — TaskXXX 기능 가용성을 `tools` 에 `TaskCreate` 가 있는지로 판정하고(버전 비교 아님, D-003), **부재(`undefined`)와 미포함은 다른 사실**이라 부재에는 안내하지 않는다(D-005)                                                                                                                               |
