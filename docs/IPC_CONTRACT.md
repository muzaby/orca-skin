...
| `orca:git:diffSummary` | R→M (invoke) | `GitDiffRequest` = `{ cwd: string; sessionId?: string; commit?: string }` | `GitDiffSummary` = `{ isRepo, base, files, filesTruncated, commits, commitsTruncated }` | 변경사항 타일의 파일 목록 + 커밋 목록(0211). **비교 범위는 main 이 정한다** — `sessionId` 의 managed worktree row 가 있으면 그 `base_oid` → 현재 작업 트리, 없으면 `HEAD` → 현재 작업 트리다. `commit` 을 주면 그 커밋 하나(`<sha>^`→`<sha>`)로 범위가 바뀌고 작업 트리는 보지 않는다. 미추적 파일(`ls-files --others --exclude-standard`)도 전량 추가로 목록에 든다 — 에이전트가 만든 새 파일은 커밋 전까지 untracked 라 빼면 목록이 조용히 빈다. 커밋 목록은 base 를 아는 격리 세션에서만 채워진다(base 를 모르면 "이 세션의 커밋" 을 셀 수 없다). 상한은 파일 200 · 커밋 100 이고 **잘렸다는 사실을 `*Truncated` 로 값에 싣는다** — 조용히 자르면 사용자가 전부 본 것으로 읽는다. 무해 read = zod 실패 시 `isRepo:false` 빈 요약 폴백. |
