...
| `orca:chat:steer` | R→M (invoke) | `SteerChatMessage` = `{ sessionId: string; text: string; clientRequestId?: string }` | `Promise<void>` | 진행 중 assistant 턴에 텍스트 피드백을 주입한다. pending 은 DB 미영속이고, **모델이 그 메시지를 받아들이는 경계**(claude=`PostToolBatch` 훅 / sub-turn `result` 중 먼저 오는 쪽)에서 `steer.flushed` 와 함께 단일 user 메시지로 영속된다. 전달 실패(턴이 이미 종료)면 `error` + `steer.cancelled` 로 마감한다. |
...
| `steer.flushed`           | `ids; text; messageId; createdAt` | 소비 경계(`PostToolBatch` 훅 / sub-turn `result` 중 먼저) 에서 pending 피드백을 단일 user 메시지로 영속함. 이벤트 순서는 `telemetry` → `steer.flushed` 라 트랜스크립트·DB 가 똑같이 `[응답-전][steer user][응답-후]` 로 정렬된다 | pending 제거 + confirmed user 버블 전환 |
...
| `telemetry`               | `usage?: ProviderReportedTelemetry` (model·input/output·캐시 토큰·costUsd·durationMs·numTurns·modelUsage) + `continuation?: true` | 어댑터 턴 종료 (SDK `SDKResultMessage`, 또는 터미널 이벤트 누락 시 폴백). **`continuation` = sub-turn 경계** — 아직 커밋 안 된 steer 피드백이 있어 응답이 더 온다(스트리밍 입력 모드의 `query()` 하나가 입력마다 `result` 를 낸다) | `lastTelemetry`·`sessionCostUsd`(누산) 갱신 → 컨텍스트 도넛/TelemetryPanel. `inflight = false` 는 **`continuation` 이 없을 때만** — 있으면 턴 유지(라이브 프리뷰·중단 버튼 존속) |
