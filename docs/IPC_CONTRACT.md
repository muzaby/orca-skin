...
| `orca:chat:discardSession` | R→M (invoke) | `DiscardSession` = `{ sessionId: string }` | `Promise<void>` | **세션 전체 중단(0151 r2)** — 진행 턴 abort + 런타임(서브프로세스) 폐기로 CLI 입력 큐 잔여를 통째로 소멸시킨다. 공개 SDK 에 provider 큐 개별 취소 표면이 없어(`cancel_async_message` 는 `Query` 메서드 아님) Stop 잔여를 없애는 유일한 수단. **백그라운드 서브에이전트도 함께 종료**되므로 `chat.residual(count>0)` 통지가 있을 때만 UI 가 제시한다. 폐기된 예약은 `message.cancelled` 로 회신돼 composer draft 로 복원된다. |
...
| `orca:session:load`       | R→M (invoke) | `LoadSessionRequest` = `{ sessionId: string }`                  | `LoadedSession \| null` | 세션 cwd 와 메시지를 순서 보존 parts(`LoadedMessage.parts: AppMessagePart[]`, provider-runtime.md §7)로 일괄 로드. Phase 3 lazy load 진입점. `LoadedSession` 은 마지막 턴 통계 `lastTelemetry?: ProviderReportedTelemetry`(`turn_usage` 최신 부모 행 + `turn_model_usage` 자식 행에서 재구성)를 실어 컨텍스트 도넛/패널을 세션 수명 동안 복원. 비용은 `turn_usage` 원장(`0006`)이 SSOT — 일/주/월 시간 집계용이며 모델별 분해는 `turn_model_usage` 에 보존한다. **0122 r2**: 세션 한정 비용 총합 `costUsd?: number`(`turn_usage` 세션 SUM, 0 이면 생략)를 실어 상태 팝오버 "이 세션에서 사용한 비용" 을 시드한다 — 이후 라이브 턴은 renderer 가 `telemetry.costUsd` 를 누산. **0064 r2**: fork/handoff 파생 세션이면 `lineage?: { parentSessionId; relation: 'fork' | 'handoff'; parentTitle }` 를 실어 출처 배너(원본 세션 링크)를 복원한다(`session_lineage` + 부모 title 스냅샷). |
...
| `chat.residual`           | `count: number` | **중단 잔여 통지(0151 r2)** — Stop 영수증(`still_queued`)과 우리 예약의 **교집합**이 비지 않았다. CLI 내부 uuid(cron·auto-resume continuation)는 제외된 수다. **transient·relay-only**(버스 미경유·미영속) | `count>0` 이면 컴포저에 '세션 전체 중단' 액션이 붙은 Notice 를 띄운다(Stop 의 통념과 어긋나는 상태를 수동 배지로 덮지 않는다). `0` 이면 통지 해제 |
...
| `chat.listen`             | `phase: 'started' \| 'ended'` | listen phase(0143) — 메인 턴 종료 후 미정착 백그라운드 서브에이전트를 기다리는 턴-후 루프 대기 구간의 진입/이탈. **transient·relay-only** — chat-turn 이 `sendChatEvent` 직행으로 방출(버스 미경유 = history/usage 미소비·미영속, `message.queued` 동렬). 개별 listen 턴 경계가 아니라 루프 스코프 1쌍 | `listening`/`listenStartedAt` 상태 — StatusLine(inflight 애니메이션)이 `inflight ‖ listening` 동안 지속되고, send busy 판정도 동일 확장(listening 중 send = steer 예약). telemetry(TURN_END_RESET)는 listening 을 건드리지 않는다. 클리어 = ended·CANCEL_CHAT·세션 로드/신규 |
