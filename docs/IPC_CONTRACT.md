...
`SendChatMessage.agentKind?: 'coding' | 'work'`는 backend·provider·권한과 독립적이다. 생략하면 신규 출생만 Coding을 기본값으로 쓰며, 기존 세션·preparing lease·fork/handoff는 소유자의 종류를 계승한다. 다른 종류를 명시하면 큐·DB를 변경하기 전에 오류로 거부한다. 시스템 지침과 프로필 key는 클라이언트가 지정하지 못한다.

`SessionListItem`·`LoadedSession`과 `session.updated.patch`는 `agentKind`를 제공한다. 구형 payload에서 필드가 빠지면 renderer는 Coding으로 읽는다. `session.updated`의 생략은 기존 값 유지이고 Main은 확정 세션의 종류를 붙인다.
...
정규화 어휘는 SDK 호환을 유지한다. Composer의 Work 선택은 default·auto_classified·bypass이며 Coding은 기존 메뉴를 사용한다. 자동 승인은 명시된 Claude 버전 >4.5에만 허용하고 지원하지 않으면 Work=default, Coding=accept_edits로 정착한다. 계획 승인 목표는 Work=default, Coding=accept_edits이며 Main이 계산한 목표를 TurnRequest와 후속 실행에 전달한다. 실제 send에서 보정한 권한은 `session.updated.patch.permissionMode`로 전달한다. 이 필드도 누락이면 무변경이다.
