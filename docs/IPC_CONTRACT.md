...
| 채널 | 방향 | 페이로드 | 응답/스트림 | 설명 |
| --- | --- | --- | --- | --- |
| `orca:provider:list` | R→M (invoke) | — | `ProviderInfo[]` | 등록된 provider 와 grant 상태. `auth` 는 선언된 인증 방식 서술자 배열이고 **선언 순서가 곧 GUI 선택지 순서**다(길이 1이면 renderer 가 선택 단계를 건너뛴다). 입력 수집형(`api-key`·`password`·`pat`)만 `fields` 가 비어 있지 않다. `tools` 는 그 provider 가 LLM 에 노출하는 도구의 **완전 이름**(`mcp__<id>-tools__<tool>`, 없으면 `[]`) — 이름이 `Provider.id` 에서 파생되므로 GUI 가 id 와 함께 보여 선언과 대조할 수 있게 한다. Plugin 행만 선택적 `catalog`를 싣는다. `catalog`는 허용된 Material icon 이름, locale별 `title`·`body`, source·version·GitHub·license attribution을 담으며 raw secret은 포함하지 않는다. Plugin이 아닌 행은 이 필드가 없다. |
