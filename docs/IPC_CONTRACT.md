...
| `orca:files:pickDirectory` | R→M (invoke) | —                                                      | `string \| null`                                      | 컴포저 cwd 패널용 디렉토리 선택 다이얼로그. 취소/빈 선택은 `null`. |
| `orca:files:openPath` | R→M (invoke) | `OpenPathRequest` = `{ path: string }`                  | `Promise<void>`                                          | 세션 cwd 를 OS 파일 탐색기로 연다. `shell.openPath` 실패 문자열은 invoke reject 로 표면화한다. |
