...
| ------------------ | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `orca:chat:send`   | R→M (invoke) | `SendChatMessage` = `{ sessionId: string \| null; projectId: string \| null; text: string; permissionMode?; providerKey?: string \| null; modelFamily?: string \| null; effort?: 'low' \| 'medium' \| 'high' \| 'xhigh' \| 'max'; attachments?: ComposerAttachment[]; attachmentViews?: AttachmentView[]; cwd?: string \| null; forkFrom?: string; handoffFrom?: string }` | `Promise<void>` (ack) | 메시지 전송. `effort` 는 Claude Code SDK `Options.effort` 로 per-turn 전달된다(기본 UI 값 `high`). `attachments` 는 모델 주입용(경로/인라인 바이트), `attachmentViews` 는 영속·렌더용 가벼운 뷰(이미지=다운스케일 썸네일 data URL)로 user 메시지에 `attachment` 파트(`AppMessagePart`)로 남아 트랜스크립트 버블에 보인다. 응답은 `orca:chat:event` 스트림으로 발행. `sessionId === null` 이면 새 세션이며, `cwd` 가 있으면 해당 세션 출생 cwd 로 고정·영속한다. 같은 세션(또는 같은 창의 새-채팅 슬롯)에 진행 중 턴이 있으면 `error` 이벤트로 거부 — **서로 다른 세션의 동시 턴은 허용**(handoff 0011 TurnRegistry). **`forkFrom`/`handoffFrom`(0064 continuity)**: 상호 배타·새 세션 send 전용. main 이 출발 세션의 cwd/project/provider 를 계승하고 SDK `forkSession` 으로 새 session_id 를 발급받아 lineage(`session_lineage`)를 남긴다. `forkFrom` 은 도착 세션에 원문 display 복사, `handoffFrom` 은 text 를 `/compact` 자동 메시지로 대체(이때 `text` 생략 가능·복사 없음). 출발 세션 턴 진행 중인 handoff 는 `error` 로 거부. |
| `orca:chat:steer` | R→M (invoke) | `SteerChatMessage` = `{ sessionId: string; text: string; clientRequestId?: string }` | `Promise<void>` | 진행 중 assistant 턴에 텍스트 피드백을 주입한다(stdin 즉시 — `clientRequestId` 를 uuid 상관키로 실음). pending 은 DB 미영속이고, **CLI 가 입력을 흡수해 user echo(`input.echo`)로 되돌린 시점**에 `steer.flushed` 가 나가며 단일 user 메시지로 영속된다(0060 D1). 턴 종료까지 echo 가 없던 잔여 pending 은 큐에 남아 같은 세션의 다음 `chat:send` 프롬프트에 병합·이월된다(0060 D2 — 이때는 `steer.flushed` 미발신, renderer 가 send 시점 로컬 커밋). |
| `orca:chat:steerCancel` | R→M (invoke) | `CancelSteer` = `{ sessionId: string; id: string }` | `Promise<void>` | 아직 flush 되지 않은 pending 피드백을 취소한다. renderer 는 취소된 내용을 draft 로 복원한다. |
...
| `orca:session:load`       | R→M (invoke) | `LoadSessionRequest` = `{ sessionId: string }`                  | `LoadedSession \| null` | 세션 cwd 와 메시지를 순서 보존 parts(`LoadedMessage.parts: AppMessagePart[]`, provider-runtime.md §7)로 일괄 로드. Phase 3 lazy load 진입점. `LoadedSession` 은 마지막 턴 통계 `lastTelemetry?: ProviderReportedTelemetry`(`turn_usage` 최신 부모 행 + `turn_model_usage` 자식 행에서 재구성)를 실어 컨텍스트 도넛/패널을 세션 수명 동안 복원. 비용은 `turn_usage` 원장(`0006`)이 SSOT — 일/주/월 시간 집계용이며 모델별 분해는 `turn_model_usage` 에 보존한다. **0064 r2**: fork/handoff 파생 세션이면 `lineage?: { parentSessionId; relation: 'fork' | 'handoff'; parentTitle }` 를 실어 출처 배너(원본 세션 링크)를 복원한다(`session_lineage` + 부모 title 스냅샷). |
...
| `steer.queued`            | `id; text; createdAt` | 피드백이 main queue 에 접수됨 | pending user 버블 표시(optimistic idempotent ack) |
| `input.echo`              | `text; uuid?` | CLI 가 stdin 주입 입력을 흡수해 user 메시지로 echo(`SDKUserMessageReplay`)한 순간 — steer 소비 확정의 정밀 신호(0060 D1) | **main 내부 전용 — renderer 로 forward 되지 않는다.** TurnCoordinator 가 uuid(1차)/text(폴백) 매칭으로 pending 을 소비 표시하고 스트림에서 흡수 |
| `steer.flushed`           | `ids; text; messageId; createdAt` | echo 로 소비 확정된 pending 피드백을 단일 user 메시지로 영속한 직후(echo 배치 종료 지점) | pending 제거 + confirmed user 버블 전환 |
| `steer.cancelled`         | `id` | pending 피드백 취소 확정 | pending 제거 |
| `message.user`            | `text; createdAt` (`sessionId` 없음) | main 이 본문을 조립해 렌더러가 내용을 모르는 커밋 user 발화(0064 handoff `/compact` 자동 메시지) — send 수리 직후(턴 시작 전, sessionId 미발급) 1회 발행(r4: SDK init 지연 시 요약이 에코보다 먼저 렌더되던 역순 차단) | 렌더러 receive 가 `pendingNewChatKey`(핸드오프 draft)로 라우팅 후 `APPEND_COMMITTED_USER_MESSAGE` 로 user 버블 커밋(steer.flushed 와 동형). 일반 send 의 user 버블은 낙관 렌더라 이 이벤트가 없다 |
| `session.compacted`       | `trigger?: 'manual' \| 'auto'; preTokens?` | SDK `system/compact_boundary`(네이티브 압축 완료) 정규화(0064) | `compact_boundary` 파트로 커밋 → transcript 압축 경계 구분선(`CompactBoundaryMarker`). main persist 도 같은 파트를 영속해 재로드 복원 |
...
| `subagent.task`           | `toolUseId; phase:'started'\|'progress'\|'settled'; taskId?; subagentType?; description?; model?; durationMs?; toolUses?; lastToolName?; status?; summary?` | 서브에이전트(Task) 라이브 메타 (SDK `task_started`/`task_progress`/`task_notification` + child assistant `message.model`) | **reducer 미경유** — chat store 가 `toolUseId` 키 transient 맵(`subagentMeta`)으로 흡수해 우측 패널·`AgentTaskRow` 의 모델·경과시간·현재도구·도구수 표시를 구동(메인 transcript 파트 비오염). `taskId` 는 `orca:chat:stopSubagent` 의 stop 대상. 영속은 부모 Task `tool.call.completed.subagentMeta` 가 담당 |
| `telemetry`               | `usage?: ProviderReportedTelemetry` (model·input/output·캐시 토큰·costUsd·durationMs·numTurns·modelUsage) | 어댑터 턴 종료 (SDK `SDKResultMessage`, 또는 터미널 이벤트 누락 시 폴백)                         | `inflight = false`, `lastTelemetry`·`sessionCostUsd`(누산) 갱신 → 컨텍스트 도넛/TelemetryPanel                                                                         |
