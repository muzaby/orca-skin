...
| ------------------ | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `orca:chat:send`   | R→M (invoke) | `SendChatMessage` = `{ sessionId: string \| null; projectId: string \| null; text: string; permissionMode?; providerKey?: string \| null; modelFamily?: string \| null; effort?: 'low' \| 'medium' \| 'high' \| 'xhigh' \| 'max'; attachments?: ComposerAttachment[]; attachmentViews?: AttachmentView[]; cwd?: string \| null }` | `Promise<void>` (ack) | 메시지 전송. `effort` 는 Claude Code SDK `Options.effort` 로 per-turn 전달된다(기본 UI 값 `high`). `attachments` 는 모델 주입용(경로/인라인 바이트), `attachmentViews` 는 영속·렌더용 가벼운 뷰(이미지=다운스케일 썸네일 data URL)로 user 메시지에 `attachment` 파트(`AppMessagePart`)로 남아 트랜스크립트 버블에 보인다. 응답은 `orca:chat:event` 스트림으로 발행. `sessionId === null` 이면 새 세션이며, `cwd` 가 있으면 해당 세션 출생 cwd 로 고정·영속한다. 같은 세션(또는 같은 창의 새-채팅 슬롯)에 진행 중 턴이 있으면 `error` 이벤트로 거부 — **서로 다른 세션의 동시 턴은 허용**(handoff 0011 TurnRegistry). |
