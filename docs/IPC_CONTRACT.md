...
| ------------------ | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `orca:chat:send`   | R→M (invoke) | `SendChatMessage` = `{ sessionId: string \| null; projectId: string \| null; text: string; permissionMode?; providerKey?: string \| null; modelFamily?: string \| null; effort?: 'low' \| 'medium' \| 'high' \| 'xhigh' \| 'max'; attachments?: ComposerAttachment[]; attachmentViews?: AttachmentView[]; cwd?: string \| null; forkFrom?: string; handoffFrom?: string; clientKey?: string; clientRequestId?: string }` | `Promise<void>` (ack) | **모든 사용자 프롬프트의 단일 입구(0067)** — 메시지는 세션별 단일 pending queue 에 텍스트·첨부와 전송 순간의 전체 선택(`providerKey/modelFamily/permissionMode/effort`, main 해석 실제 모델)을 함께 보존한다. **유휴(idle)** 면 즉시 아이템 배치로 flush(스폰 초기 프롬프트 또는 장수명 채널 `pushTurn`). **진행 중(busy)** 은 런타임이 steer 를 지원하고 현재 턴과 provider·실제 모델·effort 가 같으며 앞선 queue barrier 가 없을 때만 PostToolBatch steer 대상이다. 그 밖의 provider/model/effort 경계와 steer 미지원은 거부하지 않고 다음 턴 queue 로 보낸다. 첫 queue 항목 뒤의 모든 메시지는 설정을 되돌려도 같은 queue 로 고정되며, 어시스턴트 턴 종료 시 남은 항목 전량을 한 번 병합해 마지막 메시지의 전체 선택으로 실행한다(permission 은 라이브 변경 가능). 커밋(user row 영속·preview·renderer 승격)은 **echo 관측 단일 경로**(`message.committed`) — send 시점 선영속은 없다. `clientRequestId` = pending 아이템 id(낙관 버블·echo 커밋 상관키), `clientKey` = 새 세션의 세션-이전 큐/라우팅 키(draft UUID — init 에서 실 id 로 rekey, 절대 영속 금지). `effort` 는 SDK `Options.effort` per-turn(변경 시 respawn 경계). 첨부 2종·cwd·fork/handoff 의미는 종전과 동일(0064). 새-채팅 슬롯의 중복 send race 만 `error` 로 거부한다. |
