...
| `orca:git:diffSummary` | R→M (invoke) | `GitDiffRequest` = `{ cwd: string; sessionId?: string }` | `GitDiffSummary` = `{ isRepo, base, files, totals, filesTruncated, commits, commitsTruncated, commitFilesUnavailable, uncommitted }` | 변경사항 타일의 **세션 전체** 파일·커밋·미커밋 변경(0211). main 이 저장된 `sessionId` baseline(커밋 + **그때의 브랜치 이름** `base.ref`, 0211 ΔV4)을 찾고, 없으면 질의 시점 `HEAD`를 쓴다. `base.ref` 는 컨텍스트 바가 그리는 유일한 비교 기준 라벨이고, `null`(이 변경 이전 세션·detached HEAD)이면 화면이 oid 7자로 접는다. `files`·`totals`는 세션 baseline→작업 트리의 추적 변경이며 미추적 파일은 명령·목록·합계 어느 곳에도 들지 않는다. totals/fileCount는 각 목록 절단 전에 계산한다. 커밋 timeline은 baseline을 아는 세션에서만 `base..HEAD`를 담고, 각 노드는 subject·author·시각·선택 body·파일·절단 전 `fileCount`/`totals`를 가진다. raw 파일 수집이 실패해 metadata fallback으로 응답하면 `commitFilesUnavailable:true`, 각 commit의 `fileCount`·`totals:null`이다(관측된 0과 다름). `uncommitted`는 timeline 뒤의 `HEAD`→작업 트리 추적 변경이다. baseline과 HEAD가 같으면 timeline은 비어도 이 블록은 남는다. 무해 read = zod 실패 시 `isRepo:false` 빈 요약 폴백. |
| `orca:git:diffPatch` | R→M (invoke) | `GitDiffPatchRequest` = `{ cwd: string; sessionId?: string }` | `GitDiffPatch` = `{ isRepo, base, files, filesTruncated, contextLimited, unavailable }` | 비교 범위 **전체**의 파일별 diff 줄(0211 ΔV4). 파일마다 묻던 옛 본문 채널을 대신한다 — 호출 수가 **파일 수와 무관하게 1** 이고, 타일이 열려 있을 때 요약 세대당 1회만 부른다. `git -c core.quotePath=false diff --unified=1000000 -M --no-color <base>` **한 호출**이라 문맥이 전문이고, 그래서 화면의 문맥 확장이 재조회 없이 순수 파생으로 끝난다. 각 파일은 `kind` 로 세 상태를 구분한다 — `text`(줄 있음) · `binary` · `too-large`(상한 초과로 줄을 싣지 않음, `added`/`removed` 는 그대로). 상한은 셋이다: 파일 200 · 파일당 줄 50,000 · 패치 전체 줄 200,000. 조회가 실패하면 같은 명령을 `--unified=3` 으로 한 번 더 부르고 `contextLimited:true` 를, 그것마저 실패하면 `unavailable:true` 를 싣는다 — 빈 배열만 주면 사용자가 '변경 없음' 으로 읽는다. **커밋 범위 인자는 없다**: 커밋 선택은 표시할 목록을 좁히는 renderer 축이고 각 파일 diff 기준은 세션 baseline 그대로다. 무해 read = zod 실패 시 `isRepo:false` 빈 패치 폴백. |
