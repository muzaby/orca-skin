...
도메인별 분포: `chat` 4 · `backend` 1 · `agent` 1 · `engine` 4 · `install` 2 · `settings` 2 · `skills` 7 · `files` 3 · `session` 6 · `project` 5 · `window` 3 · `search` 1 · `mcp` 4 · `cost` 2 · `concurrency` 1 · `permission` 2 (`respond` · `setMode`) · `debug` 2 (dev 전용 — `getMock` · `setMock`).
...
| ------------------ | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `orca:chat:send`   | R→M (invoke) | `SendChatMessage` = `{ sessionId: string \| null; projectId: string \| null; text: string; permissionMode?; providerKey?: string \| null; modelFamily?: string \| null; effort?: 'low' \| 'medium' \| 'high' \| 'xhigh' \| 'max'; attachments?: ComposerAttachment[]; attachmentViews?: AttachmentView[] }` | `Promise<void>` (ack) | 메시지 전송. `effort` 는 Claude Code SDK `Options.effort` 로 per-turn 전달된다(기본 UI 값 `high`). `attachments` 는 모델 주입용(경로/인라인 바이트), `attachmentViews` 는 영속·렌더용 가벼운 뷰(이미지=다운스케일 썸네일 data URL)로 user 메시지에 `attachment` 파트(`AppMessagePart`)로 남아 트랜스크립트 버블에 보인다. 응답은 `orca:chat:event` 스트림으로 발행. `sessionId === null` 이면 새 세션. 같은 세션(또는 같은 창의 새-채팅 슬롯)에 진행 중 턴이 있으면 `error` 이벤트로 거부 — **서로 다른 세션의 동시 턴은 허용**(handoff 0011 TurnRegistry). |
...
구 `orca:runtime:{status,prepare,statusEvent}` 3채널은 renderer 소비처(과거 `features/runtime` RuntimeStatus 위젯)가 제거된 뒤 preload 에만 노출된 고아 채널이라 **2026-06-11 제거**됐다. handoff 0049 PR-B 에서 main 내부 `PythonRuntime`(uv 격리 인터프리터)와 `RuntimeStage`/`RuntimeStatus` 타입, `ensure()` 부팅, `UV_*`/`PATH` SDK env 주입도 제거됐다. 현재 런타임 IPC 채널은 없고, Python MCP 는 시스템 Python 또는 사용자 MCP 설정에 맡긴다. 런타임 UI 재도입 시 §6 변경 절차로 신규 채널을 추가한다.
