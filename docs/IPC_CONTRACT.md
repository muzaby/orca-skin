...
| `tool.call.completed`     | `toolRunId; result; isError; durationMs?; parentToolRunId?`                                                                 | 도구 실행 완료 (SDK `tool_result` block)                                                       | `tool_result` 파트 append (`toolRunId` 로 `tool_call` 과 페어링). `parentToolRunId` 가 있으면 child transcript 에만 표시                                                                                                       |
