...
| `orca:git:diffSummary` | R→M (invoke) | `GitDiffRequest` = `{ cwd: string; sessionId?: string }` | `GitDiffSummary` = `{ isRepo, base, files, totals, filesTruncated, commits, commitsTruncated, commitFilesUnavailable, uncommitted }` | 변경사항 타일의 **세션 전체** 파일·커밋·미커밋 변경(0211). main 이 저장된 `sessionId` baseline을 찾고, 없으면 질의 시점 `HEAD`를 쓴다. `files`·`totals`는 세션 baseline→작업 트리의 추적 변경이며 미추적 파일은 명령·목록·합계 어느 곳에도 들지 않는다. totals/fileCount는 각 목록 절단 전에 계산한다. 커밋 timeline은 baseline을 아는 세션에서만 `base..HEAD`를 담고, 각 노드는 subject·author·시각·선택 body·파일·절단 전 `fileCount`/`totals`를 가진다. raw 파일 수집이 실패해 metadata fallback으로 응답하면 `commitFilesUnavailable:true`, 각 commit의 `fileCount`·`totals:null`이다(관측된 0과 다름). `uncommitted`는 timeline 뒤의 `HEAD`→작업 트리 추적 변경이다. baseline과 HEAD가 같으면 timeline은 비어도 이 블록은 남는다. 무해 read = zod 실패 시 `isRepo:false` 빈 요약 폴백. |
| `orca:git:diffFile` | R→M (invoke) | `GitDiffFileRequest` = `GitDiffRequest & { path: string }` | `GitDiffFileContent` = `{ kind:'text', oldValue, newValue, truncated }` \| `{ kind:'binary' }` \| `{ kind:'unavailable', reason }` | 파일 하나의 diff 본문(0211). **old/new 전문 두 벌**이고 unified patch 가 아니다 — 소비자 `DiffTable` 의 계약이 `{oldValue,newValue}` 라 patch 를 주면 renderer 에 파서가 새로 필요하다. 어떤 timeline node에서 파일을 골라도 **같은 세션 baseline** 대비 현재 작업 트리 본문을 돌려준다. 파일을 펼칠 때만 부른다(요약에 본문을 실으면 타일을 여는 순간 저장소 전체를 읽는다). 각 측 1 MiB 를 넘으면 `too-large`, NUL 을 담으면 `binary` 다. `path` 는 스키마가 절대경로와 `..` 상승을 막는다 — 그 문자열이 `git show <base>:<path>` 와 작업 트리 파일 읽기 두 곳으로 그대로 간다. 무해 read = zod 실패 시 `unavailable` 폴백. |
