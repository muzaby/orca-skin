...
> **응답 DTO 에 raw secret 이 없다.** `ProviderInfo` 는 상태·만료·표시용 `principal`과 plugin row의 표시 전용 `plugin:{icon,copy}`만 싣고, 값은 main의 vault(safeStorage)에만 있다. 앱 로그인은 **UX 게이트이지 보안 경계가 아니다** — 인증 전에도 main IPC 는 열려 있다.
...
| 채널 | 방향 | 페이로드 | 응답/스트림 | 설명 |
| --- | --- | --- | --- | --- |
| `orca:provider:list` | R→M (invoke) | — | `ProviderInfo[]` | 등록된 provider와 grant 상태. `auth`는 선언된 인증 방식 서술자 배열이고 선언 순서가 GUI 선택지 순서다. `tools`는 LLM에 노출하는 완전 이름이다. plugin row만 `plugin?:{icon;copy:Record<locale,{title;body}>}`를 가지며 icon은 local `IconName` key다. non-plugin row는 field를 생략하고 plugin copy가 비면 기존 label/detail을 유지한다. |
