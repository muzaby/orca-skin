...
| `message.user`            | `text; createdAt` | main 이 본문을 조립해 렌더러가 내용을 모르는 커밋 user 발화(0062 handoff `/compact` 자동 메시지) — coordinator 가 `session.updated` 승격 직후 1회 발행 | `APPEND_COMMITTED_USER_MESSAGE` 로 user 버블 커밋(steer.flushed 와 동형). 일반 send 의 user 버블은 낙관 렌더라 이 이벤트가 없다 |
