...
- 입력 검증: 모든 invoke 핸들러는 `app/src/main/infra/ipc/handle.ts` 의 `handle(channel, schema, invalid, fn)` 헬퍼를 경유해 **zod 스키마 (`app/src/shared/protocol.ts`)** 로 safeParse 검증한다. 채널별 실패 정책은 등록부에 명시:
...
| ------------------ | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `orca:chat:send`   | R→M (invoke) | `SendChatMessage` = `{ sessionId: string \| null; projectId: string \| null; text: string; agentKind?: 'code' \| 'work'; permissionMode?; providerKey?: string \| null; modelFamily?: string \| null; effort?: 'low' \| 'medium' \| 'high' \| 'xhigh' \| 'max'; attachments?: ComposerAttachment[]; attachmentViews?: AttachmentView[]; requirements?: DiffRequirementAnchor[]; cwd?: string \| null; extraDirs?: string[]; worktreeIsolation?: boolean; worktreeBaseRef?: string; forkFrom?: string; handoffFrom?: string; continuityLang?: 'ko' \| 'en'; clientKey?: string; clientRequestId?: string }` | `Promise<void>` (ack) | **모든 사용자 프롬프트의 단일 입구(0067·0166)** — 메시지는 논리 세션 lease의 pending queue에 먼저 적재된다. 신규 일반 세션이 `worktreeIsolation:true`이면 Main이 Agent 실행 전에 managed worktree를 만들고 `executionCwd`만 기존 턴 경로에 넣는다. base 는 `worktreeBaseRef`(컴포저에서 유예된 로컬 브랜치)의 커밋이고, 없으면 현재 HEAD다 — **source 트리가 dirty여도 거부하지 않는다**(0210 D-105, `git worktree add`는 source를 건드리지 않는다). `worktreeBaseRef`는 `worktreeIsolation:true`와 함께만 통과하며 브랜치 이름 규칙은 `orca:git:checkout`과 같은 SSOT(`GitBranchNameSchema`)다. resume·fork·handoff에서는 격리 선택을 거부한다. resume 턴에서 실행 경로가 사라져 있으면 Main이 원본 작업 경로로 폴백하고 `session.updated`의 `patch.cwd`로 알린다(0210 D-107·D-109). `requirements`는 변경사항 패널의 diff 줄에 적은 요구사항이고 `DiffRequirementAnchor`만 실린다 — `{ sessionId, baselineCommit, filePath, oldLine, newLine, hunkHeader, contextBefore, contextAfter, comment, createdAt }`. `baselineCommit`은 댓글을 작성한 패치의 실제 `base.oid`다. UI wrapper의 재anchor 상태(`located`)와 선택 범위 표식(`commitSha`)은 **wire anchor에 포함하지 않는다**. Main 이 이 값을 sentinel 블록으로 직렬화해 턴 콘텐츠에 합류시키며 본문의 sentinel·닫는 태그는 중화한다 — 목록이 비면 콘텐츠 문자열은 **한 글자도 달라지지 않는다**. `attachments` 와 같은 홉(큐 적재·배치 병합·연속 턴)을 그대로 지난다. 나머지 큐·커밋·`extraDirs` 계약은 기존과 같다. |
...
| `orca:chat:cancel` | R→M (invoke) | `CancelChat` = `{ sessionId: string }` | `Promise<void>` | 현재 응답을 취소한다. 살아 있는 세션 예약이 있으면 세션 lease와 채널을 유지해 다음 예약 입력을 받는다. 세션 전체 종료는 `discardSession` 경로다. |
| `orca:chat:discardSession` | R→M (invoke) | `DiscardSessionSchema` = `{ sessionId: string }` | `Promise<void>` | **세션 전체 중단(0151 r2, 0167 갱신)** — 진행 턴 abort + active/idle 런타임(서브프로세스) 폐기로 CLI 입력 큐 잔여를 통째로 소멸시킨다. 공개 SDK 에 provider 큐 개별 취소 표면이 없어 Stop 잔여를 없애는 유일한 수단. **백그라운드 서브에이전트도 함께 종료**되므로 `chat.activity.residualCount>0` 일 때만 UI 가 제시한다. 폐기된 open 예약은 `message.cancelled` 로 회신돼 composer draft 로 복원된다. |
...
Composer의 `dialog`·`drag_drop`·`clipboard` 첨부는 `chat:send` 정규화 단계에서 OS 사용자 임시 폴더(Windows 기본 `%LOCALAPPDATA%\Temp`)에 고유 이름으로 보관한다. `AttachmentView.path`·`sha256`은 Main이 실제 바이트에서 확정하며 renderer가 보낸 값을 그대로 사용하지 않는다. 원래 파일명·이미지 썸네일은 유지하고 저장 경로/hash는 user attachment 파트, 큐 병합과 재로드를 거쳐 보존한다. 경로가 없는 이전 이력도 읽을 수 있다. 입력 형식은 기존 텍스트/이미지 지원 범위를 유지하며 실제 바이트 기준 32 MiB를 검사한다.
...
| `orca:files:pickDirectory` | R→M (invoke) | —                                                      | `string \| null`                                      | 컴포저 cwd 버튼용 디렉토리 선택 다이얼로그. 기본 시작 위치는 OS Desktop이며, 취소/빈 선택은 `null`. |
...
| 채널 | 방향 | 페이로드 | 응답 | 설명 |
| --- | --- | --- | --- | --- |
| `orca:git:status` | R→M (invoke) | `GitPathRequest` = `{ cwd: string }` | `GitStatus` = `{ isRepo: boolean; branch: string \| null; detached: boolean; root: string \| null; githubUrl?: string \| null }` | 작업 경로의 git 상태. `githubUrl`은 origin의 HTTPS/SCP/SSH 원격에서 hostname을 소문자로 바꿔 `github` 포함 여부로 판정하고, 실제 호스트를 보존한 자격증명 없는 `https://<hostname>/owner/repo`로 정규화한 웹 주소다. 사내 `github.company.com` 같은 호스트도 지원하며 서버 제품을 네트워크로 검증하지 않는다. 원격 없음·미지원은 null이며 이전 snapshot의 미지정도 같은 의미다. 로컬 설정만 읽고 네트워크 조회는 하지 않는다. `isRepo:false`(저장소 아님·git 미설치·경로 부재)가 **정상 응답**이고 renderer 는 그때 브랜치 칩을 렌더하지 않는다. `branch` 는 `symbolic-ref --short` 라 커밋 0개인 unborn 브랜치에서도 이름이 나온다. **변경량 축은 이 응답에 없다**(0211 D-027) — `diff HEAD --shortstat` 은 격리 세션에서 커밋된 작업을 세지 못해 `+0 −0` 을 만들었다. 표시 변경량의 정본은 `orca:git:diffSummary` 의 `totals` 이고, checkout 의 더티 게이트는 `orca:git:checkout` 이 자기 stat 을 부른다. 무해 read = zod 실패 시 `isRepo:false` 폴백. |
| `orca:git:branches` | R→M (invoke) | `GitPathRequest` | `GitBranchList` = `{ current: string \| null; branches: string[] }` | 로컬 브랜치 목록(`for-each-ref refs/heads/`). 현재 브랜치를 맨 앞에 고정하고 나머지는 이름순 — **정렬은 main 이 소유**한다. 원격 전용 브랜치·worktree 는 다루지 않는다. 폴백 = 빈 목록. |
| `orca:git:checkout` | R→M (invoke) | `GitCheckoutRequest` = `{ cwd: string; branch: string; resolution?: 'stash' \| 'commit-wip' \| 'discard' }` | `GitCheckoutResult` | 브랜치 전환. **`resolution` 없이 더티 트리를 만나면 아무것도 하지 않고** `{ ok:false, reason:'dirty', from, stat }` 을 돌려준다 — renderer 가 처리 방식을 묻는 모달을 띄우고 고른 값으로 재호출한다. 해소 3종은 전부 추적 변경만 건드린다(`stash push` / `commit -a -m WIP` / `reset --hard HEAD`). 실패는 예외가 아니라 `{ ok:false, reason:'error', message }` 값이다(zod 실패만 reject). **해소는 성공했는데 checkout 이 실패하면 그 값에 `applied: 'stash' \| 'commit-wip' \| 'discard'` 가 실린다** — 작업 트리는 이미 바뀌고 브랜치만 그대로인 부분 실패라, renderer 가 "변경은 스태시됐고 브랜치는 그대로" 를 말할 수 있어야 한다(`discard` 는 되돌릴 수 없어 이것이 없으면 데이터 유실과 구분되지 않는다). `branch` 는 스키마가 문자셋으로 잘라 옵션 주입을 막고, **main 의 실행부가 execFile 앞에서 같은 스키마로 한 번 더 검사한다**. |

| `orca:git:diffSummary` | R→M (invoke) | `GitDiffRequest` = `{ cwd: string; sessionId?: string }` | `GitDiffSummary` = `{ isRepo, base, files, totals, filesTruncated, commits, commitsTruncated, commitFilesUnavailable, uncommitted }` | 세션 baseline→HEAD에 **커밋된** 파일·합계·커밋 목록이다. main은 저장된 세션 baseline(커밋과 당시 브랜치 이름 `base.ref`)을 우선하고, 없으면 세션 출생 시각 이전 커밋을 찾는다. 현재 커밋이 있으나 출생 시각 이전 커밋이 없으면 empty tree가 기준이며 baseline과 출생 시각이 모두 없으면 질의 시점 HEAD를 쓴다. 커밋이 전혀 없으면 `base.kind:'none'`으로 빈 결과를 반환한다. 미커밋·미추적 변경은 제외하고 `uncommitted`는 빈 파일·합계로 유지한다. totals/fileCount는 목록 절단 전에 계산한다. timeline은 해석된 baseline의 `base..HEAD`이며 각 노드는 subject·author·시각·선택 body·파일·`fileCount`/`totals`를 가진다. raw 파일 수집 실패 시 metadata fallback과 `commitFilesUnavailable:true`, 각 commit의 `fileCount`·`totals:null`로 알린다. zod 실패 시 `isRepo:false` 빈 요약 폴백. |
| `orca:git:diffPatch` | R→M (invoke) | `GitDiffPatchRequest` = `{ cwd: string; sessionId?: string; commitSha?: string }` | `GitDiffPatch` = `{ isRepo, base, files, filesTruncated, contextLimited, unavailable }` | `commitSha`를 생략하면 세션 baseline→HEAD, 지정하면 **첫 부모→선택 커밋**의 파일별 diff를 일괄 조회한다. root 커밋은 empty tree 대비이며 선택 커밋 본문에는 세션 누적 변경·후속 HEAD·미커밋 변경이 섞이지 않는다. `commitSha`는 `GitCommitOidSchema`가 완전한 40자리 hex OID만 허용하며 브랜치 이름·축약 SHA·옵션 문자열은 거부한다. 전문맥 조회와 문맥 축소 폴백의 비교 좌표는 동일하다. 파일의 `kind`는 `text`·`binary`·`too-large`이고 상한 초과 파일도 변경량을 유지한다. 문맥 축소 성공은 `contextLimited:true`, 조회 불가는 `unavailable:true`로 알린다. zod 실패 시 `isRepo:false` 빈 패치 폴백. |

커밋 선택 패치의 `base`는 `{ kind: 'commit-parent'; oid: string; commitOid: string }`이다. `oid`는 첫 부모 OID(root이면 empty-tree OID), `commitOid`는 선택 커밋의 완전한 OID다. 객체나 부모를 읽을 수 없으면 `unavailable:true`를 반환하며 세션 전체 범위로 폴백하지 않는다. 전체 모드의 `base`는 세션 baseline 해석에 따른 `worktree-base`·`head`·`none`을 사용한다.

패치 실행은 [`git-diff.ts`](../app/src/main/infra/git/git-diff.ts)의 읽기 전용 Git 관문을 사용한다. 전문맥 조회가 실패하면 같은 범위에서 문맥을 줄여 재시도하고, 파일·줄 상한은 [`git-diff-parse.ts`](../app/src/main/infra/git/git-diff-parse.ts)가 적용한다. Renderer의 타일 수명·범위별 요청·응답 폐기와 캐시 재사용은 [상태 관리 문서](arch/frontend/state.md#121-git-패치와-댓글-범위)를 따른다.

`GitDirtyStat` = `{ files: number; insertions: number; deletions: number }`.

> **worktree 는 이 도메인의 범위가 아니다** (제품 결정) — 채널도 UI 도 두지 않는다.

### 2.7 Session

| 채널                      | 방향         | 페이로드                                                        | 응답                    | 설명                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------- | ------------ | --------------------------------------------------------------- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `orca:session:cwd`        | R→M (invoke) | —                                                               | `Promise<string>`       | 새 대화의 기본 작업 디렉토리(OS Desktop). 프로젝트 landing은 Project.cwd를 우선한다.                                                                                                                                                                                                                                                                                                                                                                                          |
| `orca:session:list`       | R→M (invoke) | —                                                               | `SessionListItem[]`     | 사이드바 '최근 대화' 메타 목록(`cwd` 포함). DB SSOT — `updatedAt` 내림차순.                                                                                                                                                                                                                                                                                                                                                                                          |
| `orca:session:load`       | R→M (invoke) | `LoadSessionRequest` = `{ sessionId: string }`                  | `LoadedSession \| null` | 세션 cwd·허용 폴더 `extraDirs`·순서 보존 메시지·마지막 턴 통계 `lastTelemetry`·비용 `costUsd`·fork/handoff `lineage`를 일괄 로드한다. **0167부터 `activity?: ChatActivitySnapshot`**을 동봉해 renderer 재접속/세션 전환 시 현재 준비·스트리밍·대기·잔여 상태를 추가 왕복 없이 복원한다. live broadcast가 load 응답보다 먼저 도착했으면 renderer는 더 높은 `revision`을 보존한다. |
| `orca:session:delete`     | R→M (invoke) | `DeleteSessionRequest` = `{ sessionId: string }` | `Promise<DeleteSessionResult>` | 일반 세션은 hard delete한다. managed worktree 세션은 clean이고 HEAD가 생성 base와 같을 때만 worktree·branch를 제거한 뒤 삭제한다. dirty·새 commit·검사/제거 실패면 이유를 반환하고 세션과 작업을 보존한다. |
...
| `orca:session:setPinned`  | R→M (invoke) | `SetSessionPinnedSchema` = `{ sessionId: string; pinned: boolean }` | `Promise<void>`    | 0129 — 세션 고정 토글. `pinned=true` 면 `pinned_at=Date.now()`(정렬 키 겸용), false 면 `null`. 좌측 nav "고정됨" 섹션이 `SessionListItem.pinnedAt` 로 파생.                                                                                                                                                                                                                                                                                            |
| `orca:session:addDirectory` | R→M (invoke) | `AddSessionDirectoryRequest` = `{ sessionId: string; directory: string }` | `AddSessionDirectoryResult` | Work 전용 허용 폴더 추가. 실제 디렉터리·canonical 경로·루트 금지·중복·상한을 검사한다. 준비/실행/listening 중이면 `busy`. 비동기 검사 후 세션과 busy 상태를 다시 읽고 DB에 추가만 저장한다. 성공 `{ ok:true, extraDirs }`, 실패 `{ ok:false, reason }`(`busy`·`not-found`·`invalid-directory`·`not-work`·`limit`·`failed`). 성공한 canonical 목록은 재로드로 복원하고 다음 요청부터 SDK와 호스트 도구에 함께 적용한다. cwd 변경·허용 폴더 제거는 제공하지 않는다. |
...
| `orca:project:create`       | R→M (invoke) | `CreateProjectRequest` = `{ name: string; instructions: string; cwd?: string }` | `Project` | cwd가 없으면 수동 프로젝트 생성. cwd가 있으면 절대 경로를 정규화해 같은 경로를 재사용하고 새 이름은 basename으로 정한다. 기존 지침·이름은 덮어쓰지 않는다. name 1–120자, instructions 최대 8000자. |
...
| `orca:project:setPinned`    | R→M (invoke) | `SetProjectPinnedSchema` = `{ id: string; pinned: boolean }` | `Promise<void>` | 고정 시각을 기록하거나 해제한다. 프로젝트 구획은 모든 프로젝트를 표시하고 고정 프로젝트를 먼저 정렬한다. |
...
- **입력어 prefix 매칭**: `toFtsMatch` (`app/src/main/infra/db/queries.ts`) 가 공백 토큰 분리 후 _모든 토큰_ 에 `*` wildcard 부착 (예: `진행 중` → `"진행"* "중"*`). 어느 토큰이든 미완성으로 타이핑 중일 수 있다는 가정. 짧은 토큰의 매치 폭증은 LIMIT + FTS5 rank 정렬로 흡수.
...
`MockScenarioId` 의 전체 목록과 개수는 **`app/src/shared/ipc.ts` 의 `MOCK_SCENARIO_IDS` 가 갖는다** — 여기에 사본을 두지 않는다(root `AGENTS.md` 원칙 4). `full` 은 text/reasoning/tool_calls/tool_approval/ask_question/plan_review 를 순차 재생한 뒤 도구호출→error 점프로 종료한다(권한 2종은 approval 스텝이 라우터를 경유해 합성). 시나리오 telemetry 는 `costUsd: 0`·`model: 'mock-sonnet'`, 컨텍스트 토큰 합 = `round(contextUsageRatio × 200_000)` 로 도넛/`nearCompaction` 경고를 구동.
...
| 채널 | 방향 | 페이로드 | 응답/스트림 | 설명 |
| --- | --- | --- | --- | --- |
| `orca:provider:list` | R→M (invoke) | — | `ProviderInfo[]` | 등록된 provider 와 grant 상태. `auth` 는 선언된 인증 방식 서술자 배열이고 **선언 순서가 곧 GUI 선택지 순서**다(길이 1이면 renderer 가 선택 단계를 건너뛴다). 입력 수집형(`api-key`·`password`·`pat`)만 `fields` 가 비어 있지 않다. `tools` 는 그 provider 가 LLM 에 노출하는 도구의 **완전 이름**(`mcp__<id>-tools__<tool>`, 없으면 `[]`) — 이름이 `Provider.id` 에서 파생되므로 GUI 가 id 와 함께 보여 선언과 대조할 수 있게 한다. |
| `orca:provider:state` | R→M (invoke) **+** M→R (send) | — | `ProviderPlatformState` = `{ gate: { required; passed; bypassed }; providers: ProviderInfo[]; step: ProviderStepInfo \| null; resuming: boolean }` | `gate.required` = `kind:'gate'` provider 선언 여부 **또는 DEV 빌드**(선언 0개여도 true). **`passed` 는 게이트 멤버가 전부 `valid` 이고 이번 실행에서 확인(`verified`)됐을 때만 true** (0184) — 로그인이 체인이라 멤버 하나만 풀려도 인증이 아니고, 멤버가 0인 DEV 게이트는 우회 없이는 열리지 않는다(빈 배열 `every` 함정을 멤버 수 병행 검사로 막는다). **복원된 grant 는 통과 근거가 아니다** — 기록(`status`)만 보던 동안 한 번 로그인에 성공한 providerId 가 영구히 통과했다(세션 grant 는 vault 도 만료도 없어 기록만으로 계속 `valid`). `verified` 는 메모리에만 살고 영속하지 않으며, 재시작 뒤에는 로그인 또는 부팅 자동 로그인(`Provider.probe`)이 다시 세운다. `bypassed` 는 dev 우회(`Settings.authBypass`, DEV 빌드 한정)로 통과했음을 표시한다. **`resuming` 은 게이트 통과 후 나머지 Auth 의 부팅 복원이 아직 진행 중인가다** (0194) — renderer 는 참인 동안 메인 셸 대신 대기 화면을 유지한다(복원이 여는 재로그인 창이 메인 UI 뒤에서 뜨지 않도록). **invoke 와 push 가 같은 값을 실어야 한다** — invoke 는 창을 열자마자 부르는 첫 스냅샷이라, 여기서만 `false` 를 주면 복원 중에 열린 창이 곧바로 메인 셸을 띄운다. renderer 가 파생할 수 없는 값이라(main 의 배치 진행 상태다) wire 로만 온다. |
| `orca:provider:login` | R→M (invoke) | `{ providerId; authKind?; input? }` (`ProviderLoginRequestSchema` — 키 64자·값 4096자·32쌍 상한) | `ProviderStepInfo` | 인증 시작 → 다음 step(`input-required`·`code-required`·`done`·`failed`). `authKind` 미지정이면 선언 배열의 **첫 방식**. (사용자가 시작하지 않는 step 이 하나 더 있다 — 부팅 자동 로그인이 도는 동안 `orca:provider:state` 의 `step` 에 `resuming` 이 실린다. invoke 응답으로는 나오지 않는다.) 입력 수집형 1회차는 필드 선언을 되돌려주고, 사용자가 채워 다시 부르면 vault 에 봉인한다(**신뢰된 prompt** — 방식이 만든 임의 UI 가 아니라 Orca 가 이 선언을 렌더링한다). **`done` 은 `Provider.probe` 가 통과한 뒤에만 나온다** — 방식과 무관하게 grant 커밋 직후 확인이 한 번 돈다. 확인이 끝나기 전에는 `orca:provider:state` push 가 나가지 않는다(떨어질 자격증명에 게이트가 잠깐 열리는 것을 막는다). 확인 실패는 입력형이면 같은 폼(`input-required` + 사유), 브라우저 흐름이면 `failed(reason:'probe_failed')` 다. |
...
`SendChatMessage.agentKind?: 'code' | 'work'`는 backend·provider·권한과 독립적이다. 생략하면 신규 출생만 Code를 기본값으로 쓰며, 기존 세션·preparing lease·fork/handoff는 소유자의 종류를 계승한다. 다른 종류를 명시하면 큐·DB를 변경하기 전에 오류로 거부한다. 시스템 지침과 프로필 key는 클라이언트가 지정하지 못한다.

현재 `SessionListItem`·`LoadedSession`의 `agentKind`는 필수이며 Main은 DB 값을 엄격히 검증해 출력한다. Renderer의 `LOAD_SESSION` 읽기 경계에서만 구형 `coding` 또는 필드 누락을 `code`로 변환하며 그 외 값은 오류로 처리한다. `session.updated.patch.agentKind`의 생략은 기존 값 유지이고 Main은 확정 세션의 종류를 붙인다.

Work에는 Main이 `{ type: 'response.boundary', sessionId, boundary }`를 발행한다. `boundary`는 `{ phase: 'begin', id }` 또는 `{ phase: 'end', id, outcome: 'ended'|'aborted'|'failed'|'unknown' }`이다. 표시 내용이 있는 수신 구간만 생성하며 중간 사용자 입력 커밋 직전에 기존 구간을 `unknown`으로 닫는다. 실제 종료 신호 없이 정리용 telemetry만 받은 경우 완료로 가장하지 않는다. 라이브 이벤트와 DB의 `response_boundary` part는 같은 payload를 사용한다. 이 경계는 표시 전용이며 권한·과금·작업 성공의 정본이 아니다.
...
| `output.captured` | `artifact?: ArtifactRef`, `toolRunId?`, `responseId?` | 일반 파일 hook 결과를 SDK result 경계에서 telemetry 전에 드레인 | Main이 소유 파일과 원래 호출 메시지 또는 응답 경계를 확인하고 artifact part를 영속한 뒤 전달. 참조가 없거나 연결 근거가 없으면 transcript를 변경하지 않음. |
| `session.updated`         | `patch: { model?; cwd?; permissionMode?: NormalizedPermissionMode; worktree?: WorktreeDisplay \| null; agentTools?: string[]; cliVersion?: string }`                                                                                 | 어댑터의 첫 메시지 (SDK `SDKSystemMessage.init`)                                               | `state.sessionId` 저장, `state.cwd` 갱신. **patch 는 전부 누락 = 무변경**이다. `worktree`(0211 AR-02)만 **세 상태**를 구분한다 — 키 부재 = 이 갱신은 표시 정본에 관해 말하지 않는다 · `null` = 표시 정본 소실(0210 D-107 폴백) · 객체 = 덮어쓰기. 그래서 소비자는 `undefined` 비교가 아니라 **키 유무**(`Object.hasOwn`)로 판정하고, 발신부는 키를 넣거나 빼거나 둘 중 하나만 한다(0211 D-022). `agentTools`·`cliVersion`(0212 R-01)은 **누락 = 무변경**으로 병합한다 — TaskXXX 기능 가용성을 `tools` 에 `TaskCreate` 가 있는지로 판정하고(버전 비교 아님, D-003), **부재(`undefined`)와 미포함은 다른 사실**이라 부재에는 안내하지 않는다(D-005)                                                                                                                               |
...
| `input.received` | `text; uuid?; origin:{kind,label?}` | SDK의 외부 입력 또는 앱 송신과 구분된 UserPromptSubmit 프롬프트. kind는 scheduled/channel/task/peer/automatic이며 출처를 텍스트로 추측하지 않는다. | main 내부 전용. 응답 앞에 user 메시지를 영속하고 origin을 담은 message.committed로 전달한다. |
| `session.schedules` | `schedules:{id,schedule,recurring,prompt}[]; pendingWakeup?:boolean` | Stop 훅의 검증된 session_crons 또는 확인된 생성·삭제 영수증. ScheduleWakeup의 양수 실행 시각 성공 응답은 ID를 만들지 않고 pendingWakeup으로 표시하며 전체 Stop snapshot이 이를 대체한다. | main 내부 전용. 채널 pump가 수신·폐기 순서대로 activity snapshot을 갱신한다. 원시 이벤트를 renderer에 재전달하지 않는다. |
| `message.committed`       | `ids; text; attachmentViews?; requirements?: DiffRequirementAnchor[]; origin?: ReceivedMessageOrigin; messageId; createdAt` | 소비 확정된 pending 배치 또는 `input.received`를 user 메시지로 영속한 직후. 턴 프롬프트·이월 프렐류드·steer 게이트 배치는 기존 병합 단위를 유지한다. 외부 수신은 `ids:[]`와 `origin`을 갖는다. | pending 제거 + 정식 user 버블 승격(`APPEND_COMMITTED_USER_MESSAGE`, 파일 첨부와 별도 `diff_requirements` 파트 포함). 수신 출처는 text 파트의 origin에 저장해 라이브와 세션 로드에서 동일하게 표시한다. 코멘트 스냅샷의 인용 첨부도 유지한다. |
...
| `tool.call.started`       | `toolRunId; toolName; args; parentToolRunId?; editPreview?`                                                                               | LLM 도구 호출 (SDK `tool_use` block)                                                           | 현재 assistant 메시지에 `tool_call` 파트 append. `parentToolRunId` 가 있으면 부모 Task 의 child transcript 로 분기. **`editPreview`(0229)** 는 `Edit` 실행 **전** 미리보기 패치 `{ structuredPatch }` 로, 어댑터가 편집 전 파일을 workspace 가드(`guardToolAccess`) 안에서 1 MiB 이하일 때만 읽어 `diff.structuredPatch` 로 만든다 — SDK 는 실행 전 diff 를 주지 않으므로 진행 중(`수정 중`) 카드가 실제 줄번호를 그리는 유일한 입력이다. **라이브 전용이라 writer 가 영속하지 않는다**(`toolCallPartPayload`) — 완료 결과의 `structuredOutput` 이 정본이고 이미 영속된다 |
| `tool.call.completed`     | `toolRunId; result; isError; durationMs?; parentToolRunId?; subagentMeta?; structuredOutput?`                                                                 | 도구 실행 완료 (SDK `tool_result` block)                                                       | `tool_result` 파트 append (`toolRunId` 로 `tool_call` 과 페어링). `parentToolRunId` 가 있으면 child transcript 에만 표시. 부모 Task tool_result 면 `subagentMeta`(`{model?,durationMs?,toolUses?}`, claude-map 이 task_*/child model 누산)를 실어 영속 — 세션 재로드 후 카드/행 모델·소요시간 복원. **`structuredOutput`(0204·0228)** 은 SDK `tool_use_result` 기반으로 **TaskXXX 도구 결과**(원문)와 **`Edit` 결과**(`{ structuredPatch }` 투영 — `originalFile` 은 파일 전체라 싣지 않는다)에만 실린다 — `result` 는 모델용 wire content 라 `TaskCreate` 의 `task.id` 같은 필드를 담지 않는다. writer 가 같은 규칙으로 `payload_json` 에 영속해 재로드 후에도 작업 목록을 접을 수 있다                                                                                                       |
| `subagent.task`           | `toolUseId; phase:'started'\|'progress'\|'settled'\|'updated'; taskKind?:'agent'\|'shell'\|'monitor'\|'workflow'\|'unknown'; runState?:'running'\|'paused'; isBackgrounded?; taskId?; subagentType?; description?; model?; durationMs?; toolUses?; lastToolName?; status?; summary?; background?; elapsedSeconds?; heartbeat?; retry?` | 서브에이전트(Task) 라이브 메타 (SDK `task_started`/`task_progress`/`task_notification` + child assistant `message.model`). `background?: true`(0143) = settled 한정 — async_launched 영수증이 관측된(실제 백그라운드 실행) 태스크의 정착에만 main(coordinator)이 부여 | chat store 가 `toolUseId` 키 transient 맵(`subagentMeta`)으로 흡수해 우측 패널·`AgentTaskRow` 표시를 구동(메인 transcript 파트 비오염). **단 settled+`background:true`(0143)는 reducer 로도 흘러 `subagent_notice` 파트(완료 통지 블록 — main writer 도 동형 영속, toolRunId 멱등)를 커밋한다.** `taskId` 는 `orca:chat:stopSubagent` 의 stop 대상. 메타 영속은 부모 Task `tool.call.completed.subagentMeta` 가 담당. **`phase:'updated'`(0212 AR-03)** = SDK `task_updated` 델타 패치 — `paused`(살아 있는 일시정지, 정착 아님)와 `is_backgrounded` 만 나른다. `killed` 는 `phase:'settled'`+`status:'stopped'` 로 접히고(D-015) `patch.error` 는 그 정착의 `summary` 로 실린다. `patch.error` 를 `updated` 에 싣지 않는 이유는 라이브 표시에 소비처가 없기 때문이다. **`taskKind`(0230)** = 실행 태스크의 종류 — 이벤트 이름은 `subagent.task` 로 남지만 대상은 서브에이전트만이 아니다(셸 명령·감시·워크플로가 같은 봉투로 온다). 판정 SSOT 는 `shared/task-kind.ts` 이고 우선순위는 **원래 도구 이름 > `task_type`** 다(`Monitor` 와 셸이 `local_bash` 를 공유해 `task_type` 단독으로는 안 갈린다). **키 부재와 `'unknown'` 은 다른 사실이다** — 부재는 판정할 입력이 없었다는 뜻이고 `'unknown'` 은 판정했으나 어휘 밖이라는 뜻이다. **`elapsedSeconds`·`heartbeat`·`retry`(0231)** = SDK **최상위** `tool_progress` 정규화 — `phase:'progress'` 로 오며 transient 다(파트를 만들지 않는다). `heartbeat` 는 연결 생존만 말하고 진척을 주장하지 않는다. `retry` 는 `heartbeat` 로 해제되지 않고 새 attempt 나 정착에서만 바뀐다 |
| `subagent.backgroundSet`  | `toolUseIds: string[]` | SDK `background_tasks_changed` (0212 AR-02) — 변화 후 살아 있는 background 태스크 전량 중 `task_id → tool_use_id` 매핑이 있는 것만 | **레벨 신호**다: coordinator 가 매 payload 로 집합을 교체하고(REPLACE) 추적에는 있는데 payload 에 없는 항목을 정착시킨다. edge(started/settled)와 짝지어 읽지 않는다 — 놓친 bookend 가 "실행 중" 표시를 영구 고착시키지 못하게 하는 것이 목적이다. 프로세스 단위라 **첫 payload 는 기준선일 뿐 아무것도 정착시키지 않고**, CLI (재)기동 시 미확립으로 되돌린다(D-012). `STOP_SETTLE_TIMEOUT_MS` watchdog 을 대체하지 않는다 — 레벨은 채널이 살아 있을 때만 오고 watchdog 은 채널이 죽는 축을 막는다(D-011). **소비자는 main 하나다** — renderer 까지 중계되지만 store 가 명시적으로 무시한다(정착은 기존 `subagent.task` 경로로 온다) |
| `worktree.preparing`      | `step: WorktreePrepareStep` (`sessionId` **없음**) | 격리 **신규 세션**의 준비 경계 5곳 — `repo`·`base`·`branch`·`worktree`(`WorktreeService.prepare` 내부 4) + `session`(`prepare-worktree.ts` 격리 분기 1) (0211 AR-01) | 세션 id 발급 **전**이라 `sessionId` 가 없다 — renderer 는 `message.queued` 와 같은 규칙으로 `pendingNewChatKey` 에 라우팅한다. 미영속 UI 신호다(버스 미경유 — history·usage 미소비). 준비가 끝나거나 거부되면 상태를 비운다(`BEGIN_TURN`·`session.updated`·`TURN_END_RESET` 세 지점, 0211 §10 EP-04). 비격리·resume 경로는 이 이벤트를 한 번도 내지 않는다(D-005) |
| `chat.activity`           | `sessionId; revision; foreground:'idle'\|'preparing'\|'streaming'; transport:'idle'\|'ready'\|'listening'; queuedCount; deliveryPendingCount; residualCount; backgroundTaskCount; sessionSchedules?: SessionSchedule[]; pendingSessionWakeup?:boolean` | lease·pending queue·background tracker·채널 활동·세션 예약의 단일 활동 스냅샷. 의미가 달라질 때만 세션별 단조 `revision`으로 모든 살아 있는 renderer에 broadcast한다. `ready`는 채널이 유휴이고 미소비 backlog 없이 입력을 받을 수 있는 수신 상태다. 실제 자동 응답·배출·held 입력 전송 중은 `listening`이며, count는 고유 사용자 메시지 수다. | 낮거나 같은 revision은 무시한다. Renderer의 `listening`은 ready/listening 모두에 세션 점유를 유지하고 `activityTransport`로 답변 표면을 구별한다. ready에서는 스피너·대기 문구·중단 버튼 없이 일반 전송을 표시하되 main의 held→flush·commit 순서를 유지한다. 라이브 `inflight`는 renderer가 소유하고 세션 로드 시에는 snapshot으로 복원한다. `sessionSchedules` 부재는 미조회, 빈 배열은 예약 없음이다. pendingSessionWakeup은 목록 도착 전 확인된 깨우기 대기이며 임의 ID·개수를 만들지 않는다. `residualCount>0`이면 상태 Notice와 '세션 전체 중단' 탈출구를 제공한다. |
...
| `turn.ended`              | 없음 (`sessionId?`)                                        | 백엔드 **Stop hook** 이 턴 정상 종료를 알린 직후. terminal 이 아니다 — 턴을 닫는 것은 여전히 `telemetry` 다 (0211 ΔV6 D-115) | `turnEndTick` 을 1 올린다. 그 값이 git 변경 목록 조회의 **유일한 계기**이고 컴포저 git 행의 닫힘도 함께 푼다. 정상 라우팅한 세션은 app을 통해 비열람 완료 아이콘에도 전달한다. 페이로드가 없다 — hook 이 결과를 기다리면 그만큼 턴 종료가 늦어진다 |
| `permission.requested`    | `approvalId; origin; action: PermissionAction`                                                            | AskUserQuestion·ExitPlanMode·**위험 도구 게이트**(canUseTool)                                  | `action.kind` 로 분기 → `pendingAsks` / `pendingPlanReview` / `pendingToolApprovals`. 응답은 단일 `permissionRespond`(`{approvalId, resolution}`, approvalId=requestId) |
...
`PermissionAction` = `{kind:'ask_question', request} | {kind:'plan_review', request} | {kind:'tool_approval', toolName, input}`. `ApprovalResolution` = `{behavior:'allow', updatedInput?, updatedPermissions?} | {behavior:'deny', message?, interrupt?, planFeedback?}` (claude `PermissionResult` 와 동형 + 앱 레벨 세션 권한 `updatedPermissions:[{toolName, scope:'session'}]`). `deny.planFeedback` = `{comments:[{id,quote,start,end,body}], note?}` — 계획(plan_review) 패널 인라인 코멘트를 담아 보내면 어댑터가 구조화 태그(`ORCA_PLAN_FEEDBACK`, `adapters/plan-feedback.ts`)로 직렬화해 ExitPlanMode deny message 로 전달한다(handoff 0047, 채널 수 불변·페이로드 확장).
...
**권한 모드 라이브 전환.** `permissionSetMode`(`orca:permission:setMode`, renderer→main invoke)는 `{sessionId, mode: NormalizedPermissionMode}`를 받고 적용된 `NormalizedPermissionMode`를 반환한다. 잘못된 요청·없는 세션·SDK 적용 실패는 `undefined`다. 같은 세션의 변경은 직렬화하며 live가 있으면 실제 spawnedModel과 종류로 정착한 모드를 `Query.setPermissionMode`에 적용한 뒤 controller에 저장한다. idle은 종류별 선택을 저장하고 다음 send에서 실제 해소 모델로 다시 검사한다. renderer는 대상·모델·요청 세대가 유효한 응답만 반영한다. 위험 모드의 기존 확인은 유지한다.

정규화 어휘는 SDK 호환을 유지한다. Composer의 Work 선택은 default·auto_classified·bypass이며 Code는 기존 메뉴를 사용한다. 자동 승인은 명시된 Claude 버전 >4.5에만 허용하고 지원하지 않으면 Work=default, Code=accept_edits로 정착한다. 계획 승인 목표는 Work=default, Code=accept_edits이며 Main이 계산한 목표를 TurnRequest와 후속 실행에 전달한다. 실제 send에서 보정한 권한은 `session.updated.patch.permissionMode`로 전달한다. 이 필드도 누락이면 무변경이다.
...
3. `app/src/main/app/handlers/<domain>.ts`(또는 chat 계열이면 `app/chat-turn/`) — `handle(channel, schema, invalid, fn)` 또는 `handlePlain` 으로 등록(실패 정책 명시). 새 도메인이면 파일 신설 후 `Bootstrap.register()` 에서 호출
