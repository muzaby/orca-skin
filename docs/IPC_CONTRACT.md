...
| `turn.ended`              | 없음 (`sessionId?`)                                        | 백엔드 **Stop hook** 이 턴 정상 종료를 알린 직후. terminal 이 아니다 — 턴을 닫는 것은 여전히 `telemetry` 다 (0211 ΔV6 D-115) | `turnEndTick` 을 1 올린다. 그 값이 git 변경 목록 조회의 **유일한 계기**이고 컴포저 git 행의 닫힘도 함께 푼다. 페이로드가 없다 — hook 이 결과를 기다리면 그만큼 턴 종료가 늦어진다 |
