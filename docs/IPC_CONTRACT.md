...
타입은 `app/src/shared/background-task.ts`, 동작 정본은 [백그라운드 작업 아키텍처](arch/backend/background-tasks.md)를 따른다. `PermissionAction.providerRequest`는 SDK requestId·toolUseId·agentId와 query generation을 승인 요청에 연결하며 일반 메시지 FIFO를 식별자로 사용하지 않는다.
...
| 채널 | 방향 | 페이로드 | 응답/스트림 | 설명 |
| --- | --- | --- | --- | --- |
| `orca:provider:list` | R→M (invoke) | — | `ProviderInfo[]` | 등록된 provider 와 grant 상태. `auth` 는 선언된 인증 방식 서술자 배열이고 **선언 순서가 곧 GUI 선택지 순서**다(길이 1이면 renderer 가 선택 단계를 건너뛴다). 입력 수집형(`api-key`·`password`·`pat`)만 `fields` 가 비어 있지 않다. `tools` 는 그 provider 가 LLM 에 노출하는 도구의 **완전 이름**(`mcp__<id>-tools__<tool>`, 없으면 `[]`) — 이름이 `Provider.id` 에서 파생되므로 GUI 가 id 와 함께 보여 선언과 대조할 수 있게 한다. 모든 connection category(`gate`·`harness`·`plugin`·`usage`)는 선택적 `catalog`를 싣을 수 있다. `catalog`는 허용된 Material icon 이름, locale별 `title`·`body`, source·version·GitHub·license attribution을 담으며 raw secret은 포함하지 않는다. 입력이 없는 행은 category별 기존 fallback을 사용한다. |
...
| `message.committed`       | `ids; text; attachmentViews?; requirements?: DiffRequirementAnchor[]; origin?: ReceivedMessageOrigin; messageId; createdAt` | 소비 확정된 pending 배치 또는 `input.received`를 user 메시지로 영속한 직후. 턴 프롬프트·이월 프렐류드·steer 게이트 배치는 기존 병합 단위를 유지한다. 외부 수신은 `ids:[]`와 `origin`을 갖는다. | pending 제거 + 정식 user 버블 승격(`APPEND_COMMITTED_USER_MESSAGE`, 파일 첨부와 별도 `diff_requirements` 파트 포함). 수신 출처는 text 파트의 origin에 보존한다. origin이 있는 자동 수신은 라이브와 세션 로드의 transcript 투영에서 제외하며 버블·복사·시간 행을 만들지 않는다. 코멘트 스냅샷의 인용 첨부도 유지한다. |
...
| `message.reasoning`       | `text; signature?; parentToolRunId?`                                                                                        | 확장사고 블록 (SDK `SDKAssistantMessage` thinking block)                                       | 부모 식별자를 보존하며 현재 assistant 메시지에 `reasoning` 파트 append (signature 는 opaque 보관) + `live.reasoning` 비움                                                                     |
...
| `tool.call.retracted` | `toolRunIds:string[]` | SDK 공개 철회 신호 | main 내부 전용. coordinator가 열린 ID만 host retracted 결과로 정착하며 원 신호는 영속·중계하지 않는다. |
| `tool.call.completed`     | `toolRunId; result; isError; durationMs?; parentToolRunId?; subagentMeta?; structuredOutput?; nonExecution?`                                                                 | 도구 실행 완료 (SDK `tool_result` block)                                                       | `tool_result` 파트 append (`toolRunId` 로 `tool_call` 과 페어링). `parentToolRunId` 가 있으면 child transcript 에만 표시. 부모 Task tool_result 면 `subagentMeta`(`{model?,durationMs?,toolUses?}`, claude-map 이 task_*/child model 누산)를 실어 영속 — 세션 재로드 후 카드/행 모델·소요시간 복원. **`structuredOutput`(0204·0228)** 은 SDK `tool_use_result` 기반으로 **TaskXXX·Agent·Bash·PowerShell·Skill·MCP 도구 결과**와 **`Edit` 결과**(`{ structuredPatch }` 투영 — `originalFile` 은 파일 전체라 싣지 않는다)에 실린다 — `result` 는 모델용 wire content 라 `TaskCreate` 의 `task.id` 같은 필드를 담지 않는다. writer 가 같은 규칙으로 `payload_json` 에 영속해 재로드 후에도 작업 목록을 접을 수 있다                                                                                                       |
| `subagent.task` | `toolUseId; phase; taskId?; status?; summary?; background?`와 선택 메타 | 기존 Agent transcript 카드용 투영. 실행 상태의 정본은 background journal과 taskId 기반 상태다. | terminal 신호가 있을 때에만 종료한다. 중단 요청이나 live snapshot 제외를 실패/중단으로 확정하지 않는다. |
| `subagent.backgroundSet` | `toolUseIds: string[]` | 레거시 호환 variant. Claude 어댑터의 현재 수신 경로는 `BackgroundEvent`다. | 이 variant로 terminal 상태를 합성하지 않는다. 현재 live 집합은 별도 background 채널의 전체 snapshot을 따른다. |
...
`tool.call.completed.nonExecution?: NonExecution`은 SDK 비실행 사유(`source:sdk`, kind, 선택적 userFeedback) 또는 host 정착 사유(`source:host`, no_result/retracted)를 운반한다. writer는 tool_result payload에 보존하고 renderer는 라이브·재로드에서 같은 분류를 사용한다. 메타데이터 부재는 기존 결과 해석으로, 미지 SDK 사유는 미실행으로 처리한다. 타입과 분류 정본은 `app/src/shared/tool-outcome.ts`다.
