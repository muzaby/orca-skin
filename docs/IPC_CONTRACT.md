...
| ------------------ | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `orca:chat:send`   | R→M (invoke) | `SendChatMessage` = `{ sessionId: string \| null; projectId: string \| null; text: string; permissionMode?; providerKey?: string \| null; modelFamily?: string \| null; effort?: 'low' \| 'medium' \| 'high' \| 'xhigh' \| 'max'; attachments?: ComposerAttachment[]; attachmentViews?: AttachmentView[]; cwd?: string \| null; extraDirs?: string[]; worktreeIsolation?: boolean; forkFrom?: string; handoffFrom?: string; continuityLang?: 'ko' \| 'en'; clientKey?: string; clientRequestId?: string }` | `Promise<void>` (ack) | **모든 사용자 프롬프트의 단일 입구(0067·0166)** — 메시지는 논리 세션 lease의 pending queue에 먼저 적재된다. 신규 일반 세션이 `worktreeIsolation:true`이면 Main이 Agent 실행 전에 clean source HEAD에서 managed worktree를 만들고 `executionCwd`만 기존 턴 경로에 넣는다. resume·fork·handoff에서는 이 선택을 거부한다. 나머지 큐·커밋·`extraDirs` 계약은 기존과 같다. |
