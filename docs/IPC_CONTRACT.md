...
| `orca:git:diffSummary` | R→M (invoke) | `GitDiffRequest` = `{ cwd: string; sessionId?: string; commit?: string }` | `GitDiffSummary` = `{ isRepo, base, files, totals, filesTruncated, commits, commitsTruncated }` | 변경사항 타일의 파일 목록 + 커밋 목록(0211). **비교 범위는 main 이 정한다** — `sessionId` 의 managed worktree row 가 있으면 그 `base_oid` → 현재 작업 트리, 없으면 `HEAD` → 현재 작업 트리다. `commit` 을 주면 그 커밋 하나(`<sha>^`→`<sha>`)로 범위가 바뀌고 작업 트리는 보지 않는다. 미추적 파일(`ls-files --others --exclude-standard`)도 목록에 들지만 **수치는 0 이다**(0211 D-026) — 목록에서까지 빼면 에이전트가 만든 새 파일이 화면에서 사라지고, 수치에 더하면 컴포저 행이 우측 패널과 다른 수를 말한다. `totals` 는 **필수 필드**이고 **200개 절단 전** 전체에서 센다 — 목록의 합이 아니다. 커밋 목록은 base 를 아는 격리 세션에서만 채워진다(base 를 모르면 "이 세션의 커밋" 을 셀 수 없다). 상한은 파일 200 · 커밋 100 이고 **잘렸다는 사실을 `*Truncated` 로 값에 싣는다** — 조용히 자르면 사용자가 전부 본 것으로 읽는다. 무해 read = zod 실패 시 `isRepo:false` 빈 요약 폴백. |
| `orca:git:diffFile` | R→M (invoke) | `GitDiffFileRequest` = `GitDiffRequest & { path: string }` | `GitDiffFileContent` = `{ kind:'text', oldValue, newValue, truncated }` \| `{ kind:'binary' }` \| `{ kind:'unavailable', reason }` | 파일 하나의 diff 본문(0211). **old/new 전문 두 벌**이고 unified patch 가 아니다 — 소비자 `DiffTable` 의 계약이 `{oldValue,newValue}` 라 patch 를 주면 renderer 에 파서가 새로 필요하다. `orca:git:diffSummary` 와 **같은 범위 해석 함수**를 쓴다: 두 벌이 되면 목록은 base 대비인데 본문은 HEAD 대비가 되어 같은 화면의 두 값이 어긋난다. 파일을 펼칠 때만 부른다(요약에 본문을 실으면 타일을 여는 순간 저장소 전체를 읽는다). 각 측 1 MiB 를 넘으면 `too-large`, NUL 을 담으면 `binary` 다. `path` 는 스키마가 절대경로와 `..` 상승을 막는다 — 그 문자열이 `git show <base>:<path>` 와 작업 트리 파일 읽기 두 곳으로 그대로 간다. 무해 read = zod 실패 시 `unavailable` 폴백. |
