...
- **심볼축 세트가 따로 있다**: 아래 1~7장이 *주제*로 자른 것이라면, [`api/`](api/) 는 **API 심볼 하나하나**를 호출 순간부터 CLI 경계까지 따라간다("`interrupt()` 를 부르면 무슨 일이 일어나나").
...
| 세트 | 내용 |
|---|---|
| [**`api/`**](api/) — 9편 | **0장 진입점 분류**(경계 문서 — 어떤 진입점이 어느 심볼로 들어가는지, 도달/미도달 전수) + **1~7장 SDK 딥다이브**: `query()` 호출 생명주기 · 입력 경로 `SDKUserMessage` · 출력 경로 `SDKMessage` · 제어 메서드(`setModel`/`setPermissionMode`/`interrupt`) · 태스크 제어(`stopTask`/`backgroundTasks`) · 역방향 콜백(`canUseTool`/`hooks`) · `Options` 표면과 실행 파일 해석. 각 장이 **①시그니처 ②SDK 내부 콜스택 ③wire 프레임 ④구현 디테일 ⑤다이어그램 ⑥관측 불가 구간** 6단 골격을 채운다. |

2부 다이어그램 **10개**(flowchart 5 · sequence 4 · state 1) — 1부와 합쳐 총 **19개**.
...
- **유일한 예외**: 2부의 [`api/00-진입점-분류.md`](api/00-진입점-분류.md) 만 "어떤 애플리케이션 진입점이 어느 SDK 심볼로 들어가는가" 를 매핑한다 — 콜스택은 서술하지 않는 **경계 문서**이며, `api/01`~`07` 은 위 원칙대로 SDK 만 다룬다.
