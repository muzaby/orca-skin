...
r1은 국소 보완에 치우쳤다. 사용자의 “리팩토링 수준” 정정에 따라, 기존 동작을 유지하면서 함께 변하는 코드의 소유자를 다시 정했다.

| 영역 | 이전 구조의 취약점 | 현재 책임과 변화 |
|---|---|---|
| 제출·대기열 | 신규/busy 경로가 payload와 queued 이벤트를 중복 조립하고 index callback을 경유 | [enqueue](../../../../app/src/main/app/chat-turn/enqueue.ts)가 적재·발신을 함께 소유. send가 직접 호출하며 busy-reserve와 중간 callback 계약 제거 |
| 턴 해석 | turn-setup이 provider/env 해석과 renderer 발신을 함께 보유해 취소·대기열에도 설정 의존 전파 | [resolve-turn](../../../../app/src/main/app/chat-turn/resolve-turn.ts)에 해석, enqueue에 submitted, send에 coordinator 발신 결집. turn-setup 제거 |
| 설정 | 서비스와 단일 소비자인 디렉토리 열거 모듈로 파편화. runtime 주입 시 models 빈 배열로 입력 보정 | [settings](../../../../app/src/main/features/harnesses/settings.ts)에 열거·캐시 결집, 기본 provider 선택은 models. 실제 입력만 받으므로 bootstrap이 서비스를 직접 주입 |
| 모델 해석 | parser와 available-models가 알고리즘을 나누고 catalog가 내부 호출 순서를 조립 | [model-parser](../../../../app/src/main/features/harnesses/claude/model-parser.ts)가 settings/runtime 진입별 완성된 목록 반환. 빈 목록 의미는 각 진입에서 유지, 최종 default 결정 중복 제거 |
| 사용량 DB | 집계기가 세션·프로젝트 등 전체 DB API에 의존 | [UsageQueries](../../../../app/src/main/infra/db/usage-queries.ts)를 DbQueries.usage에 고정 구성. 같은 연결·SQL·준비 정책을 유지하며 forwarding facade는 두지 않음 |
| 사용량 기록·복원 | subscriber가 tracker와 DB를 동시에 조작하고 usage-map에 기록 판정과 wire 변환 혼재 | [tracker](../../../../app/src/main/features/usage/tracker.ts)가 기록·집계·방송 소유, 기존 dto가 row 변환 소유. subscriber·usage-map 제거 |
| 이력 조회 | session IPC callback이 메시지·usage·비용·lineage·worktree 복원을 직접 조립 | [history reader](../../../../app/src/main/features/history/reader.ts)가 복원 담당. handler는 검증·조회 호출·현재 activity 결합만 수행 |
| MCP 변환 | convert가 expand의 결과 속성명만 바꾸는 중간 wrapper | [convert](../../../../app/src/main/features/extensions/mcp/convert.ts)가 확장·서버별 제외·target 반환을 함께 소유. 문자열 치환과 secret resolver 경계 유지 |

삭제된 생산 모듈은 busy-reserve, turn-setup, settings-entries, available-models, subscriber, usage-map, expand다. 새 생산 모듈은 usage-queries와 history/reader이며, 기존 계층 안에서 책임을 분리한 것이다. 새로운 registry·DI container·백그라운드 서비스·의존성은 없다.

전수 import 조사와 큰 파일·작은 단일 소비자 모듈 검토도 수행했다. auth lifecycle, adapter 이벤트 매핑, SessionRuntime과 pool, admission/continuation, runtime model startup은 독립 불변식·수명·정책이 있어 유지했다. Git root 해석의 realpath/오류 처리와 HTTP header의 set-cookie/빈 값 정책은 서로 달라 이름만 보고 통합하지 않았다. 파일 길이만을 기준으로 한 분할이나 병합은 하지 않았다.
...
| 모델 목록 | 불변 선언의 canonical key 집합을 catalog 생성 시 구성해 조회마다 반복 정규화하지 않음. 최종 목록의 기본 모델 결정 중복 제거 | 전체 앱 지연의 감소율을 측정한 것은 아님 |
...
| 세션 로드: [queries](../../../../app/src/main/infra/db/queries.ts)의 `loadParts().all()` → [history reader](../../../../app/src/main/features/history/reader.ts) → [DTO](../../../../app/src/main/infra/ipc/dto.ts) | 해당 세션의 모든 part row를 배열로 받은 뒤 payload를 해석해 메시지 객체를 조립. 원본 JSON과 파싱 결과, IPC 직렬화 비용이 겹칠 수 있음. renderer 가상화는 이 Main 작업을 줄이지 않음 | part 수·payload 총량별 조회/해석 시간과 메모리 최고치를 분리 측정. 부담이 확인되면 동일 결과를 만드는 row 순회부터 검토. 페이지 로딩·공개 IPC 변경은 별도 설계 |
| 사용량: [tracker](../../../../app/src/main/features/usage/tracker.ts) → [기간 SUM](../../../../app/src/main/infra/db/usage-queries.ts) | 유효한 telemetry마다 `recordAndBroadcast`가 전역 월 범위와 해당 provider 범위를 다시 집계. 월 원장 행이 늘면 집계 비용도 증가 가능. 전체 provider를 매번 순회하는 구현은 아님 | 월 범위 행 수별 SQL 시간과 이벤트 처리 지연 측정, `EXPLAIN QUERY PLAN`으로 전역/provider 쿼리 확인. 측정 없이 증분 집계 캐시·상주 집계 서비스를 추가하지 않음 |
...
구조 리팩토링은 위 r2에 반영했다. 남은 성능 후보는 대형 입력에서 실제 지연이나 메모리 부담이 관측될 때 별도로 변경한다.
...
Main 전체 테스트, node/web/test 타입 검사, Main/shared lint, 문서 링크·인벤토리, 실-git 시험 시간 예산을 확인했다. 실제 busy 제출·사용량 이벤트 순서·MCP 배포 인자를 우회시키는 변이도 실패를 검출했다. 이동 전후 SQL·준비 순서와 모델 해석 결과를 별도 대조했다. 수치와 실행 조건은 [0221 구현 보고](../../../handoff/0221-main-structural-simplification/plan.md)에 기록한다.

현재 리팩토링은 구현 자기검증을 마쳤으며 독립 handoff verify는 대기 중이다. 실제 GUI·대형 입력의 체감 성능 및 미구현 adapter의 종단 호환성은 측정하지 않았다.
