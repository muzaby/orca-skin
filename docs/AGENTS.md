...
| `guides/closed-network-extensions.md` | **폐쇄망(사내) 확장 정본 (0130 → 0157 개정)** — 확장 모델 축("빌드 타임 내장 ↔ 런타임 MCP")·불변 확장점 2파일(`contracts/auth-plugin.ts`·`connector-plugin.ts`·`usage-report.ts`)·opt-in 레지스트리 2곳·additive-optional-only 정책 | 사내 인증 provider·connector·정적 사용량 provider 를 main 브랜치 수정 없이 붙일 때. |
