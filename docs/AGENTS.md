...
| `etc/study/orca/` | **외부 연구 기반 Orca 인증 플랫폼 설계** — 설치형 auth provider·connector 계약, 앱 로그인/서비스 연결 공통 lifecycle, ADFS/WIA shared session, API key·Auth token·PAT, broker 보안 경계와 인수 기준. | 인증 플러그인 플랫폼의 요구사항·모듈 구조·secret 비노출 경계를 결정할 때. |
