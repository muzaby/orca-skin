...
| `arch/backend/` + `ARCHITECTURE.md`(인덱스) | **Main 진실 원천 (분해됨)** — Backend Adapter 추상화 (SessionAdapter — LLM Provider 가 아님), electron-store 영속화(18 키·마이그레이션 16종), **2 계층 영속성 모델 (로컬 DB 구현 완료 + FS 잔여)**, **자격증명 모델 (safeStorage — MCP 비밀. 인증 플랫폼 vault 는 0180 에서 제거)**, IPC 핸들러, 보안 경계(**main 원격 요청 = Chromium `net.fetch` 단일 스택, 0173/0174**), **자동 업데이트 (electron-updater, 0084~0086 — runtime-ipc §3.1) + 스케줄러 (croner, 0091 — runtime-ipc §3.1-b) + skills 부트 시딩 (0078) + 인증 플랫폼·connector 전면 제거 (0180 — main 수직 슬라이스 11→9, contracts 6모듈, `infra/auth`→`infra/net` 이설)**, **§12 Provider Runtime Model (범용 정규화 계층 — NormalizedEvent · PermissionBridge · AppCommandPolicy · SessionCapability · RevertManager · ErrorClassifier · Telemetry · AuthStore · AuditLog · DirectBackendAPI · WorkspaceManager · ConfigManager · 우선순위 P0/P1; 정본 타입 SSOT — **`NormalizedEvent` + `PermissionBridge`(`permission.requested` 1급 이벤트)는 스테이지 B 로 구현 완료 (PR #47), 잔여 항목 설계 대기**)** | Main 작업할 때. SDK 호출 / 어댑터 / 영속성 / 자격증명 / IPC 핸들러 / 범용 정규화 계층 설계. |
| `IPC_CONTRACT.md` | **Main ↔ Renderer 채널 SSOT** — **총 71 채널 · 21 도메인** (0180 에서 auth 7 + plugin 4 제거) (skills 7 · session 7 · update 6 · project 6 · cost 6 · chat 6 · files 5 · engine 5 · mcp 4 · window 3 · settings 2 · permission 2 · install 2 · debug 2 · boot 2 · search 1 · notify 1 · log 1 · concurrency 1 · backend 1 · agent 1), NormalizedEvent variant 21종, ErrorCategory, 검증 실패 정책, 변경 절차. **분포·총계의 정본은 IPC_CONTRACT §2** — 본 표의 수치는 사본이므로 채널 변경 시 §2 를 먼저 고치고 여기를 맞춘다. | IPC 채널 추가/변경할 때. FRONTEND/BACKEND 가 이 문서를 인용. |
...
| `guides/closed-network-extensions.md` | **폐쇄망(사내) 확장 정본 — ⚠️ 0180 에서 확장점이 사라져 현재 코드와 맞지 않는다.** 0181 이 `Provider` 선언 하나를 축으로 재작성한다. 그전까지 이 문서를 근거로 코드를 붙이지 마라 | (0181 재작성 대기) |
