...
| [providers.md](./arch/backend/providers.md) | **Provider 플랫폼(인증) 정본 (0180 제거 → 0181 재작성)** — 왜 지웠나(축의 교차 진단·제거 인벤토리·살려둔 4종)·`Provider` 단일 축(`kind:'gate'\|'llm'\|'service'`)·`AuthSpec` 5분기(OAuth code→token 포함)·`Grant`·모듈 지도·라이프사이클(부팅/로그인/재인증/만료)·**등록하는 법**·**쓰는 법**(`ProviderApi` 소비 표면 4종)·게이트 진리표·뒤집으면 안 되는 결정 |
