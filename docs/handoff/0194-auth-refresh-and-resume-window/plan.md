...
| 일자 | 2026-08-20 |
| 매핑 | 0193 후속 |
| 상태 | DRAFT → READY → IMPL_DONE (r1) → verify/FAIL (r1) → IMPL_DONE (r2) → verify/FAIL (r2) → IMPL_DONE (r3) → verify/FAIL (r3) → IMPL_DONE (r4) → verify/FAIL (r4) → plan/READY (r5) → IMPL_DONE (r5) → verify/PASS (r5) |
...
| D21 | **만료가 재시작을 넘게 되면서 입력형(값형) 연결은 자동 회복 경로가 0이 된다.** `status==='expired'` 인 secret grant 는 refresh 가 `unsupported`(`login.ts:374` — `kind!=='token'`)이고 `AUTO_RELOGIN_KINDS`(`auth-resume.ts:56`)가 재로그인을 막으며, `policy.ts:68-70` 이 요청을 막아 probe 도 낼 수 없다 | verify r5 §2 — 검증자의 producer→JSON→파서→`status()` 왕복 2케이스(`markExpired` 가 쓴 `expiresAt` 이 재시작 후에도 `expired` 를 낸다) | **사용자 결정.** 이전에는 재시작이 강등을 잊어 일시적 401·망 장애로 강등된 api-key 가 부팅 한 번에 되살아났다 — 그 회복은 **필드 유실의 부산물**이라 의도가 아니었다. ⓐ 현행 유지(사용자가 값을 다시 입력) ⓑ 부팅 1회에 한해 값형 만료 grant 의 probe 를 허용(정책 예외가 필요해 한 줄이 아니다) — 검증자가 고르지 않는다 | **사용자 결정 대기** |
| D22 | **`auth.md:119` 의 `expiresAt … secret 은 없다` 가 거짓이다.** `runtime.ts:142` 는 갈래를 가리지 않고 `grant.expiresAt` 을 snapshot 에 싣고, `ProviderDetail.tsx:99-102` 가 그것을 `만료` 로 그린다 — 값형에서 그 값은 **강등 시각**이다 | verify r5 §9 — 표 §4.1 은 AC20 이 이름 붙인 세 문서(§5.2·guide §3-b·IPC_CONTRACT) 밖이라 이번 라운드 채점 대상이 아니었다 | 문면을 `발급 시점 만료는 토큰만 선언한다 — 값형·세션은 강등이 채운다` 로 고친다. 0194 이전에도 실행 중에는 참이었고, 이번 수정으로 **재시작을 넘어** 보인다 | 보고만 |
