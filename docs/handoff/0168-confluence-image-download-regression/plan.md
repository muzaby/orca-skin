...
| # | 이슈 | 출처 | 대응 방향 | 상태 |
|---|---|---|---|---|
| D1 | **진단 출력에 상한이 없다.** 새 `unreferenced` 나열이 첨부 목록 상한(`ATTACHMENT_LIST_LIMIT=200`) × 배치 상한(`MAX_PAGES_PER_CALL=50`)까지 무제한으로 모델 컨텍스트에 실린다. 같은 파일이 본문 미리보기는 `PREVIEW_CHARS=4000` 으로 **일부러 자르고 있다** | verify r1 §0 | 모델에게 나가는 이름 수를 상수로 제한하고, 잘리면 그 사실 + 전체 개수를 문장에 남긴다. **`manifest.json` 은 전량 유지**(디스크는 컨텍스트 비용이 없다). 상한을 잠그는 테스트 추가 | **해결 (r2)** |
| D2 | `uniqueName` 충돌 시 본문 링크 desync — 같은 위생화 이름 2건이 `x-1.png` 로 저장되면 markdown 은 `x.png` 를 가리킨다 | 구현자 코멘트 #4 (⚠️ 보고만) · verify r1 | 이번 변경이 만든 결함이 아니고 저장 파일명 규칙 변경이 필요하다 — **후속 핸드오프 후보**. 이 라운드의 액션 아님 | open (비범위) |
