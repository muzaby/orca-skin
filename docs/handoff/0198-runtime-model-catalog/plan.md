...
| D-008 | runtime contribution fetch는 Gate 로그인 인증 성공 시 contribution별 1회만 수행하고, 새 세션 생성·턴 실행은 cache만 읽는다. Auth 밖 설정 변경이 cache를 무효화하면 같은 canonical key의 settings 행을 포함한 자동 항목 전체를 두 UI에서 숨긴다. **fetch 횟수 조항은 0202 D-004가, "Gate 로그인" 한정은 0202 D-008이 대체한다** — 명시적 invalidate 1회당 replay 1회가 추가되고, 트리거는 Gate뿐 아니라 부팅 복원 batch가 만든 `verified` 전이 전부다. cache-only 읽기와 미노출 조항은 그대로 ACTIVE다. | Gate 인증을 권위 트리거로 쓰며 세션마다 network 요청할 수 없다. 사용자는 D18에서 재fetch보다 fail-closed 미노출을 선택했다. | 사용자 후속 결정 (2026-08-24) + D25 정정 | **부분 SUPERSEDED** | 0202 D-004 (fetch 횟수) · 0202 D-008 (Gate 한정) |
