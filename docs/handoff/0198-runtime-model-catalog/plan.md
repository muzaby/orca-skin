...
| D-010 | 컴포지션 seam(`bootstrap.ts`가 helper에 무엇을 넘겼는가)은 순서 단언이 아니라 **유일성 가드**로 강제한다 — `auth.subscribe(`를 부르는 production 파일은 하나이고 배포 선언은 변형 없이 넘어간다. 순서·동작은 주입 테스트가 계속 맡는다. | `bootstrap.ts`는 `@electron-toolkit/utils`의 CJS `require('electron')` 때문에 vitest가 열지 못한다(verify r7 §8 실측). 저장소는 같은 문제를 음성 소스 스윕으로 이미 두 번 풀었다(`no-node-fetch.test.ts`·`no-cookie-token.test.ts`). | 설계 정정 (verify r7 D41, 사용자 결정 아님) | ACTIVE | — |
...
- **컴포지션 유일성(D-010)**: 순서는 helper 안에서 주입 관측으로 잠기고, "`bootstrap.ts`가 그 helper를 실제로 쓰는가"는 **음성 스윕** 두 술어가 잠근다 — `src/main`에서 `auth.subscribe(` 호출 파일 1개(`runtime-model-startup.ts`)·`RUNTIME_MODEL_CONTRIBUTIONS.` 0건. 두 술어 모두 현재 코드에서 이미 참이거나(전자, 실측 2호출 전부 helper 안) 이번 라운드가 참으로 만든다(후자, 현재 `bootstrap.ts:465`가 `.filter(...).map(...)`). 가드 자신은 `infra/source-scan.ts` 관용구를 따르고 **판정 지점 3종**(대상 집합·술어 2)에 자기 테스트를 둔다.
...
| 부팅 시퀀스 순서 | `startRuntimeModelCatalogAfterDeploy` 5단계 + `createRuntimeModelAuthResume` 유일 설치 | 컴포지션 루트(`bootstrap.ts`) · 음성 가드 | helper 내부 5단계 순서 1지점 · **listener 설치 유일성** 1지점(`src/main` 전체에서 `auth.subscribe(` 호출 파일 = `runtime-model-startup.ts` 1개) = 2지점 | invalidate가 attach 뒤로 가면 Gate fetch가 지워지고(r5 D24), attach가 subscribe 뒤로 가면 부팅이 network를 await한다. 호출 자체의 소멸은 `noUnusedLocals`가 typecheck를 red로 만든다(verify r7 M-J) |
...
| `app/no-stray-auth-subscribe.test.ts` **(r8 신규)** | 컴포지션 유일성 가드 | `infra/source-scan.ts`로 `auth.subscribe(` 호출 파일 1개·`RUNTIME_MODEL_CONTRIBUTIONS.` 0건을 단언하고, 대상 집합·술어 2에 가드 자기 테스트를 둔다 | 순수(node) |
