...
> **[구현자 기입] r3** — ΔV2 구현 + D7 재구현. r1·r2 필드는 위에 그대로 두고 같은 이름의 필드를 다시 채운다.

## [구현자 기입] 설계 리뷰 (r3)

- 동의 / 그대로 진행: ΔV2 의 두 판단이 다 맞았다 — **브랜드가 r2 에서 무음이던 G4 원본 변이를 `TS2322` 로 잡고**(M15), **양성 짝이 r2 에서 미검출이던 D7 원본 변이를 잡는다**(M17).
- 이견 / 현실성 문제: **없음.** 다만 §7-C 가 적은 EP-11 분모 `3` 은 실측과 다르다 — 아래 #1.
- ACTIVE Decision 과 충돌하는 설계 발견: 없음. **D-013 유지 확인** — 추가한 의존성 0(`react-dom/server` 는 이미 devDependency, 브랜드는 타입).

## [구현자 기입] 강제 지점 전수 (§10 대조) (r3)

| Pair | 계약/필드 | §10이 적은 지점 | 닫은 지점 | 재현 명령 / 관측 | 남긴 곳 |
|---|---|---|---|---|---|
| VP-18·05b | 구획 목록 prop 은 슬롯 브랜드를 요구한다 | `EP-11` (3) | **5/5** | 술어 = *구획 컴포넌트에서 세션 목록을 나르는 prop 선언*. 브랜드 요구 **5**(`PinnedSection:11`·`SessionList:22`·`PinnedProjectsSection:19`·`:66`·`:162`) · 평범한 `SessionListItem[]` 로 남은 것 **0** | — |
| VP-18 | 브랜드는 파티션이 부여한다 | `EP-12` (1) | 1/1 | `grep "as PinnedSessions\|as RecentSessions\|as ProjectChildSessions"` 프로덕션 **3건 전부 `navSections.ts:43·49·61`** — `splitNavSections` 밖 **0** | — |
| VP-13·05b (회귀) | 한 대화는 정확히 한 구획 | `EP-1a` (1) | 1/1 | 브랜드 도입 **후** 고정 필터 제거 → **4 failed / 499**. 잠금이 살아 있다 | — |
| VP-18·05b | 구획은 props 를 재파생하지 않는다 | `EP-9` (3) | **3/3** | r2 는 2/3 이었다(프로젝트 지점이 접힘 뒤라 공허). 이제 `PinnedProjectChildren` 을 직접 렌더해 **M17 이 red** | — |
| VP-02a·17 | 고정 프로젝트 파생은 순수 함수 | `EP-10` (1) | 1/1 | `pinnedAt != null` 프로덕션 5건 중 프로젝트 목록 필터는 `navSections.ts:60` 1건 | — |
| VP-14·03·04·11·12 | `V1` 승계 5지점 | `EP-4`~`EP-8` (5) | 5/5 | typecheck 0 · renderer 499 전건 통과로 회귀 없음 | — |

**전수 = 16/16.** **plan §7-C 의 유효 전수 `14` 는 EP-11 을 `3`(구획 수)으로 셌다** — 불변식의 주어인 *목록을 나르는 prop 선언*으로 세면 **5**다(프로젝트 축이 record·row·children 3단). 분모를 설계자 값에 맞추지 않고 전수 검색으로 다시 셌다 → #1.

**V-pair 자기확인 (r3)** — `SELF_PASS 10 / SELF_BLOCKED 4`.

| Pair | requiredness | 자기 상태 | 직접 관측 | 선택된 적대 증거 결과 |
|---|---|---|---|---|
| VP-18 | REQUIRED | SELF_PASS | 렌더 양성+음성 쌍 전건 · `@ts-expect-error` 3줄 | **M14′ 3× `TS2578`**(잔여물 0 상태) · **M15 `TS2322`** · **M16·M17 red** |
| VP-05b | REQUIRED | SELF_PASS | 파티션 + 렌더 + 이음매 타입 셋이 경로 전 구간을 덮는다 | M14′·M15·M16·M17 공유 |
| VP-13·14·17·02a·06a·07a | INHERITED(회귀) | SELF_PASS | ΔV2 가 `navSections.ts` 를 건드렸으므로 재확인 — 파티션 12케이스 통과 + EP-1a 변이 4 red | 위 EP-1a 행 |
| VP-12·03·08 | INHERITED | SELF_PASS | renderer 499 전건 통과 | not selected |
| VP-01·04·09·10·11·15 | REQUIRED | SELF_BLOCKED | 시각·IPC 실기(Electron GUI 부재). AC9 의 빈 헤더 절반·AC3 은 렌더로 닫혔다 | 해당 없음 |

## [구현자 기입] 이번 라운드 수정의 잠금 (r3)

| 심은 결함 | 출처 | 실패한 테스트 / 케이스 수 | 결과 |
|---|---|---|---|
| **M14′** 브랜드 3개를 평범한 별칭으로 되돌리고 **잔여물(`Slot`·`slot`)까지 제거** | **VP-18 등록 변이** | **`TS2578` 3건** (`navSections.render.test.ts:179·181·193`) | 잠김 — 1단계에선 `TS6196` 잔여물이 섞여 4건이었다. **잔여물 0 상태로 판정**했고 3건이 실제 잠금이다 |
| **M15** 어댑터가 `pinned` 대신 `recent` 를 넘긴다 | **G4 원본 변이**(r2 에서 무음) | **`TS2322` 1건** | 잠김 — **r2 에서 vitest 497 초록·typecheck 0 이던 바로 그 결함이다** |
| **M16** 하위 목록이 아무것도 그리지 않는다 | AT-13a 양성 방향 | **2건** | 잠김 — 양성 짝이 없으면 통과했을 변이다 |
| **M17** 하위 목록이 준 적 없는 대화를 끌어온다 | **D7 원본 변이**(r2 에서 미검출) | **1건** | 잠김 — **r2 에서 497 전건 통과하던 바로 그 결함이다** |
| **EP-1a 회귀** 브랜드 도입 후 고정 필터 제거 | §5.3 — 새 표면 위의 기존 잠금 | **4건** | 잠김 — 브랜딩이 기존 잠금을 약화하지 않았다 |

- 신설 oracle 의 production 경로 진입: `@ts-expect-error` 3줄은 **프로덕션 컴포넌트의 prop 타입**(`PinnedSectionView`·`PinnedProjectChildren`·`SessionListView`)에 직접 대입한다 — 동명 로컬 재구현이 아니다. 브랜드는 `splitNavSections` 가 실제로 반환하는 타입이다.
- **`@ts-expect-error` 는 다음 라운드에도 남는 눈이다** — 한 번 심었다 지우는 변이와 달리 오류가 사라지는 순간 `typecheck` 가 스스로 깨진다(선례 `src/shared/obj.test.ts:36`).

## [구현자 기입] Product/UX 파생 검토 (r3)

| 질문 | 판정 | 후속 |
|---|---|---|
| 새로 만든 사용자 대면 문구·상태에 소비자가 있는가 | ✅ 신규 문구 0 | 이번 변경은 타입과 테스트뿐이다. 렌더 출력은 r2 와 동일 |
| 이번에 만든 실패 경로가 Part I 상태 전이표의 어느 행인가 | **새 실패 경로 없음** | 브랜드는 컴파일 타임에만 산다 — 런타임 분기 0 |
| 실패가 화면에서 "아무 일도 안 일어남"으로 보이지 않는가 | ⚠️ 변화 없음 | 조회 실패가 여전히 "대화 없음"으로 보인다 — D2 유지 |
| 늦게 도착한 응답이 화면을 되돌리지 않는가 | ⚠️ 변화 없음 | `loadProject` 세대 토큰 부재 — r1·r2 와 동일 |

## [구현자 기입] 놓친 잠재 문제 + 대응 (r3)

| # | 문제 | 대응 | 근거 |
|---|---|---|---|
| 1 | **§7-C 의 EP-11 분모 `3` 이 실측 `5` 와 다르다** — 프로젝트 축이 record·row·children 3단이라 목록을 나르는 prop 선언이 5개다 | ✅ **선조치(전수 재계수)** — 설계자 분모에 맞추지 않고 5/5 로 닫았다. 규범 행 정정이 필요하면 설계자 몫이나, **닫힌 지점이 더 많으므로 계약 위반은 아니다** | 술어 재검색: 브랜드 요구 5 · 평범한 `SessionListItem[]` 0 |
| 2 | `@ts-expect-error` 가 **여러 줄 호출을 덮지 못한다** — 초안의 세 번째 지시자가 `TS2578` 로 뒤집혔다 | ✅ **선조치** — 나머지 props 를 먼저 묶어 호출을 한 줄로 좁혔다. 주석에 이유를 남겼다 | 초안 typecheck: `(188,5) TS2578` + `(190,7) TS2769` |
| 3 | 브랜드 도입으로 **기존 렌더 테스트 fixture 가 전부 타입 오류**가 됐다(`TS2769` 3건) | ✅ **선조치** — `asPinned`·`asRecent`·`asChildren` 캐스트 헬퍼 3개를 테스트에만 두었다. 프로덕션에는 이 경로가 없다(EP-12 전수 3건이 전부 `splitNavSections` 안) | 브랜딩 직후 typecheck 4건 |
| 4 | **음성 단언 7건 중 4건에 양성 짝이 없었다** — D7 이 지적한 프로젝트 구획 하나가 아니다 | ✅ **선조치(불변식 전수 적용)** — `:100`(프로젝트)·`:108`(최근)·`:112`·`:113`(빈 목록) 넷 모두에 양성 짝을 붙였다 | 전수 검색 `not.toContain\|not.toMatch` → 7건, 짝 있음 3 · 없음 4 |
| 5 | 브랜드가 **모든 대입을 막지는 않는다** — `SessionListItem[]` 를 브랜드 타입 자리에 넣는 것은 여전히 오류지만, 그 반대(브랜드 → 평범한 배열)는 통과한다 | ⚠️ **보고만** — 의도한 방향이다. 막아야 할 것은 *칸 혼동*이지 소비가 아니다. `SessionRow` 가 개별 항목을 받는 경로는 브랜드 밖이다 | 타입 정의상 교집합(`&`)이라 상위 대입은 허용 |

### 설계 대비 명시적 차이 (r3)

- plan 이 지정한 것과 다르게 구현한 것: **없음.** §7-C 의 구현 설계 4행(브랜드 정의·부여 · 세 View prop · 렌더 테스트 3항 · `PinnedProjectChildren` export)을 그대로 수행했다. 유일한 차이는 **분모**(#1)이고 설계 메커니즘 변경이 아니다.

| 축 | 대체물에만 있는 실패 모드 | 재확인한 AC·§10 행 / 관측 |
|---|---|---|
| 만료 | 해당 없음 — 브랜드는 컴파일 타임 전용이라 런타임 수명이 없다 | — |
| 공유 (누가 함께 쓰고 누가 비울 수 있는가) | 해당 없음 — 타입은 상태가 아니라 공유·소거 주체가 없다. 다만 **캐스트로 만들 수 있는 주체**는 존재하고 그것이 EP-12 다 | EP-12 재확인 — 프로덕션 캐스트 3건 전부 `splitNavSections` 안, 밖 0 |
| 재진입 | 해당 없음 — 런타임 분기 0 | — |
| 다른 무효화 축 | **테스트 캐스트 헬퍼가 브랜드를 우회한다** — fixture 는 아무 배열이나 브랜드로 만들 수 있다 | 의도한 범위다. 프로덕션 어댑터가 받는 값은 항상 `splitNavSections` 산출이라 캐스트 경로가 없다(EP-12 관측). **테스트가 자기 fixture 를 잘못 만들면 그 테스트가 틀리는 것이지 프로덕션 이음매가 열리는 것은 아니다** |

## [구현자 기입] 구현 보고 (r3)

| 항목 | 내용 |
|---|---|
| 변경 파일 | **5** — `lib/navSections.ts`(브랜드 정의·부여) · 세 구획 컴포넌트(prop 타입 + `PinnedProjectChildren` export) · `components/navSections.render.test.ts`(양성 짝 4건 추가 · 하위 목록 직접 렌더 · `@ts-expect-error` 3줄) |
| 실행 명령 | `npm run lint` · `npm run typecheck` · `./node_modules/.bin/vitest run src/renderer` · `./node_modules/.bin/vitest run` |
| **관측한 게이트 산출**(exit code 아님) | lint **0 error / 1 warning**(`useTranscriptVirtualizer.ts:22`, nav 무관·기존) · typecheck **`error TS` 0건** · renderer vitest **63파일 499케이스 전건 통과**(r2 497 → **+2**) · 전체 vitest **2349 중 48 실패** |
| 환경 기인 실패 분리 | 48건은 r1·r2 와 **같은 5파일**(`chat-turn.continuity`·`builder`·`fork`·`migrate`·`queries`), better-sqlite3 ABI. renderer 499 전건 통과로 분리된다 |
| 게이트가 트리를 바꿨는가 | `npm run lint`(`--fix`) 실행 후 변경 파일은 **내 5개뿐** — 남의 파일 0 |
| V-pair 자기확인 | `SELF_PASS 10 / SELF_BLOCKED 4`; pair 별 상세는 위 표 |
| 강제 지점 전수 | **16/16** (EP-11 을 설계자 분모 3 이 아니라 실측 5 로 셈, #1) |
| **AC 자기보고**(`Criteria-Met`) | **10/15.** ✅ AC2a · AC3 · AC5a · AC6a · AC7a · AC8 · AC11 · **AC13a**(양성 짝 전건 + M16·M17) · AC14 · **AC15**(신설, `@ts-expect-error` 3줄 + M14′) |
| **합계 검산** | 표의 ✅ 를 세어 **10**, 현재 AC 총수를 다시 세어 **15**(ΔV2 가 AT-15 신설, AT-13→13a 는 번호 승계) → `✅ 10 · ⚠️ 5 · ❌ 0 = 총 15`. ⚠️ = AC1·AC4·AC9·AC10·AC12(전부 시각 실기). **분모가 14→15 로 바뀌어** r2 의 `9/14` 와 직접 비교하지 않는다 |
| 블로커 / 역질문 | **없음.** #1(EP-11 분모)은 닫힌 지점이 설계자 값보다 **많아서** 생긴 차이라 blocker 가 아니다 |
| 대상 커밋 | `(r3 구현 — 좌표는 INDEX)` |

## [구현자 기입] Review Signals — 사실만 (r3)

- 이번에 닫은 불변식이 이전 라운드와 같은 축인가: **그렇다 — 네 번째 변주다.** r1 = 배치 규칙 미잠금 · r2 = 렌더 oracle 이 자기 범위를 덜 덮음 + 두 oracle 사이 이음매 · r3 = 그 둘을 각각 양성 짝과 타입으로 닫았다. 축은 매번 **"장치가 주장하는 범위 ≠ 실제로 반응하는 범위"**.
- 그것을 막았어야 할 plan 지침·AC 가 있었는가: `handoff-plan §5` 의 **음성 게이트 양성 짝 규칙**("음성 게이트는 '없다'만 잠근다 — '있다/쓰인다'가 불변식이면 양성 단언과 짝지어야 잠긴다")이 r1 plan 작성 시점부터 있었다. ΔV1 의 AT-13 이 그 규칙을 구획별로 적용하지 않아 r2 에서 열렸다.
- 반복해서 부딪히는 환경 한계: **3건 유지** — ① better-sqlite3 ABI(48케이스) ② Electron GUI 부재(시각 5 AC) ③ store 구독 컴포넌트의 SSR 한계(이번에 **타입으로 우회**했다).
- 현재 라운드 수: **3** — `docs/handoff/AGENTS.md §handoff-review 트리거` 의 "impl 라운드가 3을 **초과**"에 아직 못 미친다. r4 가 열리면 재구현 전에 review 를 수행한다.
