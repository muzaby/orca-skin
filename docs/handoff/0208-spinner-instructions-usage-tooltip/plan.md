...
# Appendix D — ΔV1 r3 구현 기록

> 사용자가 `npm test` red 를 보고해 연 라운드다. r2 산출을 되돌리지 않고, **같은 코드가 OS 에
> 따라 다른 결과를 내던 두 축**만 닫는다.

## [구현자 기입] 설계 리뷰

- 동의 / 그대로 진행: Part I·Part II 전부. r2 의 런타임·문구 산출은 이번 라운드에서 바뀌지 않는다.
- **PLAN_GAP 1건 → 설계 정정 선행.** AT-21 은 원본의 바이트 수·SHA 를 단언하는데, 그 바이트가
  **체크아웃 설정에 좌우된다**는 사실을 §10 이 지점으로 갖지 않았다. `.gitattributes` 가 그
  지점이라 EP-09 의 분모가 2 로는 닫히지 않는다. 규범 행(D-021 · EP-09 · AT-21 · ΔVP-01)을
  **구현과 분리한 설계 커밋**으로 먼저 정정했다(`897cbcf`, `Status: designed`).
- 사용자 결정: 고정 방식은 **LF** 다. 테스트가 CRLF 를 정규화해 받아주는 대안은 선택되지 않았다.
- ACTIVE Decision 과 충돌하는 설계 발견: 없음. D-021 은 `app/.editorconfig` 의 `end_of_line = lf`
  와 prettier 기본값이 **이미 선언한 것을 git 이 강제**하게 만들 뿐 새 정책이 아니다.

## [구현자 기입] 강제 지점 전수 (§10 대조)

| Pair | 계약/필드 | §10이 적은 지점 | 닫은 지점 | 재현 명령 / 관측 | 남긴 곳 |
|---|---|---|---|---|---|
| ΔVP-01 | EP-09 원본 보존 (r3 정정으로 2→3) | reference 파일 1 + plan 상대 링크 1 + `.gitattributes` LF 고정 1 **(3)** | 3/3 | (1) `sha256sum` → `ee57259b…f79b`·`wc -c` → **54,553B**(r2 값 불변). (2) plan 의 `spinner-reference.svg` 등장 **11건**, `check-doc-inventory --check` → `links ok`. (3) `grep -n 'eol=lf' .gitattributes` → **2행**(`* text=auto eol=lf` · 원본 전용 고정). 고정이 먹은 결과 = 워킹트리 CR 보유 파일 **차집합 0** | — |
| ΔVP-02·05 | EP-10 원본→runtime | 4 | 4/4 (재확인) | r3 은 이 네 파일을 건드리지 않았다. `sparkFrames`·`sparkCss` 재실행 **28케이스 green** 으로 원본 파싱값과의 등호가 이번 턴에도 성립 | — |
| ΔVP-04 | EP-11 성능 | 4 | 4/4 (재확인) | 렌더러 전체 재실행 **79파일 665케이스 green**. leak 스윕은 술어의 **전수 공급원**이 바뀌었으므로 아래 잠금 표에서 따로 잰다 | — |
| ΔVP-06·07·08 | EP-12 안내 위치 | 7 | 7/7 (재확인) | 술어를 고친 AT-29 를 다시 실측: production 등장 **1건** — `UsageTab.tsx:132`(`grep -rn "'usage.estimateNote'" … | grep -v .test. | grep -v /i18n/` 차집합). 나머지 6지점은 재실행 green | — |

**ΔV1 합계 18/18** (r2 의 17 + EP-09 신설 1). **상속 13/13** 은 r2 관측이 유효하고 이번 턴 재실행에서 green — **총 31/31.**

- §10에 없는데 같은 불변식이 필요했던 지점: **1건** — 아래 `놓친 잠재 문제` #1(경로 구분자).
  계약이 아니라 **술어 위생**이라 PLAN_GAP 이 아니고 선조치했다(r2 #1 `codeOf()` 와 같은 분류).

**V-pair 자기확인**

| Pair | requiredness | 자기 상태 | 직접 관측 | 선택된 적대 증거 결과 |
|---|---|---|---|---|
| ΔVP-01 | REQUIRED | SELF_PASS | `sparkFrames.test.ts` AT-21 **3케이스**(신설 `.gitattributes` 고정 1 + 기존 2) | **검출** — 전역 규칙 제거(M24) 1 red · 원본 전용 규칙 제거(M25) 1 red · CRLF 체크아웃(M26) **4 red** |
| ΔVP-08 | REQUIRED | SELF_PASS | `usageTooltip.render.test.ts` AT-29 — 구분자 차집합 0 + 등장 1 | **검출** — Windows 구분자 재현(M27) **1 red**(값 비교) |
| 그 밖의 pair | — | SELF_PASS (재확인) | 렌더러 79파일 665케이스 green · 전체 249파일 2572케이스 green | r2 의 23변이 결과를 재사용하지 않고 **재실행 green** 만 주장한다 |
| ΔVP-09 | REGRESSION | **SELF_BLOCKED** | 사람 실기 — 헤드리스 | not selected — 시각·OS 폰트 |

## [구현자 기입] 이번 라운드 수정의 잠금

**심은 결함 4종 · 전건 검출.** 각 행은 변이 후 대상 스위트를 돌리고 **백업 사본에서 복구**한 실측이다.

| 심은 결함 | 출처 | 실패한 케이스 수 | 결과 |
|---|---|---|---|
| M24 `.gitattributes` 에서 `* text=auto eol=lf` 제거 | EP-09 신설 지점 | 1 | 잠김 |
| M25 원본 전용 `… spinner-reference.svg text eol=lf` 제거 | EP-09 신설 지점 | 1 | 잠김 |
| M26 원본을 CRLF 로 받은 워킹트리(사용자 보고 재현) | ΔVP-01 `CRLF 체크아웃` | **4** — 신설 고정 케이스 + AT-21 SHA + AT-23 변조 fixture 2 | 잠김 |
| M27 `walkSourceFiles` 가 Windows 구분자 값을 돌려줌 | 경로 구분자 불변식 | 1 (값 비교) | 잠김 |

- **M26 이 r2 의 침묵을 드러냈다.** CRLF 트리에서 AT-23 의 여러 줄 변조 fixture 2개는 패턴이
  `\r` 때문에 매치되지 않아 **변조가 적용되지 않은 채** 단언에 도달한다. 즉 그 두 케이스는
  "변조를 넣었는데 검출 못 함" 이 아니라 **"변조가 아예 들어가지 않음"** 이었다. 신설 케이스가
  `REFERENCE_TEXT` 의 CR 0건을 먼저 단언하므로, 이제 그 상황은 바이트 수 불일치보다 **원인을
  직접 가리키는 줄**에서 red 가 난다.
- **M27 의 충실도 한계를 적는다.** `win32.join` 으로 재귀까지 바꾸면 Linux 가 백슬래시 경로를
  `scandir` 하지 못해 **ENOENT** 로 죽는다 — 그건 Windows 동작이 아니다. 그래서 재귀는 그대로
  두고 **반환값만** Windows 가 돌려줄 형태로 바꿨다. 이 재현에서 `sparkCss` 2케이스도 red 가
  되지만 그 둘은 Linux 의 `readFileSync` ENOENT 이지 Windows 예측이 아니다 — **사용자의 실제
  Windows 실행에서 red 는 값 비교 1건뿐이었고**, 그것이 이 변이가 겨눈 축이다.

## [구현자 기입] Product/UX 파생 검토

| 질문 | 판정 | 후속 |
|---|---|---|
| 사용자가 관측하는 것이 달라졌는가 | ✅ **아니다** — 이번 변경은 `.gitattributes`·테스트 술어·`.editorconfig` 줄바꿈뿐이다. 렌더러 프로덕션 코드·i18n·CSS 는 diff 에 없다(`git status` 의 수정 5건 중 프로덕션 0건) | — |
| 이번에 만든 실패 경로가 Part I 상태 전이표의 어느 행인가 | ✅ 새 실패 경로 없음 — 런타임 I/O·타이머·상태를 추가하지 않았다 | — |
| 실패가 "아무 일도 안 일어남" 으로 보이지 않는가 | ⚠️ **개발자 대면 축에서 1건이었고 닫았다** — CRLF 트리에서 AT-23 변조 fixture 는 *조용히 적용되지 않아* "oracle 이 눈을 가졌다" 는 케이스가 아무것도 재지 않는 상태가 될 수 있었다. 신설 CR 단언이 그 상태를 먼저 red 로 만든다 | 잠금 완료(M26) |
| 새로 만든 규칙에 소비자가 있는가 | ✅ `.gitattributes` 의 두 규칙 모두 신설 테스트가 값으로 단언한다(M24·M25 각 1 red). producer 만 남고 아무도 안 보는 규칙이 아니다 | — |

## [구현자 기입] 놓친 잠재 문제 + 대응

| # | 문제 | 대응 | 근거 |
|---|---|---|---|
| 1 | **경로를 값으로 비교하는 술어가 OS 구분자에 좌우된다** — 사용자 Windows 실행에서 AT-29 가 `features\settings\components\UsageTab.tsx` 로 red. 내가 Linux 에서 CRLF 만 재현했을 때는 **보이지 않던 축**이다 | ✅ 선조치 — 불변식을 *"저장소 상대경로를 값으로 다루는 술어는 OS 구분자에 좌우되면 안 된다"* 로 올리고, 지점을 지점 수만큼 닫았다. `walk()` 사본 **2곳**(`usageTooltip.render.test.ts` · `sparkCss.test.ts`)을 `sourceScan.testlib.ts` 의 `walkSourceFiles()` **한 곳**으로 합쳐 POSIX 구분자를 고정했다. 잔여 `join(dir, e.name)` **차집합 0** | 한 곳만 고치면 다른 사본이 다음에 같은 자리에서 틀린다 — r2 #1 이 `codeOf()` 로 같은 판단을 한 선례 |
| 2 | 사본만 고치고 **원본을 면제**할 뻔했다 | ✅ 빨간 곳은 `usageTooltip` 하나였지만 `sparkCss` 의 walk 도 같은 조립을 갖는다. 그쪽은 확장자 판정만 해서 *우연히* 통과하던 것이라 함께 옮겼다 — 술어가 OS 를 보는 것은 red 여부와 무관하게 같은 결함 | §5 "불변식을 낳은 문장 자신부터" |
| 3 | `app/.editorconfig` 가 저장소의 **유일한 CRLF 블롭**이었다 | ✅ 선조치 — 자기가 `end_of_line = lf` 를 선언하면서 스스로 CRLF 였다. LF 로 정규화해 `.gitattributes` 적용 시 나중에 놀랄 diff 를 없앴다(워킹트리 CR 차집합 0) | `git grep -I --cached -l $'\r'` → 정규화 전 1건 |
| 4 | `.gitattributes` 가 **바이너리를 텍스트로 오판**할 위험 | ✅ 선조치 — `*.png`·`*.ico`·`*.webp` 를 `binary` 로 못박아 `text=auto` 자동 판정에 맡기지 않았다. 저장소의 바이너리는 이 3종뿐(`git ls-files` 확장자 차집합) | 오판하면 아이콘·webp 가 손상된다 |
| 5 | **이미 CRLF 로 받아둔 작업 트리는 `.gitattributes` 만으로 낫지 않는다** | ⚠️ 보고 — git 은 다음 체크아웃부터 규칙을 적용한다. 기존 트리는 재정규화가 필요하다: `git rm --cached -r . && git reset --hard` (또는 새로 clone) | 사용자 환경에서 red 가 남으면 이 절차가 먼저다 |
| 6 | CI(windows-latest)는 왜 red 가 아니었는가 | ⚠️ 보고 — GitHub Windows 러너는 `core.autocrlf` 를 켜지 않아 LF 로 체크아웃한다. 즉 **이 결함은 CI 가 구조적으로 못 보는 축**이고 개발자 로컬에서만 났다. `.gitattributes` 는 양쪽을 같은 바이트로 만든다 | 사용자 로컬 red ↔ CI green 의 차이가 그 증거다 |

### 설계 대비 차이

- plan 이 지정한 것과 다르게 구현한 것: **1건.** §11 은 테스트 헬퍼로 SVG 파서만 지시했는데,
  `sourceScan.testlib.ts` 에 `walkSourceFiles()` 를 **하나 더** 뒀다(위 #1 의 선조치). r2 가 같은
  파일에 `codeOf()` 를 둔 것과 같은 자리·같은 이유다.
- 신규 의존성: **0**. 새 패키지 없음.
...
| 축 | 대체물에만 있는 실패 모드 | 재확인한 AC·§10 행 / 관측 |
|---|---|---|
| 만료 | **없다** — `.gitattributes` 는 정적 규칙이고 시간에 따라 상태를 잃지 않는다 | 해당 없음 |
| **공유** (누가 함께 쓰고 누가 비울 수 있는가) | **있다 — 두 곳.** (a) `* text=auto eol=lf` 는 **저장소 전역**이라 이 handoff 밖의 모든 파일에 걸린다. 누가 뒤에 더 좁은 규칙을 덧붙이면 원본만 예외가 될 수 있다. (b) `walkSourceFiles()` 는 이제 **두 테스트가 공유**하므로 한쪽 편의로 반환 형태를 바꾸면 다른 쪽 전수가 조용히 달라진다 | (a) 그래서 원본 전용 고정 행을 **따로** 뒀고 신설 케이스가 **두 행을 각각** 단언한다(M24·M25 가 각 1 red 로 분리 검출). (b) 호출부 전수 = **2**(`grep -rn 'walkSourceFiles'` → 정의 1 + import 2 + 호출 2), 양쪽 다 이번 턴에 재실행 green |
| 재진입 | **없다** — 파일 시스템 읽기뿐이고 상태를 쌓지 않는다 | 해당 없음 |
| 다른 무효화 축 | **있다 — 하나.** `.gitattributes` 는 **다음 체크아웃부터** 적용된다. 이미 CRLF 인 트리에서는 규칙이 존재해도 파일이 CRLF 다 — 규칙 단언만으로는 그 상태를 못 본다 | 그래서 신설 케이스가 규칙 2행 **과** `REFERENCE_TEXT` 의 CR 0건을 **둘 다** 단언한다. M26 에서 규칙은 그대로 두고 트리만 CRLF 로 만들어 red 를 확인했다(위 #5) |

## [구현자 기입] 구현 보고

| 항목 | 내용 |
|---|---|
| 변경 파일 | 신규 1(`.gitattributes`) · 수정 4(테스트 3 + `app/.editorconfig`) · 프로덕션 코드 **0** |
| 실행 명령 | `npm run lint` · `npm run typecheck` · `npx prettier --check ./src` · `npm test` · `node scripts/check-doc-inventory.mjs --check` |
| **관측한 게이트 산출**(exit code 아님) | lint **0 error · 1 warning**(`useTranscriptVirtualizer.ts:22` react-compiler — 이번 diff 밖, r2 와 같은 건) · typecheck **3구성 전부 출력 0줄, exit 0** · prettier **1 warn = `src/main/AGENTS.md`**(이번 diff 가 안 건드린 파일, `git status` 미변경) · **`npm test` = vitest 249파일 2572케이스 전건 green + `node --test scripts` 59/59 pass, exit 0** · doc-inventory **3검사 ok, exit 0** |
| 환경 기인 실패 분리 | **이번 환경에서는 0건.** egress 가 열려 electron 바이너리·better-sqlite3 Node ABI 가 정상 설치돼 r2 가 분리 보고한 DB 로드 5파일 48케이스도 green 이다. (r2 의 그 red 는 차단 환경 기준선이었고 코드 무관이라는 판정이 이번 실행으로 확인됐다.) |
| V-pair 자기확인 | `SELF_PASS 10 / SELF_BLOCKED 1` — 상세는 위 표 |
| 강제 지점 전수 | **ΔV1 18/18 + 상속 13/13 = 31/31** (EP-09 가 2→3) |
| 심은 결함 | **4종 전건 검출**(M24~M27) |
| **AC 자기보고**(`Criteria-Met`) | **18/19** — AT-21 은 신설 지점까지 3케이스로 닫았고, AT-22~AT-29 와 상속 AT-01~04·AT-10~14 는 재실행 green 으로 재확인. **AT-30 은 ⚠️**(사람 실기, 헤드리스라 앱 미기동) |
| **합계 검산** | `✅ 18 · ⚠️ 1 · ❌ 0 = 총 19`. **분모는 r2 와 같은 19** — 이번 라운드가 AC 를 더하거나 가르지 않았다(AT-21 의 *케이스* 가 2→3 으로 늘었을 뿐 AC 수는 그대로다) |
| 블로커 / 역질문 | 없음. 사용자 트리가 이미 CRLF 라면 재정규화(위 #5)가 선행돼야 red 가 사라진다 |
| 대상 커밋 | `(r3 구현 — 좌표는 INDEX)` |

## [구현자 기입] Review Signals — 사실만

- **이번에 닫은 불변식이 이전 라운드와 같은 축인가: 예 — r2 의 (b) 축이 한 단계 넓어졌다.**
  r2 는 *"원문 술어는 코드를 봐야지 산문을 보면 안 된다"* 를 닫았다. r3 의 두 결함은 같은 술어들이
  이번엔 **실행 환경**을 본 것이다 — 줄바꿈(CRLF)과 경로 구분자(`\`). 한 문장으로 올리면
  **"술어는 대상을 봐야지 그것을 실행하는 환경을 보면 안 된다"** 이고, r2 의 주석 축은 그 특수형이다.
- **그것을 막았어야 할 plan 지침·AC 가 있었는가: 2건.**
  - (a) AT-21 의 oracle("54,552 bytes·업로드 SHA 동일")은 **무엇의** 바이트인지를 정하지 않았다 —
    커밋된 blob 인지 체크아웃된 워킹트리인지. 둘이 갈릴 수 있다는 것이 §10 에 지점으로 없었다.
    이번에 D-021 · EP-09 3번째 지점으로 규범화했다.
  - (b) AT-29 의 oracle 은 r2 에서 "파일 목록 → 등장 횟수" 로 한 번 강화됐지만, **비교하는 값이
    OS 마다 다르다**는 축은 그때도 지금도 §10 에 없다. 술어 위생으로 선조치했다.
- 이번 턴에 만든 장치 중 방향이 틀린 것: **1건** — M27 의 첫 시도(`win32.join`)는 Linux 에서
  ENOENT 로 죽어 재현이 아니라 파손이었다. 반환값만 바꾸는 형태로 고쳐 다시 쟀다.
- 반복해서 부딪히는 환경 한계: **헤드리스라 시각 실기 불가**(AT-30·ΔVP-09). **개발자 로컬(Windows)
  과 CI·에이전트 환경(Linux/LF)의 차이는 이번에 축 자체가 드러났고 `.gitattributes` 로 닫았다** —
  다만 남은 축(경로 구분자)은 Linux 에서 자연 재현되지 않아 **변이로만** 확인할 수 있다.
- 현재 라운드 수: **3**
