...
| D7 | **[실기 회귀 · r3] 게이트 훅 콜백이 spawn 시점에 고정돼 두 번째 send 부터 commit fence 가 항상 어긋난다.** `wrapRequest` 가 `takeSteerFlush` 만 delegate 로 감싸고 `commitSteerFlush`/`rollbackSteerFlush` 는 spawn 요청의 클로저를 그대로 넘긴다 → take 는 **현재** 체인, commit 은 **spawn 체인**의 `lease.chainId` 를 본다. 결과: 배치가 stdin 으로 나갔는데(어시스턴트가 처리) 큐는 `submitting` 에 머물러 **정식 버블로 승격되지 않는다** | 사용자 실기 보고 + 로그(`haveUnconfirmed:false` ↔ `deliveryPendingCount:4`) | 세 콜백을 **모두** delegate 경유로 — `takeSteerFlush` 와 같은 패턴 | **해소(r3)** — `session-runtime.ts` wrapRequest·delegate + 회귀 1건(수정 없으면 실패 확인) |
| D8 | **`submitting` 이 confirm·orphan 어느 쪽에도 안 잡혀 영구 고착된다.** `confirm` 의 open 술어는 `submitted\|orphaned`, `orphanUnconfirmed` 는 `submitted` 만 — 그런데 `counts` 의 `isOpen` 은 `submitting` 을 **포함**한다. 그래서 갇힌 배치가 세션을 영구히 busy 로 만든다 | r3 원인 분석 | 체인 종료 시 `submitting` 도 orphaned 로 — push 실패는 이미 rollback 됐으므로 남은 것은 "보냈는데 fence 어긋남" 뿐이다 | **해소(r3)** — `pending-message-queue.ts` + 회귀 2건 |
| D9 | **영수증(echo)보다 장부(commit fence)를 우선해 실물 전달 증거를 버렸다.** uuid 는 우리가 만들어 push 한 값이라 되돌아온 것 자체가 전달 증거인데, `confirm` 의 open 술어가 `submitting` 을 제외해 **확정을 거부**했다. D7 의 증상(모델은 답하는데 버블은 안 승격)이 그래서 회수 불가였다. 부수 효과로 **이중 전달 위험**도 있었다 — echo 를 받고도 `submitting` 이면 `takeForRespawn` 이 `state!=='confirmed'` 로 다시 이월한다 | 사용자 제안(“echo 를 확인하는 것은 어떤가”) | **uuid 로 지목된 신호**(echo·model-output)는 `submitting` 도 확정. **텍스트 폴백은 제외** — 아직 push 되지 않은 예약과 본문이 같으면 오확정한다 | **해소(r3)** — `confirm` 술어 분리 + 회귀 3건 |
