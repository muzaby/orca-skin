...
| # | 이슈 | 출처 | 대응 방향 | 상태 |
|---|---|---|---|---|
| D1 | 명시 결정 ⑦ "행 = 플러그인 패키지" 가 **0164 에서 커넥터 단위로 뒤집혔다**(사용자 지시). 결함이 아니라 결정 변경 — 그 결정의 *전제*(패키지가 provider+connector 를 함께 담는다)를 고정하는 인수 기준이 없었던 것이 교훈. | verify r1 | 기록만 | 종결 |
| D2 | INDEX 의 `대상 커밋` `f055b14..7175e57` 이 **두 hash 모두 저장소에 존재하지 않는다**. | verify r1 §메타 | 실재 hash 로 정정 | **open** |
