...
|---|---|---|---|---|---|
| D1 | `read()` 인자를 `read(edit.filePath)` 로 바꿔도 12케이스 전건 통과 — EP-Δ1 의 "가드가 통과시킨 **그 경로**만" 축에 oracle 이 없다 | verify r1 · `VP-Δ4` · `§10 EP-Δ1` · AC6 | 가드와 read 가 같은 해석 경로를 쓰는지 단언하는 케이스 추가(상대 경로 입력) | BLOCKING | **r2 closed** — 인용 변이가 이제 red(케이스 `reader 는 가드가 통과시킨 그 경로를 받는다`) |
| D2 | `EDIT_PREVIEW_MAX_BYTES` 값 자체 단언은 동어반복 | verify r1 · 비귀속 | 기록만 | NON_BLOCKING | **r2 closed** — 기본 reader 가 그 상한을 경계로 쓰는지 보는 케이스로 교체 |
...
- 동의 / 그대로 진행: r1 과 같다 — 규범 행(Decision·AC·V node/pair·§10)은 손대지 않았다. r2 는 `D1`(BLOCKING)·`D2`(NON_BLOCKING)의 **증거만** 보강한다.
- 이견 / 현실성 문제: 해당 없음 — `D3`(§10 EP-Δ4 지점 수 1 → 4)는 설계자 권한이라 고치지 않았다.
- ACTIVE Decision 과 충돌하는 설계 발견: 없음.
...
| Pair | 계약/필드 | §10이 적은 지점 | 닫은 지점 | 재현 명령 / 관측 | 남긴 곳 |
|---|---|---|---|---|---|
| VP-Δ4 | 가드가 통과시킨 **그 경로**만 읽는다 | 미리보기 생성 (1) | 1/1 — r1 은 **호출 여부**만 닫았고 r2 가 **경로 동일성**을 닫는다 | 케이스 `reader 는 가드가 통과시킨 그 경로를 받는다` → `toHaveBeenCalledExactlyOnceWith(<WS>/nested/hello_world.ts)` | — |
| VP-Δ4 | 1 MiB 상한 | 읽기 (1) | 1/1 | 케이스 `기본 reader 가 EDIT_PREVIEW_MAX_BYTES 를 경계로 쓴다` → 경계값 본문 길이 = 상한, 상한+1 은 `null` | — |
| VP-Δ3·VP-Δ8 | 일치 1회 | 생성 (1) | 1/1 | r1 과 동일 — `edit-preview.ts:73` | — |
| VP-Δ1·VP-Δ5·VP-Δ7 | 렌더 우선순위 | `DiffBody` (1) | 4/4 | r1 과 동일. 검증자 재열거가 4지점을 확인했고 전부 red 관측(`D3` 는 지점 수 표기 정정 요청) | — |
| VP-Δ6 | 비영속 | writer (1) | 1/1 | r1 과 동일 | — |

- 전수 합계 **8/5** — §10 이 적은 5지점 + EP-Δ4 형제 3지점. r1 대비 새로 닫은 것은 EP-Δ1 의 **경로 동일성 축**이다.
- §10에 없는데 같은 불변식이 필요했던 지점: r1 과 같은 3건(EP-Δ4) — `D3` 로 이관돼 있다.

**V-pair 자기확인**

| Pair | requiredness | 자기 상태 | 직접 관측 | 선택된 적대 증거 결과 |
|---|---|---|---|---|
| VP-Δ4 | REQUIRED | SELF_PASS | `read` 인자 = `<WS>/nested/hello_world.ts`, main cwd 기준 경로가 **아님** · 상한 경계 2점 | required — D1 인용 변이 red(1케이스) · N1 red · N3 red |
| VP-Δ1·Δ2·Δ3·Δ5·Δ6·Δ7·Δ8 | REQUIRED | SELF_PASS | r1 관측 유지 — 이번 변경은 테스트 2건 추가·1건 교체뿐이라 프로덕션 경로 무변경 | r1 과 동일 |
| VP-R1·VP-R2 | REGRESSION | SELF_PASS | 0228 스위트 전건 통과 | not selected |
...
| 심은 결함 | 출처 | 이전 라운드 결과 | 실패한 테스트 / 케이스 수 | 결과 |
|---|---|---|---|---|
| `edit-preview.ts:70` — `read(path.resolve(roots.ws, …))` → `read(edit.filePath)` | `D1 인용 변이` | **green (r1 — 잠금 없음)** | `reader 는 가드가 통과시킨 그 경로를 받는다` 1건 | **잠김** |
| N1 `edit-preview.ts:68` — 가드 판정을 항상 통과로 | `VP-Δ4 선택 증거` | red (r1) | `workspace 밖 경로는 읽지 않는다` 1건 | 잠김 |
| N3 `edit-preview.ts:94` — 상한 확인을 읽기 뒤로 | EP-Δ2 순서 | red (r1) | `상한 초과 파일은 null` 1건 | 잠김 |

- **분모 검산**: `선택 증거 1(VP-Δ4) · 인용 변이 1(D1) · 새 oracle 0 = 표 행 2` + 순서 축 N3 재실행 1행 = **3행**. r2 는 새 구조적 proxy·0건 스윕·배선 oracle 을 만들지 않았다(추가한 둘은 직접 인자·경계값 관측이다).
- **덮개 회귀**: 이전 라운드에 red 였는데 이번에 green 인 행 **0건** — N1·N3 를 재실행해 둘 다 여전히 red 임을 확인했다. 교체한 장치(`기본 상한은 1 MiB 다` → 경계값 케이스)가 구 장치보다 약하지 않은지도 봤다: 구 케이스는 상수 값만 봐서 어떤 프로덕션 변이에도 반응하지 않았고, 새 케이스는 기본 인자 배선이 끊기면 red 다.

## [구현자 기입] Product/UX 파생 검토 (r2)

| 질문 | 판정 | 후속 |
|---|---|---|
| 새로 만든 사용자 대면 문구·상태에 소비자가 있는가 | 해당 없음 — r2 는 테스트만 바꿨다 | 없음 |
| seam을 만들려고 production을 재배치했다면 정리 코드가 보던 변수가 여전히 그 스코프에 있는가 | 재배치 없음 | 없음 |
| 이번에 만든 실패 경로가 Part I 상태 전이표의 어느 행인가 | 새 실패 경로 없음 | 없음 |
| 실패가 화면에서 "아무 일도 안 일어남"으로 보이지 않는가 | r1 판정 유지 — 모든 폴백이 변경 전 카드다 | 없음 |
| 늦게 도착한 응답이 화면을 되돌리지 않는가 | r1 판정 유지 | 없음 |

## [구현자 기입] 놓친 잠재 문제 + 대응 (r2)

| # | 문제 | 대응 | 근거 |
|---|---|---|---|
| 1 | `D1` 이 드러낸 축은 **부작용 부재 단언의 한계**다 — "부르지 않았다" 는 "무엇으로 불렀나" 를 말하지 않는다. 같은 형태가 이 저장소의 다른 주입 포트에도 있을 수 있다 | ⚠️ 보고만 — 이번 범위 밖이고 전수 조사가 별도 작업이다 | `D1` 재현 |
| 2 | 경로 동일성 케이스는 `path.join` 으로 기대값을 만든다 — Windows 구분자에서도 같은 문자열이어야 한다 | ✅ 선조치 — `path.join`/`path.resolve` 로 양쪽을 만들어 플랫폼 리터럴을 쓰지 않았다 | `edit-preview.test.ts` |
| 3 | r1 의 `Criteria-Met: 10/10` 이 검증자 재측정 `9/10` 과 갈렸다 | ✅ 선조치 — r2 trailer 를 재산정한 값으로 적는다(아래 합계 검산) | verify r1 §11 |

### 설계 대비 명시적 차이 (r2)

- plan 이 지정한 것과 다르게 구현한 것: **없음**.

| 축 | 대체물에만 있는 실패 모드 | 재확인한 AC·§10 행 / 관측 |
|---|---|---|
| 만료 | 해당 없음 — 차이 없음 | — |
| 공유 (누가 함께 쓰고 누가 비울 수 있는가) | 해당 없음 — 차이 없음 | — |
| 재진입 | 해당 없음 — 차이 없음 | — |
| 다른 무효화 축 | 해당 없음 — 차이 없음 | — |

## [구현자 기입] 구현 보고 (r2)

| 항목 | 내용 |
|---|---|
| 변경 파일 | 1 (`src/main/adapters/edit-preview.test.ts` — 케이스 2건 추가, 1건 교체). 프로덕션 코드 무변경 |
| 실행 명령 | `npm run typecheck` · `npm run lint` · `./node_modules/.bin/vitest run` |
| **관측한 게이트 산출**(exit code 아님) | typecheck `error TS` **0건**(3구성) · lint **0 error / 1 warning**(변경 무관 기존 항목) · vitest **486파일 4525케이스: 4336 pass · 173 fail · 16 skip**. 실패 30파일은 0228 기준선과 동일 집합 |
| V-pair 자기확인 | `SELF_PASS 10 / SELF_BLOCKED 0`(REQUIRED 8 · REGRESSION 2) · `NOT_REQUIRED 1` |
| 강제 지점 전수 | 8/5 (§10 5지점 + EP-Δ4 형제 3) |
| **AC 자기보고**(`Criteria-Met`) | 10/10 — r1 에서 ⚠️ 였던 **AC6 만 갱신**: `read` 인자가 `<WS>/nested/hello_world.ts` 이고 main cwd 기준 경로가 아님을 단언한다. 나머지 9건은 r1 관측 유지(프로덕션 무변경) |
| **합계 검산** | `✅ 10 · ⚠️ 0 · ❌ 0 = 총 10` — §7 표에서 분모를 다시 세어 10. r1 의 재측정값 9 에서 AC6 하나가 올라왔다 |
| 블로커 / 역질문 | 없음 |
| 대상 커밋 | `(r2 구현 — 좌표는 INDEX)` |

## [구현자 기입] Review Signals (r2)

- 이번에 닫은 불변식이 이전 라운드와 같은 축인가: 예 — r1 의 EP-Δ1 과 같은 축이고, 빠진 것은 규칙이 아니라 **증거의 한 차원**(호출 여부 vs 호출 인자)이었다.
- 그것을 막았어야 할 plan 지침·AC 가 있었는가: §10 EP-Δ1 은 "그 경로" 라고 적었으나 §7-A VP-Δ4 의 oracle 은 "호출 횟수 0" 만 적었다 — 강제 지점의 문장과 pair 의 oracle 이 같은 축을 가리키지 않았다.
- 반복해서 부딪히는 환경 한계: better-sqlite3 ABI(30파일) · `claude.ts` 직접 테스트 불가.
- 현재 라운드 수: 2
