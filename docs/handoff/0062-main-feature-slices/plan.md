...
| 항목 | 내용 |
|---|---|
| 진척 | **부분 구현(진행 중).** 인수 2·3·4·12 충족(버스), 6 충족(adapters 디커플링), 9 부분. 재배치: `infra/`·`adapters/` 포트·`features/{usage,approvals,providers,extensions,chat,history,sessions}`·`contracts/`·`app/` 완성. `src/main` 최상위 = 목표 5디렉토리 + `index.ts`/`env.d.ts`/`prompts`(잔여). 남은 본체=prompts 데드코드(+doc)→InflightTurn/ports 중복 해체→RouterContext 해체→eslint 엄격화(AC7·8)→네이밍→AGENTS.md(AC16). |
| 재배치 현황 | **★ lifecycle·ipc 해체 완료(C13·C14).** `contracts/`(turn·bus-events·ports·session-state) 신설, `features/sessions`(런타임 거버넌스 11파일)·`features/chat`(턴 오케스트레이션 7파일)·`features/history/writer`(persist)·`features/approvals/coordinator`(승인 IPC), `app/{bootstrap,chat-turn,context,handlers/*}`(컴포지션 루트), `infra/ipc/{handle,send,dto}`. installer→`adapters/installer`, 무회귀 배럴 제거. **잔여 dir=`prompts`만**(빈 POLICY 체인·문서화 메커니즘 — `system-prompt.md` 동시 갱신 필요, 별도 wave). RouterContext·send.ts 세부분해·ports 중복은 최종 엄격화 wave 로 이연(전환 eslint features↔features/legacy 허용에 기대). |
| 실행 명령 | `npm run lint` / `typecheck` / `test` / `build` (매 wave 후 green) |
| 게이트 결과 | lint ✅ / typecheck(node+web+test) ✅ / test ✅ (**649 passed**, electron path 스텁 후) / build ✅ (electron-vite 3-config) |
| 커밋 | 설계 `f7cfe4d`·버스 `6a98ab1`·제거 `c9e8e6e`·waveA `495c777`·waveB `c6fbcb5`·risky-tools `2be7858`·전환eslint+mcp `d41c8f8`·…·adapters 디커플링 `3507ffb`·**lifecycle C13 `6014124`·ipc C14 `10b6d80`** |
| 블로커/역질문 | 없음. lifecycle/ipc 는 전환 eslint(features↔features 허용)에 기대 wave 로 분해했다 — 순수 이동+경로 재배선, 로직 무변경. 다음=prompts 데드코드 정리(+`system-prompt.md`) 후 최종 엄격화 wave. |
