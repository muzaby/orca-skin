...
| 재배치 현황 | **★ adapters 도메인 디커플링 완료(AC6) — `grep '../' src/main/adapters` 의 domain/features 참조 0.** 완료: `infra/*`·`adapters/*` 포트(turn·types·mcp-config·provider-config·descriptor·risky-tools·hooks·error-classifier·claude-plugin·attachment-prompt·plan-feedback)·`features/{usage,approvals,providers,extensions/{builder,mcp,skills,deployer,…},chat/{attachments,scan,title,image}}`. **남은 dir(3개 + 인프라)**: `lifecycle`(→features/{sessions,chat} 분해 — 强결합 클러스터, 최난도)·`ipc`(→features/{chat,history,approvals}+app/bootstrap+infra/ipc+RouterContext 해체+send.ts 분해)·`prompts`(빈 POLICY 체인, 문서화 메커니즘 — `system-prompt.md` 동시 갱신 필요)·`installer`(24LoC→providers ipc 인라인). 이후 contracts/·최종 eslint 엄격화(legacy 제거)·네이밍·AGENTS.md. |
