...
# Verify — r3 (D-019 cwd 축 확장)

> r1 판정 원문은 위에 그대로 둔다. 이 절은 **r3 만** 다룬다.

## 메타 (r3)

| 항목 | 값 |
|---|---|
| 대상 커밋/range | `321f243..a07e8d5` |
| 구현 전 plan 기준 | `321f243` (cwd 확장 설계 커밋) |
| 라운드 | 3 |
| 상태 | **PASS** |
| 자기 검증 여부 | 예 — 완화책은 §4 |

**판정: PASS.** AC **✅25 · ⚠️1 · ❌0 / 26** — 자기보고(24/26)보다 하나 높다(AC24 가 이 턴에 CI green 으로 닫혔다). 강제 지점 **23/23**. 남은 ⚠️ 는 **AC16 하나**이고 그것은 구현 노력이 아니라 **신규 의존성 승인**이 막고 있다(검증 책임 분리상 사람 몫). 기준 밖 결함 2건은 전부 문서·형식이라 비차단으로 판정했다 → §13.

## 0. 기준선 (r3)

- **성립한다** — 설계 `321f243`(`Status: designed`)과 구현 `a07e8d5`가 분리돼 있다. `git log --oneline cd91736..a07e8d5` → 2커밋.
- 규범 3절(§3·§7·§10)을 두 커밋에서 뽑아 diff → **차집합 0줄**(base 106 = head 106). 구현 커밋이 규범 행을 바꾸지 않았다.
- 채점 기준 AC 총수: `321f243` 의 §7 표 → **26행**.

## 1. 구현 비판적 검토 (r3)

| 질문 | 판정 | 근거 |
|---|---|---|
| **throw 의 폭발 반경** | **안전** | `resolveGuardRoots` 는 `makeWorkspaceGuardHook` **본문**(콜백 밖, `workspace-guard.ts:141`)에서 동기 호출되고, 그 호출부는 `claude.ts:392` 의 `query()` 옵션 조립부다. 즉 throw 는 **spawn 시점**에 터지지 훅 콜백에서 터지지 않는다 — **툴별 fail-open 이 불가능하다** |
| throw 를 누가 받는가 | **받는다** | `session-runtime.ts:345` 가 catch → `rollbackInitialSubmission()` → `engine.spawn.failed` 로그 → 재전파. 롤백이 있으므로 부분 상태가 남지 않는다 |
| false success 가능성 | 없음 | 네 지점이 전부 값/예외로 끝난다. 오류를 삼키는 하위 호출 없음 |
| Product/UX 의 A 대신 B | 아니오 | D-021 의 세 갈래(막기·폴백·throw)를 그대로 구현 |
| 새 조기 반환이 무엇을 건너뛰는가 | **확인** | `SET_CWD` 의 루트 거부가 신설 조기 반환이다. `extraDirs` 를 비우지 않는 것이 옳다(cwd 가 안 바뀌었으므로) — 전용 케이스로 단언돼 있다 |
| 최적화가 잃은 관측 | 해당 없음 | 캐시·순서 변경 0 |
| **미보고 동작 변화 1건** | **개선 방향** | `usableCwd` 가 `''` 도 `null` 로 접는다. 이전에는 `'' ?? x` 가 `''` 를 통과시켰다 — 빈 cwd 는 무의미하므로 개선이지만 보고에는 없다. 스키마 `min(1)` 이라 IPC 로는 도달 불가, 세션행만 해당 |

## 2. 역방향 탐색 (r3)

`scan-surface.sh 321f243..a07e8d5` — 후보 전부 **같은 파일 소비처**(스크립트는 cross-file 만 센다):

| 후보 | 판정 | 근거 |
|---|---|---|
| `resolveTurnCwd` 프로덕션 0 | 정상 | `turn-context.ts:161` 이 같은 파일 소비처 |
| `guardToolAccess`·`GuardRoots` | 정상 | `workspace-guard.ts:144` 콜백이 소비 |
| 형제 정책 비대칭 | **(없음)** | 스크립트 §3 |
| 신규 export | **0건** | r3 은 신규 파일·신규 export 0 — 두 축이 같은 SSOT·같은 상태·같은 문구를 쓴다 |
| producer ↔ consumer | 일치 | 신규 i18n 키 0 — r2 의 `extraDirRejectRoot` 를 두 축이 공유. `CwdPanel.tsx:61` 이 유일 소비처이고 **고르는 경로도 그 안 하나뿐**(`CwdButton` 은 `sessionStarted` 면 열기만 — `CwdButton.tsx:34`) |

## 4. 잠금 재측정 (r3) — 검증자가 다시 심는다

> 구현자 표(C1~C6)를 증거로 쓰지 않고 독립으로 심었다. r3 은 파생 이슈가 없어 분모는 고친 지점이다.

| 검증자 변이 | 스위트 | 결과 |
|---|---|---|
| W1 `SET_CWD` 루트 거부 제거 | `chatReducer.extraDirs` | **5/20 red** ✅ |
| W2 `usableCwd` 를 resume 경로에만 적용(비대칭) | `turn-context` | **1/23 red** ✅ |
| W3 가드 throw 를 정규화 결과에만(플랫폼 의존) | `workspace-guard.extra-dirs` | **2/12 red** ✅ |
| W4 스키마 `cwd` refine 을 `nullable` 뒤로(적용 무효화) | `absolute-path` | **3/42 red** ✅ |
| **W5 cwd 거부만 다른 사유값**(두 축 분기) — 구현자가 심지 않은 축 | `chatReducer.extraDirs` | **4/20 red** ✅ |

- 검출 **5/5**. 구현자 보고와 일치하며, 보고에 없던 두 축(W2 비대칭 · W5 사유 분기)도 잠겨 있다.
- 소거 변이 잔여물 수렴: 해당 없음 — 이번 변이는 전부 조건 무력화라 잔여물이 생기지 않았다.

## 5. 요구사항 충족 매트릭스 (r3)

| # | 결과 | 검증 증거 (검증자 재측정) |
|---|---|---|
| AC25 | ✅ | `SET_CWD('/'·'C:\'·'\\srv\share')` → `cwd` 불변 + 사유 세팅. 두 축의 사유가 **같은 값**(W5 가 분기를 4케이스로 검출). 거부된 SET_CWD 가 `extraDirs` 를 비우지 않음 |
| AC26 | ✅ | 스키마 `cwd:'/'` 거부(W4) · `resolveTurnCwd` 가 세션행/요청 루트에서 `getCwd(projectId)` 폴백(W2) · `resolveGuardRoots` 가 원문·정규화 양쪽에서 throw(W3). **`writeRoots[0]` 이 `/` 인 상태를 만들 수 없다** |
| AC24 | ✅ | **이 턴에 닫혔다** — `gate`(windows-latest) `conclusion: success`, 03:44:33→03:47:57Z, head `a07e8d5` |
| AC16 | ⚠️ | **r1 이래 불변 — 사람 결정 대기.** 호출부 스윕이며 AC 가 적은 "컴포넌트 테스트" 가 아니다. 막고 있는 것은 구현 노력이 아니라 신규 devDependency 승인이다 |
| AC1~AC15 · AC17~AC23 | ✅ | 전체 스위트 **227파일 2294케이스 전건 통과**로 회귀 없음 |

- **합계 재측정**: `✅ 25 · ⚠️ 1 · ❌ 0 = 총 26`(§7 표 26행 직접 셈).
- **합계 사본 대조**: 본문 `24/26` ↔ trailer `Criteria-Met: 24/26` ↔ INDEX `✅24·⚠️2·❌0/26` — **세 사본 일치**. 검증자 재측정과 1건 차이(AC24, 자기보고 시점엔 CI 미완). 과대보고 아님.

### §10 강제 지점 — AC와 별개로 걸음 (r3)

| 계약/필드 | plan 지점 | 확인한 지점 | 결과 |
|---|---|---|---|
| **`cwd` 비루트** | 선택 · `chat:send` · 세션행 · 가드 ws (4) | `chatReducer.ts:635` · `protocol.ts:91` · `turn-context.ts:54` · `workspace-guard.ts:68` | **4/4** ✅ |
| `extraDirs` 절대·비루트 | (4) | r2 유지, 회귀 없음 | **4/4** ✅ |
| 나머지 5행 | 2+2+2+6+3 | r1·r2 유지 | **15/15** ✅ |

- **합계 23/23.**
- **불변식의 주어로 전수 재검색**: `resolveGuardRoots` 프로덕션 호출부는 `workspace-guard.ts:141` **1곳**, 그 호출부(`makeWorkspaceGuardHook`)의 프로덕션 호출부는 `claude.ts:392` **1곳**이고 인자는 `cwd`·`additionalDirectories` **둘뿐**이다. 둘 다 닫혔다 — **구현자의 "이 축은 끝났다" 주장이 성립한다.**
- **표에 없는데 같은 불변식이 필요한 지점**: **0건.**
- **`실패 의미` 재현**: `writeRoots[0] === '/'` 상태를 만들 수 없다(W3 원복 상태에서 3형태 + 별칭 2형태 전부 throw).

## 9. 게이트 재실행 (r3)

- 관측한 산출: **typecheck `error TS` 0줄**(3구성) · **vitest 227파일 / 2294케이스 전건 통과** · **scripts 59/59** · migrations `no-copies ok: scanned 750 source files, 2 list owners` · doc-inventory prose/links ok.
- **게이트가 트리를 바꾸지 않았다** — 실행 후 `git status --short` **0건**(`--fix` 없는 명령만 사용).
- 검증 중 잔여물: 없음(변이 원복 후 0건 확인).
- CI(별도 정본): `a07e8d5` `gate` success.

## 11. Repository operation checks (r3)

- trailer 파싱 ✅ — `321f243` 6키 · `a07e8d5` 8키 그대로 반환.
- trailer 허용값 ✅ — 설계 `Status: designed`(`Criteria-*` 없음) · 구현 `Status: implemented` + `Criteria-*` + `Verified-By: pending`.
- 인용 커밋 실재 ✅ — 6개 전부 `git cat-file -t` → `commit`.
- `[구현자 기입]` r3 필드 **8절**(요구 7 + AC 자기보고). 산문으로 접힌 필드 0.
- 변경 파일 재측정 ✅ — `git diff --name-status` → **8 수정 · 0 신규**, 구현자 보고와 일치.
- **INDEX 비고 6문장 — 5 초과** ❌ → D6.
- 대상 커밋 좌표: 구현자가 `(r3 구현 — 검증자 기입)` 을 남겼다 → 이번 턴에 `a07e8d5` 로 채운다.

## 13. 파생 이슈 (r3) — 비차단

| # | 이슈 | 출처 | 대응 방향 | 상태 |
|---|---|---|---|---|
| D5 | **`extraDirRejection` 에 주석이 두 벌이고 앞의 것이 틀렸다.** r2 주석("마지막 **참조 경로 추가**가 거부된 이유")이 남은 채 r3 주석이 아래에 덧붙었다 — `SET_CWD` 도 이 값을 쓰므로 첫 주석은 이제 거짓이다. 계약 파일 안의 문서라 다음 구현자가 좁은 계약으로 읽고 cwd 용 상태를 따로 만들 수 있다(D-019 가 일부러 합친 것을 다시 가르는 방향) | `chatReducer.ts:95-100` | 앞 주석을 지우고 뒤 주석 하나만 남긴다. 코드 변경 0 | open |
| D6 | INDEX 0201 행 비고가 **6문장**으로 `산출물 문장 규칙 3`(5줄 이내)을 넘는다 | `docs/handoff/AGENTS.md §산출물 문장 규칙 3` | 이번 검증 턴이 그 행을 `verify/PASS` 로 다시 쓰면서 함께 줄인다 | **이 턴에 해소** |

> 둘 다 문서·형식이고 제품 동작·계약·게이트에 영향이 없어 **PASS 를 막지 않는다**고 판정했다. D5 는 다음 touch 에서 정리한다.

## Review Signals (r3) — 사실만

- **이전 라운드와 동일/유사 증상인가**: r3 은 r1·r2 와 **같은 축의 마지막 지점**이었다. 전수 재검색으로 그 축이 닫혔음을 확인했다 — `resolveGuardRoots` 인자 2개가 전부다.
- **관련 plan 지침/AC 가 있었는가**: r1·r2 의 §10 은 계약별 행 구조라 `extraDirs` 행의 `실패 의미` 가 `cwd` 로 번지지 않았다. `cwd` 행이 아예 없었다 — 구현자의 §5 형제 축 전수 검색이 아니었으면 열리지 않았을 지점이다.
- **사용자 결정 변경 근거**: D-019a → D-019 확장은 사용자 지시가 근거다. 무단 변경 0.
- **플랫폼 의존 판정이 두 번째다**: r1 `toPosix`(`path.sep`), r3 가드 throw(`path.resolve` 후에만). 둘 다 테스트 작성 중에 잡혔고 둘 다 "정규화 결과만 보면 러너마다 답이 갈린다" 는 같은 형태다.
- **반복된 검증 환경 한계**: 렌더 하네스 부재 — AC16 이 **3라운드 연속** 같은 이유로 ⚠️. 이제 그것 하나만 남았다.
- **자기 검증**: 설계·구현·검증이 같은 에이전트다. 완화로 규범 절 차집합(0줄)·독립 재심기 5건(구현자가 심지 않은 2축 포함)·불변식 주어 전수 재검색을 썼다.

## 결론 (r3)

**PASS (r3)** — 다만 **archive 이동은 보류**한다. AC16 이 사람 결정(렌더 하네스 도입) 대기이고, 그 결정이 나면 AC16 이 닫히거나 "도입하지 않는다" 로 종결된다. 0198 이 AC9 시각 확인을 남기고 PASS 한 것과 같은 형태다.

- 두 축(`extraDirs`·`cwd`)의 루트 거부가 **8지점** 전부 닫혔고, 불변식의 주어로 전수 재검색해 **표 밖 지점 0건**을 확인했다.
- D-021 의 세 갈래 처리가 실제로 다르게 끝난다 — 막기(리듀서) · 폴백(세션행) · throw(가드). throw 는 spawn 경계에서 터지고 롤백까지 붙어 있어 fail-open 이 불가능하다.
