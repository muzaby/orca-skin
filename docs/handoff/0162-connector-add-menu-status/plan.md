...
| # | 이슈 | 출처 | 대응 방향 | 상태 |
|---|---|---|---|---|
| D2 | `pluginTone` 죽은 코드(0164 가 소비처 변경 후 잔존) — 0164 D2 와 동일 항목. | verify r1 | 0164 라운드에서 함께 정리 | **open** |
| D7 | AC5·AC6 을 "사람 실기" 로 적어 **실제 결함이 통과했다**(생성 후 무갱신 · 인증 방식 0개). 둘 다 순수 로직으로 내릴 수 있었다. | verify r1 §자기 리뷰 | 0163 이 순수 모듈로 내려 해소 | 해결 |
