...
> **verify r2 = PASS** (2026-08-16). 전문은 [`verify.md`](verify.md). AC **15 ✅ / 0 ❌**(자기보고
> `15/15` 가 재측정과 **일치** — r1·0187 r1 에서 반복된 과대 보고가 재발하지 않았다) ·
> §10 강제 지점 **23/23 지점 · 8/8 행**(구현자 소관 22 + verify 몫 1) · 표에 없는 새 지점 **0건**.
>
> 재측정 결과: 인용 **51/51** 이 줄 **내용**까지 일치 · 규모 수치 12종 + r2 정정 수치 2종(62·ACTIVE 61·
> SUPERSEDED 1·D-053 결번 / 금지표 데이터 20행) **전부 정확** · 전칭 주장 6종 성립 · r2 가 새로 실측한
> baseline 주장(U1 3층)도 `git show ad10f6c:` 3홉 추적으로 확인 · **역방향 탐색에서 뒤집힌 판정 0건**.
> AC·Decision Ledger 는 `702cd3c` 와 **바이트 동일**(무단 완화 0).

### r2 — 비차단 파생 이슈 (PASS 를 막지 않는다)

전부 문서 위생이고 판정·수치·코드 사실을 바꾸지 않는다. 0190 또는 다음 문서 손질 때 정리한다.

- [ ] **D-A — 연역 전수표의 전칭 문구가 발견 밖에서 성립하지 않는다 (낮음).** `audit.md:28` 의 "아래가
      이 문서의 **연역 항목 전부**" 는 **발견 13건 축에서는 참**(AC3 충족)이나, 발견 번호가 없는 서술
      최소 3건이 런타임 추론이다 — `:132-133`(턴당 I/O 증가 0 · vault −1) · `:204-206`(읽기 IPC 가 방송·
      도구 회수를 유발) · `:207-208`(재인증 실패 시 row 가 '연결됨' 유지). 전칭을 "**발견** 항목의 연역
      전부" 로 좁히거나 세 문장에 표기를 단다.
- [ ] **D-B — §14 인용 상한 개정이 산출을 여전히 못 덮는다 (낮음).** r2 가 "인과 사슬은 홉 수만큼"
      예외를 넣었으나 실제 인용은 **F1 5 · F2 5 · U1 5** 다(U1 = 4홉 + baseline 1, F1·F2 는 인과 사슬이
      아니라 **2·3층 근거**). 초과분이 전부 정확한 인용이라 해는 없다 — 상한이 3층 채점(D-008)이 요구하는
      근거량을 반영하지 못한 것이다. 예외를 "층별 근거 1건씩 추가 허용" 으로 넓히거나 발견 유형별로 나눈다.
- [ ] **D-C — 0189 자신의 Decision 인용 2곳에 정본 표기가 없다 (낮음).** `audit.md:20` "(D-006)" ·
      `:313` "D-009 대로" 는 **이 plan** 의 Decision 인데 같은 문서가 0188 Decision 26종을 인용하고
      **0188 에도 D-006·D-009 가 실재**한다(내용은 전혀 다르다). 같은 문서 `:217` 이 이미
      `[plan.md](plan.md) D-002` 로 정본을 밝히는 관례를 갖고 있으므로 두 곳에 같은 관례를 적용한다.

### r1 — FAIL 파생 이슈 (전부 해소, 이력 보존)

> verify r1 = **FAIL**. AC **13 ✅ / 2 ❌**(자기보고 15/15 는 과대) ·
> §10 강제 지점 **5/7 충족 · 2 부분**(자기보고 7/7 은 과대).
>
> **감사 내용 자체는 뒤집히지 않았다** — 인용 40/40 재현, 규모 수치 11종 전부 정확, 전칭 주장 8종
> 성립, 철회 2건 모두 baseline 대조에서 정당. FAIL 사유는 **계약한 표기와 보드 정합**이었다.
> 아래 5건은 r2 에서 전부 닫혔고 verify r2 가 각각을 재현해 확인했다 — 조용히 지우지 않고 남긴다(D-009).

- [x] **D1 — INDEX 21행 비고가 철회된 판정을 현재형으로 싣는다** (repository operation mismatch).
      ...
      `(구현자 산출)` → `dddd05b`.
- [x] **D2 — AC3 미충족: "연역" 표기가 `audit.md` 에 0회.** U1 은 `void` 연산자와 catch 부재로부터의
      **연역**인데 단정문으로 적혀 있다(전역 `unhandledrejection` 리스너는
      ...
      명시)는 P2·P6 에서 충족.
- [x] **D3 — AC12 미충족: 축 3 회귀표에 `층` 열이 없다.** `audit.md:171` 헤더가
...
- [x] **D4 — 수치 2건 과다(판정 불변).** ① "ACTIVE Decision **63건**" → 실제 **총 62건**
...
- [x] **D5 — AC11 부분 · 자기 상한 초과 (낮음).** ① 36불릿 ↔ AC1~25 "전수 매핑" 이 *방법*으로만
...
> *(r1 시점의 다음 단계였던 "재구현 r2" 는 `19baf21`·`5a4176d` 로 수행됐고 verify r2 가 PASS 를 줬다.)*

**다음 = 종료.** 이 핸드오프는 `verify/PASS` 로 닫히고 보드 행은 archive history 로 이동한다.
**F1~F3 · P1~P2 · U2~U3 의 시정 여부는 사용자 결정**이며 별도 핸드오프(0190)로 설계한다 —
이 감사는 코드를 바꾸지 않았고 바꾸지 않는 것이 D-001 이다. 위 r2 파생 이슈 3건(D-A·D-B·D-C)은
비차단 문서 위생이라 0190 이나 다음 문서 손질 때 함께 정리한다.
