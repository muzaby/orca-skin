...
| r5 | **PASS** | 형제 3섹션의 본문 귀속과 헤더 순서가 잠겼다 — 맞바꿈·순서·제거 변이 5종 전건 red |
...
| 검증자 | Claude Code |
| 일자 | 2026-08-27 |
| 대상 커밋/range | `74566f7..7b45fa3` (r5 구현 = `7b45fa3`) |
| 구현 전 plan 기준 | `74566f7` (r4 검증 커밋) |
| V mode / 유효 V | `Baseline V + ΔV1 + ΔV2` / `V1 + ΔV1 + ΔV2` |
| 라운드 | 5 |
| 상태 | **PASS** |
| 자기 검증 여부 | **예** — 기준선은 커밋 분리로 성립한다(§0) |
...
**PASS.** ΔV1·ΔV2 의 REQUIRED/REGRESSION pair **17 전건 PASS**, gate 4종 PASS, `PLAN_GAP` **0**.

D15 가 닫혔다 — `진행 상황`↔`출력` 본문을 맞바꾸는 변이 **M-S 가 이제 1 red** 다. 구현자가 지목된 1지점이 아니라 **형제 3섹션 전수**를 닫아 `출력`↔`컨텍스트` 맞바꿈(M-S2)과 섹션 순서 변이(M-O)도 함께 red 다. r4 가 닫은 D10 회귀(V1·V2)도 유지된다.

**NON_BLOCKING 1건 신규** — 검증자가 심은 **M-N**(`출력` 섹션에 목록을 *추가*로 넣는다)은 무음이다. 현재 PASS 를 막지 않는 이유는 §3 에 적었다.

---

### 0. 기준선 / plan 변경 확인

**기준선 성립.** 검증 커밋(`74566f7`)과 구현 커밋(`7b45fa3`)이 분리돼 §0 의 자기 증명 방지가 작동한다.

- 규범 구간(plan.md 1~825행, 첫 `[구현자 기입]` 직전) 두 커밋 버전을 `diff` → **0줄**. Decision·AC·V node/pair·§10 무변경.
- 규범 구간 밖 변경은 **2행** — 파생 이슈 표의 D15·D16 `상태` 칸이 `open` → `구현 완료 (r5)` 다. `closed` 가 아니라 구현 보고 문면이라 검증자 판정을 선점하지 않는다.
- 채점 기준은 r4 가 고정한 AC28·AC29 원문과 VP-21·VP-22 의 path·oracle 이다.

### Plan validity

| 검사 | 판정 | 근거 |
|---|---|---|
| V mode·유효 V 재구성 | 유효 | `none → V1(72766d2) → ΔV1 → ΔV2`. r5 는 V 를 바꾸지 않았다 |
| 재검증 범위 | 유효 | production diff 0줄이라 root 실패 pair 1개(VP-22) + 회귀 pair 1개(VP-21) + gate 만 실행 대상. 나머지 15 는 r3 좌표 참조 |
| pair별 path·oracle | **충족** | VP-22 의 `3섹션` hop 이 이제 세 섹션을 구별한다 — r4 의 미달이 해소됐다 |
| 적대 증거 | 유효 | plan 이 VP-22 를 `적대 증거 required` 로 등록한 pair라 변이 심기가 규정된 절차다 |
| 운영 gate 범위 | 유효 | 4종. 무관한 기존 실패(better-sqlite3)를 blocking 으로 올리지 않음 |

### 1. AC 전 diff 비판적 읽기

변경은 **테스트 1파일**(`+21/-9`)이다. production·i18n·IPC·DB·의존성 **0건** — `git show --stat 7b45fa3` 이 3파일(테스트 1 · plan.md · INDEX.md)만 낸다.

| 질문 | 판정 | 근거 |
|---|---|---|
| 새 장치가 눈이 있는가 | **있다** | 변이 5종이 각각 1 red. 전부 같은 문자열을 돌려주는 눈먼 장치가 아니다 |
| 장치가 fail-closed 인가 | **예** | 검증자 변이 **M-C**(`TileSection` 기본 접힘 → `pb-3` div 부재) → `bodies` 가 비어 **1 red**. 조용히 통과하지 않는다 |
| 기존 단언을 약화했는가 | **아니오** | 교체된 5 `toContain` 이 전부 더 좁은 단언으로 승계됐고, `aria-expanded` 3개 단언은 그대로다 |
| 헤더 존재 단언이 사라졌는가 | **아니오** | `Object.keys(bodies)` 가 세 제목을 **순서까지** 단언해 기존 3 `toContain` 보다 강하다 |
| false success 가능한가 | **1건** | M-N(아래 §3) — *여분의 사본* 축은 여전히 무음이다 |

### 2. 역방향 탐색

`scan-surface.sh 74566f7..7b45fa3` → **변경된 소스 파일 0**(테스트 전용이라 스크립트 분모 밖). 직접 본 것:

| 후보 | 판정 | 근거 |
|---|---|---|
| 신규 헬퍼 `sectionBodies()` | 정상 | 같은 파일 안 1소비처. production 심볼을 재구현하지 않고 렌더 산출만 판다 |
| 장치가 묶인 DOM 형태 | **fail-closed** | `<section`·`pb-3` 는 `TaskTileSections.tsx:26,40` 실측과 일치. 형태가 바뀌면 red 다(M-C 로 확인) |
| 형제 파일 `navSections.render.test.ts`(0203) | 무영향 | 같은 축을 다른 장치로 이미 닫는다 — 구획마다 별도 렌더 + 슬롯 브랜드 타입. 이 불변식이 열린 다른 지점 0건 |
| D12 고아 i18n 키 | **잔존 2건** | `chat.taskTile.emptyTitle` · `chat.taskTile.viewTranscript` 각 0 소비처. r5 범위 밖 |

### 3. 등록·인용 변이 재측정 + 검증자 신설 변이

구현자 보고를 증거로 받지 않고 **7종 전부 검증자가 직접 심어 재실행**했다(대상: renderer chat 44파일).

| 변이 | 출처 | 재측정 | 판정 |
|---|---|---|---|
| V1 진행 상황 View 제거 | D10 회귀 | **1 failed / 422 passed** | 검출 |
| V2 background View 제거 | D10 회귀 · VP-21 | **1 failed / 422 passed** | 검출 |
| M-S 진행 상황↔출력 맞바꿈 | **D15 인용 변이** | **1 failed / 422 passed** | **검출 — D15 닫힘** |
| M-S2 출력↔컨텍스트 맞바꿈 | 형제 전수 | **1 failed / 422 passed** | 검출 |
| M-O 섹션 순서 뒤집기 | D-017 | **1 failed / 422 passed** | 검출 |
| **M-N** `출력`에 목록을 *추가* | **검증자 신설** | **423 전건 통과** | **무음 — D17** |
| **M-C** 섹션 기본 접힘 | **검증자 신설**(장치 무결성) | **1 failed / 422 passed** | 검출 |

- 구현자 보고(`각 1 failed / 11 passed`)와 **행마다 일치**한다 — 분모만 파일 1개 ↔ 44개로 다르다.
- 자기보고 합계 대조: 본문 `✅2 · 총 2` ↔ trailer `Criteria-Met: 2/2` ↔ INDEX 비고 — **세 곳 일치**.
- 7종 복원 후 production 3파일 `git diff --quiet` 통과.

**M-N 을 blocking 으로 올리지 않는 이유** — 세 가지가 함께 성립한다.

1. AC29 의 음성 절(`출력`·`컨텍스트`는 설명문만)의 **plan 지정 장치는 §10 EP-16**(두 섹션 컴포넌트가 `parts` 를 읽지 않는다, 지점 2)이고 그 분모는 전건 닫혀 있다. M-N 이 치는 것은 EP-16 표 밖 지점이다.
2. **D-022 가 두 섹션의 빈 상태를 이번 라운드 한정으로 명시**했다 — 다음 handoff 가 실제 내용을 채운다. 지금 래퍼에 "설명문만" 잠금을 걸면 그 라운드가 그것을 걷어내야 한다.
3. 축이 다르다 — M-S 는 *자리*(그럴듯한 리팩터 실수), M-N 은 *여분의 사본*(의도적 추가). 후자는 diff 리뷰가 보는 형태다.

→ **`NON_BLOCKING` (D17)**. 래퍼 수준의 "only" 장치가 필요한지는 `출력`·`컨텍스트`를 채우는 handoff 의 설계 질문이다.

### 4. §8 엄격화 — 장치의 실재 판정 기준을 좁혀 재측정

구현자가 이번 라운드에 만든 구조적 장치라 그대로 재현하는 것은 검증이 아니다. **판정 기준을 한 단계 좁혀 다시 쟀다.**

```text
느슨(현 구현): /<div class="pb-3">([\s\S]*)$/          ← chunk 끝까지
엄격(재측정):  /<div class="pb-3">([\s\S]*?)<\/div><\/section>/   ← 섹션 경계까지
  → 변이 7종 재실행 → 7/7 판정 동일
```

**차집합 0.** 느슨한 기준이 엄격한 기준보다 더 통과시키는 케이스가 없다 — `split('<section')` 이 이미 chunk 를 섹션 단위로 가르기 때문이다. **다만 엄격화는 이 스윕이 전수인지를 재지 방향을 재지 않는다** — 방향은 위 소거·맞바꿈 변이가 판정했다.

### 5. V-pair closeout — `UT → IT → ST → AT`

production diff 0줄이므로 실행 대상은 root 실패 pair 와 그 회귀 pair 다. 나머지 15 는 r3 §5 좌표를 참조한다(이번 전체 스위트 재실행에서 그 증거가 전건 green 임도 확인).

| Pair | 레벨 | requiredness | 결과 | 직접 검증 증거 |
|---|---|---|---|---|
| **VP-22** (R-12↔AT-26/27/29) | R↔AT | REQUIRED | **PASS** | 본문 귀속 3 + 헤더 순서 `toEqual` · 변이 M-S·M-S2·M-O·V1 각 1 red · 엄격화 차집합 0 |
| **VP-21** (R-11↔AT-28) | R↔AT | REQUIRED | **PASS** | r4 잠금 유지 — 변이 V2 1 red |
| VP-20·23·24·25·26·27·28·29 | 각 레벨 | REQUIRED | PASS(참조) | r3 §5 좌표. production diff 0줄이라 무영향 |
| VP-06·08·12·16·17·18 | 각 레벨 | REGRESSION | PASS(참조) | 같음 |

**합계: PASS 17 · PAIR_FAIL 0 · BLOCKED_BY 0 · PLAN_GAP 0 / 17.**

### 강제 지점 재측정

구현자가 보고한 `3/3` 을 다시 셌다 — `rg "<TileSection" app/src | rg -v '\.test\.'` → **3지점**(`TaskTileContent.tsx:307·310·313`), 전부 한 부모 아래 형제다. 구현자 분모와 **일치**.

§10 production 강제 지점 **32** 는 이번 라운드 대상이 아니다 — production diff 0줄이고 r3 §강제 지점 분모 재측정이 정본이다.

### AC 세부와 합계 — r5 분모

| AC | 결과 | 증거 |
|---|---|---|
| AC29 / VP-22 | ✅ | 세 본문 귀속 + 헤더 순서. M-S·M-S2·M-O·V1 각 1 red |
| AC28 / VP-21 | ✅ | V2 1 red |

**검산: `✅ 2 · ⚠️ 0 · ❌ 0 = 총 2`** — 분모는 r4 와 같다(D15 는 AC 를 늘리지 않고 AC29 단언을 좁혔다). ΔV1·ΔV2 의 나머지 10 AC 는 r3 좌표, `V1` 의 AC11·AC20·AC25 는 여전히 사람 실기다.

### 6. 게이트 (검증자 실행)

| Gate | 관측한 산출 | 판정 |
|---|---|---|
| `npm run lint` | **0 error · 1 warning** — `useTranscriptVirtualizer.ts:22`(기존·변경 무관) | PASS |
| `npm run typecheck` | **error TS 0건** (3구성) | PASS |
| `vitest run` (전체) | **231파일 green / 5파일 red · 2388케이스 green / 46 red** | PASS(분리 보고) |
| `check-doc-inventory --check` | `9 items, 79 channels` · prose ok · links ok | PASS |

- **5파일 red 는 알려진 환경 서명** — `Could not locate the bindings file … better_sqlite3.node`. `app/AGENTS.md §제약 환경` 실측 5파일과 정확히 일치한다.
- **게이트가 트리를 바꿨는가**: `lint --fix` 후 `git status --short` **0줄**.
- **검증 중 잔여물**: 변이 7종 + 엄격화 1종 전부 복원(`git diff --quiet`). 백업·스크립트는 scratchpad 에만.

### 7. 사람 실기 경계

r3 §7 과 같다 — 3섹션 시각 대조 · 긴 제목의 버튼 잘림 · `72766d2` 시각 동일성 · AC11·AC20 · J1(SDK `updatedFields`). **r5 가 새로 사람에게 넘긴 것은 없다.**

### 8. Repository operation checks

| 검사 | 판정 | 관측 |
|---|---|---|
| 인용 커밋 실재 | ✓ | `34db51d`·`4b3310b`·`e459be0`·`7b45fa3`·`8fbfae5` 전부 `git cat-file -t` = `commit` |
| trailer 파싱 | ✓ | `7b45fa3` → **9키 전부 반환** |
| trailer 허용값·사실성 | ✓ | `Agent: claude` 가 ACTIVE **D-014** 와 일치한다 — **D16 닫힘** |
| `[구현자 기입]` 7필드 | ✓ | r5 = **7**(기계 추출). 산문으로 접힌 필드 0 |
| INDEX 비고 5줄 | ✓ | 352자 = 100자 기준 **4줄** |
| INDEX 대상 커밋 | 기입 | `(r5 구현 — 검증자 기입)` → `7b45fa3` 로 채웠다 |
| 지침 변경 커밋 분리 | ✓ | `8fbfae5`(`Handoff: none` 카브아웃, handoff-review Tier 1)는 r5 구현과 **다른 커밋**이라 §0 기준선을 흐리지 않는다 |
...
|---|---|
| lint/typecheck/테스트/inventory | 검증자 실행·산출 관측 완료 |
| 변이 재현 + 신설 | 등록 5종 재현 · 신설 2종(M-N·M-C) |
| 장치 엄격화 | 판정 기준 좁혀 재측정, 차집합 0 |
| UI 시각 품질 · merge | 사람 |

### Review Signals — 사실만

- **이전 라운드와 같은 축인가**: **닫혔다.** r3 D10(배선 존재) → r4 D15(자리)로 좁아지던 축이 r5 에서 형제 전수로 닫혔다. 이번에 새로 관측한 **M-N(여분의 사본)** 은 같은 장치-방향 계열의 세 번째 변형이지만 plan 이 EP-16 으로 따로 다루는 축이다.
- **관련 plan 지침/AC 가 있었는가**: **r5 직전에 생겼다.** 사용자가 지침 적용을 지시해 `8fbfae5` 가 plan §5·impl §3·verify §4·두 template·`docs/handoff/AGENTS.md` 에 자리 축을 더하고 corpus 에 **P49** 를 신설했다(review round 24, Tier 1). 이번 검증의 맞바꿈 변이는 그 새 규칙을 처음 적용한 것이다.
- **사용자 결정 변경 근거**: 없다. r5 는 Decision 을 건드리지 않았다.
- **반복된 검증 환경 한계**: better-sqlite3 ABI/egress(5라운드 연속) · zustand SSR 스냅샷이 store 연결 컴포넌트를 시드하지 못함(3라운드 연속, 구현자 K1 과 동일).
- **자기 검증 한계**: 설계·구현·검증이 같은 에이전트다. 기준선 커밋 분리와 **검증자가 새로 심은 변이 2건**(M-N·M-C)으로 완화했다 — r3·r4 는 그 신설 변이가 FAIL 을 만들었고 이번에는 하나가 무음(D17, non-blocking)이다.
- **라운드 수**: **5**. 다음 재구현이 있으면 `handoff-review` 를 다시 수행한다(round 24 는 r5 전에 수행했다).
