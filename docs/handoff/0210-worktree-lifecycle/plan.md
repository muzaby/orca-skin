...
> **라운드 ↔ 턴 라벨 매핑**: verify 사이의 impl 턴은 몇 번이든 **한 라운드**다. 아래 절 제목의
> `(r1)`~`(r4)`는 당시 표기를 보존한 **턴** 라벨이고, 실제 라운드는 `r1`·`r2` = **라운드 1**(verify
> `bb3d2156` FAIL) · `r3`·`r4` = **라운드 2**(검증 대기)다. [`verify.md`](verify.md)의 `Verify r2` 도
> **라운드 1**의 판정이며, 두 문서의 본문은 당시 기록이라 재서술하지 않는다.
...
- 동의 / 그대로 진행: 규범 행 변경 없음. r4 는 **CI red gate 수복**이고 §10·AC·Decision 을 건드리지 않는다.
- 이견 / 현실성 문제: 없음.
- ACTIVE Decision과 충돌하는 설계 발견: 없음.

## [구현자 기입] 강제 지점 전수 (§10 대조) (r4)

r4 가 닫은 것은 §10 행이 아니라 **현재 산출물의 필수 gate**(CI vitest)다. r3 의 17/17 은 그대로다.

| Pair | 계약/필드 | §10이 적은 지점 | 닫은 지점 | 재현 명령 / 관측 | 남긴 곳 |
|---|---|---|---|---|---|
| WP-07·15·17 | EP-17 폴백 3쓰기의 **oracle 무결성** | (행 변화 없음) | 3/3 유지 | 워밍업 추가 후 M-J red 2건 · M-K red 1건 — 장치가 약해지지 않았다 | — |

- 전수 합: **17/17**(r3 유지). 이번 diff 는 프로덕션 로직 0이라 지점 자체가 이동하지 않았다.
- §10에 없는데 같은 불변식이 필요했던 지점: 없음(r3 의 제안 1건은 그대로 설계 몫이다).

**V-pair 자기확인 (r4)**

| Pair | requiredness | 자기 상태 | 직접 관측 | 선택된 적대 증거 결과 |
|---|---|---|---|---|
| WP-07·15·17 | REQUIRED / REGRESSION | SELF_PASS (r3 유지) | `worktree-recover.test.ts` 4 + `worktree-bind.test.ts` 3 = **7케이스 green**, 케이스별 206~293ms | M-J RED 2건 · M-K RED 1건(워밍업 후 재실행) |
| 그 밖 18 pair | — | r1~r3 상태 유지 | 이번 diff 가 그 경로에 닿지 않는다 | 재실행 없음 |

## [구현자 기입] 이번 라운드 수정의 잠금 (r4)

| 심은 결함 | 출처 | 이전 라운드 결과 | 실패한 테스트 / 케이스 수 | 결과 |
|---|---|---|---|---|
| M-J `service.ts:193` `updateSessionCwd` 호출 제거 | `WP-07 형제 슬롯 ①` — 그 장치 파일이 이번 diff 에 있다 | red(r3) | AC15 · AC16 2건 | 잠김(유지) |
| M-K `service.ts:194` `deleteManagedWorktree` 호출 제거 | `WP-07 형제 슬롯 ②` — 같은 이유 | red(r3) | AC15 1건 | 잠김(유지) |

- **분모 검산**: `선택 증거 2 · 인용 변이 0 · 새 oracle 0 = 표 행 2`. 이번 라운드는 `SELF_PASS` 를 새로 올리지 않았고(r3 상태 유지) `closed` 로 적은 파생 이슈도 없다. 표에 오른 둘은 **이번 diff 가 그 장치 파일을 건드렸기 때문**이다.
- **덮개 회귀**: 이전 라운드에 red 였는데 이번에 green 인 행 **0건**. 워밍업은 비용을 훅으로 옮길 뿐 단언을 바꾸지 않는다 — M-J·M-K 가 그것을 실측으로 확인한다.

## [구현자 기입] Product/UX 파생 검토 (r4)

| 질문 | 판정 | 후속 |
|---|---|---|
| 새로 만든 사용자 대면 문구·상태에 소비자가 있는가 | 해당 없음 — 테스트 하네스 변경이다 | — |
| seam을 만들려고 production을 재배치했다면 정리 코드가 보던 변수가 여전히 그 스코프에 있는가 | 재배치 없음 | — |
| 이번에 만든 실패 경로가 Part I 상태 전이표의 어느 행인가 | 해당 없음 — 새 실패 경로 0 | — |
| 실패가 화면에서 "아무 일도 안 일어남"으로 보이지 않는가 | 해당 없음 | — |
| 늦게 도착한 응답이 화면을 되돌리지 않는가 | 해당 없음 | — |
...
| # | 문제 | 대응 | 근거 |
|---|---|---|---|
| 1 | **이 수정은 로컬에서 반증할 수 없다** — 로컬 7케이스는 206~293ms 로 고르고 첫 케이스 스파이크가 0이다. 즉 실패하는 테스트를 먼저 만들 수 없었다 | ⚠️ 보고만 — 그래서 **해석을 미리 등록한다**: ① CI green → 프로세스 1회 비용 가설 확정 ② `beforeAll` 이 10s hookTimeout 으로 red → 비용은 1회지만 10s 초과 ③ **첫 케이스가 다시 5s red → 가설 반증**, 다음 수단은 이 파일군의 예산 확대 | 세 갈래가 서로 다른 실패 좌표를 낸다 |
| 2 | `migrate.test.ts` 도 파일 DB 를 연다 — 같은 축인데 워밍업을 넣지 않았다 | ⚠️ 보고만 — 이번 CI 에서 green 이라 변경 범위를 최소로 뒀다. 같은 증상이 그 파일에서 열리면 다음 후보다 | `rg "new Database\(" src/main/**/*.test.ts` → 파일 기반 3파일(bind·recover·migrate), 나머지 5는 `:memory:` |
| 3 | `warm-file-sqlite.ts` 는 프로덕션 트리에 있는 **테스트 전용 심볼**이다 — 프로덕션 참조 0건 | ✅ 선조치 — `infra/source-scan.ts` 와 같은 선례를 따르고 헤더 첫 줄에 "런타임 경로에서 부르지 않는다"를 명시했다 | `source-scan.ts:1` 이 같은 문장을 쓴다 |
| 4 | 원인은 확정했으나 **OS 수준 기전(Defender 스캔 등)은 특정하지 못했다** | ⚠️ 보고만 — 특정해도 수정은 같다(비용을 재는 창 밖으로). 못 본 것을 못 봤다고 적는다 | CI 로그만으로는 프로세스 내부 stall 을 가르지 못한다 |

### 설계 대비 명시적 차이 (r4)

- plan이 지정한 것과 다르게 구현한 것과 그 이유: **없음** — production 로직 diff 0.

| 축 | 대체물에만 있는 실패 모드 | 재확인한 AC·§10 행 / 관측 |
|---|---|---|
| 만료 | 해당 없음 — 대체물 없음 | — |
| 공유 (누가 함께 쓰고 누가 비울 수 있는가) | 해당 없음 — 워밍업은 자기 temp 를 만들고 즉시 지운다 | `warm-file-sqlite.ts` `finally` 의 `rm` |
| 재진입 | 해당 없음 — `beforeAll` 은 파일당 1회 | — |
| 다른 무효화 축 | 해당 없음 | — |
...
| 항목 | 내용 |
|---|---|
| 변경 파일 | **프로덕션 로직 0 · 테스트 2 수정 · 테스트 헬퍼 1 신규**. `worktree-bind.test.ts`·`worktree-recover.test.ts` 에 `beforeAll(warmFileSqlite)`, 신규 `infra/db/warm-file-sqlite.ts` |
| 실행 명령 | `node node_modules/vitest/vitest.mjs run [--reporter=verbose]` · `npx tsc --noEmit -p tsconfig.{node,web,test}.json`(각각) · `npm run lint` · `node --test scripts/*.test.mjs` |
| **관측한 게이트 산출**(exit code 아님) | **vitest 275파일 2692케이스 전건 green** · 두 파일 케이스별 **206~293ms**(첫 케이스 스파이크 0) · **typecheck 3구성 진단 0줄** · **lint 0 error / 1 warning**(기존분) · **scripts 59/59** |
| V-pair 자기확인 | r3 상태 유지 · `SELF_BLOCKED` 0 · WP-07·15·17 의 장치 무결성을 M-J·M-K 로 재확인 |
| 강제 지점 전수 | **17/17** (r3 유지) |
| **AC 자기보고**(`Criteria-Met`) | **20/21** — r3 과 같다. 이번 수정은 테스트 하네스라 AC 판정을 바꾸지 않는다 |
| **합계 검산** | `✅ 20 · ⚠️ 1 · ❌ 0 = 총 21` — 분모 21 재확인(분할·추가 없음) |
| 블로커 / 역질문 | 없음. 다만 **이 수정의 참·거짓은 다음 CI 가 판정한다**(§[놓친 잠재 문제] 1번의 3분기) |
| 대상 커밋 | `(r4 구현 — 좌표는 INDEX)` |

## [구현자 기입] Review Signals — 사실만 (r4)

- 이번에 닫은 불변식이 이전 라운드와 같은 축인가: **아니오.** r1~r3 은 "무엇을 잠그는가" 축이었고 r4 는 "무엇을 재는가" 축이다 — 케이스 예산 안에 계약이 아닌 프로세스 초기화가 섞여 있었다.
- 그것을 막았어야 할 plan 지침·AC가 있었는가: **없다.** §19 게이트 절은 ABI·egress 만 환경 제약으로 적고 **러너의 시간 변동**은 적지 않는다. 같은 축이 r1 에도 있었고(`service.test.ts` 에 `30_000` 을 달았다) 그때도 규범으로 올라가지 않았다.
- 반복해서 부딪히는 환경 한계: **windows CI 러너의 파일 I/O 시간 변동**. 이 handoff 에서 두 번째다(r1 `service.test.ts` 30s · r4 파일 DB 워밍업). 0209 도 같은 러너에서 EBUSY 로 한 번 깨졌다(`b7792119`).
- 현재 라운드 수: 4
