...
**verify r2 = PASS** (2026-08-17). 판정 원문은 [`verify.md`](verify.md).
독립 채점 **✅ 18 · ⚠️ 1 · ❌ 0**(분모 19) · r2 강제 지점 **13/13** · r1 강제 지점 **25/25 재측정
유지** · 기준 밖 중대 결함 **0**. 검증자가 characterization 12건을 **리팩토링 전 코드**에 직접
올려 12/12 green 을 재현했고, `revoke` 를 `entries.delete()` 로 바꾸는 변이를 주입해 그물이
실제로 잡는 것(1 failed)까지 확인했다. D1~D4 는 전부 닫혔다.

**r2 신규 파생 이슈 (비차단 — PASS 를 뒤집지 않는다)**

- [ ] **D5 — 자기보고의 재현 명령 2건이 문자 그대로는 재현되지 않는다.** ① `rg 'this\.(grants|verified|revisions|expirySettled)'`
      의 "통합 전 18건" 은 실제 **36 occurrence / 34 줄**이고 18 은 쓰기 지점(`.set/.add/.delete/.clear`)
      수다. ② `rg "kind: 'secret'" login.ts` 는 **2건**(`:41` 은 `AuthResult` 유니언 타입 선언,
      조립부는 `:534` 하나). **두 주장 모두 결론은 옳다** — 어긋난 것은 명령과 숫자의 짝이다.
- [ ] **D6 — AC10 문구가 원인 대신 대리 지표를 적었다 (plan 결함, 무해).** "mutator 별 쓰기
      횟수가 준다" 는 목표가 아니라 부작용이고, `put`·`revoke` 가 네 필드를 계속 쓰는 것은 그
      전이가 원래 네 축을 다 바꾸기 때문이다. 실제 목표("동기화를 강제하는 것이 없다")는 충족됐다.
