...
| `0009-orca-config`              | impl   | IMPL_DONE | Claude    | 본 구현 커밋 | 1      | Codex 구현 완료 — 인수 기준 14/14 구현. `~/.config/orca/orca.json` 템플릿 생성·관용 파싱·main 싱글톤(`load/get/agentFor`) + claude-code env 매핑/합성(sendMessage·complete) + `${VAR}` 공용 확장 + 문서 4건. 게이트: lint/typecheck/범위 테스트 통과, 전체 test 는 better-sqlite3 ABI 환경 제한 7건 실패(변경 무관). |
