...
| `0010-agent-model-select`       | impl   | IMPL_DONE | Claude     | c2a90d8   | 1      | 0009 잔여 범위(TRD §6.8) — `${adapter}-${provider}` 합성 키 + `orca:agent:list` 채널(도메인 `agent` 신설, 38→39) + Composer 모델 선택 UI(`${key}/${family}`, 턴 단위 적용 — 라이브 채널 없음) + 세션 잠금은 adapter 단위·provider 는 턴 전환 가능(`sessions.provider_key` = 마지막 사용 provider 기록, adapter+provider 조합 중복 불허, auth token 은 secret store(safeStorage) `provider:${provider key}` 에만·DB 비밀 0) + `apiKey`→`authToken` 리네임(별칭 호환) + `/agents` 동적화(파생 필드 제거) + mock UI 빗금 규약 신설. 사용자 확정 결정 6건은 plan §Context — 임의 변경 금지. |
