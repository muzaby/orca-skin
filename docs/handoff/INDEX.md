...
| `0004-auto-session-title`       | impl   | IMPL_DONE | Claude    | 9c4f1a1 | 1      | 새 세션 첫 턴 후 Haiku 1-shot 제목 요약(60자 절단 대체). `title_source` 컬럼(0007)으로 사용자 rename 보호, `SessionAdapter.complete` seam(provider 중립), telemetry 트리거 fire-and-forget, `sessionTitleEvent` 전-창 push. 인수기준 9개. PHASES Future Scope "자동 제목 생성 (요약)" 승격 예정.                                |
