...
| `0006-composer-status-line`     | plan   | READY     | Codex     | —         | 1      | Composer 상태 알림 줄 — Tier1 pill + Tier2 팝오버 **presentational UI 셸**. view-model prop 구동, 카피는 plan §카피 표 SSOT, 새대화/오늘비용(`useCost`) 실배선·요약(`onCompact`) 스텁, 상태판정은 임시 근사(TODO). 정식 상태판정·compact 실동작·Tier3 는 비범위(후속). |
