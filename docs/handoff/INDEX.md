...
| `0010-agent-model-select`       | verify | FAIL      | Codex      | 0a3c043   | 1      | 검증 FAIL(r1) — 인수 **14/15** 충족, 게이트 lint/typecheck ✅. **#14 무회귀 미충족**: 공유 테스트 헬퍼 `insertSession`(`queries.test.ts:40`)에 `provider_key` 추가로 0008 이전 스키마(`dbBefore0006`) 사용하는 기존 `0006_turn_usage` 마이그레이션 테스트가 `SqliteError: table sessions has no column named provider_key` 로 회귀(ABI 환경 제한과 무관 — Node ABI 재빌드 시 `queries.test.ts` 8/9, 신규 provider_key 2건은 통과). 수정 범위 `queries.test.ts` 1곳(verify §근본원인). 기능 로직·신규 비-DB 테스트·문서 6건은 정상. (대상 커밋 INDEX 기재 `c2a90d8`→실 `0a3c043`, 위생 노트 ①) 사용자 확정 결정 6건은 plan §Context — 임의 변경 금지. |
