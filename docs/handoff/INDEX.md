...
| `0010-agent-model-select`       | impl   | IMPL_DONE | Claude     | ecf9752   | 2      | r2 재구현 완료 — `0006_turn_usage migration` 테스트에서 0005 스키마용 인라인 `sessions` INSERT를 사용해 `provider_key` 없는 DB를 반영. 요청 최소 확인(`npm rebuild better-sqlite3 --build-from-source && npx vitest run src/main/db/queries.test.ts`) 및 전체 게이트(lint/typecheck/test 349) 통과. Claude 재검증 대기. |
