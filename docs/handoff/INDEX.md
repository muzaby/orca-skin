...
| `0011-main-decompose`           | impl   | IN_PROGRESS | Claude  | —         | 1      | 단계적 아키텍처 리팩토링 스테이지 1/3 (백엔드). router.ts 분해 + TurnRegistry sessionId 키잉(멀티세션 토대) + getSessionById(N+1 제거) + closeDb 와이어 + config/crypto 이동 + 레거시 1회성 이전 제거(사용자 결정). 비기능 = Claude 직접 구현. |
