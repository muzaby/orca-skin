...
- 재구현 턴에서는 지적을 **불변식으로 올려 전수 적용**한다.
...
- 노드 provenance는 `NEW` · `CHANGED` · `INHERITED` · `SUPERSEDED`다. `INHERITED`는 변경하지 않은 기준선의 출처, `SUPERSEDED`는 대체 노드를 반드시 가리킨다. **pair의 왼쪽 노드가 `SUPERSEDED`로 분해되면 그 pair가 들던 AC와 선택 적대 증거가 어느 대체 pair로 가는지 적고, 어디로도 가지 않으면 폐기 근거를 적는다** — 노드만 옮기면 pair가 들던 증거가 조용히 사라진다(0238 ΔV1: V1 VP-06·12의 header-index 변이가 대체 pair 어디에도 없어 AC11 키보드가 미잠금).
...
- **강제 지점 `N`은 §10 항목 수가 아니라 자리 수다.** production path의 edge 중 그 계약 값을 운반하는 edge는 모두 자리이고, §10 항목 하나가 자리 여럿을 담을 수 있다. 등록 변이에는 심을 자리를 적고, **자리가 적히지 않은 등록 변이는 이번 변경이 닿은 자리마다 심는다**(0238: VP-10 `callback 맞바꿈`을 컴포넌트 안에서만 심어, 같은 경로의 `ProviderDetail` props 전달이 두 라운드 동안 미잠금).
...
- 라운드가 **3을 초과**(§라운드).
- 같은 라운드에서 verify 없이 impl 턴이 **3을 초과** — 사용자 요구 변경에 따른 턴은 세지 않는다(§라운드).
...
- **대상 커밋 좌표는 검증자가 기입한다.** 설계자·구현자는 공유 브랜치에서 `git cat-file -t`로 확인하지 못한 해시를 좌표로 적지 않고 `(rN 구현 — 검증자 기입)` 같은 자리표시자를 남긴다 — 두 에이전트는 분리 환경이라 로컬 해시가 공유 브랜치에 그대로 도착하지 않는다(0198: r2·r3·r6 구현 해시, 0238: ΔV1 설계가 INDEX·plan 메타에 적은 V1·r1 해시 전부 죽음).
...
**라운드 1회는 plan(또는 정정) → impl → verify 한 주기다.** 라운드 번호는 verify 턴이 `verify/FAIL` 또는 `verify/RETURN_TO_PLAN`으로 끝날 때만 1 오르고, `verify/PASS`면 종료한다.

- **verify 없이 반복된 plan·impl 턴은 라운드를 올리지 않는다** — 사용자 요구 변경에 따른 Delta V 설계·재구현, 외부 피드백 재구현, red gate 수정이 모두 같은 라운드 안의 턴이다. 사용자 요구 변경은 실패가 아니므로 라운드로 세지 않는다.
- 라운드 N의 첫 impl 턴은 `rN`, 같은 라운드의 두 번째 이후 impl 턴은 `rN.k`(k=2,3…)로 적는다. `[구현자 기입]` 표제와 INDEX 대상 커밋 자리표시자가 이 라벨을 쓴다.
- INDEX `라운드` 칸은 현재 라운드 번호 = 끝난 FAIL/RETURN_TO_PLAN verify 수 + 1이다.
- 진행 중 handoff는 다음 INDEX 갱신 때 라운드 칸만 이 정의로 다시 센다. 이미 쓴 `rN` 라벨은 고치지 않고, 다음 impl 라벨이 기존 라벨과 겹치면 기존 순번을 이어 쓴다.

`handoff-review`는 별도 lifecycle state가 아니라 **메타 유지보수 경로**다. 라운드가 3을 초과하거나 같은 라운드에서 verify 없는 impl 턴이 3을 초과하면(사용자 요구 변경 턴 제외) 다음 재구현 전에 review를 수행해 전제/지침/소통 실패를 분리한다.
...
- 그 피드백으로 재구현하면 라운드는 그대로이고 같은 라운드의 impl 턴(`rN.k`)이 하나 는다. 상태는 `impl/IMPL_DONE` 으로 돌아오고 **다음 주체는 여전히 검증자다.**
...
- **밀린 verify 가 있는데 외부 피드백이 또 도착하면, 그 피드백이 무엇을 말하는지로 갈린다.** 보드의 다음 주체가 이미 검증자일 때:
  - **이번 라운드 산출물의 필수 gate 가 red 라는 보고**(자기 커밋이 CI 에서 컴파일·lint·테스트 실패)면 묻지 않고 고친다 — red gate 는 그 라운드가 아직 안 끝났다는 뜻이라 갈림길이 아니다. 라운드는 그대로 두고 impl 턴만 하나 늘리며, 보드 상태는 `impl/IMPL_DONE` 그대로 두고 verify 는 여전히 밀려 있다.
  - **새 결함·설계 의견·범위 제안**이면 순서를 사용자에게 한 줄로 묻는다 — "이 피드백을 먼저 반영할지, 밀린 verify 를 먼저 수행할지" 는 **두 해석이 모두 합리적인 갈림길**이고, 에이전트가 조용히 재구현을 고르면 verify 턴이 계속 밀린다.
  위 규칙들은 *무엇을 verify 라고 부를지* 만 정하고 *무엇을 먼저 할지* 는 정하지 않는다.

> **사례(0188).** impl 턴 **10회** 동안 verify 턴이 0회였다 — 보드는 내내 "다음 주체 = Claude(검증)"였고 매 라운드가 외부 리뷰 → 재구현으로 흘렀다. r5에서 신설된 위 규칙은 라벨 축에서는 지켜졌다(보드 비고와 `Verified-By: pending` 유지, 거짓 PASS 0). 라벨을 고정하는 규칙이 순서를 바꾸지 못해 순서 질의 조항이 붙었다. r10 이후 실제 verify 턴의 신규 코드 결함은 0건 — 비용은 "놓친 결함"이 아니라 "닫히지 않는 핸드오프"였다.
...
- 명시적인 기존 V가 없으면 Baseline V를, 기존 V를 일부 바꾸면 Delta V를 선언하고 `R↔AT`, `SD↔ST`, `AR↔IT`, `MD↔UT` 중 영향 수준을 고른다. 기준 V·이전 라운드 커밋 좌표는 공유 브랜치에서 확인한 해시만 적는다(§INDEX.md 운영).
...
- **그 분모는 이번 라운드가 한 주장에서 유도한다.** `SELF_PASS`로 올린 pair의 선택 증거 + `closed`로 적은 이슈의 인용 변이 + 이번 턴에 만든 oracle을 각각 세어 `선택 증거 N · 인용 변이 M · 새 oracle K = 표 행 T` 검산 줄을 적고, **행이 없는 claim은 `SELF_PASS`·`closed`로 적지 않는다.** 자기가 고른 변이 하나가 red인 것은 그 변이가 잠겼다는 뜻이지 그 pair·그 이슈가 닫혔다는 뜻이 아니다. (skill §8)
- **장치를 교체·삭제했다면 구 장치가 red로 만들던 변이를 새 장치도 red로 만드는지 확인한다.** 이전 라운드가 red로 잡던 자리를 이번 라운드가 green으로 되돌리면 개선이 아니라 **덮개 회귀**다 — 새 장치가 잡는 것만 보고하면 그 회귀는 보고서에 나타나지 않는다. (skill §3)
...
- **plan `§10 강제 지점` 표에 지점이 여럿이면 그 개수만큼 닫고 개수를 보고한다.** 한 지점만 닫아도 대표 경로 AC는 통과하므로 게이트 green은 전수를 뜻하지 않는다. 일부만 닫았으면 남긴 곳을 적는다. **모든 행의 지점 수를 전수 검색으로 세고 검색 명령을 적는다 — 세는 단위는 §10 항목이 아니라 자리이고, pair production path에서 그 계약 값을 운반하는 edge를 포함한다** — 설계자·자기가 정한 분모에 그대로 맞추면 `N/N`은 아무것도 말하지 않는다. **그 검색의 술어는 불변식의 주어로 쓴다 — 네가 쓴 해법의 이름이 아니다.** 해법 이름으로 세면 이미 고친 지점만 분모에 오르고, 결함 심기는 그 집합의 감도만 증명한다. (skill §2)
- **지적으로 도는 모든 재구현 턴(verify/FAIL 파생 이슈·외부 PR 리뷰·사용자 지적)는 지적을 재현하는 데서 멈추지 않는다** — 불변식을 한 문장으로 올리고, 성립해야 할 지점을 **그 불변식을 낳은 문장부터** 전수로 닫고, 이번 수정이 만든 새 표면을 스스로 검사한다. **파생 이슈의 계약은 `출처`가 가리키는 V-pair·Decision·AC·§10·현재 산출물 gate이고 `대응 방향`은 제안이다** — 닫힘은 제안 수행이 아니라 그 계약의 성립이다. Decision으로 규칙을 적는 것은 적용을 보장하지 않는다. (skill §5)
...
- **재구현 턴도 `[구현자 기입]`의 같은 이름 필드를 다시 채운다** — 표제만 바꾸고 필드를 줄이지 않으며, 해당 없는 필드는 지우지 말고 `해당 없음`으로 남긴다. 산문 목록으로 갈아타면 필드 이름과 함께 그 필드가 요구하던 증거도 사라진다. (skill §8)
...
- **등록된 적대 증거를 다시 센다** — 구현자 보고와 무관하게 pair가 선택한 변이·닫는 파생 이슈의 인용 변이·새 oracle의 민감도 검사를 다시 실행한다. 자리가 적히지 않은 등록 변이는 이번 변경이 닿은 pair path의 자리마다 심는다. 검출되지 않은 인용 변이가 있으면 구현자가 `closed`로 적었어도 그 이슈는 닫히지 않으며, 소거 변이는 잔여물 진단이 0이 될 때까지 밀어 그 상태의 게이트로 판정하고, 형제 슬롯이 서로 다른 계약을 가지면 두 산출을 맞바꾸는 변이까지 본다. (skill §4)
- **이전 라운드가 red로 관측한 변이를 다시 실행해 결과를 나란히 적는다.** `red → green`이면 그 자체가 finding이다 — 구현자가 장치를 교체하며 구 장치가 잡던 자리를 잃은 것이고, 새 장치가 무엇을 잡는지만 보면 보이지 않는다. (skill §4)
- **구현자가 이번 라운드의 검증자와 같은 에이전트면 분모에 구현 보고가 이름을 대지 않은 적대 축을 최소 하나 넣는다.** 보고된 변이의 재실행은 자신이 할 때 자기 목록의 재실행이라 자기 사각을 통과시킨다 — 다른 지점·형제 지점·분모 독립 재열거 중 하나를 만들고, 못 만들었으면 그 사실을 적고 PASS로 올리지 않는다. (skill §4)
