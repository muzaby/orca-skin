...
- **이번 턴에 만들거나 고친 검사 장치는 결함을 볼 수 있음을 먼저 보인다.** 테스트·grep 스윕·게이트 모두 해당하며, 결함을 심어 실패하는지 확인한다. **고친 지점 하나가 아니라 장치의 판정 지점마다 하나씩 심는다** — 지적받은 지점에만 심으면 검사가 지적한 사람의 시야를 물려받는다. 눈이 없는 장치의 `0건`은 전수의 증거가 아니다. (skill §3·§8)
...
- **이번 라운드가 닫았다는 잠금을 다시 센다** — 구현자 보고와 무관하게, 닫는 파생 이슈가 **인용한 변이**를 그대로 다시 심는다(인용 변이가 없을 때만 고친 hunk가 분모다). 검출되지 않은 인용 변이가 있으면 구현자가 `closed`로 적었어도 그 이슈는 닫히지 않았다. (skill §4)
