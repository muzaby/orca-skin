...
- Product/UX에서 모듈 내부까지 **Delta V 노드와 검증 pair**를 먼저 선언해 구현·검증의 공통 추적축을 만든다.
...
- `REQUIRED`·`REGRESSION` V-pair와 전역 불변식을 구현하고 pair별 자기검증을 남긴다.
...
- AC 밖 결함을 역방향으로 찾되, 현재 V-pair·ACTIVE Decision·기존 blocking 이슈·전역 불변식에 귀속되는 결함만 현재 handoff를 막는다.
...
V는 기존 Decision·AC·강제 지점·production-path·mutation 규칙을 대체하지 않고 **어떤 설계 계약을 어떤 증거로 닫는지 연결하는 추적축**이다. 관계는 many-to-many이며 억지로 1:1로 만들지 않는다.
...
- pair requiredness는 `REQUIRED` · `REGRESSION` · `NOT_REQUIRED`다. 모든 `NEW`·`CHANGED` 왼쪽 노드는 같은 레벨의 `REQUIRED` pair가 필요하다. 변경 경로가 기존 상위 동작에 닿으면 그 `INHERITED` 노드는 `REGRESSION` pair로 다시 닫는다.
- `NOT_REQUIRED`는 영향받지 않은 `INHERITED` 노드에만 쓰며 출처·기존 증거·비영향 근거를 적는다. 필요한 행이 없는 것은 암묵적 `NOT_REQUIRED`가 아니라 `PLAN_GAP`이다.
- 각 pair는 stable ID, 좌·우 노드, 레벨, `start → edges → end` production path, 강제 지점 전수 `N`, 직접 판정 oracle, 음성 대조/결함 변이를 갖는다. 강제 지점이 없는 pair도 행을 생략하지 않고 `0 + 이유`로 적는다. 증거 하나가 여러 pair를 직접 판정하면 중복 실행하지 않고 어느 pair를 함께 닫는지 명시한다.
...
보안·데이터 무결성·현재상태 문서 정합·repository/message-bus 정합·수정 subtree의 필수 gate는 **적용되는 handoff 전체**를 가로지르는 전역 불변식이다. 해당 불변식에 직접 매핑되는 결함은 현재 pair 밖에서 발견돼도 blocking이다. plan은 이번 범위에 적용할 전역 불변식과 증거를 열거하고, 각 subtree `AGENTS.md`가 명령의 정본을 유지한다.

검증 finding은 다음 네 disposition 중 하나다.

| disposition | 조건 | handoff 결과 |
|---|---|---|
| `BLOCKING` | 현재 pair·ACTIVE Decision·기존 blocking 이슈·전역 불변식 위반 | `FAIL` |
...
명시된 기존 계약 위반은 증거 행이 빠졌더라도 `PAIR_FAIL`이다. 동시에 계획 누락을 기록할 수 있지만 결함을 `PLAN_GAP`으로 낮추지 않는다. 반대로 구현자가 새 요구나 pair를 발명하거나 모순된 기준 중 하나를 골라야만 고칠 수 있다면 구현 실패가 아니라 `PLAN_GAP`이며 설계자가 Delta V를 정정한다.
...
Part I/II, Decision Ledger, Technical Design AS-IS/TO-BE, Delta V가 포함된 새 템플릿은 **신규 handoff부터 적용**한다.
...
- 단순 verify/FAIL 파생 이슈나 RETURN_TO_PLAN Delta 정정은 영향받은 규범 행만 갱신하고 기존 상단 전체를 강제로 재작성하지 않는다.
...
plan/DRAFT
  ...
  → impl/IMPL_DONE
  → verify/PASS ── 종료
  ├ verify/FAIL ── pair/전역 불변식 위반 → 재구현 → 재검증
...
| verify/PASS | 완료 | — |
| verify/FAIL | 명시 계약 pair 또는 전역 불변식 미충족 | 구현자 |
...
- `R↔AT`, `SD↔ST`, `AR↔IT`, `MD↔UT` Delta V와 적용 전역 불변식을 선언한다.
...
- `REQUIRED`·`REGRESSION` pair와 전역 불변식을 작업 목록으로 만들고 pair별 자기검증을 남긴다.
...
- **이번 턴에 만들거나 고친 검사 장치는 결함을 볼 수 있음을 먼저 보인다.** 테스트·grep 스윕·게이트 모두 해당하며, 결함을 심어 실패하는지 확인한다. **고친 지점 하나가 아니라 장치의 판정 지점마다 하나씩 심는다** — 지적받은 지점에만 심으면 검사가 지적한 사람의 시야를 물려받는다. **장치가 반응하는 방향도 맞춘다** — 불변식이 “X가 쓰인다”인데 장치가 여분의 사본·남은 잔여물에만 반응하면 X를 지운 회귀에 침묵하므로 X를 지우는 변이도 함께 심는다. 눈이 없는 장치의 `0건`은 전수의 증거가 아니다. (skill §3·§8)
- **이번 라운드가 닫는 것이 잠겼음을 결함을 심어 보인다.** 앞 항목의 쌍대다 — 검사 장치에 눈이 있어도 *내 수정이 그 시야 안에 없으면* 배선을 지운 회귀가 초록으로 통과한다. **분모는 닫는 파생 이슈가 인용한 변이이고, 인용된 변이가 없을 때만 고친 hunk다** — 동작을 보존하는 추출·재배치 라운드는 hunk를 되돌려도 초록이 정답이라 그 되돌림이 아무것도 재지 못한다. 무엇을 심었고 무엇이 실패했는지 적는다 — hunk 분모에서 아무것도 실패하지 않으면 그 수정은 잠기지 않은 것이므로 테스트를 먼저 만들고, 인용 변이가 검출되지 않으면 그 이슈는 닫히지 않았다. (skill §3·§5)
...
- **지적으로 도는 모든 재구현 라운드(verify/FAIL 파생 이슈·외부 PR 리뷰·사용자 지적)는 지적을 재현하는 데서 멈추지 않는다** — 불변식을 한 문장으로 올리고, 성립해야 할 지점을 **그 불변식을 낳은 문장부터** 전수로 닫고, 이번 수정이 만든 새 표면을 스스로 검사한다. **파생 이슈의 계약은 `출처`가 가리키는 V-pair/전역 불변식·AC·§10 행이고 `대응 방향`은 제안이다** — 닫힘은 제안 수행이 아니라 그 계약의 성립이다. Decision으로 규칙을 적는 것은 적용을 보장하지 않는다. (skill §5)
...
- **이번 라운드가 닫았다는 잠금을 다시 센다** — 구현자 보고와 무관하게, 닫는 파생 이슈가 **인용한 변이**를 그대로 다시 심는다(인용 변이가 없을 때만 고친 hunk가 분모다). 검출되지 않은 인용 변이가 있으면 구현자가 `closed`로 적었어도 그 이슈는 닫히지 않았다. **소거 변이는 잔여물 진단(unused import·dead local)이 0이 될 때까지 밀어 그 상태의 게이트로 판정한다** — 첫 red는 치우면 사라지는 부산물이다. (skill §4)
- pair/전역 불변식 위반은 `FAIL`, 계획 누락은 `RETURN_TO_PLAN`, 그 밖의 발견은 `NON_BLOCKING` 또는 `NEXT_HANDOFF`로 이관한다.
