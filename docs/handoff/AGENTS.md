...
2. **Create (없으면)** — 기존 행 중 **`max(번호)+1`**(아래 "디렉토리 구조" / INDEX 말미 규칙)로 `<NNNN-slug>/plan.md`(plan.template 복사)를 만들고 **설계 턴부터 진입**한다. 구현 요청이라도 **코드로 바로 건너뛰지 않는다** — §1 설계 강화 규칙(의도/조사/의존/UX/리스크 + READY 전 self-review)을 거친다.
...
- **FAIL**: verify "미충족 요구사항" 체크리스트 작성 → **미해결 문제는 plan 에 "파생 이슈(Derived Issues)" 챕터를 신설**해 이관(구현자 코멘트 참조, 또는 사용자 결정이 필요하면 **검증자가 사용자 결정을 대리 기록** — plan.md 는 검증자가 쓴다) → `INDEX.md` `verify/FAIL`, 다음=Codex, 라운드 +1 로 구현 턴 루프백.
