...
- **외부 피드백 재구현 라운드는 지적을 재현하는 데서 멈추지 않는다** — 불변식을 한 문장으로 올리고, 성립해야 할 지점을 **그 불변식을 낳은 문장부터** 전수로 닫고, 이번 수정이 만든 새 표면을 스스로 검사한다. Decision으로 규칙을 적는 것은 적용을 보장하지 않는다. (skill §5)
