...
- **이번 라운드에 고친 프로덕션 지점을 되돌리면 실패하는 테스트가 있어야 한다.** 앞 항목의 쌍대다 — 검사 장치에 눈이 있어도 *내 수정이 그 시야 안에 없으면* 배선을 지운 회귀가 초록으로 통과한다. 고친 hunk를 되돌려 무엇이 실패했는지 적고, 아무것도 실패하지 않으면 테스트를 먼저 만든다. (skill §3)
