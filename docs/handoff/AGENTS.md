...
> **실측 이력.** 0188 은 impl 라운드 **10회**를 도는 동안 verify 턴을 한 번도 거치지 않았다 — INDEX 는 내내 "다음 주체 = Claude(검증)" 였는데 매 라운드가 외부 리뷰 → 재구현으로 흘렀다. 위 규칙은 r5 에서 신설됐고 **라벨 축에서는 지켜졌다**(라운드 6~10 의 보드 비고가 매번 "다음은 재구현이 아니라 verify 턴이다" 라고 스스로 적었고 `Verified-By: pending` 도 유지됐다 — 거짓 PASS 는 없었다). **그런데도 라운드는 계속 올라갔다.** 라벨을 고정하는 규칙은 순서를 바꾸지 못하기 때문이고, 그래서 위에 순서 질의 조항이 붙었다. (r10 이후 실제 verify 턴에서 신규 코드 결함은 0건이었다 — 밀린 verify 의 비용은 이 사례에서 "놓친 결함" 이 아니라 "닫히지 않는 핸드오프" 로 나타났다.)
