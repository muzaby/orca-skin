...
- **밀린 verify 가 있는데 외부 피드백이 또 도착하면, 그 피드백이 무엇을 말하는지로 갈린다.** 보드의 다음 주체가 이미 검증자일 때:
  - **이번 라운드 산출물의 필수 gate 가 red 라는 보고**(자기 커밋이 CI 에서 컴파일·lint·테스트 실패)면 묻지 않고 고친다 — red gate 는 그 라운드가 아직 안 끝났다는 뜻이라 갈림길이 아니다. 라운드는 올리되 보드 상태는 `impl/IMPL_DONE` 그대로 두고, verify 는 여전히 밀려 있다.
