...
window.orca = {
  ...
  artifacts: {
    ...
    save: async req => { report.calls.push(['save', req]); const failure = failSave; failSave = false; if (cancelSave) { cancelSave = false; return { outcome: 'cancelled', items: [] } }; return { outcome: 'completed', items: req.publicationIds.map(publicationId => ({ publicationId, outcome: failure ? 'failed' : 'saved', ...(failure ? { reason: 'forbidden' } : {}) })) } },
...
Object.defineProperty(navigator, 'clipboard', { configurable: true, value: {
  writeText: async text => {
    report.calls.push(['copy', text])
    if (failCopy) { failCopy = false; throw new Error('Synthetic clipboard denial') }
  }
} })
...
window.nativeMount = async () => {
  ...
  check('title-and-two-tabs', find('[data-artifact-catalog] h1').textContent === '아티팩트12개' && all('[role="tab"]').length === 2 && tab('all').textContent === '전체' && tab('pinned').textContent === '고정됨')
  check('initial-total-count', find('[data-artifact-catalog-count]').textContent === '12개')
...
window.nativePinned = async () => {
  ...
  check('pinned-preserves-total-count', find('[data-artifact-catalog-count]').textContent === '12개')
  return report
}
window.nativeSearch = async () => {
  ...
  check('search-preserves-total-count', find('[data-artifact-catalog-count]').textContent === '12개')
  return report
}
...
window.nativePreview = async () => {
  ...
  check('viewer-copy-exact-content', report.calls.some(([kind, value]) => kind === 'copy' && value === markdown))
  check('viewer-copy-success-feedback', find('[data-artifact-viewer] [role="status"]')?.textContent.includes('복사했습니다'))
  failCopy = true
  await click(action('copy'))
  check('viewer-copy-failure-feedback', find('[data-artifact-viewer] [role="status"]')?.textContent.includes('복사하지 못했습니다'))
...
window.nativeExpand = async () => {
  ...
  check('viewer-restore-label', action('expand').getAttribute('aria-label') === '원래 크기로')
...
window.nativeDownload = async () => {
  ...
  failSave = true
  await click(action('download'))
  check('viewer-save-failure-feedback', find('[data-artifact-viewer] [role="status"]')?.textContent.includes('이 대화에서 파일을 사용할 수 없습니다'))
  cancelSave = true
  await click(action('download'))
  check('viewer-save-cancel-feedback', find('[data-artifact-viewer] [role="status"]')?.textContent.includes('취소'))
  await click(action('expand'))
...
window.nativeConfirmDelete = async () => {
  ...
  check('delete-updates-total-count', find('[data-artifact-catalog-count]').textContent === '11개')
  await click(nav('새 대화'))
  await click(nav('아티팩트'))
...
window.nativeOutputCards = async () => {
  ...
  check('ordinary-reveal-material-file-open', reveal?.querySelector('path')?.getAttribute('d') === 'M240-80q-33 0-56.5-23.5T160-160v-640q0-33 23.5-56.5T240-880h320l240 240v240h-80v-200H520v-200H240v640h360v80H240Zm638 15L760-183v89h-80v-226h226v80h-90l118 118-56 57Zm-638-95v-640 640Z')
  await click(reveal)
  check('ordinary-reveal-exact-target', report.calls.some(([kind, req]) => kind === 'reveal' && req.sessionId === 'native-work' && req.publicationId === 'ordinary-pdf'))
...
window.nativeTaskExpand = async (kind = 'work') => {
  ...
  const button = [...tile.querySelectorAll('button')].find(node => node.getAttribute('title') === '패널 펼치기')
  await click(button)
