...
const currentFile = output("current-file", "file", "두 번째 턴의 일반 파일");
...
const messages = () => [
  ...
  },
  {
    ...
    parts: [
      ...
      text("두 번째 확정 응답입니다."),
      { type: "artifact", artifact: currentFile },
...
const checkOrder = (label) => {
  const user = find(
      '.app-frame-transcript [title="이어서 두 번째 결과도 만들어 주세요."]',
    ),
    committed = body("두 번째 확정 응답입니다."),
    live = body("두 번째 실시간 응답이 이어집니다."),
    status = spark(),
    file = card("current-file"),
    footer = card("published");
  check(
    label + "-user-body-live-file-artifact-spark-dom-order",
    precedes(user, committed) &&
      precedes(committed, live) &&
      precedes(live, file) &&
      precedes(file, footer) &&
      precedes(footer, status),
  );
  check(
    label + "-user-body-live-cards-spark-vertical-order",
    user.getBoundingClientRect().bottom <=
      committed.getBoundingClientRect().top &&
      committed.getBoundingClientRect().bottom <=
        live.getBoundingClientRect().top &&
      live.getBoundingClientRect().bottom <= file.getBoundingClientRect().top &&
  ...
  check(
    label + "-spark-last-in-assistant-response",
    status.closest(".group\\/msg").lastElementChild.contains(status) &&
      precedes(footer, status),
  );
};
...
window.nativeLifecycle = async () => {
  const file = card("current-file"),
    publishedCard = card("published");
  const download = [...file.querySelectorAll("button")].find(
    (node) => node.textContent.trim() === "다운로드",
  );
  const pending = {};
  delayedSave = pending;
  ...
  check(
    "pending-save-started",
    typeof pending.resolve === "function" && download.disabled,
  );
        ...
        sessions: {
          "native-turn": {
            ...
            session: {
              ...
              inflight: value,
              activityForeground: value ? "streaming" : "idle",
              turnStartedAt: value ? Date.now() : null,
            },
          },
        },
      })),
  ...
  await tick();
  check(
    "complete-retains-both-card-dom-and-save",
    card("current-file") === file &&
      card("published") === publishedCard &&
      download.disabled &&
      !spark(),
  );
  setPending(true);
  await tick();
  check(
    "resume-retains-both-card-dom-and-save",
    card("current-file") === file &&
      card("published") === publishedCard &&
      download.disabled,
  );
  pending.resolve({
    outcome: "completed",
    items: [
      { publicationId: "current-file", outcome: "failed", reason: "forbidden" },
    ],
  });
  await tick();
  check(
    "save-result-survives-pending-state-round-trip",
    !download.disabled &&
      [...file.parentElement.querySelectorAll('[role="status"]')].some((node) =>
        node.textContent.includes("current-file.md"),
      ),
  );
  await click(download);
  check(
    "save-success-clears-preserved-failure",
    ![...file.parentElement.querySelectorAll('[role="status"]')].some((node) =>
      node.textContent.includes("current-file.md"),
    ),
  );
  checkOrder("after-pending-round-trip");
  return report;
};
...
window.nativeFirstResponse = async (hasUser = true) => {
  flushSync(() =>
    useChatStore.setState((state) => ({
      sessions: {
        "native-turn": {
          ...
          session: {
            ...state.sessions["native-turn"].session,
            messages: hasUser ? [messages()[0]] : [],
            inflight: true,
            activityForeground: "streaming",
            turnStartedAt: Date.now(),
            activityBackgroundTaskCount: 0,
          },
          live: { text: "첫 실시간 응답입니다.", reasoning: "" },
        },
      },
    })),
  );
  await tick();
  const live = body("첫 실시간 응답입니다."),
    status = spark();
  const label = hasUser ? "first-user-response" : "empty-pending-response";
  const user = find(
    '.app-frame-transcript [title="첫 번째 결과를 만들어 주세요."]',
  );
  check(
    label + "-body-before-spark",
    (!hasUser || precedes(user, live)) &&
      precedes(live, status) &&
      live.getBoundingClientRect().bottom <= status.getBoundingClientRect().top,
  );
  check(
    label + "-single-spark-without-stale-cards",
    all('.app-frame-transcript svg[viewBox="0 0 100 100"]').length === 1 &&
      !all(".app-frame-transcript [data-artifact-preview]").length,
  );
  return report;
};
