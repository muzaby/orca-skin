...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await run('recent-expanded', 'window.nativeRecentExpanded("initial")')
    await run('recent-collapsed', 'window.nativeRecentCollapse()')
    win.webContents.focus()
    win.webContents.sendInputEvent({ type: 'keyDown', keyCode: 'Return' })
    win.webContents.sendInputEvent({ type: 'char', keyCode: '\r' })
    win.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'Return' })
    await run('recent-enter-expanded', 'window.nativeRecentExpanded("enter")')
    win.webContents.sendInputEvent({ type: 'keyDown', keyCode: 'Space' })
    win.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'Space' })
    await run('recent-arrival-collapsed', 'window.nativeRecentArrival()')
    win.webContents.sendInputEvent({ type: 'keyDown', keyCode: 'Return' })
    win.webContents.sendInputEvent({ type: 'char', keyCode: '\r' })
    win.webContents.sendInputEvent({ type: 'keyUp', keyCode: 'Return' })
    await run('recent-arrival-expanded', 'window.nativeRecentExpanded("after-arrival")')
    ...
    await run('catalog-controls', 'window.nativeCatalogControls()')
