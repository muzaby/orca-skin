...
const recentGroup = () => document.querySelector('[data-context="recents"]')
const recentToggle = () => recentGroup().querySelector(':scope > button')
const recentBody = () => document.getElementById(recentToggle().getAttribute('aria-controls'))
window.nativeRecentExpanded = async phase => {
  await tick()
  const toggle = recentToggle(), body = recentBody()
  check(`recent-${phase}-expanded`, toggle.getAttribute('aria-expanded') === 'true' && !!body)
  check(`recent-${phase}-control-label`, toggle.textContent.trim() === '최근 대화' && toggle.type === 'button')
  const allRows = [...recentGroup().querySelectorAll('[data-session-id]')]
  check(`recent-${phase}-all-rows-under-body`, allRows.length >= 4 && allRows.every(row => body?.contains(row)))
  for (const [id, title] of [['nav-draft', '분기 / 초안'], ['s1', '카메라 / 센서 검증 결과 검토']]) {
    const row = body?.querySelector(`[data-session-id="${id}"]`)
    check(`recent-${phase}-${id}-title-without-project-prefix`, row?.title === title && row.querySelector(':scope > span:last-of-type')?.textContent === title)
  }
  check(`recent-${phase}-pinned-chat-in-own-group`, !!document.querySelector('[data-context="pinned"] [data-session-id="pinned"]') && !body?.querySelector('[data-session-id="pinned"]'))
  if (phase !== 'initial') check(`recent-${phase}-toggle-keeps-focus`, document.activeElement === toggle)
  if (phase === 'after-arrival') check('recent-arrival-visible-after-reopen', !!body?.querySelector('[data-session-id="recent-arrival"]'))
}
window.nativeRecentCollapse = async () => {
  await click(recentToggle())
  check('recent-click-collapses-whole-body', recentToggle().getAttribute('aria-expanded') === 'false' && !recentBody() && !recentGroup().querySelector('[data-session-id]'))
  check('recent-collapse-preserves-other-groups', !!navRow('a') && !!document.querySelector('[data-context="pinned"] [data-session-id="pinned"]'))
  recentToggle().focus()
}
window.nativeRecentArrival = async () => {
  await tick()
  check('recent-space-collapses-whole-body', recentToggle().getAttribute('aria-expanded') === 'false' && !recentBody())
  const arrival = { ...sessions[0], id: 'recent-arrival', projectId: null, title: '새로 도착한 대화' }
  sessions.unshift(arrival)
  flushSync(() => useSessionsStore.setState(state => ({ byId: { ...state.byId, [arrival.id]: arrival }, recentIds: [arrival.id, ...state.recentIds] })))
  await tick()
  check('recent-arrival-preserves-manual-collapse', recentToggle().getAttribute('aria-expanded') === 'false' && !recentBody())
  await navigate('/new')
  check('recent-route-change-preserves-manual-collapse', recentToggle().getAttribute('aria-expanded') === 'false' && !recentBody())
  recentToggle().focus()
}
