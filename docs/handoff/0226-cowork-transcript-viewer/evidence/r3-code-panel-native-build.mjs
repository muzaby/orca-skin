...
const cache = fs.mkdtempSync(path.join(cacheRoot, 'r3-code-panel-native-'))
const built = await require('esbuild').build({
  absWorkingDir: repo,
  entryPoints: [path.join(fixture, 'r3-code-panel-native-browser.tsx')],
...
fs.writeFileSync(path.join(cache, 'fixture.html'), `<!doctype html><html lang="ko" data-theme="white"><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="${csp}"><link rel="stylesheet" href="${pathToFileURL(path.join(assets, styles[0])).href}"><div id="root"></div><script src="fixture.js"></script></html>`)
fs.writeFileSync(path.join(cache, 'manifest.json'), JSON.stringify({
  ...
  stylesheet: createHash('sha256').update(fs.readFileSync(path.join(assets, styles[0]))).digest('hex')
}, null, 2))
