...
현재 피드백은 [r4 패널 계획](panel-plan-r4.md)이 우선한다. 이 문서 본문은 V1 기준선이며 [r2 패널 계획](panel-plan.md)과 [r3 패널 계획](panel-plan-r3.md)의 대체되지 않은 결정·AC·V·강제 지점을 유지한다. Codex r3 자기확인은 [구현 보고 r3](impl-r3.md)에 기록했다. [r1 보고](impl.md)와 [r2 보고](impl-r2.md)는 과거 증거이며 독립 verify는 미수행이다. 0223의 일반 생성물 기준과 미완료 상태는 이 핸드오프로 닫지 않는다.
