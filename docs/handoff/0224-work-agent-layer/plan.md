...
| D-004 | 게시 원본·직접 호출·삭제 UX와 세 섹션 패널을 유지 | 이미 선택한 제품 동작 보존 | 앞선 사용자·0223 | ACTIVE |
| D-005 | 컨텍스트 수집·뷰어·일반 생성물 감지는 기존 미정/후속 상태 유지 | 새 답변이 있을 때만 범위 변경 | 앞선 사용자 | ACTIVE |
...
## 7. Requirements / Acceptance — 제안 V1

`P-*`는 §8의 실제 경로, `EP-*`는 §10의 강제 지점이다. READY 전에는 구현 합격 기준으로 잠그지 않는다.
