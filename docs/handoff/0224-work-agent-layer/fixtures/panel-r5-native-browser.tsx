...
window.orca = {
  ...
  artifacts: {
    list: async () => (outputs ? structuredClone(refs) : []),
...
window.panelInteractions = async () => {
  ...
  checks.workExpandAbsent =
    !button("작업 넓게 보기") && !button("작업 원래 크기로");
  checks.workSaveAllAbsent = ![...task.querySelectorAll("button")].some(
    (button) => button.textContent.includes("모두 저장"),
  );
  flushSync(() => chatActions.setRightPanelColWidth(0, 640));
  await settled();
  const resizedWidth = task.getBoundingClientRect().width;
  checks.workResizePreservesContent =
    resizedWidth > width && body === task.querySelector("section");
  ...
  const taskButton = document.querySelector(
    '[data-context="plan"] [data-task-detail-trigger="agent:1"]',
  );
  flushSync(() => taskButton.click());
  await settled();
  const codingOverview = document.querySelector("[data-plan-task-overview]");
  checks.codingDetail =
    codingOverview.hidden &&
    codingOverview.inert &&
    document
      .querySelector("[data-plan-task-detail]")
      ?.textContent.includes("상태");
  await click("목록으로");
  checks.codingDetailBack =
    !codingOverview.hidden &&
    !codingOverview.inert &&
    document.querySelector('[data-context="plan"] h1')?.textContent ===
      "테스트 플랜" &&
    document.activeElement === taskButton;
  let copied = null;
...
window.panelLandingChecks = async () => {
  ...
  checks.largeNormalHero =
    styles.hero.fontSize === "32px" && styles.hero.fontWeight === "400";
  checks.noModeTooltip = !controls.querySelector(
    '[role="tooltip"], [title], [aria-describedby]',
  );
  ...
  await settled();
  checks.workGitAbsent =
    !document.querySelector(
      '[data-surface="git-row"], [data-surface="branch-worktree-group"]',
    ) && gitCalls.length === workStart;
  const cwdPanel = document.querySelector('[data-surface="cwd-panel"]');
  const cwdButton = cwdPanel.querySelector(
    'button[aria-label="작업 폴더 선택"]',
  );
  const addDirectoryButton = [...cwdPanel.querySelectorAll("button")].find(
    (button) => button.title === "다른 폴더 추가",
  );
  assert(cwdButton && addDirectoryButton, "Landing directory controls absent");
  const workAddLeft = addDirectoryButton.getBoundingClientRect().left;
  checks.workAddDirectoryNoGap =
    Math.abs(
      workAddLeft -
        cwdButton.getBoundingClientRect().right -
        parseFloat(getComputedStyle(cwdPanel).columnGap),
    ) < 1;
  ...
  checks.codingGitRestored =
    !!cwdPanel.querySelector('[data-surface="branch-worktree-group"]') &&
    addDirectoryButton.getBoundingClientRect().left > workAddLeft;
  const isolation = cwdPanel.querySelector('input[type="checkbox"]');
  assert(isolation, "Coding isolation checkbox absent");
  flushSync(() => isolation.click());
  await settled();
  checks.codingIsolationSelected =
    useChatStore.getState().sessions.landing.session.worktreeIsolation === true;
  ...
  checks.workGitReturnAbsent =
    !cwdPanel.querySelector('[data-surface="branch-worktree-group"]') &&
    Math.abs(addDirectoryButton.getBoundingClientRect().left - workAddLeft) <
      1 &&
    useChatStore.getState().sessions.landing.session.worktreeIsolation === true;
  flushSync(() => unselected.click());
  await settled();
  checks.codingSelectionRetained =
    cwdPanel.querySelector('input[type="checkbox"]')?.checked === true;
  flushSync(() => selected.click());
  await settled();
  for (const [name, ok] of Object.entries(checks))
...
const longTitle =
  "아주 긴 제목의 다운로드 폴더 분석과 정리 보고서 및 참조 리소스 "
    .repeat(8)
    .trim();
function manyTasks(count) {
  ...
  for (let i = 1; i <= count; i++) {
    const id = String(i),
      subject = i === 1 ? longTitle : `자료 정리 작업 ${i}`;
    const run = `density-create-${id}`;
    parts.push(
      {
        type: "tool_call",
        toolRunId: run,
    ...
    if (i <= 2)
      parts.push(
        {
          type: "tool_call",
          toolRunId: `density-update-${id}`,
          toolName: "TaskUpdate",
          args: { taskId: id, status: i === 1 ? "in_progress" : "completed" },
        },
        {
          type: "tool_result",
          toolRunId: `density-update-${id}`,
          ...
          structuredOutput: {
            ...
            statusChange: {
              from: "pending",
              to: i === 1 ? "in_progress" : "completed",
            },
          },
        },
      );
  }
  return [{ role: "assistant", createdAt: 1700000000000, parts }];
}
function setRefs(count) {
  refs.splice(
    0,
    refs.length,
    ...Array.from({ length: count }, (_, i) => ({
      ...originalRefs[0],
      publicationId: `density-pub-${i}`,
      artifactFileId: "file-1",
      title: longTitle + i,
    })),
  );
...
function panelMeasures() {
  const available = document.querySelector("[data-work-panel-available]");
  const card = document.querySelector('[data-context="task"]');
  assert(available && card, "Work available-height boundary missing");
  return {
    available: available.getBoundingClientRect().height,
    card: card.getBoundingClientRect().height,
    sections: Object.fromEntries(
      ["progress", "output", "context"].map((name) => {
        const section = card.querySelector(`[data-task-section="${name}"]`);
        const body = section?.querySelector(
          `[data-task-section-scroll="${name}"]`,
        );
        assert(section && body, "Section and scroll owner missing: " + name);
        return [
          name,
          {
...
window.panelR5Interactions = async () => {
  const checks = {},
    metrics = {};
  window.panelR5LastResult = { checks, metrics };
  mainDirectory = false;
  composerChecks = false;
  fullComposerChecks = false;
  transcriptChecks = false;
  closeMenu();
  seed("work", false, "density-work");
  flushSync(() => root.render(<Shell />));
  await settled();
  await refreshArtifactList("density-work");
  await settled();
  metrics.empty = panelMeasures();
  checks.emptyNaturalHeight = metrics.empty.card < metrics.empty.available - 20;
  outputs = true;
  setRefs(1);
  patchSession({ messages: manyTasks(1), extraDirs: ["C:/" + longTitle] });
  await refreshArtifactList("density-work");
  await settled();
  metrics.short = panelMeasures();
  const workRow = document.querySelector("[data-task-row]");
  metrics.workRow = rowStyles(workRow);
  checks.workTaskEllipsis =
    ellipsis(workRow.querySelector("[data-task-title]")) &&
    workRow.textContent.includes(longTitle);
  const outputTitle = document.querySelector(
    '[data-task-section="output"] article [title]',
  );
  checks.outputEllipsis =
    ellipsis(outputTitle) && outputTitle.title.includes(longTitle);
  const directory = document.querySelector(
    '[data-surface="context-directory"]',
  );
  checks.contextEllipsis =
    ellipsis(directory.querySelector("span")) &&
    directory.title.includes(longTitle) &&
    directory.getAttribute("aria-label").includes(longTitle);
  patchSession({ messages: manyTasks(4) });
  await settled();
  metrics.growing = panelMeasures();
  checks.naturalGrowth =
    metrics.growing.sections.progress.height >
      metrics.short.sections.progress.height &&
    metrics.growing.card > metrics.short.card;
  checks.noSiblingHeightAllocation = ["output", "context"].every(
    (name) =>
      Math.abs(
        metrics.growing.sections[name].height -
          metrics.short.sections[name].height,
      ) < 1,
  );
  patchSession({ messages: manyTasks(45) });
  await settled();
  metrics.progressOverflow = panelMeasures();
  checks.progressOnlyScroll =
    metrics.progressOverflow.sections.progress.scrollHeight >
      metrics.progressOverflow.sections.progress.clientHeight &&
    ["output", "context"].every(
      (name) =>
        Math.abs(
          metrics.progressOverflow.sections[name].height -
            metrics.short.sections[name].height,
        ) < 1,
    );
  setRefs(35);
  ...
  await settled();
  patchSession({
    extraDirs: Array.from({ length: 35 }, (_, i) => `C:/資料${i}/${longTitle}`),
  });
  await settled();
  metrics.allOverflow = panelMeasures();
  checks.eachSectionThirdCap = Object.values(
    metrics.allOverflow.sections,
  ).every(
    (section) =>
      section.height <= metrics.allOverflow.available / 3 + 1 &&
      section.scrollHeight > section.clientHeight,
  );
  checks.outerStaysWithinAvailable =
    metrics.allOverflow.card <= metrics.allOverflow.available + 1;
  checks.outputSaveAllAbsentWithMany = !document
    .querySelector('[data-task-section="output"]')
    .textContent.includes("모두 저장");
  const overview = document.querySelector("[data-work-task-overview]");
  const scrollOwners = ["progress", "output", "context"].map((name) =>
    overview.querySelector(`[data-task-section-scroll="${name}"]`),
  );
  scrollOwners.forEach((owner) => {
    owner.scrollTop = 24;
  });
  const beforeScroll = scrollOwners.map((owner) => owner.scrollTop);
  ...
  await settled();
  const detail = document.querySelector("[data-work-task-detail]");
  checks.workDetailFullHeight =
    overview.hidden &&
    overview.inert &&
    detail.getBoundingClientRect().height > metrics.allOverflow.available / 3;
  flushSync(() =>
    detail
      .querySelector('[data-behavior="action:back-to-work-overview"]')
      .click(),
  );
  await settled();
  checks.allScrollOwnersRetained =
    overview === document.querySelector("[data-work-task-overview]") &&
    scrollOwners.every((owner, i) => owner.scrollTop === beforeScroll[i]);
  checks.densityBackFocus = document.activeElement === trigger;
  seed("coding", true, "density-coding");
  patchSession({
    messages: manyTasks(3),
    planContent:
      "# 긴 테스트 계획\n\n" +
      Array.from({ length: 45 }, (_, i) => `계획 본문 ${i}`).join("\n\n"),
  });
  flushSync(() => root.render(<Shell />));
  await settled();
  const codingCard = document.querySelector('[data-context="plan"]'),
    planOverview = document.querySelector("[data-plan-task-overview]");
  await Promise.all(
    [...document.querySelectorAll(".animate-tile-in")]
  ...
  const codingHeight = codingCard.getBoundingClientRect().height;
  metrics.codingHeight = { overview: codingHeight };
  const codingRow = document.querySelector("[data-task-row]");
  metrics.codingRow = rowStyles(codingRow);
  checks.sharedRowStyles =
    JSON.stringify(metrics.workRow) === JSON.stringify(metrics.codingRow);
  checks.codingQuestionAbsent = !codingCard.querySelector(
    '[data-behavior="action:ask-about-task"]',
  );
  const documentNode = codingCard.querySelector("h1");
  const codingTrigger = document.querySelector(
    '[data-task-detail-trigger="agent:1"]',
  );
  flushSync(() => codingTrigger.click());
  await settled();
  checks.codingWholeDepth =
    planOverview.hidden &&
    planOverview.inert &&
    !!document.querySelector("[data-plan-task-detail]");
  checks.codingHeightUnchangedInDetail =
    Math.abs(codingCard.getBoundingClientRect().height - codingHeight) < 1;
  metrics.codingHeight.detail = codingCard.getBoundingClientRect().height;
  await click("목록으로");
  checks.codingDocumentIdentity =
    documentNode === codingCard.querySelector("h1") &&
    planOverview === document.querySelector("[data-plan-task-overview]") &&
    !planOverview.hidden &&
    document.activeElement === codingTrigger;
  patchSession({ messages: [], planContent: null });
  await settled();
  checks.codingEmptyHeightUnchanged =
    Math.abs(codingCard.getBoundingClientRect().height - codingHeight) < 1;
  metrics.codingHeight.empty = codingCard.getBoundingClientRect().height;
  refs.splice(0, refs.length, ...structuredClone(originalRefs));
  ...
  return (
    <section className="flex h-full min-w-0 flex-1 bg-bg">
      <aside className="w-80 border-r border-border p-4">
        <p className="mb-4 text-footnote text-ink3">
          완료한 비열람 대화는 모드 아이콘으로 표시합니다.
        </p>
        ...
        <button
          data-native-leave-session
          data-native-route={pathname}
          onClick={() => handlers.handleOpenProject("fixture-project")}
        >
          다른 화면
        </button>
      </aside>
...
window.panelR5Nav = async () => {
  closeMenu();
  seed("work", false, "nav-work");
  ...
  await settled();
  const checks = {};
  window.panelR5LastNav = { checks };
  const workRow = document.querySelector('[data-session-id="nav-work"]'),
    codingRow = document.querySelector('[data-session-id="nav-coding"]');
  const icon = workRow.querySelector('[data-context="session-agent-kind"]');
  const title = workRow.querySelector("span.min-w-0");
  const initialColor = getComputedStyle(icon).color,
    titleColor = getComputedStyle(title).color;
  checks.modeIconsDistinct =
    icon.getAttribute("aria-label") === "작업" &&
    codingRow
      .querySelector('[data-context="session-agent-kind"]')
      .getAttribute("aria-label") === "코딩" &&
    workRow.querySelector("svg").innerHTML !==
      codingRow.querySelector("svg").innerHTML;
  checks.noTrailingModeLabel =
    ![...workRow.querySelectorAll("span")].some(
      (span) => span.textContent === "작업",
    ) &&
    ![...codingRow.querySelectorAll("span")].some(
      (span) => span.textContent === "코딩",
    );
  flushSync(() =>
    ingestChatEvent({
  ...
  await settled();
  checks.intermediateNotCompleted = icon.dataset.state === "default";
  flushSync(() =>
    ingestChatEvent({ type: "turn.ended", sessionId: "nav-work" }),
  );
  await settled();
  checks.unviewedCompletionBlue =
    icon.dataset.state === "unseen-complete" &&
    getComputedStyle(icon).color !== initialColor &&
    icon.classList.contains("text-selected");
  checks.onlyIconBlue =
    getComputedStyle(title).color === titleColor &&
    !workRow.classList.contains("text-selected");
  const oldList = window.orca.session.list;
  window.orca.session.list = async () => navItems;
  await sessionsActions.refresh();
  await settled();
  checks.refreshRetainsCompletion = icon.dataset.state === "unseen-complete";
  window.orca.session.list = oldList;
  flushSync(() => workRow.click());
  await settled();
  checks.openAcknowledgesCompletion =
    icon.dataset.state === "default" &&
    !icon.classList.contains("text-selected") &&
    useSessionsStore.getState().viewedSessionId === "nav-work";
  checks.sessionClickRoutes =
    document.querySelector("[data-native-route]").dataset.nativeRoute ===
    "/chat/nav-work";
  flushSync(() =>
    ingestChatEvent({ type: "turn.ended", sessionId: "nav-work" }),
  );
  await settled();
  checks.viewedCompletionNotBlue = icon.dataset.state === "default";
  flushSync(() =>
    document.querySelector("[data-native-leave-session]").click(),
  );
  await settled();
  checks.leaveDoesNotReplay = icon.dataset.state === "default";
  checks.projectRouteNotViewed =
    document.querySelector("[data-native-route]").dataset.nativeRoute ===
      "/projects/fixture-project" &&
    useSessionsStore.getState().viewedSessionId === null;
  flushSync(() =>
    ingestChatEvent({
      type: "turn.aborted",
      sessionId: "nav-work",
  ...
  await settled();
  checks.abortNotCompleted = icon.dataset.state === "default";
  flushSync(() =>
    ingestChatEvent({ type: "turn.ended", sessionId: "nav-work" }),
  );
  await settled();
  return {
    ...
    colors: {
      ...
      completed: getComputedStyle(icon).color,
      title: titleColor,
    },
  };
};
window.panelR5HeightSnapshot = async () => {
  seed("work", true, "density-resize");
  setRefs(35);
  patchSession({
    messages: manyTasks(45),
    extraDirs: Array.from({ length: 35 }, (_, i) => `C:/資料${i}/${longTitle}`),
  });
  flushSync(() => root.render(<Shell />));
  await refreshArtifactList("density-resize");
  await settled();
  const metrics = panelMeasures();
  const checks = {
    resizedThirdCaps: Object.values(metrics.sections).every(
      (section) =>
        section.height <= metrics.available / 3 + 1 &&
        section.scrollHeight > section.clientHeight,
    ),
    resizedOuterFits: metrics.card <= metrics.available + 1,
    noHorizontalOverflow: document.documentElement.scrollWidth <= innerWidth,
  };
  refs.splice(0, refs.length, ...structuredClone(originalRefs));
  return { name: "work-density", checks, metrics };
};
window.panelR5CodingSnapshot = async () => {
  seed("coding", true, "coding-detail-snapshot");
  flushSync(() => root.render(<Shell />));
  await settled();
  flushSync(() =>
    document.querySelector('[data-task-detail-trigger="agent:1"]').click(),
  ...
  await settled();
  assert(
    document.querySelector("[data-plan-task-overview]").hidden &&
      document.querySelector("[data-plan-task-detail]"),
    "Coding whole detail screenshot failed",
  );
  return { name: "coding-detail", width: innerWidth };
};
...
window.panelR5Tooltip = () => {
  ...
  if (!tip)
    return { checks: { tooltipVisible: false, tooltipUnclipped: false } };
  ...
  return {
    checks: {
      tooltipVisible: box.width > 0,
      tooltipUnclipped: clipping.length === 0,
    },
    box: { top: box.top, bottom: box.bottom, left: box.left, right: box.right },
    clipping,
  };
};
window.panelR5TooltipScroll = async () => {
  const owner = document.querySelector('[data-task-section-scroll="progress"]');
  owner.scrollTop = owner.scrollHeight;
  await settled();
  const last = [
    ...document.querySelectorAll('[data-behavior="action:ask-about-task"]'),
  ].at(-1);
  const box = last.getBoundingClientRect();
  return {
    checks: {
      tooltipDismissesOnScroll: !document.querySelector('[role="tooltip"]'),
    },
    target: { x: box.left + box.width / 2, y: box.top + box.height / 2 },
  };
};
window.panelR5TooltipFinish = async () => {
  const observation = {};
  window.panelR5TooltipLifeObservation = observation;
  const inspectPointerAndFocus = () => ({
    hoveredAskKeys: [
      ...document.querySelectorAll('[data-behavior="action:ask-about-task"]'),
    ]
      .filter((button) => button.matches(":hover"))
      .map((button) => button.closest("[data-task-row]")?.dataset.taskRow),
    focusedElement: document.activeElement?.outerHTML.slice(0, 400),
    focusedRowKey:
      document.activeElement?.closest("[data-task-row]")?.dataset.taskRow ??
      null,
    focusedTriggerKey:
      document.activeElement?.dataset.taskDetailTrigger ?? null,
    originalRowPresent: !!document.querySelector('[data-task-row="agent:45"]'),
    tooltip: window.panelR5Tooltip(),
  });
  const last = [
    ...document.querySelectorAll('[data-behavior="action:ask-about-task"]'),
  ].at(-1);
  flushSync(() => last.click());
  await settled();
  const draft = useChatStore.getState().draftRestore;
  const checks = {
    tooltipQuestionDismisses: !document.querySelector('[role="tooltip"]'),
    tooltipQuestionAppends:
      draft?.mode === "append" && draft.text.includes("자료 정리 작업 45"),
  };
  last.focus();
  await settled();
  checks.tooltipKeyboardFocus = !!document.querySelector('[role="tooltip"]');
  const trigger = document.querySelector(
    '[data-task-detail-trigger="agent:45"]',
  );
  flushSync(() => trigger.click());
  await settled();
  const detail = document.querySelector("[data-work-task-detail]");
  ...
  checks.tooltipDetailDismisses =
    !document.querySelector('[role="tooltip"]') &&
    document.activeElement === back;
  flushSync(() => back.click());
  await settled();
  checks.tooltipBackRemainsDismissed =
    !document.querySelector('[role="tooltip"]') &&
    document.activeElement === trigger;
  observation.afterBack = inspectPointerAndFocus();
  last.focus();
  await settled();
  checks.tooltipBeforeAnchorDeletion =
    !!document.querySelector('[role="tooltip"]');
  patchSession({ messages: manyTasks(44) });
  await settled();
  checks.tooltipAnchorDeletionDismisses =
    !last.isConnected && !document.querySelector('[role="tooltip"]');
  observation.afterDeletion = inspectPointerAndFocus();
  return checks;
};
