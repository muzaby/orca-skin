...
| 결정 | 사용자 결과 | 승계 |
|---|---|---|
| D-047 | `claudecode-sonnet-5`, `claudecode-opus4.8`처럼 claude/claudecode 접두사와 계열 뒤 하이픈 생략을 허용한다. 계열 없는 `claudecode-4.8[1m]`·`claudecode-4-8[1m]`도 버전을 판정한다. | D-045/046의 모델명 형식 확장. 네 계열·4.6 이상·custom/미확정 제외 유지 |
...
세 번째 사용자 예시의 `4`와 `8[1m]` 사이 줄바꿈은 실제 ID 확인 질문을 보냈다. 버전 구분자는 기존처럼 점·하이픈을 모두 지원하므로 두 표기를 함께 구현한다. 내부 개행을 임의로 지우거나 모델 ID를 SDK 전달 전에 바꾸지는 않는다.
