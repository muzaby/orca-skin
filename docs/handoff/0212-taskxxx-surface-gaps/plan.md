...
> r1 검증 = **FAIL + PLAN_GAP**, r2 검증 = **FAIL**(`PLAN_GAP` 0), r3 검증 = **PASS**(BLOCKING 0). 판정 원문과 관측은 [`verify.md`](verify.md) — 아래는 이관 표다.
...
| P12 | **win32 분기의 실행 확인은 CI 몫이다** — 검증 환경이 Linux 라 `commandForTarget` 이 내는 명세(`shell:true`)까지만 잠근다 | §7-A 선행 조건 gate | — | NON_BLOCKING | **closed (r3 검증)** — windows-latest [run #462](https://github.com/muzaby/orca-skin/actions/runs/33579701559) @ `db1509fd` 9스텝 success. 직전 run #461 @ `62eb2c76` 은 `Install dependencies` 에서 failure |
| P13 | **Windows 에서 `postinstall` 등 4개 훅이 수명 내내 무음 no-op 이었다**(0212 r1 이 진입 가드를 고치기 전까지). `app/AGENTS.md §ABI 가이드` 의 "4개 명령이 ABI 를 뒤집는다" 가 Windows 에서는 참이 아니었다 | `app/AGENTS.md` 서술 ↔ 코드 | 코드는 r3 로 서술과 일치. 문서 정정 여부는 설계자 판단 | NON_BLOCKING | open (r3 제기) |
| D15 | **`defaultRunner` 가 `shell` 판정을 버려도 무음이다.** `{stdio,shell:false,...options}` 를 `{stdio,...options,shell:false}` 로 뒤집으면 win32 에서 이번 CI red 가 그대로 재발하는데 **scripts 67/67 green · typecheck 0** 이다 — 모든 테스트가 fake runner 를 주입해 실제 runner 를 지나지 않는다 | §7-A 선행 조건 gate (0212 V·§10 밖) | `spawnSync` 를 주입 가능하게 하거나 `process.execPath` 로 실자식을 띄워 `shell` 전달을 관측한다 | NON_BLOCKING | open (r3 검증) |
| D16 | **"실패 사유 3홉 닫음 3/3" 은 재측정 1/3 이다.** `runCli` 가 `result.reason` 을 버려도(= 수정 전 로그 문장 그 자체) **67/67 green**, `defaultRunner` 가 `error` 를 버려도 **67/67 green**. 닫힌 것은 가운데 홉뿐이다 | 같은 gate · 구현 보고 정확도 | `runCli` 의 `console.error` 를 포획하는 케이스 1개로 hop 3 을 닫는다. hop 1 은 D15 와 같은 수단 | NON_BLOCKING | open (r3 검증) |
| D17 | **`ClaudeAdapter` 의 인자 전달 위임은 4개인데 3개만 잠겼다.** `claude.ts:489` `setPermissionMode: (mode) => handle.setPermissionMode(mode)` 를 상수로 굳혀도 **2800 green · typecheck 0** — `:503`·`:505`·`:507` 과 같은 형상·같은 계약이다 | 구현자가 선언한 "§5 전수" 밖 · 0212 V 밖 | `claude.live-control.test.ts` 두 번째 케이스에 한 쌍 더한다 | NON_BLOCKING | open (r3 검증) |
| D18 | **보고 수치 2건.** ① "8사이트 중 잠긴 것 **5부류**(②③④⑤⑥⑦)" — 괄호 라벨은 6개이고 사이트 단위로는 9다(검증자 재열거: 9사이트 / 7홉). ② 신설 전수 테스트가 target 을 `['node','electron']` 리터럴로 가져 "새 target 이 늘어도 함께 걸린다" 는 `VALID_TARGETS` 와 우연히 같아서 참이다 | 구현 보고 정확도 | 다음 보고에서 정정 · 테스트는 `VALID_TARGETS` 를 읽게 한다 | NON_BLOCKING | open (r3 검증) |
| D19 | **구현자가 이 표를 직접 편집했다** — D3·D11·D12 상태와 P9·P10·P11 행. 같은 사실이 `구현 보고` 의 `파생 이슈 처리(자기보고)` 칸에도 있어 사본이 둘이다. 재측정 결과 세 상태는 참이라 되돌리지 않았고, 빠져 있던 P12·P13 은 검증자가 이번에 넣었다 | 산출물 경계 · `AGENTS.md §산출물 문장 규칙` | 구현자 보고는 `구현 보고` 칸 한 곳으로 — 이 표의 상태 열은 검증자가 쓴다 | NON_BLOCKING | **closed (r3 검증)** |
| D20 | **r2 verify 가 인용한 `229a0e6c` 는 죽은 좌표다** — 실제 `229a0e67`. r2 §11 이 "전건 `commit`" 이라 적었으나 `git cat-file -t 229a0e6c` = `Not a valid object name` (0190 D3 과 같은 형태) | 검증자 산출 정확도 | r3 메타가 `229a0e67` 로 정본을 고정한다. r2 원문은 그 자리에 둔다 | NON_BLOCKING | **closed (r3 검증)** |
