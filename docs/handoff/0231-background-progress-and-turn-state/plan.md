...
| D-101 | "생각 중" spark 라인과 **백그라운드 실행 줄을 분리**한다. 후자는 턴이 끝난 뒤에도 마지막 어시스턴트 턴 아래에 남는다 | D-001 과 D-002 를 동시에 만족하는 유일한 배치다 — 턴이 끝나 보이면 `PendingAssistant` 가 사라지므로 백그라운드 정보를 그 안에 둘 수 없다 | 설계자 추론 | **OPEN** — 사용자 확인 필요 | — |
...
- 새로 추가된 결정: D-101(OPEN) · D-102 · D-103 · D-104.
- 승계: D-001 · D-002 (0230 Decision Ledger).
- **`ACTIVE 결정 ↔ AC` 대조**: READY 승격 시 수행한다. 현재 D-101 이 OPEN 이라 §5 배치가 확정되지
  않았고, AC 도 그 위에 선다.
...
| ACTIVE 결정과 충돌하는가 | 주의 | 0153(낙관 커밋 금지) · 0167 AC21(foreground 구간 무활동 라벨 금지) · 0208 D-002(스피너는 셋이 분기 없이 같은 것). D-101 이 새 줄을 만들면 0208 D-002 의 "분기 없음" 이 깨지는지 확인해야 한다 |

- **사용자에게 올릴 결정**: D-101(백그라운드 실행 줄을 어디에 둘 것인가).
- 코드 조사로 닫은 사실: `sessionResponding` ↔ `shouldQueueAsPending` 의 판정 불일치.

## 5. 동작 / 사용자 흐름 (D-101 확정 전 초안)
...
|---|---|---|
| 실행 줄의 배치(D-101) | **예 — 사용자가 보는 결과** | 지금 사용자에게 묻는다 |
...
- 변경이 시작되는 수준: `R`.
- 노드·pair 는 D-101 확정 후 채운다. 확정 전 고정된 것:
...
| 0208 D-002("스피너는 분기 없이 같은 것") 위반 | 새 실행 줄은 **스피너가 아니다** — 별도 컴포넌트로 만들고 `SparkSpinner` 를 쓰지 않는다. D-101 확정 시 이 제약을 명시한다 |
| `tool_progress` 영속으로 파트 폭증 | transient 로 둔다(`message.delta` 선례) |

## 10~15

D-101 확정 후 작성한다. 확정 전에 고정된 강제 지점 후보:
