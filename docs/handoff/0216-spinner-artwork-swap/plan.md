...
<details><summary>r2 구현 보고 (2026-09-06 · 접힘)</summary>
...
- 동의 / 그대로 진행: rev.4 대로 30Hz 계단을 확정하고, 계단이 사라져도 green 이던 구멍(AT-202)과 allowlist(AT-208)를 함께 막았다.
- **선조치(구현 세부)**: 하네스에 **rep 산포 가드**(`STABILITY_MS_PER_SEC = 12`)를 신설했다. 측정 중 세 조건이 모두 fps 60 을 지키면서 rep 사이 구/신 순서가 뒤집히는 실행을 만났다(rep별 초과분 `[49.2, 31.0, -5.6]`) — 평균만 보면 그 실행도 숫자를 하나 내놓는다. EP-209 지점 ①("측정 유효성")의 실현 강화이고 지점 수·계약 방향은 그대로다(더 엄격해진다).
- 이견 / 현실성 문제: plan §10 EP-209 지점 ①의 **문구**가 `rAF 프레임 > 0` 만 말한다 — 실현은 이제 산포도 본다. 문서 정합성 정정이 필요하다(검증자에게 보고).
- ACTIVE Decision 과 충돌하는 설계 발견: 없음.
...
| Pair | 계약/필드 | §10이 적은 지점 | 닫은 지점 | 재현 명령 / 관측 | 남긴 곳 |
|---|---|---|---|---|---|
| ΔVP-202 | 기하·주기·타이밍·계단 | 6 | **6/6** | `vitest run …/sparkCss.test.ts` 15케이스 green — 그중 신설 `값이 바뀌는 구간 전수에 30Hz 계단이 걸려 있다` 가 마크 5종 × 구간 **16곳**을 원본 키타임에서 파생한 `steps(N, end)` 와 차집합 비교 | — |
| ΔVP-207 | 인스턴스 비용 | 5 | **5/5** | 같은 스위트 — allowlist 가 `animation-timing-function` 을 **값이 `steps(...)` 일 때만** 통과시킨다 | — |
| ΔVP-209 | 런타임 비용 | 4 | **4/4** | 아래 측정 절 — ① 무효 행 0 + rep 산포 가드 ② 예산 판정 ③ 하네스↔컴포넌트 등가 ④ fixture provenance | 판정에 쓴 실행은 **앞선 조용한 2회**(아래) |
| ΔVP-201·203~206·208·210 | (변경 없음) | 16 | **16/16** | 렌더러 전 스위트 재실행으로 회귀 0 — 151파일 **1177케이스** green | — |

- 합계: **31/31**. r2 의 `31/31 실행 · 30 충족` 에서 EP-202 가 5 → 6 으로 늘어 분모가 **32**가 아니라 31 인 이유는 EP-209 지점 ②의 기준만 바뀌고 개수는 그대로이기 때문이다 — EP-202 6 + EP-207 5 + EP-209 4 + 나머지 16 = **31**.

## [구현자 기입] 이번 라운드 수정의 잠금 — r3

| 심은 결함 | 출처 | 이전 라운드 결과 | 실패한 테스트 / 케이스 수 | 결과 |
|---|---|---|---|---|
| `steps(5, end)` 한 줄 삭제 | ΔVP-202 등록 변이(rev.4 신설) | r2 에는 장치가 없어 **green 이었다** | `sparkCss` 1 failed / 15 | RED ✅ |
| `steps(5)` → `steps(6)` (단계 수만 어긋남) | 새 oracle(파생 기대값) 민감도 | 없음 | `sparkCss` 1 failed / 15 | RED ✅ |
| `steps(5, end)` → `cubic-bezier(...)` (계단을 연속으로 되돌림) | allowlist 예외가 구멍인지 | 없음 | `sparkCss` **2** failed / 15 — 계단 단언 + allowlist 둘 다 | RED ✅ |
| rep 산포 가드 — 실측 잡음 행 `[49.2, 31.0, -5.6]` 주입 | 이번 턴 신설 oracle 민감도 | 없음 | `measure-spinner-perf.test.mjs` 의 `rep 산포가 크면…` 케이스 | RED ✅ |
| 예산 경계 ±0.1 · 신이 더 싼 입력 | 같은 oracle 양방향 | 없음 | 같은 스위트 3케이스(`예산 경계에서 갈린다` 외) | 양방향 green ✅ |

- 검산: 선택 증거 **1**(ΔVP-202 의 `steps()` 제거) · 인용 변이 **0** · 새 oracle **4**(계단 파생 기대값 · allowlist steps 한정 · rep 산포 가드 · 예산 경계) = 표 행 **5**.

## [구현자 기입] Product/UX 파생 검토 — r3

| 질문 | 판정 | 후속 |
|---|---|---|
| 사용자가 보는 것이 달라지는가 | ✅ 그렇다 — 마크가 뜨고 지는 순간이 5단계(마크 `a` 는 3단계)로 나뉜다. **사용자가 실기로 확인하고 채택했다**(D-211) | — |
| 감속 모션이 영향을 받는가 | ✅ 아니오 — `prefers-reduced-motion` 은 `animation: none` 이라 계단과 무관하다 | — |
| transcript flush 와 간섭하는가 | ✅ 아니오 — flush 는 rAF **JS 콜백 큐**이고 스피너는 그 큐에 없다. 같은 프레임 예산을 나눠 쓰므로 스피너 몫이 줄면 flush 여유가 는다 | §14 rev.4 |
| 실패가 화면에서 무엇으로 보이는가 | 불변 — 트랙 미방출이면 마크 5개가 동시에 정지 표시된다 | AT-212 |

## [구현자 기입] 놓친 잠재 문제 + 대응 — r3

| # | 문제 | 대응 | 근거 |
|---|---|---|---|
| 1 | fps 60 만으로는 측정 유효성이 부족하다 | 선조치 — rep 산포 가드. 임계 12 는 조용한 실행 4회(2.5·3.5·4.3·5.8)와 오염된 실행(54.8) 사이에서 골랐다 | 위 잠금 표 |
| 2 | plan §10 EP-209 지점 ① 문구가 rAF 만 말한다 | 보고만 — 규범 행 정정은 설계자 몫 | 위 설계 리뷰 |
| 3 | **이번 턴의 라이브 측정 2회가 가드에 막혔다** | 보고 — 기계 CPU 57~80%, 기준선 task 가 90 → 198ms 로 2배였다. 판정은 같은 페이지를 조용할 때 잰 앞선 2회를 쓴다 | 아래 측정 절 |
| 4 | 계단이 원본과 다른 모션이라는 사실이 `spinner-reference.svg` 에는 없다 | 보고만 — 원본은 건드리지 않는다(D-206). 차이는 D-211·AT-202 가 갖는다 | — |
...
| 변경 파일 | 5 — `styles/app.css`(계단 16곳) · `shared/ui/sparkTracks.ts`(`SPARK_STEP_HZ`) · `shared/ui/sparkCss.test.ts`(계단 전수 + allowlist) · `scripts/measure-spinner-perf.mjs`(예산·산포 판정) · `scripts/measure-spinner-perf.test.mjs`(양방향 잠금) |
| 관측한 게이트 산출 | `lint` **0 error** / 1 warning(`useTranscriptVirtualizer.ts:22` 기존) · `typecheck` node·web 0 error, test 2 error(`@opencode-ai/sdk` 베이스라인) · `vitest run src/renderer` **151파일 1177케이스 green** · `node --test scripts/*.test.mjs` **88 pass** · `check-doc-inventory --check` ok |
| **측정** | 같은 페이지(계단 적용본)를 조용할 때 잰 2회 — 초과분 **+3.5 · +3.0 ms/s**(예산 5.0) · rep 산포 **5.8 · 2.5**(임계 12) · 무효 행 0 · fps 60. layout **144.7 → 66·65.7회**. 이번 턴의 라이브 2회는 rep 산포 32.7·54.8 로 **가드가 판정을 거부**했다(기계 부하) |
| AC 검산 | ✅ 11(AT-201~209·211·212) · ⏸ 1(AT-210 사람 실기) · ❌ 0 = 총 **12** |
| 설계 대비 차이 | 없음 — rev.4 의 파일·형상 그대로다. 하네스의 산포 가드는 §10 지점 수를 바꾸지 않는 실현 강화다 |

- `Criteria-Met: 11/12` · `Criteria-Pending: AT-210(사람 실기)`.

## [구현자 기입] Review Signals — 사실만 — r3

- 이전 라운드와 같은 축인가: **아니오** — r1·r2 는 "신 > 구" 였고 r3 는 그 초과분을 예산 안으로 넣었다. 다만 **측정 유효성** 축은 r2(rAF 가드)에 이어 r3(rep 산포)로 두 번째다.
- 막았어야 할 plan 지침이 있었는가: 계단 구멍은 AT-202 의 술어가 `transform`·`opacity` 만 읽어서 생겼다 — r2 verify 가 그 사실을 적었고 rev.4 가 지점을 신설해 닫았다.
- 반복해 부딪히는 환경 한계: 측정이 **기계 부하에 좌우된다**. 가드가 그것을 거짓 통과 대신 판정 거부로 바꿨지만, 조용한 창을 기다려야 한다는 사실은 남는다.
- 현재 라운드 수: 3.
...
| D3 | **재측정에서 신 > 구** | ΔVP-209 · AT-211 · D-205 | 30Hz 계단 + 예산 재정의(D-211) | ~~BLOCKING~~ | **closed (r3)** — 초과분 +15.2 → +3.0~3.5, 예산 5.0 안 |
...
| D8 | plan §10 EP-209 지점 ① 문구가 `rAF 프레임 > 0` 만 말한다 — 실현은 rep 산포도 본다 | EP-209 문구 | 다음 설계 턴에서 한 줄 정정(계약은 더 엄격, 지점 수 불변) | NON_BLOCKING | open |
| D9 | 최종 커밋본으로 낸 **라이브 AT-211 PASS 가 없다** — 가드가 4회 거부(기계 CPU 44~59%) | ΔVP-209 | 조용할 때 `cd app && npm run measure:spinner-perf` 1회 | NON_BLOCKING | open |
