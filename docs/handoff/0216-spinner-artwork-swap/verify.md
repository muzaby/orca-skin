...
> 라운드가 셋이다. **r1 은 아래 §0~§15 원문** · **r2 는 §Verify r2** · **r3 은 §Verify r3** 다 — 같은 판정을 두 곳에 쓰지 않는다.
...
| 라운드 | 1 |
| 상태 | r1 **FAIL + PLAN_GAP → RETURN_TO_PLAN** · r2 **FAIL** · r3 **PASS**(§Verify r3, AT-210 실기 남음) |
...
# Verify r2 — 커밋된 오라클로 다시 잰다

## 메타

| 항목 | 값 |
|---|---|
| 대상 커밋/range | `bae8190a..f3944d57` (rev.3 설계 `c6b0e00f` · r2 구현 `f3944d57`) |
| 검증 기준 plan revision | `c6b0e00f:ΔV2 rev.3` |
| 라운드 | 2 |
| 상태 | **FAIL** — `PLAN_GAP` 은 닫혔고 `PAIR_FAIL` 하나가 남는다 |
| 다음 주체 | **사람** — 남은 것은 코드가 아니라 D-205 수용 여부다 |
| 자기 검증 여부 | 구현자 = 검증자. 구현 보고에 없던 축 **3건** — V-E1(오라클 재실행 재현성) · V-E2(fixture 변조 3단계) · V-E3(EP-209 4지점 독립 재열거) |

## 0. 기준선

- 구현 커밋 `f3944d57` 이 `plan.md` 를 건드린 범위: **`[구현자 기입]` 뿐**이다. 규범 정정(rev.3)은 앞선 설계 커밋 `c6b0e00f` 에 있다 — §0 의 자기 증명 방지 장치가 이번에도 성립한다.
- 규범 변경 확인: AT-211 의 **판정 기준은 그대로**(`신 ≤ 구`)이고 오라클 위치와 EP-209 지점 수(2→4)만 바뀌었다. 구현자가 자기 결과에 맞춰 기준을 완화하지 않았다.
- `git cat-file -t c6b0e00f`·`f3944d57` = 둘 다 commit.

## 1. r1 PLAN_GAP(D4) 의 닫힘

| 요구 | 관측 | 판정 |
|---|---|---|
| 오라클이 저장소 안에 있다 | `app/scripts/measure-spinner-perf.mjs` (245줄) + CJS 셸 + companion + fixture | ✅ |
| 한 명령으로 재현된다 | `npm run measure:spinner-perf` — 검증자가 독립 실행해 표를 얻었다 | ✅ |
| 산출이 결정적이다(같은 절차 → 같은 결론) | 구현 실행 `신 46.7 / 구 31.1` · **검증 재실행 `신 47.4 / 구 32.2`** — 차이 1.5%·2.4%, 판정 동일 | ✅ |
| 구 조건의 provenance 를 재생성할 수 있다 | git 블롭에서 CSS·마크업을 다시 만들어 커밋본과 **바이트 동일**(14,528 / 2,192) | ✅ |

- **D4 는 닫혔다.** r1 이 "두 재구성이 반대 방향" 이라 판정할 수 없던 자리가, 이제 같은 명령이 같은 결론을 낸다.

## 2. AT-211 재측정 — 검증자 독립 실행

| mode | fps | recalc | recalc ms | layout | layout ms | task ms | 순증가 ms/s |
|---|---|---|---|---|---|---|---|
| none | 60.1 | 0회 | 0 | 0회 | 0 | 89.6 | 0 |
| old | 60.2 | 301.3회 | 98.7 | **0회** | **0** | 250.5 | **32.2** |
| new | 60.2 | 301회 | 90.5 | **144.7회** | **12.2** | 326.4 | **47.4** |

- 무효 행 0(rAF 298~301프레임/5s). 세 조건 모두 60fps 다.
- **네 번째 독립 측정이고 네 번 모두 신이 비싸다** — r1 임시 하네스 2회(41.9·38.9) · r2 구현 1회(46.7) · r2 검증 1회(47.4 ms/s).
- 방향은 r1 §5 의 원인 분리(`opacity: 0` 마크가 레이아웃에 남는다)와 일치한다 — 그 절을 재서술하지 않는다.

## 3. 자기검증 분모 — 구현 보고에 없던 축

| 축 | 무엇을 했는가 | 결과 | 귀속 |
|---|---|---|---|
| V-E1 오라클 재현성 | 같은 명령을 검증자가 독립 실행 | 판정 동일 · 수치 오차 ≤2.4% | EP-209 지점 ①② 재측정 |
| V-E2a fixture 변조(형상) | 구 fixture 에서 `<text>` 글리프 1개 삭제 | companion **red**(1 failed / 16) | 새 oracle 민감도 |
| V-E2b fixture 변조(값) | 개수는 그대로 두고 구 트랙의 `visibility: visible` 3곳을 `hidden` 으로 | **green — 아무 게이트도 잡지 않는다**(companion 16 pass · 렌더 10 pass) | **D7** |
| V-E3 §10 독립 재열거 | EP-209 의 4지점을 구현 보고와 무관하게 다시 셈 | 4/4 실행 · 지점 ② 불충족 — 보고와 일치 | 분모 확인 |

- V-E2b 의 의미: 동결 fixture 는 **비교의 기준선**인데 그 값이 바뀌어도 침묵한다. 개수·치수 단언은 형상만 본다. 조용히 바뀌면 "구" 가 다른 것이 되고 AT-211 의 결론이 따라 움직인다. 지금은 실제로 바뀌지 않았다(§1 provenance 가 바이트 동일을 보였다) — **잠재 결함이지 현재 회귀가 아니다**.

## 4. V-pair closeout — r2 실행 범위

> 이번 라운드는 렌더러 코드를 건드리지 않았다. 영향받지 않은 pair 는 r1 §5 의 증거 좌표를 참조하고 다시 세지 않는다.

| Pair | requiredness | 결과 | 근거 |
|---|---|---|---|
| ΔVP-209 | REQUIRED | **PAIR_FAIL(유지)** | §2 — 신 47.4 > 구 32.2 ms/s · layout 신 12.2 > 구 0 ms |
| ΔVP-201~208·210 | REQUIRED/REGRESSION | **PASS(참조)** | r1 §5. 이번 라운드 회귀 0 — `vitest run src/renderer` 151파일 **1176케이스** green |
| ΔVP-211 | REGRESSION | 사람 실기 대기 | AT-210 |

### AC 합계

- `✅ 10 · ❌ 1(AT-211) · ⏸ 1(AT-210) = 총 12`. 자기보고 `Criteria-Met: 10/12` — **일치**.
- 합계 사본 대조: verify 본문 `10/12` ↔ trailer `Criteria-Met: 10/12` ↔ INDEX 비고 — 일치.
- r1 대비 분모 변화 없음(12).
...
| Gate | 결과 | 증거 |
|---|---|---|
| subtree `app/**` | **PASS** | `lint` **0 error** / 1 warning(기존) · `typecheck` node·web 0 error, test 2 error(`@opencode-ai/sdk` 베이스라인) · `vitest run src/renderer` 151파일 1176케이스 · **`node --test scripts/*.test.mjs` 83 pass**(새 companion 16 포함) |
| repository (docs) | **PASS** | `check-doc-inventory --check` ok |
| message bus | **PASS** | `git log -1 --format='%(trailers:only=true)' f3944d57` 이 7키 반환 |

- 게이트가 작업 트리를 바꿨는가: **예 — `npm run lint` 의 `--fix` 가 새 companion 의 포맷을 고쳤다**(줄바꿈·괄호). 구현자 자신의 신규 파일이고 커밋 전에 반영됐다. 검증 시점 트리는 클린이다.

## 5. 이번 라운드가 새로 만든 표면

| 표면 | 검토 | 판정 |
|---|---|---|
| `npm test` 에 16케이스 추가 | 순수 테스트 · Electron 미기동 — `node --test` 4.9s | 무해 |
| CJS ↔ ESM 두 파일 분리 | 셸 18줄에 로직 0. `runMeasurement` 시그니처가 갈리면 즉시 런타임 실패 | 무해 |
| 렌더 테스트가 `scripts/` 를 import | 테스트 파일 한정. `boundaries` 는 `src/**` 에 걸리고 lint 0 error | 무해 |
| 하네스가 `app.css`·`tokens.css` 를 **실행 시점에** 읽는다 | 스타일이 바뀌면 다음 측정이 자동으로 새 값을 쓴다 — 의도된 배선 | 무해 |

## 6. Finding disposition — r2

| # | finding | 귀속 | disposition | 후속 |
|---|---|---|---|---|
| D3 | **신 > 구** — 네 번째 독립 측정에서도 같은 방향 | ΔVP-209 · AT-211 · D-205 | **BLOCKING(유지)** | 아래 §7 — 사람 결정 |
| D4 | 오라클이 재현되지 않는다 | ΔVP-209 | **CLOSED** | `npm run measure:spinner-perf` · provenance 바이트 동일 |
| D7 | 동결 fixture 의 **값** 변조를 아무 게이트도 잡지 않는다(형상 변조는 잡는다) | EP-209 지점 ④ | NON_BLOCKING | fixture 해시를 companion 에 박을지 판단 |
| D1·D2·D5 | (r1 항목 유지) | — | NON_BLOCKING / NEXT_HANDOFF | 변동 없음 |

## 7. 결론 — r2

- 상태: **FAIL**. `PLAN_GAP` 은 없다(D4 closed).
- pair: **10 PASS(참조) · 1 PAIR_FAIL(ΔVP-209) · 1 사람 실기 대기**.
- 사용자가 고른 ③은 **완수됐다** — 오라클이 저장소에 있고, 한 명령으로 돌며, 두 실행이 같은 결론을 냈고, 구 기준선의 출처가 재생성으로 증명된다.
- **그 오라클이 낸 답은 "신이 더 비싸다" 이다.** 3개 동시에서 프레임당 **0.26ms**(16.7ms 예산의 1.6%), fps 는 두 조건 모두 60.
- 남은 것은 코드가 아니라 **D-205 의 수용 범위**다. 구현자가 더 할 일은 없다 — 다음 주체는 사람이고, 결정에 따라 ①(마크를 레이아웃에서 빼기, D-201 과 상충 가능) 또는 ②(AT-211 재정의)로 갈린다.
- 남은 사람 확인: AT-210 시각 실기 + 위 결정.

---

# Verify r3 — 30Hz 계단 채택

## 메타

| 항목 | 값 |
|---|---|
| 대상 커밋/range | `e93b2754..4af38eaf` (rev.4 설계 `17d95799` · r3 구현 `4af38eaf`) |
| 검증 기준 plan revision | `17d95799:ΔV2 rev.4` |
| 라운드 | 3 |
| 상태 | **PASS** — 남은 것은 AT-210 사람 실기 |
| 다음 주체 | **사람** (AT-210 시각 + 조용한 기계에서 AT-211 라이브 1회) |
| 자기 검증 여부 | 구현자 = 검증자. 구현 보고에 없던 축 **4건** — V-E4(계단 16곳 독립 재열거) · V-E5(형제 자리 맞바꿈) · V-E6(예산·임계 상수 변조) · V-E7(라이브 재측정 시도) |

## 0. 기준선

- 구현 커밋 `4af38eaf` 이 `plan.md` 를 건드린 범위: **`[구현자 기입]` 뿐**이다. 규범 정정(rev.4)은 앞선 설계 커밋 `17d95799` 에 있다 — §0 의 기준선 잠금이 세 라운드 내내 성립했다.
- 규범 변경 확인: D-211 이 신설되고 **D-201 의 타이밍 등가 축만** 대체됐다. 그림·주기·색·감속 마크는 원문 그대로다.
- **판정 기준 완화 여부**: AT-211 이 `신 ≤ 구` → 예산 `≤ 5.0 ms/s` 로 **완화됐다**. 구현자가 자기 결과에 맞춰 고친 것이 아니라 **사용자 결정(D-211)** 이고, 근거 수치(30Hz 실측 +3.5·+2.9, 구 조건 편차 ±1.1)가 §3 갱신 메모에 남아 있다.
- `git cat-file -t 17d95799`·`4af38eaf` = 둘 다 commit.

## 1. r2 가 남긴 구멍의 닫힘

| r2 finding | 요구 | 이번 관측 | 판정 |
|---|---|---|---|
| 계단이 통째로 사라져도 green | 계단을 보는 단언 | `steps()` 한 줄 삭제 → `sparkCss` **red**(1/15) | ✅ 닫힘 |
| — | 단계 수가 틀려도 잡히는가 | `steps(5)` → `steps(6)` → **red** | ✅ |
| — | allowlist 예외가 구멍인가 | `steps(...)` → `cubic-bezier(...)` → **red 2건**(계단 단언 + allowlist) | ✅ 구멍 아님 |
| D7 fixture 값 변조 무검출 | (이번 범위 밖) | 변동 없음 | NON_BLOCKING 유지 |

## 2. 자기검증 분모 — 구현 보고에 없던 축

| 축 | 무엇을 했는가 | 결과 |
|---|---|---|
| V-E4 계단 독립 재열거 | 테스트의 파생 로직을 쓰지 않고 `app.css` 를 직접 파싱해 구간 길이 → 30Hz 기대값을 다시 계산 | 마크별 `a 2 · b 8 · c 2 · d 2 · e 2` = **16곳**, 값 `[3,5]`·`[5×8]`·`[5,5]`·`[5,5]`·`[5,5]` 전부 일치. `steps(` 등장 총수도 16 |
| V-E5 형제 자리 맞바꿈 | `spark-a` 안에서 `steps(3)` ↔ `steps(5)` 를 맞바꿈(개수·존재는 불변) | `sparkCss` **red**(1/15) — 존재가 아니라 **자리**를 본다 |
| V-E6a 예산 상수 변조 | `BUDGET_MS_PER_SEC` 5.0 → 50 | companion **2 failed** |
| V-E6b 임계 상수 변조 | `STABILITY_MS_PER_SEC` 12 → 999 | companion **2 failed** |
| V-E7 라이브 재측정 | 커밋된 하네스를 검증자가 직접 실행 | **가드가 판정 거부**(아래 §3) |

## 3. AT-211 — 무엇을 봤고 무엇을 못 봤나

**본 것**: 같은 페이지(계단 적용본)를 조용할 때 잰 **2회**. 판정은 검증자가 raw 행에서 다시 계산했다 — 구현 보고의 수치를 그대로 받지 않았다.

| 실행 | 초과분(신 − 구) | rep 산포 | 무효 행 | fps | 판정 |
|---|---|---|---|---|---|
| 조용 #1 | **+3.5** ms/s | 5.8 | 0 | 60 | PASS |
| 조용 #2 | **+3.0** ms/s | 2.5 | 0 | 60 | PASS |

- 예산 5.0 대비 여유 **1.5·2.0 ms/s**. layout 은 연속 144.7회 → **66·65.7회**.
- 그 두 실행은 **측정 경로가 지금과 같다**(`buildPages`·`diffMetrics`·Electron 루프 불변). 이번 라운드가 바꾼 것은 판정 함수뿐이고, 그 함수는 companion 21케이스가 양방향으로 잠그며 상수 변조가 red 다(V-E6).

**못 본 것**: **최종 커밋본 바이너리로 낸 라이브 PASS.** 검증 시점 기계가 CPU 44~59%·기준선 task 90 → 170~219ms 였고, 가드가 **4회 연속 판정을 거부**했다.

| 시도 | 기준선 task | rep별 초과분 | 산포 | 결과 |
|---|---|---|---|---|
| r3 ① | 95.4 | — | — | 무효 행 3(fps 40대) |
| r3 ② | 198.5 | `28, -4.7, -3.4` | 32.7 | 거부 |
| r3 ③ | 169.6 | `-17.6, 14.7, 0.2` | 32.3 | 거부 |
| 검증 ④ | 218.7 | `0, 15.1, 12.9` | 15.1 | 거부 |

- **이 거부는 결함이 아니라 가드가 일하는 모습이다.** ②·③은 평균만 보면 각각 `+6.6`·`-0.9` 라는 숫자를 내놓았고, 후자는 "신이 더 싸다" 로 읽혔을 것이다. 같은 실행의 rep 안에서 구/신 순서가 뒤집혀 있었다.
- 판정: ΔVP-209 **PASS**. 관측(조용한 2회)과 판정 경로(단위 테스트 + 변조 red)가 각각 닫혔고, 남은 것은 두 조각을 한 번에 잇는 라이브 실행이다 — **D9 로 남기고 사람에게 한 줄 명령으로 넘긴다.**

## 4. V-pair closeout — r3 실행 범위

| Pair | requiredness | 결과 | 근거 |
|---|---|---|---|
| ΔVP-202 | REQUIRED | **PASS** | 계단 전수 16 독립 재열거 일치 · 변이 3종 red · §10 6/6 |
| ΔVP-207 | REQUIRED | **PASS** | allowlist 가 `steps()` 값만 통과(`cubic` 변이 red) · §10 5/5 |
| ΔVP-209 | REQUIRED | **PASS** | §3 |
| ΔVP-201·203~206·208·210 | REQUIRED/REGRESSION | **PASS(참조)** | r1 §5 좌표. 회귀 0 — `vitest run src/renderer` 151파일 **1177케이스** green |
| ΔVP-211 | REGRESSION | 사람 실기 대기 | AT-210 |

### AC 합계

- `✅ 11(AT-201~209·211·212) · ⏸ 1(AT-210) · ❌ 0 = 총 12`. 자기보고 `Criteria-Met: 11/12` — **일치**.
- 합계 사본 대조: verify 본문 `11/12` ↔ trailer `Criteria-Met: 11/12` ↔ INDEX 비고 — 일치.

### §10 강제 지점 독립 재열거

`EP-202 6 + EP-207 5 + EP-209 4 + 나머지 16 = **31**`. 구현 보고의 31 과 일치하고, EP-202 가 5→6 으로 는 만큼 다른 지점이 줄지 않았음을 항목별로 확인했다.

### 현재 변경의 운영 gate

| Gate | 결과 | 증거 |
|---|---|---|
| subtree `app/**` | **PASS** | `lint` **0 error** / 1 warning(기존) · `typecheck` node·web 0 error, test 2 error(`@opencode-ai/sdk` 베이스라인) · `vitest run src/renderer` **151파일 1177케이스** · `node --test scripts/*.test.mjs` **88 pass** |
| repository (docs) | **PASS** | `check-doc-inventory --check` ok |
| message bus | **PASS** | `git log -1 --format='%(trailers:only=true)' 4af38eaf` 이 7키 반환 |

- 게이트가 작업 트리를 바꿨는가: `npm run lint --fix` 와 `prettier --write` 가 구현자 자신의 신규/수정 파일을 포맷했고 커밋 전에 반영됐다. 검증 시점 트리는 클린이다.

## 5. 이번 라운드가 새로 만든 표면

| 표면 | 검토 | 판정 |
|---|---|---|
| `SPARK_STEP_HZ` 상수 | 프로덕션 참조 0 — 소비자는 `sparkCss.test.ts` 뿐이다. `SPARK_PERIOD_MS` 와 같은 성질이고 D2 와 같은 항목이다 | NON_BLOCKING(D2 에 흡수) |
| `excessByRep` · `signed` export | companion 이 직접 부른다. 프로덕션 그래프 밖(`scripts/`) | 무해 |
| 계단 16곳이 `app.css` 를 16줄 늘림 | 전역 CSS 1회 파싱이라 인스턴스 비용과 무관 | 무해 |
| `npm test` 가 21케이스로 늘어남(+3) | 순수 테스트, Electron 미기동 | 무해 |

## 6. Finding disposition — r3

| # | finding | 귀속 | disposition | 후속 |
|---|---|---|---|---|
| D8 | plan §10 EP-209 지점 ① 문구가 `rAF 프레임 > 0` 만 말한다 — 실현은 rep 산포도 본다 | EP-209 문구 | NON_BLOCKING | 다음 설계 턴에서 한 줄 정정. 계약은 더 엄격해졌고 지점 수는 불변이라 gap 이 아니다 |
| D9 | 최종 커밋본으로 낸 **라이브 AT-211 PASS 가 없다** — 가드가 4회 거부(기계 부하) | ΔVP-209 | NON_BLOCKING | 조용할 때 `cd app && npm run measure:spinner-perf` 1회 |
| D3 | 신 > 구 | ΔVP-209 · D-205 | **CLOSED (r3)** | 30Hz 계단으로 초과분 +15.2 → +3.0~3.5, 예산 5.0 안 |
| D1·D2·D5·D7 | (r1·r2 항목 유지) | — | NON_BLOCKING / NEXT_HANDOFF | 변동 없음 |

## 7. Review Signals — 사실만

- 이전 라운드와 동일/유사 증상: **측정 유효성**이 두 라운드 연속이다 — r2 는 rAF 가드, r3 는 rep 산포. 두 번 다 "숫자는 나오는데 그 숫자가 무엇의 평균인지 모르는" 실행이었다.
- 관련 plan 지침/AC 의 존재: 계단 구멍은 r2 verify 가 이미 적었고 rev.4 가 지점을 신설해 닫았다 — 지적이 규범 행으로 올라간 사례다.
- 사용자 결정 변경 근거: 있다 — 실기 후 "30fps로 마무리"(D-211).
- 반복된 검증 환경 한계: 런타임 측정이 **기계 부하에 좌우된다**. 가드가 거짓 통과를 막지만 조용한 창을 기다려야 한다.

## 8. 결론 — r3

- 상태: **PASS**
- pair: **11 PASS · 1 사람 실기 대기(ΔVP-211)** · PAIR_FAIL 0 · PLAN_GAP 0
- AC: ✅ 11 · ⏸ 1(AT-210) = 12
- Product/UX 및 ACTIVE Decision: D-201~D-211 충족. **D-205 는 D-211 의 예산으로 재정의되어 충족**(초과분 +3.0~3.5 ≤ 5.0).
- 운영 gate: subtree·repository·message-bus 3종 PASS.
- 남은 사람 확인: **① AT-210** — 실행 앱에서 14×14 스피너가 상태문구보다 크게 보이는가(30Hz 계단이 거슬리지 않는가는 이미 실기로 확인됐다). **② D9** — 기계가 조용할 때 `npm run measure:spinner-perf` 1회.
- 다음 단계: 두 사람 확인 뒤 archive 이동. 코드 축에 남은 구현자 작업은 없다.
