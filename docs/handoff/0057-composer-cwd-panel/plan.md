...
- 동의 / 그대로 진행: Composer 패널 삽입 위치, files IPC 분리, send payload 기반 세션 출생 cwd 고정, `sessions.cwd` 영속화 방향으로 구현했다. 사용자 후속 피드백에 따라 **프로젝트 랜딩 Composer도 `/new`와 동일 정책**(랜딩 baseline은 default, 선택 cwd는 해당 새 세션 출생 시 고정, 세션 진입 후에는 열기만)으로 포함했다.
...
- [x] Composer cwd 패널 추가 및 도구승인↔Notice 사이 삽입
