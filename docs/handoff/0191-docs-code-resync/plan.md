...
| 일자 | 2026-08-18 |
| 매핑 | — (문서 전용) |
| 상태 | … → IMPL_DONE (r6) → **verify/FAIL (r6)** |
...
### 라운드 6 파생 이슈 (verify r6)

> 판정 근거·재현 명령은 [`verify.md`](verify.md) §3·§5·§12. **계측 축은 이번에 닫혔다** — 남은 것은 분류 완결성 하나다.

| # | 이슈 | 출처 | 대응 방향 | 상태 |
|---|---|---|---|---|
| L1 | `filterEscalatingDefaultMode` 2사이트(`TRD.md:378`·`adapters.md:67`)가 버킷 6행 어디에도 없다. delta 산문이 "정정 대상이었던 것" 으로 따로 적어 **버킷 배정을 건너뛰었다** — 그런데 강제 지점 표·INDEX 는 `미분류 0` 을 ✅ 로 적는다 | verify r6 §5·§12 | `외부`(SDK 함수명) 버킷에 넣고 그 행 +2, 총계 재검산. 정정 이력은 delta 에 남기되 배정을 대체하지 않는다 | **r7 대상** |
| M1 | 실재 판정이 **줄머리만** 본다 — `code // Sym` 후행 주석을 코드로 센다. 후행 주석까지 세면 292 → **296**, 차집합 4건 전부 `Query`(통과시킨 줄 = `adapters/descriptor.ts:37`). **문서 결함 0** | verify r6 §3 | 넓히거나, 넓히지 않으면 §19 한계 표에 차집합 4·`Query`·통과 줄을 근거로 적는다 | **r7 대상**(경미) |

- 수정 불요 관찰 2건(INDEX 비고 841자 · r6 이 지적 밖 결함을 스스로 냈다)은 verify r6 §12 파생 관찰.
