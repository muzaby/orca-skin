...
| # | 이슈 | 출처 | 대응 방향 | 상태 |
|---|---|---|---|---|
| D3 | `providerMap` 미사용 + `new Map(...)` 이중 구현 — 0164 D3 와 동일 항목. | verify r1 | 0164 라운드에서 함께 정리 | **open** |
| D8 | `VIEW_SEGMENTS`(뷰 경로 목록)를 **Atlassian 1차 출처로 대조하지 않았다** — 0161 승계값. 틀리면 컨텍스트 경로 오판 → 전 요청 404. | verify r1 §자기 리뷰 | 공식 문서 대조 또는 사내 서버 실기 | **open** |
