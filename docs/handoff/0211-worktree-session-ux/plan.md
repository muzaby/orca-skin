...
| 작성자 | Claude Code |
| 일자 | 2026-08-30 (V1) · 2026-08-31 (ΔV1 · ΔV2) · 2026-09-02 (ΔV3 · ΔV4) · 2026-09-03 (ΔV5) |
| 매핑 | 0209·0210 격리 기능의 사용자 대면 잔여 3건 (준비 안내 · 표시 이름 · diff 실데이터) + 라운드 1 사용자 피드백 3건 (표시 정본 소멸 · 변경량 출처 · 조회 계기) + 변경사항 패널 UI/UX 명세 (Session Git Panel) + git 우측 패널 재설계 제안서 (Git Review Surface) + 싱크 계기 축소 · 첨부 GUI 재대조 · 로딩 교착 회귀 |
| 상태 | READY (ΔV5 — 싱크 계기 축소 + 첨부 GUI 재대조 + 로딩 교착 회귀) |
| V mode | `Baseline V` + `Delta V` |
| 기준 V | `V1` @ `0d8cf037` (ΔV1 의 기준) · `V1 + ΔV1` @ `553da6a8` (ΔV2 의 기준) · `V1 + ΔV1 + ΔV2` @ `d23c5be` (ΔV3 의 기준) · `V1 + ΔV1 + ΔV2 + ΔV3` @ `46047ac` (ΔV4 의 기준) · `V1 + ΔV1 + ΔV2 + ΔV3 + ΔV4` @ `177def67` (ΔV5 의 기준) |
| 이번 V revision | `ΔV5` |
...
| D-083 | 탐색 사이드바는 **기본 숨김**이고 컨텍스트 바의 폴더 버튼 또는 `⋮ › 파일 표시` 가 연다. 열리면 좌측에 **상단 변경 파일 트리 + 하단 커밋 목록** 두 구획이 선다. | 제안서 §8·§9 — “파일 트리는 diff width를 침범하기 때문에 필요 시에만 보여준다”. 우측 패널 최소 폭이 280px 라(`chatReducer.ts:374`) 고정 노출하면 diff 가 읽히지 않는다. | 사용자 명시 | ACTIVE | — |
...
| D-100 | 턴 종료 감지는 **지금의 `busy` 전이 그대로**다. main 이벤트·hook·신규 IPC 를 만들지 않는다. | 사용자 명시 — “Hook이 이니라면 현재 동작을 유지하겠다. 추후 구조적 개선을 하겟음”. 조건절이 대안을 지정한다: hook 이 아니면 **현재 동작 유지**이지 다른 새 배선이 아니다. | 사용자 명시 | ACTIVE | D-004(턴 경계 전이) 를 그대로 승계 |
...
| D-104 | 컨텍스트 바 라벨은 **`기준 → 현재`** 두 값이다. | 사용자 명시 — 참조 캡처의 `main → claude/git-uphuk-panel-redesign-xybgmd` 를 다시 보이고 “참조대로 `기준 → 현재`” 를 골랐다. D-069 의 금지를 사용자가 스스로 뒤집었다. | 사용자 명시 | ACTIVE | D-069 를 대체 |
