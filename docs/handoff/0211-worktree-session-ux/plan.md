...
| 상태 | **verify/PASS** (라운드 7 — D49·D50·D51 closed · VP-97 PASS; 사람 몫 시각 3건 잔여) |
...
> 새 ΔV 가 아니다. 라운드 4 verify 가 남긴 **BLOCKING 9행**을 닫는 재구현 턴이고, 계약은
> 그 행들이 인용한 V-pair · §10 지점 그대로다. 프로덕션 동작 변경은 마커 1개뿐이다.

### 설계 리뷰

계약 변경 0건. 아홉 행 전부 “배선·값은 이미 있는데 그것을 보는 오라클이 없다” 라 **plan 정정이
필요 없다**(verify §0 도 `PLAN_GAP 없음` 으로 적었다). 라운드 3 이 지시한 `handoff-review` 를
재구현 **전에** 수행했다(D44) — 모드·분류는 아래 Review Signals.

선조치 1건: `DiffReview` 빈 안내 `<div>` 에 `data-diff-empty` 를 붙였다. 형제 상태 넷이 이미
`data-diff-error`·`data-diff-not-synced`·`data-diff-loading` 을 갖는데 이 갈래만 없어 **대상 요소를
지목할 수 없었다**(D-123 이 정한 “요소 자신이 단언을 소유한다” 를 이 자리에서만 지킬 수 없었다).

### 강제 지점 전수 / V-pair 자기확인

| EP / pair | 이번 라운드 관측 | 결과 |
|---|---|---|
| EP-46 (4) / VP-72 | ① `query` 옵션의 `Stop` 매처 — M-A1 red 4/4 ② 두 드레인 — M-A2·M-A3 각각 red 1/4 ③ 이벤트 형태 `{type:'turn.ended', sessionId:'s1'}` 직접 단언 ④ 리듀서 tick — M-A6 red 5/10 | **4/4**, SELF_PASS |
| EP-53 (5) / VP-79 | ① 타일 헤더 32px — M29·M29b red ② 컨텍스트 바 트리거 24px — M-B1 red(**이번 라운드 신설**, 어느 라운드도 재지 않던 지점) ③ 사이드바 240px · ④ 파일 밴드 32px — M26·M28 재현 red ⑤ scroll owner 첫 자식 기존 단언 | **5/5**, SELF_PASS |
| EP-61 (2) / VP-88 | ① draft 자동 높이 — M50 red 1/45 ② 저장 카드 줄 라벨·거터 기존 단언(이번 라운드 미측정) | 1/2 측정, SELF_PASS |
| EP-71 (4) / VP-97 | ① 선택 액션 배선 — M13b red ③ `activeRequirementId` 전달 — M13 red 2/10. ②④ 기존 단언(미측정) | 2/4 측정, SELF_PASS |
| EP-73 (4) / VP-99 | ① `onRefresh` 배선 — M43 red ② `gitRefreshTick` 전이 — M42 red 2/5. ③④ 기존 단언(미측정) | 2/4 측정, SELF_PASS |
| EP-74 (3) / VP-100 | ① cwd 전달 → 그 경로로 조회 — M41 red 5/6 ② 메뉴 epoch 별 query/재시도 — 신규 6케이스. ③ 기존 `GitIdentityMenu.test.ts`(미측정) | 2/3 측정, SELF_PASS |
| EP-76 (3) / VP-102 | ③ GitRow 갱신 세대 전달 — M8·M8b·M8c red. ①② resource 공유·재시도는 기존 팩토리 + 신규 훅 케이스 | 3/3, SELF_PASS |
| EP-77 (3) / VP-103 | ① 빈 본문 중앙 — M5 red(마커 유지, 중앙 3토큰만 제거). ②③ 트리·토글 기존 red 좌표 | 1/3 측정, SELF_PASS |

`측정` 표기가 이번 턴에 결함을 심어 본 범위다 — 미측정 지점을 SELF_PASS 근거로 쓰지 않았고, 그
pair 의 자기판정은 측정한 지점의 red 에 근거한다.

### 이번 라운드 수정의 잠금

| 변이 | 분모 갈래 | 결과 | 검출 케이스 |
|---|---|---|---|
| M-A1 `claude.ts` 에서 `makeTurnEndHook(…)` 제거 | D25 인용 변이 | **red** | 신규 4/4 · `turn-end-hook.test.ts` 는 4/4 green |
| M-A2 스트림 중 `drainTurnEnded()` 제거 | D26 인용 변이 | **red** | 1/4 |
| M-A3 꼬리 `drainTurnEnded()` 제거 | D26 인용 변이 | **red** | 1/4 (다른 케이스) |
| M-A4 `chatStore` 의 `case 'turn.ended'` 절 제거 | D27 인용 변이 | **green — 등가 변이** | `chatStore.ts:644` `default:` 가 같은 `dispatchTo(RECV_EVENT)` 를 한다 |
| M-A5 스토어가 그 이벤트를 삼킨다 | D27 계약(대체 측정) | **red** | 2/10 |
| M-A6 리듀서가 tick 을 세지 않는다 | D27 계약 · EP-46 ④ | **red** | 5/10 |
| M-A7 `turn.ended` 가 형제 `turn.aborted` 의 일을 한다 | D27 형제 맞바꿈 | **red** | 1/10 |
| M41 훅의 `void cache.ensure()` 제거 | D35 인용 변이 | **red** | 5/6 · 팩토리 테스트는 green |
| M8 `identityGeneration` 상수 | D35 인용 변이 | **red** | 2/3 |
| M8b 세대를 메뉴 owner key 에서 제거 | 새 oracle 민감도 | **red** | 1/3 |
| M8c 새로 고침 축만 떨어뜨림 | 새 oracle 민감도 | **red** | 2/3 |
| M13 `activeRequirementId` → `null` | D37 인용 변이 | **red** | 2/10 · 기존 선택 테스트는 green |
| M13b 선택 액션 배선 제거 | 새 oracle 민감도 | **red** | 1/10 |
| M43 `onRefresh` 배선 제거 | D36 인용 변이 | **red** | 1/10 |
| M42 `REFRESH_GIT_SNAPSHOT` 무효화 | D36 인용 변이 | **red** | 2/5 |
| M5 빈 안내 중앙 3토큰 제거 | D38 인용 변이 | **red** | 1/45 |
| M29 헤더 밴드 32 → 48px | D39 인용 변이 | **red** | 1/45 |
| M29b 두 밴드 갈래를 한 모양으로 합침 | 형제 맞바꿈 | **red** | 1/45 |
| M50 `[field-sizing:content]` 제거 | D40 인용 변이 | **red** | 1/45 |
| M-B1 컨텍스트 바 24px → 32px | 새 지점(EP-53 ②) oracle | **red** | 1/18 |
| M26 사이드바 240 → 300px | r4 red 재현 | **red** | 1/13 |
| M28 파일 밴드 32 → 48px | r4 red 재현 | **red** | 1/13 |
| M23 빈 안내 문구 제거 | r4 red 재현 | **red** | 3/13 |

**분모 검산**: 인용 변이 **11**(D25 1 · D26 2 · D27 1 · D35 2 · D36 2 · D37 1 · D38 1 · D39 1 · D40 1 — 합 12 중 D27 인용 1건이 등가) · 새 oracle 민감도 **6**(M-A5·M-A7·M8b·M8c·M13b·M-B1, M-A6 은 EP-46 ④ 인용축과 공유) · r4 red 재현 **3** = 표 행 **23**. **red 22 · green 1**.

**덮개 회귀 0** — 장치를 교체·삭제하지 않았다(전부 추가). r4 가 red 로 본 M26·M28·M23 을 다시
심어 셋 다 red 로 재현했다.

**M-A4 를 `closed` 로 세지 않았다.** D27 이 인용한 변이가 검출되지 않으므로 그 행은 **부분
closed** 로 남기고, 대신 그 계약(스토어→리듀서 도달 · tick · 형제 비대칭)을 M-A5·M-A6·M-A7 로
닫았다. 등가인 근거는 `chatStore.ts:644` 한 줄이다 — 그 `default:` 가 삭제된 case 와 같은
`dispatchTo(key, { type: 'RECV_EVENT', event: ev })` 를 한다.

### Product/UX 파생 검토

사용자가 관측하는 것은 **바뀌지 않는다** — 이번 변경은 테스트 8파일(신규 5 · 추가 3)과 마커
1개다. `data-diff-empty` 는 DOM 속성이라 화면·문구·순서에 영향이 없다.

닫은 아홉 축이 각각 사용자에게 무엇이었는지는 그대로다: 메뉴가 영영 `확인 중` 에 머무는 것(D35) ·
‘새로 고침’ 이 아무 일도 안 하는 것(D36) · 코멘트를 눌러도 diff 가 반응하지 않는 것(D37) ·
턴이 끝나도 변경 목록이 갱신되지 않는 것(D25~D27). 넷 다 **오류 없이 조용히** 나타나므로
사용자에게는 “앱이 멈춘 것” 으로 보인다 — 그래서 게이트가 그것을 봐야 한다.

### 놓친 잠재 문제 + 대응

| 발견 | 대응 |
|---|---|
| D27 인용 변이가 **등가**다 — `default:` 절이 같은 일을 한다 | 보고에 그대로 적고 계약을 다른 변이로 닫았다. 프로덕션 case 절을 지우는 정리는 하지 않았다(문서 가치가 있고 동작 변경이 아니다) |
| EP-53 ② (컨텍스트 바 24px) 는 **plan 이 센 5지점에 있는데 어느 라운드도 재지 않았다** | 같은 EP 를 닫는 김에 오라클을 만들었다(M-B1 red). 형제 지점만 고치고 넘어가면 다음 라운드가 이 자리를 연다 |
| 훅 effect 를 node 환경에서 돌릴 DOM 이 없다 | 형제 `useGitPatch.commitScope.test.ts` 의 react 대역 패턴을 따라 슬롯 기반 하네스를 테스트 안에 뒀다 — 신규 의존성 0. **프로덕션 훅 자신**을 돌리므로 동명 재구현이 아니다 |
| `git checkout --` 로 변이를 되돌리면 같은 파일의 **정당한 편집까지** 사라진다 | 실제로 `data-diff-empty` 마커를 한 번 잃었고 그 상태의 red 를 잘못 읽을 뻔했다. 이후 변이 스윕은 파일 백업 → 복원 → 바이트 동일성 확인으로 바꿨다 |
| `mutation-queue.test.ts:35` 간헐 실패(D45) | 이번 라운드 전체 스위트 3회 중 1회 재현. 변경 무관이고 기존 D15·D34 와 같은 축이다 |

### 구현 보고

| 닫은 행 | 판정 | 이번 턴 증거 |
|---|---|---|
| D25 | ✅ | `src/main/adapters/claude.turnEnd.test.ts` 4케이스 · M-A1 red |
| D26 | ✅ | 같은 파일 · M-A2·M-A3 각각 red |
| D27 | ⚠️ | `chatStore.gitTriggers.test.ts` 3케이스 · M-A5·M-A6·M-A7 red. **인용 변이 M-A4 는 green(등가)** |
| D35 | ✅ | `gitIdentityRemoteWiring.test.ts` 6 + `gitRowIdentityGeneration.test.ts` 3 · M41·M8·M8b·M8c red |
| D36 | ✅ | `diffTileWiring.test.ts` 1 + `chatStore.gitTriggers.test.ts` 2 · M43·M42 red |
| D37 | ✅ | `diffTileWiring.test.ts` 4 · M13·M13b red |
| D38 | ✅ | `diffPanel0211dv6.render.test.ts` +2 · M5 red |
| D39 | ✅ | 같은 파일 +2 · M29·M29b red |
| D40 | ✅ | `diffTile.render.test.ts` 1행 추가 · M50 red |

✅ 8 · ⚠️ 1 · ❌ 0 = 라운드 5 대상 **9**. (ΔV1~ΔV14 의 AC 분모는 이 라운드가 바꾸지 않았다 —
이번 턴의 AC 변경 0건이다.)
...
| Gate | 산출 |
|---|---|
| `npm run lint` (`--fix`) | **0 error · 1 warning** — 기존 `useTranscriptVirtualizer`. 실행 후 트리에 도구 변경분 0 |
| `npm run typecheck` node/web/test | 3구성 **0 error** |
| `vitest run` 전체 | **347파일 / 3,352케이스 / 실패 0 / skip 0** (r4 기준선 342파일 3,324케이스 → 파일 +5 · 케이스 +28) |
| `node --test scripts/*.test.mjs` | **67/67** |
| `check-doc-inventory --check` | generated(9 items · 82 channels) · prose · links 3종 ok |
| `check-migrations-appendonly` | 마이그레이션 **20** · 소스 936파일 스캔 |
| `prettier --check` 변경 8파일 | 전건 통과 |
| `git diff --check` | **0건** |

**케이스 증분 검산**: 신규 5파일 **23**(4+5+6+3+5) + 기존 파일 추가 **5**(diffPanel +4 · GitContextBar +1) = **28**. 3,324 + 28 = **3,352** — 실측과 일치.

환경: `ELECTRON_SKIP_BINARY_DOWNLOAD=1 npm ci` 뒤 `npm test` 의 `pretest` 가 Node ABI 를 세우고,
`node node_modules/electron/install.js` 로 바이너리를 받아 `chat-turn.continuity.test.ts` 까지
수집됐다 — 라운드 4 가 적은 환경 한계(ABI·electron 미설치)는 이번 실행에서 **0** 이다.

### Review Signals

- **이번에 닫은 불변식은 라운드 3·4 와 같은 축이다** — “단위는 잠갔고 그 단위를 부르는 배선은
  안 잠갔다”. ΔV4 r1 D1(컨텍스트 바 미렌더) → ΔV6 D25~D27 → ΔV14 D35~D37 로 **네 라운드 연속**이다.
- **막았어야 할 지침은 셋 다 있었다**: `handoff-plan` §“적대 증거는 배선 존재처럼 방향·민감도를
  별도로 입증해야 할 때 선택” · `handoff-impl` §3 “내가 만든 oracle 이 주장하는 production 경로에
  실제로 진입하는가” · `handoff-verify` §0 “나머지는 직접 oracle 로 충분한 이유가 있는가”.
- **`handoff-review` round 26 을 재구현 전에 수행했다(D44). 모드 `DIAGNOSE_ONLY`** — 사용자 요청은
  “verify 지적사항 보완”(구현)이고 지침 변경 요청이 아니다. **지침 파일 변경 0건.**
  - 위 세 축은 **B(실행 누락)** 로 판정했다 — 규칙이 있었고 정상 수행하면 막힌다. review §3-5 에
    따라 같은 문장을 다시 넣지 않았다.
  - **A 후보 2건은 적용하지 않고 사용자 판단으로 남긴다**(둘 다 Tier 1):
    ① **증거의 지속성** — impl §8 은 “산출물에서 다시 찾은 표식” 을 요구하는데, ΔV14 는 `EP-76
    3/3`·`EP-77 3/3` 을 **일회성 브라우저 실측**(`status=2 유지` · `x=-0.006px`)으로 닫았고 그
    관측은 게이트에 남지 않았다. 규칙을 정상 수행해도 통과한다.
...
| D-133 | 우측 변경사항 본문의 코드·줄 번호·파일명·경로·입력 글자 크기는 좌측 nav 메뉴의 `text-footnote`와 같게 한다. 코드의 mono 서체는 유지한다. | ACTIVE · 같은 턴 추가 사용자 지시; Sidebar.tsx 메뉴의 실제 토큰으로 확인 |
...
> **ΔV4 진입 사유**(2026-09-02). 사용자가 git 우측 패널 UI/UX 재설계 제안서 18절과 실제 화면
> 캡처 8장을 제출했다 — “첨부이미지와 제안서를 참고하여 git 우측 패널의 기능 및 ui를 첨부와
> 똑같이 수정하라”. 전부 **사용자 결정 변경**이고 결함 보고가 아니다: 두 화면(목록↔peek)을
> 한 화면의 연속 review 로, 상단 요약을 컨텍스트 바로, 파일별 지연 조회를 전체 1회 조회로
> 바꾼다. **라운드는 그대로 1** 이다 — 사용자 명시(“설계변경으로 라운드를 증가하지 않는다”).
> ΔV3 은 아직 검증되지 않았고, ΔV4 가 그 UI 표면을 대체하므로 **V1+ΔV1+ΔV2+ΔV3+ΔV4 를 한 번에
> 검증한다** — ΔV3 의 백엔드 축(D-062~D-064)은 그대로 살아 REGRESSION 으로 닫힌다.

> **ΔV5 진입 사유**(2026-09-03). 사용자가 ΔV4 r3 구현물을 실기하고 세 가지를 지적했다 —
> ① git 표시의 싱크 계기를 “에이전트 턴 반환” 하나로 좁힐 것 ② 첨부 GUI 와 배치가 다르니
> 다시 대조해 반영할 것 ③ 커밋을 시킨 뒤에도 우측 패널이 `내용을 불러오는 중…` 만 그리는
> 회귀의 원인을 찾을 것. ①②는 **사용자 결정 변경**이고 ③만 결함 보고다. **라운드는 그대로
> 3** 이다 — 사용자 명시(“그것은 사용자변심이다. 라운드를 증가하지 않는다”).
> ΔV4 r3 의 `verify/FAIL` 이 남긴 blocking 파생 이슈 **D22**(VP-58 / AT-50 — 이동의 소유자
> 인자가 무관측)는 소멸하지 않는다. ΔV5 가 그 표면을 그대로 이어받으므로 D-110 으로 흡수해
> 같은 pair 에서 닫는다.

> **ΔV6 진입 사유**(2026-09-03). 사용자가 ΔV5 구현물을 실기하고 여섯 가지를 지시했다 —
> ① 컴포저 git 패널에 우측 정렬 닫기 ② 갱신 계기를 result 가 아닌 **stop 훅**으로 옮기고 비동기화
> ③ 커밋 없이 변경만 있는 경우는 노출하지 않음 ④ 빈 커밋일 때 `표시할 변경 사항이 없습니다.`
> ⑤⑥ 버튼 상태 토큰과 폰트·크기·색을 첨부 이미지대로 재연. **여섯 다 사용자 결정 변경**이고 결함
> 보고가 아니다. **라운드는 그대로 3** 이다 — 사용자 명시(“사용자 피드백이다. 라운드를 유지한다”).
> ΔV5 의 verify 는 아직 수행되지 않았다 — 이 갱신은 `verify/FAIL` 이 아니라 **설계 입력**이고,
> ΔV5+ΔV6 을 한 번에 검증한다.
...
### ΔV4 — git 우측 패널 재설계 (2026-09-02)

출처는 사용자가 첨부한 재설계 제안서 18절과 실제 화면 캡처 8장, 그 뒤 4건의 질의 응답이다.
제안서 문장은 요약하지 않고 절 번호와 함께 원문으로 인용한다.

| 구분 | 내용 | 출처 |
|---|---|---|
| 명시 요구 | “첨부이미지와 제안서를 참고하여 git 우측 패널의 기능 및 ui를 첨부와 똑같이 수정하라.” | 라이브 세션 |
| 명시 요구 | “이것은 핸드오프 211의 작업이며, 설계변경으로 라운드를 증가하지 않는다.” | 라이브 세션 |
| 명시 요구 | 목적 — “단순한 `diff viewer`가 아니라, 사용자가 현재 작업 트리의 변경사항을 빠르게 탐색하고 비교 기준을 바꾸며 파일별 변경 내용을 연속적으로 검토할 수 있는 **Git Review Surface**” | 제안서 §1 |
| 명시 요구 | 좁은 폭 원칙 4종 — “기본 화면에서 diff 영역의 가용 폭을 최대화한다” · “파일 탐색은 항상 고정 노출하지 않고 필요할 때만 펼친다” · “`File / Diff` 같은 별도 탭 전환 없이 파일과 diff를 하나의 연속적인 review flow로 구성한다” · “변경된 줄만 보여주지 않고 필요 시 주변 문맥을 단계적으로 펼칠 수 있어야 한다” | 제안서 §1 |
| 명시 요구 | 기본 레이아웃 — 컨텍스트 바 아래 파일 헤더가 “diff 본문 위에서 각각 section header 역할을 한다”, “파일별 diff는 접기/펼치기가 가능하다”, “별도의 `File` 탭과 `Diff` 탭은 두지 않는다” | 제안서 §3 |
| 명시 요구 | 컨텍스트 바 5요소 — 폴더 아이콘(navigator toggle) · `main → 현재 브랜치 ▾`(비교 기준 선택) · `⋮`(diff view settings) · `↗`(패널 확대) · `×`(패널 닫기) | 제안서 §4 |
| 명시 요구 | 비교 모드 둘 — `모든 변경사항`(“현재 작업 브랜치의 전체 변경사항을 기준 브랜치와 비교한다”) · `커밋 ›`(“특정 커밋을 선택하여 해당 커밋의 변경사항만 본다”) | 제안서 §5 |
| 명시 요구 | UX 원칙 — `HEAD`·`index`·`working tree`·`merge-base`·`git diff <sha>` 를 “기본 UI에서 직접 노출하지 않는다” | 제안서 §5 |
| 명시 요구 | 파일 행 6요소 — 접기 chevron · file SVG · filename · parent path · 추가 줄 수 · 삭제 줄 수 | 제안서 §6 |
| 명시 요구 | 아이콘 — “파일과 폴더는 문자 기호가 아니라 SVG 아이콘으로 표현한다”, `▢`·`□`·`📄`·`📁` 를 “사용하지 않는다” | 제안서 §6 |
| 명시 요구 | 사이드바 — “상단의 변경 파일 트리 + 하단의 커밋 목록”, “기본 화면에서는 숨긴다” | 제안서 §8·§9 |
| 명시 요구 | 파일 트리 — “Repository Explorer X / Changed File Navigator O”, “현재 선택된 비교 범위에서 변경된 파일만”, “사용자가 파일을 선택하면 우측 diff 영역의 해당 파일 위치로 이동한다” | 제안서 §8.1 |
| 명시 요구 | 커밋 목록 — `All changes` 항목 + commit card(subject · short SHA · author · relative time) | 제안서 §8.2 |
| 명시 요구 | 문맥 — “︿ 18 unmodified lines” / “﹀ 46 unmodified lines”, “사용자가 선택하면 해당 방향의 숨겨진 원문을 더 펼친다” | 제안서 §10·§11 |
| 명시 요구 | `⋮` 메뉴 8항목 — 파일 표시 · 모든 파일 접기 · 모든 파일 펼치기 · 나란히 · ✓자동 줄 바꿈 · ✓변경된 단어 강조 · 공백 변경 숨기기 · 새로고침 | 제안서 §12 + 캡처 5 |
| 명시 요구 | 비목표 — stage/unstage · commit UI · push/pull · branch 관리 · merge · rebase · history explorer · PR/CI 대시보드는 “IDE의 Git 도구 전체를 재현하는 것이 목적이 아니다” | 제안서 §18 |
| 명시 요구 | 비교 기준 좌측 라벨 — “세션 시작시 지정한 브랜치 (당시 head) 만 출력 우측화살표+우측값 표시하지 말것” | 질의 응답 §비교 기준 |
| 명시 요구 | 커밋 모드 — “아니다 — 목록만 좁히고 diff는 세션 기준 유지” | 질의 응답 §커밋 모드 |
| 명시 요구 | 기본 펼침 — “전부 펼침 + 파일 1개씩 조회가 아닌 전체 조회로 1회만 실행” | 질의 응답 §기본 펼침 |
| 명시 요구 | 상단 요약 6값 — “전부 제거” | 질의 응답 §요약 6값 |
| 추론 의도 | 제안서의 `main`·`All changes` 는 참조 제품의 화면 예시이지 영어 UI 요구가 아니다 — 라벨은 한국어 카탈로그다. | 저장소 규칙 (root AGENTS §8, `i18n/resources/ko.ts`) |
| 추론 의도 | 제안서 §13 이 문맥 확장 중 보존할 것에 **“현재 comment”** 를 명시한다 — 요구사항(diff 줄 `+`) 기능은 폐기가 아니라 존속이다. | 제안서 §13 원문 |
| 추론 의도 | 제안서에 `미커밋` 블록이 없는 것은 *레이아웃 재구성*이고 그 범위를 지우라는 문장은 없다 — 능력은 새 IA 의 범위 선택 자리로 옮긴다(D-082). | §4 규칙 “이동은 제거가 아니다” |

### ΔV5 — 싱크 계기 · GUI 재대조 · 로딩 회귀 (2026-09-03)

출처는 사용자 지시 3항과 그 뒤 3라운드의 질의 응답, 그리고 다시 첨부한 참조 화면 캡처 2장이다.

| 구분 | 내용 | 출처 |
|---|---|---|
| 명시 요구 | “그것은 사용자변심이다. 라운드를 증가하지 않는다.” | 라이브 세션 |
| 명시 요구 | “우측패널, 컴포저 패널스택의 git은 에이전트의 작업 결과로 판정한다. 외부 변경이 있더라도 실시간 동기화하지 않는다. 오직 에이전트 메시지 턴 반환시 싱크하며 이때 발견된 외부 환경에서 변경한 경우도 에이전트가 인식하는 것으로 간주한다.” | 라이브 세션 §도입 |
| 명시 요구 | “1. 우측패널, 컴포저 패널스택의 git은 에이전트의 작업의 결과물이다. 어시스턴트턴의 메시지 반환때 hook으로 비동기 실행” | 라이브 세션 §1 |
| 명시 요구 | “2. 작업물의 결과와 예시로 전달했던 gui 배치가 다르다. 첨부 gui를 다시한번 검토하고 반영하라.” | 라이브 세션 §2 |
| 명시 요구 | “3. 커밋요구후에도 우측패널에서 내용을 불러오는중... 만출력한다. 회귀의 원인을 찾고 정상동작하도록 하라.” | 라이브 세션 §3 |
| 명시 요구 | 비교 기준 라벨 = “참조대로 `기준 → 현재`” (D-069 의 “우측화살표+우측값 표시하지 말것” 을 사용자가 스스로 뒤집었다) | 질의 응답 §기준 라벨 |
| 명시 요구 | 파일 기본 상태 = “참조대로 항상 기본 접힘” | 질의 응답 §파일 기본상태 |
| 명시 요구 | 나머지 배치 차이 4건 = “모두 첨부이미지의 배치를 따르라” (커밋 서브메뉴 · 파일 헤더 1줄 + `↗` · 라벨 `모든 변경사항` · `미커밋 변경` 행 제거) | 질의 응답 §배치 차이 |
| 명시 요구 | 계기 축소 범위 = “둘 다 없앨다” (**대화를 열 때** 읽기와 **다시 읽기 버튼** 둘 다 제거) | 질의 응답 §다시 읽는 순간 |
| 명시 요구 | 턴 반환 방식 = “Hook이 이니라면 현재 동작을 유지하겠다. 추후 구조적 개선을 하겟음” | 질의 응답 §턴 반환 방식 |
| 추론 의도 | “git 은 에이전트의 작업 **결과물**” 이 가리키는 것은 **변경 목록**(`diffSummary`·`diffPatch`)이다 — 저장소·브랜치 이름(`gitApi.status`)은 세션의 식별이지 작업의 산출이 아니다. | 조사: 이름은 `baseline_ref` 로 세션 출생 때 이미 고정된다(D-070) |
| 추론 의도 | 계기가 턴 종료 하나로 줄면 **한 번도 싱크하지 않은 세션**이 정상 상태가 된다 — 그 자리를 로딩 문구로 두면 사용자 지시 §3 의 증상과 화면이 구분되지 않는다. | 조사: `DiffReview.tsx:87` 이 `patch===null` 하나로 두 상태를 그린다 |
| 추론 의도 | `↗` 의 동작은 참조 캡처가 말하지 않는다 — Orca 에서 그 자리의 기존 관용은 탐색기 열기다(`CwdButton`). | 조사: `filesOpenPath` 가 디렉토리만 열고 파일은 못 연다(`handlers/files.ts:66`) |

### ΔV6 — 커밋 전용 범위 · Stop hook 싱크 · 컴포저 닫기 · 참조 GUI 재대조 (2026-09-03)

출처는 사용자 지시 6항, 그 뒤 질의 응답 6건, 그리고 다시 첨부한 참조 화면 캡처 5장이다.

| 구분 | 내용 | 출처 |
|---|---|---|
| 명시 요구 | “핸드오프 211. 사용자 피드백이다. 라운드를 유지한다.” | 라이브 세션 §도입 |
| 명시 요구 | “composer git 패널스택의 우측 정렬로 닫기버튼 (x) 추가” | 라이브 세션 §1 |
| 명시 요구 | “git 정보 갱신시 어시스턴트 메시지 (result )가 아닌 stop 훅에 추가할것. 비동기로 변경” | 라이브 세션 §2 |
| 명시 요구 | “커멋없이 변경만 있는 경우 노출하지 않는것으로 변경” | 라이브 세션 §3 |
| 명시 요구 | “빈 커밋일때는 '표시할 변경 사항이 없습니다.' 출력” | 라이브 세션 §4 |
| 명시 요구 | “버튼 클릭상태, 호버 시 디자인 토큰을 첨부 이미지의 디자인을 충실히 재연” | 라이브 세션 §5 |
| 명시 요구 | “폰트, 크기, 색상 등 모든 디자인 요소를 첨부이미의 디자인을 충실히 재연” | 라이브 세션 §6 |
| 명시 요구 | §3 의 뜻 = “미커밋 변경분을 항상 목록에서 제외” | 질의 응답 §미커밋 처리 |
| 명시 요구 | §4 의 ‘빈 커밋’ = “세션 시작 기준 브랜치(head) 이후에 추가된 커밋이 없을때” | 질의 응답 §빈 커밋 |
| 명시 요구 | `×` 수명 = “다음 턴 종료 때 다시 뜸” | 질의 응답 §닫기 수명 |
| 명시 요구 | 헤더 라벨 = “참조대로 모드별로 바꾼다” (`all` = `기준 → 현재` · `commit` = `<sha7> <제목>`) | 질의 응답 §헤더 라벨 |
| 명시 요구 | 선택/활성 색 = “형태만 참조, 색은 Orca 러스트” | 질의 응답 §선택 색 |
| 명시 요구 | 헤더 좌측 = “참조대로 버튼 둘 (목록 없음 / 목록 있음)” | 질의 응답 §헤더 아이콘 |
| 추론 의도 | “미커밋 제외” + “커밋 0 이면 표시할 변경 없음” 은 한 계약의 두 면이다 — 미커밋을 빼면 커밋이 0 인 세션에 남는 것이 없다. | 조사: `readDiff(diffRevArgs(base))` 가 base→작업트리 한 범위만 낸다(`git-diff.ts:268`) |
| 추론 의도 | ‘stop 훅’ 은 Orca 가 이미 정규화한 `on-turn-end`(claude `Stop`)를 가리킨다 — 새 백엔드 개념이 아니다. | 조사: `adapters/hooks.ts:14` 가 `on-turn-end // Stop` 을 정의하고 `claude.ts:398` 이 `adaptHooks` 로 주입한다 |
| 추론 의도 | ‘비동기’ 는 **hook 이 조회를 기다리지 않는다**는 뜻이다 — hook 콜백이 git 을 await 하면 그만큼 턴 종료가 늦어지고 그것이 사용자가 없애려는 지연이다. | 조사: claude SDK hook 은 `Promise<HookJSONOutput>` 을 await 한다(`claude-adapt.ts:327` 콜백 계약) |
| 추론 의도 | 참조 캡처의 파랑(#cce2fa 채움 · #3975bd 체크)은 참조 제품의 accent 이고 Orca 의 accent 는 러스트다 — 사용자가 “형태만” 을 골라 그 축을 닫았다. | 실측: `tokens.css:81` `--color-accent: var(--color-rust)` |
...
| D-005 | 단계 줄은 **격리 준비 경로에서만** 뜬다. 비격리 새 세션·resume 의 화면은 바뀌지 않는다. | 요구 범위가 “워크트리로 세션 시작 시”다. 모든 턴으로 넓히면 요구에 없는 회귀 표면을 만든다. | 사용자 요구 범위 | **부분 SUPERSEDED** | → D-114 (‘닫기는 그리지 않는다’ 절만 대체 — 단계 줄의 격리 경로 한정은 ACTIVE) |
...
| D-030 | diff 타일 헤더에 **수동 새로고침** 버튼을 둔다. | D-028 이 “타일 열림” 계기를 없애면서 *사용자가 스스로 갱신할 수단*도 함께 사라진다 — 앱 밖에서 커밋/체크아웃하면 다음 턴까지 화면이 낡는다. 능력을 옮기는 것이 아니라 없애는 것이므로 대체 affordance 를 같은 변경에서 둔다. 0206 D-013 의 금지 셋(설정·펼치기·이동)에 걸리지 않는다 — 대응 동작이 실재한다. | 파생 정책 | **SUPERSEDED** | → D-086 (능력은 승계, 자리만 헤더 버튼 → `⋮` 메뉴 항목) |
...
| D-035 | 미추적 파일은 패널 범위에서 **완전히 제외**한다 — 목록·수치·트리 어디에도 넣지 않고 `ls-files --others` 조회 자체를 없앤다. 상단 요약이 “미추적 제외”를 문구로 말한다. | 사용자 명시 — “Untracked file은 Session Git Panel의 범위에서 완전히 제외한다”(명세 §3). 사용자는 “에이전트가 만든 새 파일이 커밋 전까지 패널에 보이지 않는다”는 결과를 제시받고 이 선택을 했다. | 사용자 명시 | **SUPERSEDED** | D-026 을 대체했고 → D-095 로 대체됨 (사용자가 그 결과를 화면에서 겪고 되돌렸다) |
...
| D-037 | 타일은 **두 화면**이다 — `Session Changes`(요약 + 커밋 타임라인 + 구분선 + Uncommitted) ↔ `Diff Peek`(뒤로 + 파일 헤더 + hunk + Prev/Next). 좌측 파일트리·커밋 목록 2분할과 파일트리 토글 버튼을 없앤다. | 명세 §4.1·§9·§20 이 두 화면을 그림으로 고정한다. 0206 D-010 의 3영역은 “배선할 IPC 가 없다”던 시점의 배치이고, 명세는 좁은 우측 패널에서 **한 축씩** 보여주기를 요구한다(§1). | 사용자 명시 | **SUPERSEDED** | D-037 이 0206 D-010·D-011 을 대체했고 → D-073 으로 대체됨 (제안서가 두 화면을 한 화면의 연속 review 로 되돌린다) |
| D-038 | 커밋 노드는 **작업 단위 grouping** 이다 — subject · 파일 수 · diffstat · 파일 일부 + `+N개 더` 인라인 확장. 확장은 같은 화면 안에서 일어나고 화면을 바꾸지 않는다. | 명세 §5·§6 — “이 동작은 동일한 panel 안에서 inline expansion으로 수행한다. 새로운 화면으로 이동하지 않는다.” | 사용자 명시 | **SUPERSEDED** | → D-079 (제안서 §8.2 의 커밋 카드는 subject·sha·author·시각 넷만 그린다 — 그 커밋의 파일은 **카드를 고르면 본문 목록이** 보여준다. 능력은 승계, 자리만 노드 안 → 선택 결과) |
| D-039 | `Uncommitted` 는 타임라인 노드가 아니라 **구분선 아래 별도 블록**이다. 같은 파일이 커밋 영역과 Uncommitted 영역에 함께 뜨는 중복을 허용한다. | 명세 §7·§8 — “commit timeline의 새로운 node로 표현하지 않는다”, “이 중복은 허용한다. 두 위치의 의미가 다르기 때문이다.” | 사용자 명시 | **SUPERSEDED** | → D-082 (성격이 다르다는 판정은 승계, 자리만 별도 블록 → 사이드바 범위 항목) |
...
| D-042 | `Prev/Next` 의 범위는 **사용자가 진입한 group** 이다 — 그 커밋의 파일 목록 또는 Uncommitted 파일 목록. | 명세 §14 — “기본 navigation 범위는 사용자가 진입한 group이다.” 세션 전체 목록으로 넓히면 사용자가 보던 작업 단위를 벗어난다. | 사용자 명시 | **SUPERSEDED** | → D-073 (peek 화면이 사라져 `이전/다음` 의 주어가 없다 — 연속 스크롤이 그 자리를 받는다) |
| D-043 | 상단 요약은 여섯 값을 말한다 — `<기준선 7자> → 현재` · 세션 전체 `+N −M` · 추적 파일 수 · “미추적 제외” · 커밋 수 · 미커밋 파일 수. | 명세 §4.1 의 상단 블록과 칩 3종이 그것이다. 기준선 sha 는 **화면에 보여야** 사용자가 “무엇 대비”를 안다 — 그래서 `GitDiffBase` 가 `head` 일 때도 oid 를 싣는다. | 사용자 명시 | **SUPERSEDED** | → D-081 (사용자가 여섯 값 “전부 제거” 를 골랐다) · 기준선 표기 축은 → D-069 |
...
| D-045 | 요약 조회 계기는 **둘 그대로**다 — cwd/세션 변경 · 턴 종료 전이. 폴링 없음, 수동 새로고침 유지(D-030). **커밋 선택은 이제 조회가 아니다** — 커밋은 grouping 이라 이미 온 요약 안에 있다. | D-036 이 커밋 범위 질의를 없애면서 ΔV1 D-028 의 마지막 절이 성립하지 않는다. 계기 축 자체(둘·폴링 금지)는 사용자 요구 그대로 유지된다. | D-028 승계 + D-036 파생 | **SUPERSEDED** | D-028 을 대체했고 → D-099 로 대체됨 (계기가 **턴 종료 하나**로 줄고 수동 새로고침이 사라진다) |
| D-046 | 세션 상태가 소유하는 것은 **셋**이다 — 요약 · peek 대상(`{group, path}`) · 커밋 파일 확장 집합. **로컬로 남는 것은 둘** — 파일 본문 캐시 · context 확장 폭. | D-029 의 이유(타일은 조건 렌더라 닫으면 언마운트된다)가 그대로 살아 있고 소유 대상만 바뀐다. 본문을 세션 상태로 올리면 세션마다 최대 1 MiB×2 가 store 에 남는다 — 타일 재열기 시 **peek 중인 파일 1건만** 재조회하는 비용이 더 싸다. | D-029 승계 + D-037 파생 | **SUPERSEDED** | D-029 를 대체했고 → D-094 로 대체됨 (peek 대상·본문 캐시가 사라지고 패치·비교 모드·표시 옵션이 그 자리를 받는다) |
...
| D-056 | 재anchor 는 **peek 시점**에 그 파일 본문으로 한다. 요약 갱신만으로는 재anchor 하지 않는다. 후보가 여럿이면 **저장된 줄 번호에 가장 가까운 것**을 고르고, 없으면 `located:false` 다. | G6. 요약 DTO 에는 줄 내용이 없고(파일 경로·수치만) 본문은 peek 이 조회한다(D-046) — 본문이 있는 유일한 시점이 peek 이다. 규칙이 없으면 구현자가 “첫 일치”를 발명하고, 반복되는 코드에서 요구사항이 엉뚱한 줄에 붙는다. | PLAN_GAP 정정 | **SUPERSEDED** | → D-093 (본문이 도착하는 유일한 시점이 peek 에서 패치 도착으로 바뀐다 — 나머지 절은 D-093 이 승계) |
...
| D-061 | 파일 본문은 **세션 상태의 LRU 캐시**에 남는다 — 같은 `(cwd, sessionId, 요약 세대, group, path)` 를 다시 열면 IPC 조회가 **0건**이다. 상한 **12항목**. | 사용자 명시 — “한 번 로드된 내용을 캐싱하라”. 지금은 캐시가 **없다**(실측: `DiffTileContent.tsx:93` 이 단일 슬롯 `useState`, `:105` 의 `body?.key === bodyKey` 가 막는 것은 **직전 하나**뿐) — Prev/Next 왕복이 매번 git 프로세스 4~5개를 다시 돈다. | 사용자 명시 | **SUPERSEDED** | → D-074·D-075 (파일별 조회 자체가 사라져 캐시할 대상이 없다) |
...
| D-065 | 목록 ↔ peek 의 **depth 전환에 애니메이션**을 준다. 저장소의 기존 어휘를 승계한다 — `animate-tile-in`(`app.css:142`, **180ms ease-out**) 과 같은 시간·이징이고 **`motion-reduce` 에서 꺼진다**. | 사용자 명시 — “depth 이동 애니메이션 효과를 주어야 한다”. 저장소에 slide 선례가 **0건**이라 keyframe 은 새로 만들되 상수는 승계한다 — 새 시간축을 발명하면 타일 마운트(180ms)와 화면 전환이 서로 다른 속도로 움직인다. | 사용자 명시 | **SUPERSEDED** | → D-092 (연출 요구는 살아 있고 주어만 두 화면 → 사이드바로 바뀐다) |
...
| D-069 | 컨텍스트 바의 비교 기준 라벨은 **세션 시작 시점의 브랜치 이름 하나**다. `→ 현재 브랜치` 화살표와 우측 값을 그리지 않는다. | 사용자 명시 — “세션 시작시 지정한 브랜치 (당시 head) 만 출력 우측화살표+우측값 표시하지 말것”. | 사용자 명시 | **SUPERSEDED** | D-043 의 기준선 표기 축을 대체했고 → D-104 로 대체됨 (사용자가 참조 화면을 다시 보고 화살표를 되살렸다) |
| D-070 | 그 이름의 정본은 **`sessions.baseline_ref` 한 컬럼**이고 **세션 출생 시 1회** 기록한다 — 격리는 `baseRef` 지정이 있으면 그 이름·없으면 `sourceCwd` 의 현재 브랜치, 비격리는 전송 시점 `sourceCwd` 의 현재 브랜치. 마이그레이션 **0020** 신설. | D-069 가 요구하는 이름이 저장소에 **없다**(실측: `managed_worktrees.branch` 는 *새* worktree 브랜치, `base_oid` 는 커밋, `sessions` 에 ref 컬럼 0건). `baseline_oid` 와 **같은 시점·같은 insert** 로 쓴다 — 두 값이 다른 시점에 기록되면 이름과 커밋이 갈라진다. | 사용자 명시 파생 | ACTIVE | D-033 의 1회 기록 규약을 이름 축으로 확장 |
| D-071 | `baseline_ref` 가 `NULL` 이면 라벨은 **기준선 sha 7자**로 접히고, 기준선도 없으면 `HEAD` 문구다. `summaryBaseLabel` 의 3상태를 4상태(`ref`·`oid`·`head`·`none`)로 넓힌다. | 이 변경 이전 세션·detached HEAD·저장소 아님 셋이 `NULL` 을 만든다. 라벨 자리를 비우면 사용자가 “무엇 대비”를 읽을 수 없다. | 파생 정책 | ACTIVE | — |
| D-072 | `PrepareWorktreeResult.managed` 가 **`baseRef: string \| null`** 을 함께 싣는다. 소비자가 DB 를 다시 조회하거나 `sourceCwd` 를 다시 읽지 않는다. | D-051 이 `baseOid` 에 정한 규약을 같은 값의 이름 축으로 승계한다 — 두 값은 `service.prepare` 안 같은 자리에서 결정된다(`service.ts:110`). | D-051 승계 | ACTIVE | — |
| D-073 | 타일은 **한 화면**이다 — 컨텍스트 바 + (선택) 탐색 사이드바 + 파일 섹션이 세로로 이어지는 diff 본문. `Diff Peek` 두 번째 화면과 `뒤로`·`이전/다음`을 없앤다. | 제안서 §3·§15 — “별도의 `File` 탭과 `Diff` 탭은 두지 않는다”, “File header → Inline diff → Next file header → Inline diff 형태의 연속적인 vertical review 를 사용한다”. | 사용자 명시 | ACTIVE | D-037·D-042 를 대체 |
| D-074 | 파일 섹션은 **기본 전부 펼침**이고 본문은 **패치 조회 1회**로 전부 온다. 파일당 조회를 하지 않는다. | 사용자 명시 — “전부 펼침 + 파일 1개씩 조회가 아닌 전체 조회로 1회만 실행”. 지금은 파일 하나를 열 때마다 `gitApi.diffFile` 이 돌고(실측: 프로덕션 호출부 1건, `DiffTileContent.tsx:148`) 전부 펼치면 그 수가 파일 수만큼 난다. | 사용자 명시 | **부분 SUPERSEDED** | D-061 을 대체. **조회 1회 축은 ACTIVE 로 남고**, “기본 전부 펼침” 만 → D-105 로 대체됨 |
| D-075 | 신규 채널 **`orca:git:diffPatch` 하나**가 비교 범위 전체의 파일별 diff 줄을 싣는다. `orca:git:diffFile` 과 `GitDiffFileContent` 를 **제거**한다. | 전문 두 벌(`{oldValue,newValue}`)과 패치 줄이 공존하면 같은 사실의 표현이 둘이 된다. 남기면 renderer 의 `buildDiffLines` O(n·m) 재계산도 그대로 남는다 — 사용자가 지적한 느림의 한 축이다. | D-074 파생 | ACTIVE | — |
| D-076 | 패치는 `git diff --unified=1000000 -M --no-color <base>` **한 호출**이다 — 전문맥이라 문맥 확장이 **재조회 0** 인 순수 파생으로 남는다. 전용 버퍼 **16 MiB**. | 부분 문맥으로 받으면 확장이 파일마다 재조회를 낳아 D-074 가 없앤 축이 되살아난다. R-30(“확장이 조회를 부르지 않는다”)은 ACTIVE 이고 이 선택이 그것을 지킨다. | D-074 파생 | ACTIVE | — |
| D-077 | 상한은 셋이고 전부 **값으로** 표시한다 — 파일 **200**(`MAX_DIFF_FILES` 승계) · 파일당 줄 **50,000** · 패치 전체 줄 **200,000**. 넘는 파일은 `kind:'too-large'` 로 줄 없이 오고 헤더의 `+N −M` 은 그대로다. **호출이 실패하면** 같은 명령을 `--unified=3` 으로 한 번 더 부르고 `contextLimited:true` 를 싣는다. | `maxBuffer` 판정은 파서보다 **앞**이라(`runner.ts:38`) 파일 상한이 그것을 막지 못한다 — D-053 이 커밋 이력에서 이미 닫은 축과 같다. 폴백까지 실패하면 `files` 가 빈 배열이고 `unavailable:true` 다. | D-053 승계 | ACTIVE | D-016 의 본문 1 MiB 를 대체 |
| D-078 | 패치 조회 계기는 **요약 세대당 최대 1회**이고 **타일이 닫혀 있으면 0회**다. 요약(`diffSummary`)의 계기 셋(cwd/세션 변경 · 턴 종료 · 새로고침)은 그대로다. | 요약은 컴포저 git 행도 읽으므로(D-025) 무거운 패치를 거기 실으면 타일을 열지 않아도 매 턴 전문맥 diff 를 돈다. 반대로 타일 열림을 *매번* 계기로 두면 사용자가 지적한 증상이 돌아온다 — “세대당 1회 + 세션 상태 보관”이 두 축을 함께 만족한다. | D-045 승계 + D-074 파생 | **부분 SUPERSEDED** | “세대당 1회 · 닫히면 0회” 는 ACTIVE. 괄호의 **요약 계기 셋** 만 → D-099 로 대체됨 |
| D-079 | 제안서가 정한 비교 범위는 **두 모드**다 — `모든 변경사항`(기본) · `커밋 <sha>`. **D-082 가 세 번째(`미커밋 변경`)를 더한다.** 커밋 모드는 **표시할 파일 목록만** 그 커밋의 것으로 좁히고, **각 파일의 diff 기준은 세션 기준선→현재 그대로**다. | 사용자 명시 — “아니다 — 목록만 좁히고 diff는 세션 기준 유지”. 그래서 D-036 은 ACTIVE 그대로이고 `GitDiffRequest.commit` 을 되살리지 않는다 — 커밋별 파일 목록은 이미 요약 안에 있다(`commits[].files`). | 사용자 명시 | ACTIVE | — |
| D-080 | 커밋 모드에서 그 커밋의 파일이 세션 diff 에 **없으면**(이후 되돌려짐) 섹션 헤더와 그 커밋 자신의 `+N −M` 을 그리고 본문 자리에 `세션 기준 변경 없음` 을 적는다. 목록에서 조용히 빼지 않는다. | D-079 가 두 목록(커밋 파일 · 세션 패치)을 교차시키면서 생기는 **새 상태**다. 빼면 사용자가 “이 커밋에 그 파일이 없었다” 로 읽는다. | D-079 파생 | ACTIVE | — |
| D-081 | 상단 요약 블록을 **제거**한다 — 기준선 · 세션 `+N −M` · 추적 파일 수 · 미추적 제외 · 커밋 수 · 미커밋 파일 수 여섯 값이 화면에서 사라진다. | 사용자 명시 — “전부 제거”. **결과**: D-035 의 미추적 완전 제외는 조회·목록·수치에서 그대로 유효하지만 그 사실을 알리는 화면 문구가 없어진다. 세션 합계는 컴포저 git 행이 계속 보여준다(D-025). | 사용자 명시 | ACTIVE | D-043 을 대체 |
| D-082 | `미커밋 변경` 은 별도 블록이 아니라 **사이드바 커밋 목록의 한 항목**이다 — `모든 변경사항` 아래, 커밋 카드 위. 고르면 `HEAD` 대비 추적 변경 파일로 목록이 좁혀진다(diff 기준은 D-079 와 같다). | 제안서 §8.2 는 `All changes` + commit card 만 그리지만 “미커밋을 없애라” 는 문장은 없다 — 레이아웃 재구성은 명시적 제거가 아니다(§4 “이동은 제거가 아니다”). D-039 의 “두 위치의 의미가 다르다” 는 사용자 결정이라 새 IA 의 범위 선택 자리로 옮겨 0 비용으로 유지한다. | D-039 승계 | **SUPERSEDED** | D-039 의 배치 축을 대체했고 → D-107 로 대체됨 (사용자가 차이를 명시적으로 보고 “첨부대로” 를 골랐다) |
| D-083 | 탐색 사이드바는 **기본 숨김**이고 컨텍스트 바의 폴더 버튼 또는 `⋮ › 파일 표시` 가 연다. 열리면 좌측에 **상단 변경 파일 트리 + 하단 커밋 목록** 두 구획이 선다. | 제안서 §8·§9 — “파일 트리는 diff width를 침범하기 때문에 필요 시에만 보여준다”. 우측 패널 최소 폭이 280px 라(`chatReducer.ts:374`) 고정 노출하면 diff 가 읽히지 않는다. | 사용자 명시 | **부분 SUPERSEDED** | → D-117 (‘폴더 버튼 하나’ 절만 대체 — 기본 숨김·두 구획은 ACTIVE) |
| D-084 | 파일 트리는 **현재 비교 범위의 변경 파일만** 담는 계층 트리다. 저장소 탐색기가 아니다. 파일을 고르면 우측 본문의 그 섹션으로 **스크롤 이동**한다(새 화면으로 가지 않는다). | 제안서 §8.1 — “Repository Explorer X / Changed File Navigator O”, “사용자가 파일을 선택하면 우측 diff 영역의 해당 파일 위치로 이동한다”. | 사용자 명시 | ACTIVE | — |
| D-085 | 아이콘은 **디자인 시스템의 SVG**(`Icon`)를 쓴다 — 파일 `doc` · 폴더 `folder` · 접기 `chevD`/`chevR` · 문맥 확장 `chevU`/`chevD` · 설정 `kebab`. Unicode 문자(`▢`·`📄`)를 쓰지 않는다. | 제안서 §6·§17. 두 이름이 저장소에 이미 있다(실측: `Icon.tsx` 의 `folder`·`doc`) — 새 asset 을 만들지 않는다. | 사용자 명시 | ACTIVE | D-066 승계 |
| D-086 | `⋮` 메뉴는 **여덟 항목**이다 — 파일 표시 · 모든 파일 접기 · 모든 파일 펼치기 · 나란히 · 자동 줄 바꿈(기본 켬) · 변경된 단어 강조(기본 켬) · 공백 변경 숨기기(기본 끔) · 새로고침. Git 조작(stage·commit·push) 항목을 두지 않는다. | 제안서 §12 도면 + 캡처 5 + §18 비목표. 수동 새로고침(D-030)이 여기로 옮겨 온다 — 능력을 없애는 것이 아니라 자리를 바꾼다. | 사용자 명시 | **SUPERSEDED** | D-030 의 자리를 대체했고 → D-106 으로 대체됨 (`새로고침` 항목이 D-099 로 사라져 **일곱 항목**이 된다) |
| D-087 | 표시 옵션 넷(레이아웃 · 줄바꿈 · 단어 강조 · 공백)과 사이드바 표시는 **세션 상태**다. 설정 파일에 영속하지 않는다. | 새 settings 키는 이번 요구에 없다(doc-inventory 가 세는 축). 타일은 조건 렌더라 닫으면 언마운트되므로(`RightPanel.tsx`) 로컬 `useState` 는 소멸한다 — D-029 가 이미 관측한 축이다. | 파생 정책 | ACTIVE | — |
| D-088 | 나란히 · 자동 줄 바꿈 · 변경된 단어 강조 · 공백 변경 숨기기는 **전부 순수 파생**이다 — 이미 받은 패치 줄에서 계산하고 재조회하지 않는다. | 넷 다 표현 축이고 git 에 다시 물을 것이 없다. 재조회를 만들면 D-078 의 “세대당 1회”가 깨진다. | 파생 정책 | ACTIVE | — |
| D-089 | 변경된 단어 강조는 **공백 경계 토큰의 공통 접두/접미 제거**로 구한다. 새 의존성을 넣지 않는다. | 필요한 것은 한 쌍(제거 줄 ↔ 추가 줄) 안의 다른 구간 하나다. 접두/접미 제거는 순수·결정적이라 단위 테스트가 쉽고, `diff` 패키지의 word API 를 새로 배선하지 않아도 된다(`diff@^9.0.0` 는 `diffLines` 로만 쓰고 있다). | 파생 정책 | ACTIVE | — |
| D-090 | 문맥 확장은 **양방향**이다 — gap 마다 가능한 방향(`︿` 위 · `﹀` 아래)의 버튼이 서고 라벨은 **숨은 줄 수**다. 한 번에 최대 **20줄**을 펼치고 남은 수가 그 이하면 gap 이 사라진다. | 제안서 §11 — “사용자가 선택하면 해당 방향의 숨겨진 원문을 더 펼친다”, 도면의 `︿ 18 unmodified lines` / `﹀ 46 unmodified lines`. 현재 `expandGap` 은 gap 의 **끝쪽 한 방향**뿐이다(실측 `diffHunks.ts:109`~). D-058 ①(키·순서 보존 + `insertedAbove`)은 그대로 승계한다. | 사용자 명시 | ACTIVE | D-058 에 방향 축을 더함 |
| D-091 | `↗` 는 **이 타일이 있는 열의 폭을 기본↔최대로 토글**한다. 새 전체화면 모드를 만들지 않는다. | 제안서 §4 “패널 확대”. 저장소에 패널 최대화 개념이 **0건**이고(실측: `maximize` 는 OS 창 컨트롤 하나뿐, `WinControls.tsx:34`), 열 폭은 이미 `SET_RIGHT_PANEL_COL_WIDTH` + `PANEL_MIN/DEFAULT/MAX_WIDTH` 로 조절된다 — 새 축을 만들지 않고 그 축을 쓴다. | 파생 정책 | ACTIVE | — |
| D-092 | depth 전환 연출은 **사이드바 열기/닫기**로 옮긴다 — 같은 상수(**180ms** · `motion-reduce` 에서 꺼짐)를 승계하고 새 시간축을 만들지 않는다. | D-065 의 요구(“depth 이동 애니메이션”)는 살아 있고 주어만 바뀐다 — 두 화면이 사라지면서 `animate-depth-in/out` 의 대상이 없어진다. 두 utility 는 사이드바 진입/복귀에 그대로 재사용한다. | D-065 승계 | ACTIVE | D-065 를 대체 |
| D-093 | 요구사항(diff 줄 `+`) 기능은 **그대로 유지**한다. 붙는 자리만 peek 본문에서 파일 섹션 본문으로 바뀌고, 재anchor 는 **패치 도착 시점**에 파일별로 돈다. | 제안서 §13 이 문맥 확장 중 보존할 것에 “현재 comment” 를 명시한다 — 존속의 증거다. D-047~D-050·D-054·D-055·D-057 은 ACTIVE 그대로이고 D-056 의 “본문이 있는 유일한 시점” 만 옮긴다. | 제안서 §13 + D-056 승계 | ACTIVE | D-056 을 대체 |
| D-094 | 세션 상태가 소유하는 것은 **여섯**이다 — 요약 · 패치 · 비교 모드 · 접힌 파일 집합 · 사이드바 표시 · 표시 옵션 넷(+요구사항은 기존 슬라이스 그대로). **로컬로 남는 것은 하나** — 파일별 문맥 확장 상태(`DiffHunkState`). | D-046 의 이유(타일은 조건 렌더라 닫으면 언마운트된다)가 그대로 살아 있고 소유 대상만 바뀐다. 문맥 확장 상태를 세션에 올리면 파일 수 × 확장 이력이 store 에 쌓이는데, 타일 재열기 시 기본 문맥으로 되돌아가는 비용이 더 싸다. | D-046 승계 | ACTIVE | D-046 을 대체 |
| D-095 | 미추적 파일은 **목록·트리·패치 목록에 남고 변경량은 0** 이다(`+0 −0`). 합계에 기여하지 않고 패치 본문도 비운다 — 이름이 남는 것이 전부다. 조회는 `ls-files --others --exclude-standard -z` 하나이고 요약·패치 각각 1회 부른다. | 사용자 명시 — D-035 아래에서 “로컬에 uncommitted 파일들이 많은데 미커밋변경에서도 안 보임” 을 겪고 **“미추적도 표시하되 +0 −0”** 을 골랐다. D-035 가 함께 요구한 “상단 요약이 ‘미추적 제외’를 문구로 말한다” 는 구현된 적이 없어(renderer·i18n 통틀어 0건) 제외가 **아무 설명 없는 빈 화면**으로 도달했다 — 되돌리는 쪽을 골랐으므로 그 문구도 만들지 않는다. | 사용자 명시 | **SUPERSEDED** | → D-111 (미커밋·미추적이 목록에서 함께 빠진다) |
| D-096 | 랜딩 칩 행에서 **브랜치 칩과 워크트리 토글은 테두리 하나가 감싸는 묶음**이다 — 버튼은 그대로 둘이고 사이의 1px 실선이 경계를 긋는다. 묶음 안 칩은 자기 테두리·반경·높이를 갖지 않고, 구분선은 **앞 칩(브랜치)이** 그린다. | 사용자 명시 — 참조 컴포저 이미지(“버튼은 분리되어있되 border가 감싸는 형태”). 둘은 “다음 세션을 어디서 시작하는가” 라는 한 결정의 두 축이라, 낱개 칩으로 흩어 두면 워크트리 토글이 옆의 참조 경로 칩과 같은 층으로 읽힌다. 구분선의 주인이 묶음이 아니라 앞 칩인 이유는 소멸이다 — git 저장소가 아니면 브랜치 칩이 스스로 사라지는데, 묶음이 그리면 그때 세로선 하나만 남는다. | 사용자 명시 | ACTIVE | D-011(칩 행 외형)에 묶음 축을 더함 |
| D-097 | 요약·상태 조회의 계기는 **넷**이다 — `initial` · `identity` · `turn-end` + **창 재포커스**. 넷째는 `refreshGitSnapshot` 을 부른다(두 조회만 다시 돌리면 요청 키가 같아 리듀서가 `patch` 를 보존해 우측 패널 본문만 옛 값에 머문다). | 사용자 보고 — “세션 초기화 이후 터미널에서 커밋을 수동으로 넣었는데 양쪽 모두 표현 안 되고 있음”. 앞의 셋은 전부 **앱 안의 사건**이라 사용자가 앱 밖에서 커밋·stage·브랜치 전환을 하면 어느 것도 발화하지 않았고, 그 변경을 보는 유일한 길이 우측 패널 `⋮` 의 새로고침 하나였다 — 그 메뉴는 타일을 열어야만 닿는다. 창으로 돌아오는 순간이 외부 변경을 관측할 수 있는 가장 이른 지점이다. | 사용자 보고 + 파생 정책 | **SUPERSEDED** | D-045 에 넷째를 더했고 → D-099 로 대체됨 (사용자가 실시간 동기화 자체를 거절했다) |
| D-098 | **기준선은 고정점이다.** `baseline_oid` 가 없는 세션은 질의 시점 HEAD 를 읽지 않고 **출생 시각(`sessions.created_at`)으로 되짚는다** — `git rev-list -1 --before=<출생> HEAD`. 그 시각 이전 커밋이 없으면(= 빈 저장소에서 시작) 저장소의 시작(empty tree `4b825dc6…`)이 기준이다. 세 경우 모두 `kind:'worktree-base'` 라 커밋 목록이 선다. | 사용자 보고 — “세션 진입 이후 터미널에서 수동 커밋을 넣었는데 패널에 안 나온다”. 질의 시점 HEAD 를 읽으면 기준선이 **커밋을 따라 움직여** 사용자가 커밋할수록 diff 가 비고 커밋 목록은 `worktree-base` 가 아니라 영영 빈다. 실측 재현: `web/movie` 세션(08:03:05, 커밋 0개 저장소 → `oid=NULL, ref='master'`) 뒤 08:04:17 터미널 커밋 → 양쪽 모두 빈 화면. `git init` 직후 `rev-parse HEAD` 는 exit 1, `symbolic-ref` 는 `master` 를 내므로 그 갈림이 NULL 의 정체다. | 사용자 보고 | ACTIVE | D-070 의 “세션 출생 때 고정된 baseline” 을 폴백 경로까지 확장 |
| D-099 | **변경 목록의 싱크 계기는 하나다 — 어시스턴트 턴 종료.** `initial`·`identity`·창 재포커스·`⋮ 새로고침` 넷을 전부 없앤다. `REFRESH_GIT_SNAPSHOT` 액션과 `refreshGeneration` 필드도 생산자가 없어져 함께 사라진다. | 사용자 명시 — “외부 변경이 있더라도 실시간 동기화하지 않는다. 오직 에이전트 메시지 턴 반환시 싱크하며 이때 발견된 외부 환경에서 변경한 경우도 에이전트가 인식하는 것으로 간주한다” + 계기 축소 범위 질의에 “둘 다 없앨다”. | 사용자 명시 | ACTIVE | D-045·D-097 을 대체 · D-078 의 계기 절을 대체 |
| D-100 | 턴 종료 감지는 **지금의 `busy` 전이 그대로**다. main 이벤트·hook·신규 IPC 를 만들지 않는다. | 사용자 명시 — “Hook이 이니라면 현재 동작을 유지하겠다. 추후 구조적 개선을 하겟음”. 조건절이 대안을 지정한다: hook 이 아니면 **현재 동작 유지**이지 다른 새 배선이 아니다. | 사용자 명시 | **SUPERSEDED** | → D-115 (사용자가 hook 을 지정해 ‘hook 을 만들지 않는다’ 가 뒤집혔다) |
| D-101 | `gitApi.status`(저장소 이름·브랜치)는 D-099 의 대상이 **아니다** — 최초 1회와 세션/cwd 변경 계기를 유지한다. | “git 은 에이전트의 작업 **결과물**” 이 가리키는 것은 변경 목록이다. 이름을 턴 종료에 묶으면 앱을 다시 켠 세션에서 컴포저 git 행이 **통째로 사라진다**(`gitRowView` 는 `status?.isRepo` 가 거짓이면 `{visible:false}`). 사용자가 요구하지 않은 소멸이다. | 파생 정책 (사용자 미확인 — 거부 시 한 줄 되돌림) | ACTIVE | D-031(소유자 하나)은 유지하고 계기만 두 축으로 가른다 |
| D-102 | **아직 한 번도 싱크하지 않은 세션**의 우측 패널은 로딩이 아니라 **“턴이 끝나면 표시됩니다”** 를 그린다. `patch===null` 하나로 두 상태를 그리던 자리를 `요청 있음/없음` 으로 가른다. | D-099 가 “싱크 전” 을 정상 상태로 만든다. 로딩 문구를 그대로 두면 사용자 지시 §3 의 고장 화면과 정상 화면이 **같은 픽셀**이 된다 — 다음 회귀를 사용자가 구분할 수 없다. | D-099 파생 | ACTIVE | — |
| D-103 | 패치 in-flight 가드는 **성공에도 해제**한다. 그리고 패치 무효화의 주체는 `RECEIVE_GIT_SNAPSHOT_SUMMARY` **하나**다. | 사용자 지시 §3 의 회귀 원인이다. 지금 `inFlight.current` 는 `.catch` 에서만 null 이 되고(`useGitPatch.ts:60`), 요약이 패치보다 늦게 도착하면 리듀서가 `patch:null` 로 되돌리는데 요청 키가 같아 가드가 재조회를 막는다 — 영구 로딩. 요약이 늦어진 것은 `base.oid !== HEAD` 가 되는 **커밋 이후** `readCommitHistory` 가 요약 경로에 붙기 때문이다(`git-diff.ts:271`). | 사용자 보고 | ACTIVE | — |
| D-104 | 컨텍스트 바 라벨은 **`기준 → 현재`** 두 값이다. | 사용자 명시 — 참조 캡처의 `main → claude/git-uphuk-panel-redesign-xybgmd` 를 다시 보이고 “참조대로 `기준 → 현재`” 를 골랐다. D-069 의 금지를 사용자가 스스로 뒤집었다. | 사용자 명시 | **부분 SUPERSEDED** | → D-116 (`all` 모드의 두 값은 ACTIVE — `commit` 모드 모양이 갈라져 나온다) |
| D-105 | 파일 섹션은 **기본 접힘**이고 본문 상단에 “대량 diff의 경우 파일이 축소되어 있습니다. 파일을 선택하면 펼칩니다.” 안내를 둔다. 세션 상태는 `collapsedFiles`(접힌 집합) → `expandedFiles`(펼친 집합)로 뒤집는다. | 사용자 명시 — “참조대로 항상 기본 접힘”. 기본이 접힘이면 세는 쪽이 반대가 된다 — 접힌 집합으로 두면 파일이 올 때마다 전부 채워 넣어야 하고 그 채움이 빠지면 조용히 전부 펼쳐진다. | 사용자 명시 | ACTIVE | D-074 의 “기본 전부 펼침” 절을 대체 (조회 1회 축은 유지) |
| D-106 | 비교 기준 메뉴는 **2행**이다 — `모든 변경사항`(우측에 `<기준> 대비` + 체크) · `커밋 ›`(**서브메뉴**에 커밋 목록). `⋮` 메뉴는 `새로고침` 이 빠져 **일곱 항목**이다. | 사용자 명시 — “모두 첨부이미지의 배치를 따르라”. 참조 캡처 2장이 2행 + 서브메뉴를 그린다. 일곱은 D-099 의 결과지 별도 선택이 아니다. | 사용자 명시 | ACTIVE | D-086 을 대체 |
| D-107 | `미커밋 변경` 진입점 둘(메뉴 행 · 사이드바 스코프 행)과 `DiffComparison` 의 `uncommitted` 멤버를 제거한다. **`GitDiffSummary.uncommitted` 계약 필드는 남긴다.** | 사용자 명시 — 차이를 “제거하면 미커밋만 보는 진입점이 사라진다” 로 보인 뒤 “첨부이미지의 배치를 따르라” 를 골랐다. 계약까지 지우는 것은 요구 밖이다(§4 “이동은 제거가 아니다” 의 반대 방향도 같다) — renderer 소비처가 0이 되는 사실만 §18 에 적는다. | 사용자 명시 | ACTIVE | D-082 를 대체 |
| D-108 | 파일 헤더는 **한 줄**이다 — `chevron · 파일 아이콘 · 파일명 · 흐린 부모 경로(인라인) · +N −M · ↗`. `↗` 는 그 파일을 **OS 탐색기에서 선택해 보여준다**(`shell.showItemInFolder`). | 사용자 명시(배치) + 파생(동작). 참조 캡처는 `↗` 의 동작을 말하지 않는다 — Orca 에서 그 자리의 관용은 탐색기 열기다(`CwdButton`). 기존 `filesOpenPath` 는 **디렉토리만** 열어 파일에 쓸 수 없으므로 파일 허용 모드를 더한다. | 사용자 명시 + 파생 정책 | ACTIVE | — |
| D-109 | `전체 변경` 라벨을 **`모든 변경사항`** 으로 바꾼다(en `All changes`). 두 자리 전부 — `⋮` 밖 메뉴와 사이드바. | 사용자 명시 — 참조 캡처의 문구다. 한 자리만 바꾸면 같은 범위가 화면 두 곳에서 다른 이름으로 불린다. | 사용자 명시 | ACTIVE | — |
| D-110 | `DiffReview` 는 스크롤 소유자를 **주입 가능한 ref prop** 으로 받는다. 기본값은 지금의 내부 ref 다. | ΔV4 r3 `verify/FAIL` 의 blocking 파생 이슈 **D22** 를 닫는다 — SSR 에서 ref 가 비어 `revealFileSection(null, path)` 가 되고, 그래서 소유자를 통째로 지운 변이가 3,080 케이스 전건 green 이었다. 주입하면 테스트가 **첫 인자 자체**를 단언한다. | verify/FAIL 파생 | ACTIVE | — |
| D-111 | **비교 범위는 커밋된 것만이다** — `기준선 → HEAD`. 미커밋 추적 변경도, 미추적 파일도 요약·패치·트리 어디에도 오지 않는다. | 사용자 명시 — “커멋없이 변경만 있는 경우 노출하지 않는것으로 변경”, 질의에서 “미커밋 변경분을 항상 목록에서 제외”. 참조 제품의 `main → 브랜치` 도 같은 범위다. | 사용자 명시 | ACTIVE | D-095 · D-026 의 “목록에 남는다” 절을 대체 |
| D-112 | 기준선 이후 **커밋이 0이면** 본문은 `표시할 변경 사항이 없습니다.` 한 줄이다. 파일 트리도 커밋 목록도 비어 있다. | 사용자 명시 — ‘빈 커밋’ = “세션 시작 기준 브랜치(head) 이후에 추가된 커밋이 없을때”. D-111 로 미커밋이 빠지면 그 상태에서 화면에 올릴 것이 남지 않는다. | 사용자 명시 | ACTIVE | — |
| D-113 | `chat.rightpanel.diffEmpty` 의 값을 **`표시할 변경 사항이 없습니다.`**(en `Nothing to show.`) 로 바꾼다. 이 키를 쓰는 자리 **넷**이 같은 문구를 쓴다. | 사용자가 문구를 그대로 지정했다. 자리마다 다른 키를 만들면 D-112 의 “빈 커밋” 화면과 “빈 트리” 화면이 다른 말을 한다. | 사용자 명시 | ACTIVE | — |
| D-114 | 컴포저 git 행에 **우측 정렬 `×`** 를 둔다. 누르면 그 행이 사라지고 **다음 턴 종료 싱크에서 다시 선다**. | 사용자 명시 — “우측 정렬로 닫기버튼 (x) 추가”, 수명은 “다음 턴 종료 때 다시 뜸”. 형제 패널(`Notice`)이 이미 같은 자리·같은 primitive 를 쓴다. | 사용자 명시 | ACTIVE | D-005 의 “닫기는 그리지 않는다” 절을 대체 (단계 줄 범위 절은 유지) |
| D-115 | **변경 목록 싱크의 계기는 Stop hook 하나다.** 정규화 `on-turn-end` 핸들러가 `turn.ended` 이벤트를 내고 renderer 가 그것을 세어 조회한다. hook 은 조회를 **기다리지 않는다**(즉시 `{}` 반환). | 사용자 명시 — “어시스턴트 메시지 (result )가 아닌 stop 훅에 추가할것. 비동기로 변경”. `busy` 전이는 result 메시지가 만드는 파생이라 사용자가 지목한 그 자리다. | 사용자 명시 | ACTIVE | D-100 을 대체 (“hook 을 만들지 않는다” 가 뒤집혔다) |
| D-116 | 컨텍스트 바 라벨은 **비교 모드별**이다 — `all` = `<기준> → <현재>`, `commit` = `<sha 7자> <커밋 제목>`. | 사용자 명시 — “참조대로 모드별로 바꾼다”. 참조 캡처 2장이 같은 자리에서 두 문자열을 보인다(이미지 1·3 = 커밋, 2·4 = 전체). | 사용자 명시 | ACTIVE | D-104 를 **부분** 대체 (`all` 모드의 두 값 라벨은 D-104 그대로) |
| D-117 | 헤더 좌측은 **세그먼트 토글 둘**이다 — `목록 없음`(diff 만) · `목록 있음`(파일 목록 + diff). 선택된 쪽이 활성 채움을 갖는다. | 사용자 명시 — “참조대로 버튼 둘 (목록 없음 / 목록 있음)”. 실측: 참조 헤더의 둘째 버튼이 `#cce2fa` 채움 + `#244d81` 글리프다(이미지 4, x 194..248 · y 256..303). | 사용자 명시 | ACTIVE | D-083 의 “폴더 버튼 하나” 절을 대체 (기본 숨김·두 구획 절은 유지) |
| D-118 | 상태 **문법**은 참조를 따르고 **색은 Orca 팔레트**다 — 활성 채움 `fill-selected`(rust-soft) · 활성 글리프와 체크 `accent` · hover `fill-uncontained-hover`. | 사용자 명시 — “형태만 참조, 색은 Orca 러스트”. 참조 실측값(활성 `#cce2fa` · 체크 `#3975bd`)은 참조 제품의 accent 라 옮기면 이 패널 하나만 톤에서 벗어난다. | 사용자 명시 | ACTIVE | — |
| D-119 | 사이드바의 선택 표시는 **채움**이다 — `모든 변경사항` 행과 커밋 카드 둘 다. 커밋 카드의 **상시 테두리를 없앤다**. | 실측: 참조의 선택 행은 `#e6e6e6` 채움이고(이미지 4, x 135..616 · y 865..931) 비선택 커밋 카드는 배경도 테두리도 없다(x=560 에서 y 932~1060 이 순백). 현재 구현은 `text-accent`+체크 아이콘과 상시 `border-t5` 다. | 참조 실측 | ACTIVE | — |
| D-120 | 참조 실측의 정규화 상수는 **k = 0.5 CSS px / device px** 다. 캡처는 2340×1080 이고 본문 글자가 28 device px = 14 CSS px 로 떨어진다. | 앞선 라운드가 쓴 “글자 크기로 정규화” 와 같은 방법이다(`composerPanel.ts` 주석). 이 캡처에서는 메뉴 행 높이 52 → 26 CSS px, 사이드바 폭 515/2077 → 24.8% 처럼 전부 정수 근처로 떨어져 상수가 맞음을 검산했다. | 파생 정책 | ACTIVE | — |
| D-121 | `git-added`/`git-removed` 토큰은 **바꾸지 않는다**. 참조 실측(`#398e4d`/`#9f3856`)과 다르다는 사실만 기록한다. | 두 토큰은 0206 D-016 이 **dark 스코프 대비**까지 검증해 정한 값이고 `DiffTable` 등 이 패널 밖 소비처를 함께 바꾼다. 사용자가 색 축에서 “Orca 러스트” 를 고른 것과 같은 방향이다. | 파생 정책 | ACTIVE | — |
...
#### ΔV4 갱신 메모 (2026-09-02)

- 신설 **D-069~D-094(26건)**. 대체 **11건** — D-030 → D-086 · D-037 → D-073 · D-038 → D-079 · D-039 → D-082 · D-042 → D-073 · D-043 → D-069·D-081 · D-046 → D-094 · D-056 → D-093 · D-061 → D-074·D-075 · D-065 → D-092. 전부 사용자 명시 변경이고 실패로 위장하지 않는다.
- **D-036 은 ACTIVE 그대로다.** 제안서 §5 는 커밋 단위 diff 를 요구하는 것처럼 읽히지만 사용자가 질의에서 “목록만 좁히고 diff는 세션 기준 유지”를 골랐다 — `GitDiffRequest.commit` 을 되살리지 않고 D-079 가 목록 축으로만 좁힌다.
- **대체된 결정에 걸려 있던 AC 를 다시 유도했다**: D-037·D-042↔AT-31(peek navigation)은 화면이 하나가 되어 **폐기**하고 AT-45 가 반대 방향(peek·뒤로·이전/다음이 **없다**)으로 잠근다. D-043↔AT-28(요약 여섯 값)은 **폐기**하고 AT-53 이 부재를 센다. D-061↔AT-38(본문 LRU)은 **폐기**하고 AT-46 이 “파일 수와 무관한 조회 1회”로 대체한다. D-065↔AT-40 의 두 화면 축은 AT-50 이 사이드바 축으로 다시 센다. D-056↔AT-35 의 “peek 시점 재anchor” 절은 AT-54 가 “패치 도착 시점”으로 다시 센다. **D-038↔AT-26**(커밋 노드의 `+N개 더` 인라인 확장)은 그 노드가 사라져 **폐기**하고, AT-49 가 반대 방향(커밋을 고르면 그 커밋의 파일이 **본문 목록에** 전부 선다)으로 다시 센다.
- **바뀌지 않은 ACTIVE 결정**: D-001~D-008 · D-020 · D-022~D-025 · D-027 · D-031~D-036 · D-040 · D-044 · D-045 · D-047~D-055 · D-057~D-060 · D-062~D-064 · D-066~D-068. 특히 **D-035(미추적 완전 제외)는 유지**된다 — D-081 이 지우는 것은 그 사실을 알리던 *문구*이고 조회·목록·수치의 제외는 그대로다. **D-062~D-064(ΔV3 백엔드)도 유지**되고 ΔV4 의 패치 조회가 같은 `run()` 관문을 지난다.
- **ΔV4 ACTIVE 결정 ↔ AC 대조**: 충돌 0(대조표는 READY self-review — ΔV4).
- **구현 중 발견한 규범 행 1건을 정정했다**(설계 커밋 분리). D-038(커밋 노드가 파일 일부 + `+N개 더` 를 갖는다)은 최초 ΔV4 메모에서 “바뀌지 않은 ACTIVE” 로 분류했으나, 제안서 §8.2 의 커밋 카드에는 파일 목록이 **없다** — 코드를 만지고서야 그 자리가 사라진 것이 보였다. 능력은 D-079(커밋 선택 → 본문 목록이 그 커밋의 파일)로 옮겼고 D-038 은 `SUPERSEDED` 다. 대체 11건.
- **두 결정의 가시적 결과를 여기 적어 둔다**(사용자 검토용). ① D-081 아래에서 **“미추적 제외”가 화면 어디에도 없다** — 에이전트가 만든 새 파일이 커밋 전까지 패널에 안 보이는데 그 이유를 말하는 문구가 사라진다(제외 자체는 D-035 로 유효). ② D-082 는 제안서에 없는 **한 항목을 더한다**(사이드바 커밋 목록의 `미커밋 변경`) — “첨부와 똑같이”를 엄격히 읽으면 빼야 하고, 빼면 미커밋만 보는 범위가 제품에서 사라진다.

#### ΔV5 갱신 메모 (2026-09-03)

- 신설 **D-099~D-110(12건)**. 대체 **7건** — D-045 → D-099 · D-097 → D-099 · D-069 → D-104 · D-082 → D-107 · D-086 → D-106 · D-074(펼침 절) → D-105 · D-078(계기 절) → D-099. 다섯은 사용자 명시 변경이고 둘(D-074·D-078)은 **절 단위 부분 대체**라 나머지 절이 ACTIVE 로 남는다.
- **D-035(미추적 완전 제외)는 이미 D-095 가 대체했고 ΔV5 는 건드리지 않는다.** D-107 이 지우는 것은 `미커밋` 범위 선택이지 미추적 표시가 아니다 — 두 개념을 같은 축으로 읽지 않는다.
- **바뀌지 않은 ACTIVE 결정**: D-001~D-008 · D-020~D-025 · D-027 · D-031~D-034 · D-036 · D-040 · D-044 · D-047~D-060 · D-070~D-073 · D-075~D-081 · D-083~D-085 · D-087~D-096 · D-098. 특히 **D-031(조회 소유자 하나)은 유지**된다 — D-101 은 소유자를 늘리지 않고 그 훅 안의 두 effect 가 서로 다른 계기 집합을 갖게 할 뿐이다.
- **ΔV5 ACTIVE 결정 ↔ AC 대조**: 충돌 0 (대조표는 READY self-review — ΔV5).
- **verify/FAIL 의 미해결 항목을 흡수했다.** ΔV4 r3 의 blocking D22 는 D-110 으로, non-blocking D23·D24 는 §7 ΔV5 주의사항으로 옮겨 같은 라운드에서 함께 닫는다 — `RETURN_TO_PLAN` 이 아니라 **설계 입력**이다(사용자 변심이 같은 표면을 다시 연다).
- **세 결정의 가시적 결과를 여기 적어 둔다**(사용자 검토용). ① D-099 아래에서 **앱을 다시 켜고 대화를 열면 변경 목록이 비어 있다** — 그 세션에서 턴을 한 번 돌려야 채워진다(D-102 가 그 자리에 문구를 둔다). ② D-099 아래에서 **터미널에서 한 커밋은 다음 턴이 끝날 때까지 보이지 않는다** — 사용자가 그것을 “에이전트가 인식하는 것으로 간주한다” 고 명시했다. ③ D-107 아래에서 **미커밋 변경만 따로 보는 진입점이 없다** — 미커밋 파일은 `모든 변경사항` 본문에 계속 섞여 나온다.


- **ΔV6 갱신(2026-09-03).** 신설 **11건**(D-111~D-121). `SUPERSEDED` **5건** — D-095 → D-111 · D-100 → D-115 · D-104 부분 → D-116 · D-083 부분 → D-117 · D-005 부분 → D-114. 전부 사용자 명시 변경이다.
- **ΔV6 `ACTIVE 결정 ↔ AC` 대조: 충돌 0.** D-111 ↔ AT-72 · D-112·D-113 ↔ AT-73 · D-114 ↔ AT-70 · D-115 ↔ AT-71 · D-116 ↔ AT-74 · D-117 ↔ AT-75 · D-118·D-119·D-120 ↔ AT-76. D-121 은 비범위 기록이라 AC 를 갖지 않는다(§8 말미에 근거).
- **ΔV6 이 드러낸 기존 drift 하나.** ΔV4 D-095 가 “미추적은 목록에 남는다”로 되돌렸는데 V 행은 R-24(“미추적 완전 제외”)를 `INHERITED` 로 계속 들고 있었다 — 코드는 D-095 를 구현했다(`readUntracked` 호출 2건 실측). D-111 이 두 사본을 다시 하나로 맞춘다.
- **ΔV6 이 유지한 ACTIVE 결정**(대조한 문장): D-066(형제 primitive) — 실측표 2행이 바꾸는 자리는 **드롭다운 버튼 라벨**이고 형제 타일 제목 5곳은 손대지 않는다. D-067(디자인 정본은 사람 실기) — AT-76 은 구조 단언이고 §19 실기 3건이 시각 판정을 갖는다. D-101(이름 축 분리) — 두 계기 함수는 그대로 둘이고 셋째 계기의 출처만 tick 으로 옮긴다.
...
### ΔV4 — 재설계의 비판적 검토 (2026-09-02)

| 축 | 판정 | 근거 |
|---|---|---|
| 증상과 원인 | 제안서는 결함 보고가 아니라 **IA 교체**다. ΔV2 의 두 화면은 “좁은 폭에서 한 축씩”이라는 같은 목표에서 나왔고, 제안서는 같은 목표를 **한 화면 연속 review** 로 푼다. | 명세 §1(ΔV2) 과 제안서 §1 이 같은 문장(“좁은 우측 패널”)을 전제로 다른 해법을 고른다 |
| 이미 충족됐는가 | **아니다.** 화면 분기가 지금 둘이고(실측: `data-session-changes-screen` 이 `list`·`peek` 2값), 파일 트리·컨텍스트 바·비교 선택·표시 옵션은 **0건**이다. | `rg 'data-session-changes-screen' app/src` = 4건(list 3 · peek 1) |
| 더 작은 해법 | **없다.** 파일 헤더를 목록 화면에 남기고 peek 만 지우면 “diff 가 헤더 아래 이어진다”가 성립하지 않는다 — 본문이 어디서 오는지가 바뀌어야 하고 그것이 D-075 다. | 제안서 §15 “File header → Inline diff → Next file header” |
| 제거 요구인가 | 제거 셋을 **각각 확인했다** — 상단 요약 6값은 사용자가 “전부 제거”를 골랐고(능력 자체 삭제), peek 화면·`이전/다음`은 연속 스크롤로 **이동**하며(능력 유지), 파일별 지연 조회는 전체 1회 조회로 **이동**한다(능력 유지). `미커밋`만 제안서에 자리가 없어 D-082 로 옮겼다. | §4 규칙 — 이동은 제거가 아니다 |
| 선행 자료 | ΔV3 은 **아직 검증되지 않았다**(보드 `impl/IMPL_DONE`). 그 산출 중 UI 축(D-065~D-067)은 ΔV4 가 대체하고 백엔드 축(D-062~D-064)은 그대로 살아 ΔV4 의 패치 조회가 같은 관문을 지난다 — 검증은 ΔV4 와 **함께** 한 번에 한다. | `git log` — `01d60fd`(ΔV3 구현) · `46047ac`(현재) |
| 기존 결정 충돌 | **한 건**. 제안서 §5 의 커밋 모드가 D-036(범위 단일화)을 정면으로 뒤집는 것처럼 읽혀 사용자에게 물었고, “목록만 좁힌다”로 **D-036 이 유지**됐다. 나머지 충돌 9건은 사용자 명시 변경이라 `SUPERSEDED` 로 처리한다. | 질의 응답 §커밋 모드 |

**이견 1건 — 기록만 남긴다.** D-081 이 “미추적 제외” 문구를 지우면 *에이전트가 만든 새 파일이 패널에 없다*는 사실을 화면이 설명하지 않는다. 사용자는 ΔV2 에서 그 결과를 제시받고 완전 제외를 골랐으므로 정책은 유지하되, 이 문구 부재가 “변경 없음”과 혼동될 수 있다는 것을 §17 리스크에 적는다. 작업은 요구대로 진행한다.

### ΔV5 — 지시 3항의 비판적 검토 (2026-09-03)

| 축 | 판정 | 근거 |
|---|---|---|
| 증상과 원인 (§3) | **원인을 코드에서 특정했다 — 회귀는 두 커밋의 곱이다.** `71fe581a` 가 창 포커스마다 `REFRESH_GIT_SNAPSHOT` 을 쏘고 그 액션이 `patch:null` 로 만들어 패치 조회를 요약과 **동시에** 출발시킨다. `51a1fc54` 이후 커밋이 하나라도 생기면 `base.oid !== HEAD` 가 되어 요약 경로에 `readCommitHistory`(`git log --raw --numstat`)가 붙어 요약이 패치보다 **느려진다**. 패치가 먼저 도착해 저장된 뒤 늦은 요약이 `patch:null` 로 되돌리는데, 요청 키가 그대로라 `inFlight.current` 가 재조회를 막는다 — 영구 로딩. | `chatReducer.ts:1249`(REFRESH 가 `patch:null`) · `chatReducer.ts:1281`(RECEIVE_SUMMARY 가 `patch:null`) · `useGitPatch.ts:53,60`(가드가 `.catch` 에서만 해제) · `git-diff.ts:271`(커밋 있을 때만 도는 history) |
| 원인이 맞는 자리인가 | **사용자가 지목한 “커밋요구후에도” 가 원인의 서명이다.** 커밋이 없으면 `base.oid === HEAD` 라 history 조회가 돌지 않아 요약이 빠르고, 그때는 순서가 뒤집히지 않는다. 증상이 커밋 이후에만 나타나는 것이 이 경로의 직접 증거다. | `git-diff.ts:271` 의 조건 `base.oid !== currentHead` |
| ①이 ③을 고치는가 | **부분만.** D-099 가 창 포커스·수동 새로고침을 없애 REFRESH 생산자가 0이 되지만, 세션 키가 바뀌는 턴 종료에서는 `BEGIN` 이 여전히 `summary`·`patch` 를 함께 비워 같은 경쟁이 남는다. 그래서 D-103 을 **따로** 둔다 — 계기 축소를 버그 수정으로 대신하지 않는다. | `chatReducer.ts:1265`(BEGIN 의 키 불일치 분기) |
| 더 작은 해법이 있는가 (§1) | **있고, 그것을 골랐다.** 사용자가 hook 배선을 조건절로 열어 뒀고(“Hook이 이니라면 현재 동작을 유지하겠다”) 현재 `busy` 전이가 이미 턴 경계를 정확히 준다 — 새 IPC 이벤트는 같은 결과에 배선만 늘린다(D-100). |  `gitRowState.ts:shouldRefetchGitStatus` 가 0206 D-004 에서 이미 그 전이를 계약으로 갖는다 |
| 제거인가 이동인가 (§2) | **제거다, 그리고 사용자가 그것을 알고 골랐다.** `미커밋 변경` 행은 ΔV4 에서 D-082 가 “이동은 제거가 아니다” 로 살려 둔 자리였다 — ΔV5 에서는 잃는 것(“미커밋만 보는 진입점이 사라진다”)을 문장으로 보인 뒤 사용자가 첨부 배치를 골랐다. 계약 필드까지 지우지는 않는다(D-107). | 질의 응답 §배치 차이 · 실측: `summary.uncommitted` renderer 소비처 **1건** |
| 기존 결정과 충돌하는가 | **둘이 정면 충돌했고 둘 다 사용자가 뒤집었다.** D-069(화살표 금지)와 D-074(기본 전부 펼침)는 **사용자 명시 인용**이라 설계자가 단독으로 뒤집을 수 없었다 — 그래서 원문을 인용해 되물었고 각각 D-104·D-105 로 대체됐다. | §3 대체 관계 열 |
| 잃는 것을 적었는가 | **셋을 적었다** — 앱 재시작 후 빈 목록 · 터미널 변경의 지연 · 미커밋 전용 진입점 소멸. 셋 다 §3 갱신 메모에 사용자 검토용으로 남긴다. | ΔV5 갱신 메모 |

### ΔV6 — 지시 6항의 비판적 검토 (2026-09-03)

| 요구 | 판정 | 근거 |
|---|---|---|
| 컴포저 `×` (§1) | **수용**. 다만 “한 번 닫으면 끝” 이면 사용자가 되살릴 진입점이 없다 | D-005 가 닫기를 막은 이유는 “배선할 채널이 없다” 였고 지금은 있다. 사용자가 수명을 “다음 턴 종료” 로 지정해 되살릴 진입점 문제가 함께 닫혔다(D-114) |
| Stop hook (§2) | **수용**. 증상은 진짜다 — `busy` 는 `result` 메시지가 만드는 파생이라 사용자 지목이 정확하다 | 실측: `sessionBusy = inflight \|\| listening`(`chatStore.ts:1459`) 이고 `inflight` 를 내리는 것은 terminal 이벤트(`telemetry`·`error`·`turn.aborted`)다. 즉 지금 계기는 **result 도착**이다 |
| Stop hook (§2) — 더 작은 해법? | **없다**. 배선 없이는 계기를 옮길 수 없다 | `NormalizedEvent` 에 턴 종료 variant 가 **0건**이다(`rg "type: 'turn\." app/src/shared/ipc.ts` = `turn.retrying`·`turn.aborted` 둘). Stop hook 신호를 renderer 로 옮길 채널을 새로 만들어야 한다 |
| 미커밋 제외 (§3) | **수용**. 다만 **부작용 하나가 크다** — 커밋하지 않고 일하는 세션은 패널이 계속 비어 보인다 | 사용자가 두 갈래를 알고 “미커밋 변경분을 항상 목록에서 제외” 를 골랐다. 그 상태의 화면을 D-112·D-113 이 문구로 설명한다 |
| 미커밋 제외 (§3) — 되돌리기 비용 | **낮다**. `diffRevArgs` 한 함수와 미추적 조회 두 호출부가 전부다 | 전수: `readUntracked` 호출 **2건**(`git-diff.ts:268`·`333`) · `diffRevArgs` 호출 **2건**(`268`·`334`). `GitDiffSummary.uncommitted` 계약 필드는 renderer 소비처가 이미 **0건**이다(ΔV5 D-107) |
| 빈 커밋 문구 (§4) | **수용**. 기존 키 값 교체라 새 상태를 만들지 않는다 | 실측: `diffEmpty` 는 이미 **본문·트리·파일 섹션·커밋 서브메뉴 넷**이 쓴다. D-112 가 요구하는 화면이 이미 그 넷 중 첫째다 |
| 디자인 재연 (§5·§6) | **수용**. 단 “충실히” 의 분모를 실측으로 고정해야 판정이 가능하다 | 앞선 두 라운드가 “첨부와 다르다” 로 되돌아왔다(ΔV4 → ΔV5). 이번에는 픽셀 실측표(§8 ΔV6)를 계약으로 두고 AC 가 그 표를 인용한다 |
| 디자인 재연 — 이미 충족? | **아니다**. 실측 5축이 어긋난다 | 사이드바 폭 38% vs 참조 24.8% · 헤더 제목 `font-serif font-semibold` vs 참조 regular · 커밋 카드 상시 테두리 vs 참조 없음 · 선택 표시 `text-accent`+체크 vs 참조 채움 · 사이드바 토글에 활성 상태 **없음** vs 참조 채움 |

**기존 결정과의 충돌 — 전부 사용자 명시 변경이다.** D-095(미추적 목록 유지) · D-100(hook 금지) · D-104(항상 두 값) · D-083(폴더 버튼) · D-005(닫기 금지) 다섯이 이번 턴에 뒤집혔고, 다섯 다 사용자가 문장으로 지정했다. 실패로 위장하지 않는다 — 라운드는 그대로 **3**이다.
...
### ΔV4 — 재설계 후 화면과 흐름 (2026-09-02)

```text
┌──────────────────────────────────────────────────────────────┐
│ [폴더]  main ▾            [⋮] [↗] [×]   ← GitContextBar      │
├───────────────┬──────────────────────────────────────────────┤
│ 변경 파일 트리 │ ▾ [파일] plan.md                             │
│  ▾ [폴더] docs│    docs/handoff/0213-…          +517  −0     │
│    [파일] …   │  ┌──────────────────────────────────────────┐│
│               │  │ ︿ 변경 없는 18줄                          ││
│───────────────│  │ 19 · 20 · 21  … +22  (추가 줄 틴트)      ││
│ 모든 변경사항  │  │ ﹀ 변경 없는 46줄                         ││
│ 미커밋 변경    │  └──────────────────────────────────────────┘│
│ ┌───────────┐ │ ▸ [파일] INDEX.md                            │
│ │ docs(han…)│ │    docs/handoff                   +1  −0     │
│ │ 82fe993 · │ │                                              │
│ │ Claude ·  │ │ (접힌 파일은 헤더만)                          │
│ │ 4시간 전  │ │                                              │
│ └───────────┘ │                                              │
└───────────────┴──────────────────────────────────────────────┘
   사이드바(기본 숨김·D-083)          연속 diff 본문(D-073)
```

**대표 흐름 — 패널을 열고 검토를 끝낸다.**

```text
타일 열기
   ↓  요약은 이미 세션 상태에 있다(D-045 계기 셋)
패치 조회 1회 (D-078 — 세대당 1회, 파일 수와 무관)
   ↓
파일 섹션이 전부 펼쳐진 채로 이어진다 (D-074)
   ↓  필요하면 헤더 chevron 으로 개별 접기
문맥이 부족하면 ︿/﹀ 로 그 방향을 펼친다 (D-090, 재조회 0)
   ↓  파일이 많으면 [폴더] 로 사이드바를 연다 (D-083)
트리에서 파일 선택 → 그 섹션으로 스크롤 (D-084)
   ↓  또는 커밋 카드 선택 → 목록만 그 커밋으로 좁힘 (D-079)
줄 옆 `+` 로 요구사항을 적으면 컴포저 스택에 쌓인다 (D-093·D-047)
```
...
| 상태 | 관측 | 전이 |
|---|---|---|
| 요약 없음 | 컨텍스트 바의 라벨 자리가 비고 본문이 빈다 | 요약 도착 → `요약만` |
| 요약만 | 라벨·사이드바 커밋 목록은 서고 본문은 로딩 문구 | 타일 열림 → 패치 조회 → `준비됨` / 타일 닫힘 → 조회 **0** |
| 준비됨 | 파일 섹션 전부 펼침 | 접기/펼치기 · 문맥 확장 · 비교 모드 전환 — **전부 재조회 0** |
| 패치 문맥 제한 | `contextLimited` 안내 한 줄 + 로드된 문맥까지만 확장 | 새로고침 → 재시도 |
| 패치 실패 | `변경 내용을 불러오지 못했습니다` + 새로고침 안내 | 새로고침 · 턴 종료 → 재조회 |
| 파일 상한 초과 | 그 섹션만 헤더 + `+N −M` + `파일이 커서 diff 를 표시하지 않습니다` | 다른 파일은 정상 |
| 커밋 모드 · 세션 diff 부재 | 헤더 + 커밋의 `+N −M` + `세션 기준 변경 없음`(D-080) | `모든 변경사항` 으로 복귀 |
| 저장소 아님 | 기존 `Git 저장소가 아닙니다.` 문구 그대로 | — |

#### ΔV4 파생 UX / 엣지케이스

- **요약 세대가 바뀌면 패치는 버려진다.** 턴이 끝나 새 요약이 오면 패치는 `null` 이 되고, 타일이 열려 있으면 곧바로 1회 다시 조회한다 — 닫혀 있으면 다음에 열 때 1회다(D-078).
- **비교 모드는 요약 세대를 넘어 보존한다.** 고른 커밋이 새 요약에 없으면(되감기·리베이스) `모든 변경사항` 으로 접는다 — 없는 커밋을 고른 채로 빈 목록을 보여주지 않는다.
- **접힌 파일 집합도 세대를 넘어 보존한다.** 사라진 경로는 자연히 무시된다(집합 원소가 경로 문자열이라 조회에 안 걸린다).
- **문맥 확장 상태는 타일을 닫으면 사라진다**(D-094 — 로컬). 기본 문맥으로 되돌아가는 것이 store 에 파일×확장 이력을 쌓는 것보다 싸다.

### ΔV5 — 계기 축소 후 화면과 흐름 (2026-09-03)

```text
세션을 연다 (턴 이력만 복원)
   ↓  변경 목록은 조회하지 않는다 (D-099)
컨텍스트 바: [폴더] [<기준> → <현재> ▾]        ⋮  ↗  ×      ← 이름은 status 가 준다 (D-101)
본문: "턴이 끝나면 표시됩니다" (D-102)
   ↓  사용자가 메시지를 보낸다 → 에이전트가 답을 마친다 (busy true→false, D-100)
요약 1회 + (타일 열려 있으면) 패치 1회
   ↓
안내 한 줄: "대량 diff의 경우 파일이 축소되어 있습니다. 파일을 선택하면 펼칩니다." (D-105)
파일 섹션이 **전부 접힌 채** 이어진다
   ↓  헤더 한 줄: [chev] [아이콘] 파일명  흐린/부모/경로   +N −M   ↗   (D-108)
헤더를 누르면 그 파일만 펼쳐진다 · ↗ 를 누르면 탐색기가 그 파일을 선택해 연다
   ↓  [폴더] 로 사이드바를 열면 트리에서 고른 파일이 먼저 펼쳐진 뒤 이동한다 (D-110)
비교 기준 ▾ : `모든 변경사항  <기준> 대비 ✓` / `커밋 ›` → 서브메뉴 (D-106)
```
...
| 상태 | 관측 | 전이 |
|---|---|---|
| **미싱크** (요청 없음) | 본문 `턴이 끝나면 표시됩니다` · 컴포저 행은 저장소·브랜치와 `+0 −0` | 턴 종료 → `요약만` |
| 요약만 | 라벨·사이드바 커밋 목록은 서고 본문은 **로딩 문구** | 타일 열림 → 패치 1회 → `준비됨` / 타일 닫힘 → 조회 **0** |
| 준비됨 | 안내 한 줄 + 파일 섹션 **전부 접힘** | 헤더 클릭 · 트리 선택 → 그 파일만 펼침. 재조회 **0** |
| 패치 실패 | `변경 내용을 불러오지 못했습니다` | **다음 턴 종료**에 재시도(수동 새로고침 없음) |
| 앱 재시작 후 기존 세션 | `미싱크` 로 되돌아간다 — 세션 상태는 메모리에만 산다 | 턴 종료 → `요약만` |

**두 문구가 서로를 대신하지 않는다**(D-102). `미싱크` 는 “아직 물어본 적이 없다”, `요약만` 은 “묻고 기다린다” 이고, 지시 §3 의 고장은 후자에 갇힌 상태였다. 같은 픽셀로 두면 다음 회귀를 사용자가 화면에서 구분할 수 없다.

#### ΔV5 파생 UX / 엣지케이스

- **패치 무효화 주체가 하나로 준다**(D-103). `BEGIN` 은 세션 키가 바뀔 때만 비우고, 같은 키의 갱신은 `RECEIVE_SUMMARY` 가 비운다. 가드는 성공·실패·폐기 **셋 다** 해제된다 — 폐기에서 해제하지 않으면 지금의 교착이 그대로 남는다.
- **펼친 집합은 요약 세대를 넘어 보존한다**(D-105). 사라진 경로는 자연히 무시된다. 세대가 바뀔 때 집합을 비우면 사용자가 열어 둔 파일이 턴마다 닫힌다.
- **`모든 파일 펼치기` 는 이제 채우는 쪽이다.** 기본이 접힘이라 `expandedFiles` 에 현재 패치의 전 경로를 넣고, `모든 파일 접기` 가 비운다 — ΔV4 와 방향이 반대다.
- **`↗` 는 파일이 사라졌을 때 조용히 실패하지 않는다.** 삭제된 파일·범위 밖 경로는 거부되고 화면은 그대로 남는다(경로 화이트리스트, D-108).
...
### ΔV4 범위 / 비범위 (2026-09-02)

**범위**: 컨텍스트 바(비교 기준 라벨 · 폴더 토글 · `⋮` · `↗`) · 연속 파일 섹션 본문 · 양방향 문맥 확장 · 탐색 사이드바(변경 파일 트리 + 커밋 목록) · 비교 모드 둘 + 미커밋 · 표시 옵션 넷 · 신규 채널 `orca:git:diffPatch` · `orca:git:diffFile` 제거 · 마이그레이션 `0020` (`sessions.baseline_ref`).

**비범위** (그리고 그것이 범위의 실행을 막지 않는 이유):

- **stage/unstage · commit · push/pull · branch 관리 · merge · rebase · history explorer · PR/CI 대시보드** — 제안서 §18 비목표 그대로. 읽기 전용 review surface 라 쓰기 능력이 없어도 모든 AC 가 닫힌다.
- **기준선 이전 커밋 열람** — D-068 이 이미 제외했고 사용자가 철회한 요구다. 사이드바 커밋 목록은 `기준선..HEAD` 만 담는다.
- **표시 옵션의 앱 전역 영속** — D-087. 세션 상태라 새 settings 키·마이그레이션이 없다.
- **side-by-side 의 가로 스크롤 동기화 튜닝** — 두 열이 같은 행 높이를 공유하는 표 한 벌이라 별도 동기화 장치가 필요 없다.
- **패치의 증분 갱신** — 세대가 바뀌면 통째로 다시 받는다. 증분은 새 프로토콜이고 이번 요구에 없다.
- **`미추적 제외` 문구의 대체 자리** — D-081 이 지운 자리를 다른 곳에 만들지 않는다(사용자가 “전부 제거”를 골랐다).

### ΔV5 범위 / 비범위 (2026-09-03)

- **ΔV5 추가 범위**: 변경 목록 싱크 계기를 턴 종료 하나로 축소(창 포커스·최초 진입·세션 전환·수동 새로고침 제거, `REFRESH_GIT_SNAPSHOT`·`refreshGeneration` 소멸) · 미싱크 상태의 별도 문구 · 패치 in-flight 교착 수정 · 컨텍스트 바 `기준 → 현재` · 파일 기본 접힘 + 안내 문구(`expandedFiles` 로 반전) · 비교 기준 메뉴 2행 + 커밋 서브메뉴 · `미커밋 변경` 진입점 제거 · 파일 헤더 1줄 + `↗` 탐색기 열기(파일 허용 모드) · 라벨 `모든 변경사항` · 스크롤 소유자 주입(D22 마감).
- **ΔV5 추가 비범위**: 턴 종료 push 이벤트/hook 신설(D-100 이 명시적으로 거절) · git 파일 워처 · 변경 목록의 **DB 영속**(앱 재시작 후 복원) · `GitDiffSummary.uncommitted` 계약 필드와 그 생산 경로 제거 · `↗` 로 편집기를 여는 경로(탐색기 선택만) · ΔV4 r3 의 non-blocking D23·D24 를 넘어서는 접근성 리팩토링.

| ΔV5 미룬 항목 | 나중에 하면 더 비싼가 | 처리 |
|---|---|---|
| 변경 목록의 재시작 보존 | 아니오 — 세션 테이블에 스냅샷 컬럼을 붙이면 된다 | D-099 아래에서 앱 재시작 후 빈 목록이 정상이다. 사용자가 그 결과를 보고 골랐고(“둘 다 없앨다”) 요구가 생기면 후속 |
| 턴 종료 hook/이벤트 | 아니오 — `busy` 전이가 이미 같은 경계다 | 사용자가 “추후 구조적 개선을 하겟음” 으로 미뤘다(D-100) |
| 미커밋 전용 진입점 | 아니오 — 순수 필터 하나다 | 사용자가 제거를 골랐다(D-107). 계약 필드가 남아 있어 되살리는 비용이 renderer 한 곳이다 |

- **비범위가 범위를 막지 않는지 확인**: 재시작 보존을 미뤄도 D-102 의 문구가 그 자리를 채워 화면이 비지 않는다. hook 을 미뤄도 D-100 이 현재 전이를 계약으로 승격해 계기 축이 관측 가능하다. `uncommitted` 계약을 남겨도 D-107 의 제거 대상(진입점 둘 + union 멤버)은 전부 renderer 안이라 전수 스윕이 성립한다.

### ΔV6 — 커밋 전용 범위 후 화면과 흐름 (2026-09-03)

```text
컴포저 패널 스택
┌──────────────────────────────────────────────────────────────┐
│ ⑂  orca-skin   feature/x            [ +12 −3 ]          ×    │  ← D-114 우측 정렬 닫기
└──────────────────────────────────────────────────────────────┘
   × 를 누르면 이 행이 사라진다 → 다음 턴 종료 싱크에서 다시 선다

우측 변경사항 타일 (사이드바 열림)
┌──────────────────────────────────────────────────────────────┐
│ [▤][▥]  main → feature/x  ⌄                      ⋮  ↗  ×    │  ← D-117 세그먼트 둘 · D-116 라벨
│  ↑ 목록없음  ↑ 목록있음(활성 채움)                             │
├──────────────┬───────────────────────────────────────────────┤
│ 📄 hello.txt │ ⌄ 📄 hello.txt   src/app      +1 −0      ↗    │
│      +1 −0   ├───────────────────────────────────────────────┤
│              │  1 + Hello world                              │
│──────────────│                                               │
│ 모든 변경사항 │                                               │  ← D-119 선택 = 채움
...
#### ΔV6 상태와 전이 — 본문 여섯 상태의 재해석

| 상태 | 판정식 | 화면 | ΔV5 대비 |
|---|---|---|---|
| 미싱크 | `gitSnapshotRequest === null` | `턴이 끝나면 표시됩니다.` | 유지 (D-102) |
| 로딩 | `request !== null && patch === null` | `내용을 불러오는 중…` | 유지 |
| 저장소 아님 | `!patch.isRepo` | `Git 저장소가 아닙니다.` | 유지 |
| 불가 | `patch.unavailable` | `변경 내용을 불러오지 못했습니다…` | 유지 |
| **비어 있음** | `sections.length === 0` | **`표시할 변경 사항이 없습니다.`** | 문구 교체 (D-113). 커밋 0 세션이 **정상적으로 이 상태에 산다**(D-112) |
| 준비됨 | 그 외 | 안내 한 줄 + 파일 섹션들 | 유지 |

#### ΔV6 싱크 계기의 전이

```text
사용자 전송 ─► 턴 진행 ─► 에이전트 Stop hook 발화
                              │  (hook 은 {} 를 즉시 반환 — git 을 기다리지 않는다)
                              ▼
                   main: turnEnded 버퍼에 적재
                              │
                   다음 SDK 메시지 경계에서 drain
                              ▼
                   `turn.ended` NormalizedEvent ─► renderer 리듀서
                              │                     turnEndTick += 1
                              ▼
                   useGitSnapshot: tick 변화 → diffSummary 1회
                              │
                              ▼
                   요약 도착 ─► 컴포저 git 행 복귀(D-114) + 우측 본문 갱신
```

- **`busy` 는 더 이상 계기가 아니다.** `gitSummaryQueryReason` 의 입력이 `{busy}` 에서 `{tick}` 으로 바뀐다 — 두 값이 같은 순간에 움직이는 것처럼 보여도 출처가 다르고, 사용자가 지목한 것은 출처다.
- **이름 축(`gitApi.status`)은 그대로다**(D-101). `initial`·`identity`·`turn-end` 셋 중 셋째만 `tick` 으로 바뀐다.
- **hook 이 오지 않는 턴**(중단·오류)에서는 tick 이 오르지 않아 목록이 그대로 남는다. 그것이 D-115 의 계약이다 — 실패한 턴은 작업 결과물을 만들지 않았다.

### ΔV6 범위 / 비범위 (2026-09-03)

| 구분 | 항목 |
|---|---|
| 범위 | 컴포저 git 행 `×` 와 그 복귀 규칙(D-114) |
| 범위 | `on-turn-end` 정규화 hook 등록 + `turn.ended` 이벤트 + renderer tick 계기(D-115) |
| 범위 | `diffRevArgs` 를 `base..HEAD` 로 좁히고 미추적 조회 2건 제거(D-111) |
| 범위 | `diffEmpty` 문구 교체와 커밋 0 세션의 본문(D-112·D-113) |
| 범위 | 컨텍스트 바 모드별 라벨(D-116) · 헤더 세그먼트 둘(D-117) |
| 범위 | 실측표(§8 ΔV6)가 정한 폰트·크기·색·상태 토큰 재대조(D-118·D-119·D-120) |
| 비범위 | `GitDiffSummary.uncommitted` **계약 필드 제거** — 소비처가 0 이 될 뿐 필드는 남긴다. 지우면 IPC 계약과 main 파서까지 번지고 이번 요구에 없다 |
| 비범위 | `git-added`/`git-removed` 토큰 값 변경(D-121) |
| 비범위 | opencode 어댑터의 Stop hook 대응 — 레지스트리에 등록된 어댑터가 **claude 하나**다(`registry.ts:19`) |
| 비범위 | 미커밋 변경을 보는 **별도 진입점** 신설 — 사용자가 “항상 제외” 를 골랐다 |
| 비범위 | 커밋/스테이지 같은 git **쓰기** 조작 — 이 표면은 읽기 전용이다(ΔV4 승계) |
...
### ΔV4 — 추가 Requirements / Acceptance

| R | AT / AC | 동작 기준 | 검증 수단 — 무엇을 단언하는가 | 프로덕션 도달 경로 |
|---|---|---|---|---|
| R-43 | AT-43 | 컨텍스트 바가 **세션 시작 시점의 브랜치 이름 하나**를 그린다 — `→` 화살표도 현재 브랜치도 그리지 않는다 | 순수: 라벨 파생이 4상태에서 각각 `{kind:'ref',ref:'main'}`·`{kind:'oid',oid:'7f8a9b0'}`·`{kind:'head'}`·`{kind:'none'}` 을 낸다고 단언. 렌더: `ref='main'` 요약에서 `main` 이 나오고 **현재 브랜치 문자열과 `→` 가 출력에 없다**고 단언(현재 브랜치를 일부러 다른 값으로 준다) | `sessions.baseline_ref` → `handlers/git.ts` lookup → `GitDiffSummary.base.ref` → `useGitSnapshot` → 세션 상태 → `GitContextBar` |
| R-44 | AT-44 | `baseline_ref` 가 세션 출생 시 1회 기록된다 — 격리는 base 의 ref, 비격리는 전송 시점 현재 브랜치. 이후 턴이 다시 쓰지 않는다 | 통합(임시 DB + 임시 저장소): 비격리 세션이 `main` 에서 시작하면 행의 `baseline_ref==='main'` 이라고 단언. 격리 세션에서 `PrepareWorktreeResult.managed.baseRef` 가 실려 왔고 그 값이 행과 **같다**고 단언(DB 재조회로 얻지 않는다). 브랜치를 바꾸고 2턴을 더 보내도 행 값이 **불변**이라고 단언. detached HEAD 에서 `null` 이라고 단언. 마이그레이션 `0020` 적용 후 기존 행의 다른 컬럼이 보존되고 `check-migrations-appendonly.mjs` 통과 | `service.prepare` → `managed.baseRef` → `prepare-worktree.ts` → `send.ts buildTurn` → `TurnContext` → `writer.persist` → `insertSession` |
| R-45 | AT-45 | 타일이 **한 화면**이다 — 파일 헤더 아래 diff 가 이어지고, 각 파일이 개별로 접힌다. `뒤로`·`이전/다음`·peek 화면이 **없다** | 렌더: 파일 2개 패치에서 두 섹션이 **같은 스크롤 컨테이너 안**에 순서대로 있고 각 헤더에 chevron `<button aria-expanded>` 가 있다고 단언. 한 파일을 접으면 그 파일의 줄이 출력에서 사라지고 **다른 파일의 줄은 남는다**고 단언. 전수: `rg 'data-session-changes-screen=\"peek\"' app/src` = **0건**, `rg 'diffPreviousFile\|diffNextFile\|header.back' app/src/renderer/.../rightpanel` = **0건** | 세션 상태(patch) → `DiffReview` → `FileDiffSection` |
| R-46 | AT-46 | 타일을 열면 패치 조회가 **1회**이고 **파일 수와 무관**하다. 닫혀 있으면 **0회**, 같은 세대에 다시 열면 증가 **0**, 턴 종료·새로고침 뒤 각각 **1** | 렌더: `gitApi.diffPatch` 스텁을 세고 ① 타일 닫힘 상태에서 요약 3회 갱신 → 증가 **0** ② 첫 열기 → **1** ③ 닫았다 열기 2회 왕복 → 증가 **0** ④ 턴 종료 전이 1회(타일 열림) → **1** ⑤ 새로고침 클릭 → **1** 이라고 단언. 파일 1개 패치와 40개 패치에서 호출 수가 **둘 다 1** 이라고 단언(파일 수 무관). 전수: `rg 'gitApi\.diffPatch' app/src/renderer` = **1건** | `DiffTileContent` 마운트 + 세션 상태 patch 부재 → `gitApi.diffPatch` → 세션 상태 |
| R-47 | AT-47 | 패치가 파일별 diff 줄을 싣고 상한 셋이 값으로 표시되며, 조회가 실패하면 **축소 문맥으로 한 번 더** 부른다 | 통합(임시 저장소): 추가·수정·삭제·rename·binary 다섯 파일에서 각 `status` 와 줄 배열이 실제와 맞는다고 단언. 삭제 파일의 모든 줄이 `removed`, 추가 파일의 모든 줄이 `added` 라고 단언. 순수: 파일 201건 → `files.length===200 && filesTruncated`; 줄 50,001건 파일 → `kind==='too-large'` 이고 `added`/`removed` 는 **그대로**; 전체 줄 200,001 → **합계를 넘기는 그 파일**이 `too-large` 이고, 뒤에 오더라도 남은 여유에 들어가는 더 작은 파일은 다시 싣는다(예산은 수집한 파일만 소비한다). 통합(인자 수집 runner): 첫 호출을 **실패시키면** 두 번째 호출 인자에 `--unified=3` 이 있고 `--unified=1000000` 이 **없으며** 결과가 `contextLimited:true` 라고 단언. 둘 다 실패하면 `files` 빈 배열 + `unavailable:true` | `gitApi.diffPatch` → `orca:git:diffPatch` → `gitDiffPatch()` → `run()` → `runGit` |
| R-48 | AT-48 | 문맥 확장이 **위·아래 둘 다** 되고 라벨이 **숨은 줄 수**이며 확장은 **조회 0**이다. 확장 후에도 기존 행의 키·순서가 보존된다 | 순수: 중간 gap 에서 `expandGap(state,id,20,'up')` 이 **끝쪽** 20줄만 드러내고 `insertedAbove===20`, `'down'` 은 **앞쪽** 20줄에 `insertedAbove===0` 이라고 단언. 선두 gap 에 `up` 만·말미 gap 에 `down` 만 후보라고 단언. 남은 수가 20 이하면 gap 이 목록에서 **사라진다**고 단언. **확장 전후 기존 행 키 배열이 부분수열로 보존**된다고 단언. 렌더: gap 버튼 라벨이 숨은 줄 수(`변경 없는 46줄`)이고, 확장 클릭 2회에서 `diffPatch` 스텁 증가 **0** 이라고 단언 | 세션 상태 patch → `patchLinesToDiffLines` → `buildDiffHunks` → `FileDiffSection` |
| R-49 | AT-49 | 비교 기준 메뉴가 `모든 변경사항`·`미커밋 변경`·커밋 목록을 주고, 커밋을 고르면 **목록만** 좁혀지며 **각 파일 diff 는 세션 기준 그대로**다 | 렌더: 커밋 A 를 고르면 A 의 파일 섹션만 남고 다른 파일이 사라진다고 단언. **같은 파일의 줄 배열이 `모든 변경사항` 모드와 깊은 동등**이라고 단언(diff 가 커밋 단위로 바뀌지 않았다는 직접 부정). `diffPatch` 스텁 증가 **0** 이라고 단언(모드 전환은 조회가 아니다). 전수: `rg 'commit' app/src/shared/ipc.ts` 의 `GitDiffRequest`·`GitDiffPatchRequest` 정의에 `commit` 멤버 **0건**(D-036 회귀). A 의 파일이 세션 패치에 없으면 `세션 기준 변경 없음` 을 그린다고 단언(D-080) | 세션 상태 comparison → 순수 필터 → `DiffReview` |
| R-50 | AT-50 | 사이드바가 **기본 숨김**이고, 열면 **변경 파일 트리 + 커밋 목록** 두 구획이며 파일을 고르면 그 섹션으로 **이동**한다 | 렌더: 기본 출력에 사이드바 마커가 **없고** 폴더 버튼 클릭 후 있다고 단언. 트리가 `docs/handoff/0213-…/plan.md` 를 **폴더 3단 + 파일 1**로 접고, 중간 폴더를 접으면 하위 파일이 사라진다고 단언. 커밋 카드가 subject·`sha7`·author·상대 시각 넷을 그린다고 단언. 파일 클릭이 그 섹션의 `scrollIntoView` 를 부른다고 단언(스텁). CSS 파싱: 사이드바 진입/복귀 utility 가 `animate-depth-in`/`-out` 과 **같은 180ms** 이고 `prefers-reduced-motion` 에서 `animation: none` 이라고 단언(D-092) | 세션 상태 sidebarVisible → `ChangedNavigationSidebar` |
| R-51 | AT-51 | `⋮` 가 **여덟 항목**을 주고 표시 옵션 넷이 **재조회 없이** 화면을 바꾼다 | 렌더: 메뉴 열기 후 항목 8개가 **정확히** 그 순서라고 단언(집합 동등). `모든 파일 접기` 후 모든 섹션이 접히고 `모든 파일 펼치기` 가 되돌린다고 단언. `나란히` 후 한 행이 old·new **두 칸**을 갖는다고 단언. `자동 줄 바꿈` 끄면 본문 컨테이너가 가로 스크롤 클래스로 바뀐다고 단언. `변경된 단어 강조` 켜짐에서 바뀐 토큰만 강조 마커를 갖고 꺼짐에서 **0개**라고 단언. `공백 변경 숨기기` 가 공백만 다른 줄 쌍을 `unchanged` 로 접는다고 단언(순수). 넷 각각에서 `diffPatch` 스텁 증가 **0**. `새로고침` 클릭이 요약·패치를 각각 **1회** 다시 부른다고 단언 | `DiffViewMenu` → `chatActions` → 세션 상태 → 순수 파생 → 렌더 |
| R-52 | AT-52 | `↗` 가 이 타일이 있는 **열 폭을 기본↔최대로** 토글한다 | 렌더/단위: `setRightPanelColWidth` 스텁을 주고 첫 클릭 인자가 `(col, PANEL_MAX_WIDTH)`, 두 번째가 `(col, PANEL_DEFAULT_WIDTH)` 라고 단언. `aria-label` 이 ko/en 카탈로그 키로 해석된다고 단언 | `GitContextBar` → `chatActions.setRightPanelColWidth` → `rightPanelColWidths` |
| R-53 | AT-53 | 제거 대상이 제품에서 사라진다 — 상단 요약 6값 · `orca:git:diffFile` 채널 · 본문 LRU 캐시 · peek 화면 | 전수: `rg 'gitDiffFile\|orca:git:diffFile\|GitDiffFileContent' app/src` = **0건** · `rg 'diffBodyCache\|diffPeekBodyKey\|DIFF_BODY_CACHE_LIMIT' app/src` = **0건** · `rg 'diffUncommittedChip\|diffTrackedFiles\|diffUntrackedExcluded\|diffCommitChip\|diffFileChip' app/src` = **0건**(ko·en 카탈로그 포함). 파일 부재: `DiffPeek.tsx`·`diffBodyCache.ts`·`diffFileCache.ts`·`peekNavigation.ts`. **양성 짝**: 같은 스위트에서 AT-46(패치 1회)·AT-45(연속 화면)가 green — 0건 스윕 단독으로는 “대체가 붙었다”를 증명하지 못한다. `docs/IPC_CONTRACT.md` 에 `orca:git:diffPatch` 행이 있고 `orca:git:diffFile` 행이 없으며 `check-doc-inventory.mjs --check` 가 차이 0 | — (제거 + 대체) |
| R-54 | AT-54 | 요구사항(diff 줄 `+`)이 새 본문 자리에서 그대로 동작하고 **재anchor 가 패치 도착 시점**에 파일별로 돈다 (회귀) | 렌더: 파일 섹션의 줄에서 `+` → 입력 → `추가` 후 컴포저 스택에 칩 1개, 그 줄 아래 요구사항 본문 1개라고 단언. 순수: 재anchor 가 **패치 도착 한 번에 여러 파일**을 처리하고 파일마다 자기 줄만 본다고 단언(파일 A 의 anchor 가 파일 B 의 줄에 붙지 않는다). 못 찾으면 `located:false` 이되 항목이 **남는다**고 단언. 회귀: AT-34~AT-37 의 기존 케이스가 전건 green(anchor 10키·전송 홉·중화) | `FileDiffSection` `+` → `chatActions.addDiffRequirement` → `RequirementTray` → `chatApi.send` |

#### ΔV4 주의사항

- **AT-43 은 부재까지 단언한다.** “브랜치명이 나온다”만 보면 `main → feature` 를 그리는 구현도 통과한다 — 현재 브랜치를 **다른 값**으로 준 뒤 그 문자열과 `→` 가 출력에 **없다**고 함께 세야 D-069 가 잠긴다.
- **AT-44 의 “1회 기록”은 불변으로 잰다.** insert 시점의 값만 보면 나중에 덮어쓰는 구현이 통과한다 — 브랜치를 바꾸고 턴을 더 보낸 뒤 **행 값이 그대로**인 것이 그 단언이다(D-070 · D-033 과 같은 축).
- **AT-46 의 “증가 0”은 소거에도 참이다**(AT-20·AT-32·AT-38 과 같은 축). 왕복 증가 0 만 보면 조회를 통째로 지운 구현이 통과한다 — ②의 **첫 열기 1**·④의 **턴 종료 뒤 1**·⑤의 **새로고침 1** 을 같은 스위트에서 짝짓는다.
- **AT-46 의 분모는 “파일 수와 무관”이다.** 호출 1회만 세면 파일 1개짜리 픽스처에서 우연히 참이다 — 40파일 픽스처에서도 **1** 이라야 D-074 가 잠긴다.
- **AT-47 의 상한 셋은 서로를 대신하지 못한다.** 파일 수 절단만 보면 줄 폭발이 그대로 통과하고(한 파일 50만 줄), 줄 상한만 보면 파일 201건이 통과한다 — 셋을 각각 센다. 폴백은 **인자 차집합**으로 확인한다(`--unified=1000000` 부재 + `--unified=3` 존재).
- **AT-48 은 방향을 맞바꾸는 변이에 반응해야 한다.** “확장이 줄을 늘린다”만 보면 `up`·`down` 을 서로 바꾼 구현이 통과한다 — `insertedAbove` 가 `up` 에서 `n`, `down` 에서 `0` 이라는 **두 값의 차이**가 방향의 oracle 이다.
- **AT-49 의 “깊은 동등”이 이 AC 의 핵심이다.** 목록이 좁혀지는 것만 보면 diff 까지 커밋 단위로 바꾼 구현이 통과한다 — 같은 파일의 줄 배열이 두 모드에서 **같은 값**이라야 사용자 결정(“diff는 세션 기준 유지”)이 잠긴다.
- **AT-51 은 항목 집합을 동등 비교한다.** 항목을 하나씩 `toContain` 으로 세면 한 항목을 지운 변이가 나머지 7개로 통과한다 — 8개 라벨 배열 자체를 비교한다(AT-35 와 같은 축).
- **AT-53 의 0건 스윕은 단독으로 목적을 증명하지 않는다.** “사라졌다”만 말하고 “대체가 붙었다”를 말하지 않는다 — AT-45·AT-46 양성 단언과 짝지어 잠근다(§5 방향 규칙). 허용 예외 없음: `GitDiffFileContent`·`diffBodyCache` 는 0211 이 이번 작업 전에 만든 식별자다.
- 사람 실기 항목: **ΔV4 에서 3건**(① 연속 스크롤에서 파일 경계가 읽히는가 ② 사이드바 열기/닫기 연출이 끊기지 않는가 ③ 형제 타일과 나란히 놓았을 때 이질감이 없는가 — D-067). ΔV2 의 1건(D-060 scroll 보정)은 **유지**되고 ΔV3 의 2건은 ①②가 **대체**한다 — 총 **4건**.

### ΔV5 — 추가 Requirements / Acceptance

| R | AT / AC | 동작 기준 | 검증 수단 — 무엇을 단언하는가 | 프로덕션 도달 경로 |
|---|---|---|---|---|
| R-55 | AT-62 | 변경 목록 조회의 계기는 **턴 종료 하나**다 — 최초 마운트·세션 전환·창 재포커스·수동 새로고침 어느 것도 조회를 만들지 않는다. 저장소·브랜치 **이름** 조회는 최초 1회·세션 변경·턴 종료 셋을 유지한다 | 순수: `gitSummaryQueryReason(null, {busy:false})` = `null`, `(…identity 'a'…, identity 'b')` = `null`, `(busy:true → busy:false)` = `'turn-end'` 라고 단언(**세 값을 한 케이스 묶음으로** — 하나만 보면 함수를 상수 `null` 로 만든 변이가 통과한다). `gitStatusQueryReason` 은 같은 입력에서 `'initial'`·`'identity'`·`'turn-end'` 를 낸다고 단언(두 함수가 **다른 값**을 낸다는 것이 D-101 의 oracle 이다). 렌더: `gitApi.diffSummary` 스텁으로 ① 마운트 **0** ② 세션 전환 **0** ③ 턴 종료 전이 **1** 이라고 단언. 전수: `rg 'subscribeGitExternalChange\|refreshGitSnapshot\|REFRESH_GIT_SNAPSHOT\|refreshGeneration\|diffRefresh' app/src` = **0건**(ko·en 카탈로그 포함) | `busy` 전이 → `useGitSnapshot` → `gitApi.diffSummary` → 세션 상태 → `GitRow` · `DiffReview` |
| R-56 | AT-63 | **한 번도 싱크하지 않은 세션**의 본문은 로딩 문구가 아니라 `턴이 끝나면 표시됩니다` 를 그린다. 요청이 나간 뒤에만 로딩 문구다 | 렌더: `gitSnapshotRequest===null` 에서 미싱크 문구가 나오고 **로딩 문구가 출력에 없다**고 단언. `request` 는 있고 `patch===null` 에서 로딩 문구가 나오고 **미싱크 문구가 없다**고 단언. 두 문구가 **서로 다른 카탈로그 키**라고 단언(같은 키를 두 번 쓰면 두 상태가 한 픽셀이 된다) | 세션 상태 `gitSnapshotRequest`·`gitSnapshot.patch` → `DiffTileContent` → `DiffReview` |
| R-57 | AT-64 | **요약이 패치보다 늦게 도착해도 본문이 온다.** 패치를 받은 뒤 같은 세대의 요약이 도착해 패치를 무효화하면 **다시 1회** 조회한다 | 순수(가드 상태 기계): `beginPatchQuery(null, {patch:null, request:{key:'k',generation:1}})` = `fetch:true` → 같은 입력 재호출 = `fetch:false`(중복 방지) → `settlePatchQuery` 뒤 같은 입력 = **`fetch:true`**(교착 부정) 라고 단언. **셋을 한 케이스에 묶는다** — 두 번째만 보면 가드를 지운 변이가 red 가 되고, 세 번째만 보면 가드를 통째로 없앤 변이가 통과한다. 리듀서: `BEGIN` 이 **같은 키**면 `summary`·`patch` 를 보존하고 다른 키에서만 비운다고 단언. 패치가 `null` 이 되는 지점이 **넷**이라고 전수(`rg 'patch: null' app/src/renderer/src/features/chat/reducer/chatReducer.ts` — 초기 상태 · cwd 변경 리셋 · `BEGIN` 키 불일치 · `RECEIVE_SUMMARY`. 변경 전 **5**에서 `REFRESH` 하나가 빠진 값이다). 앞 셋은 `gitSnapshotRequest` 도 함께 바꾸므로 가드 키가 달라지고, **같은 요청을 유지한 채 비우는 것은 `RECEIVE_SUMMARY` 하나**라는 것이 해제 규칙이 필요한 이유다 | `gitApi.diffPatch` 응답 → `receiveGitPatch` → `RECEIVE_GIT_SNAPSHOT_SUMMARY` → `useGitPatch` 재조회 |
| R-58 | AT-65 | 컨텍스트 바가 **`<기준> → <현재>`** 두 값을 그린다. 현재 브랜치를 모르면 화살표와 우측 값을 함께 생략한다 | 순수: 라벨 파생이 `{base:'main', head:'feature'}` 에서 두 값과 구분자를 내고, `head:null` 에서 **화살표가 없는** 한 값이라고 단언. 렌더: 기준 `main`·현재 `feature` 요약에서 **두 문자열이 모두** 출력에 있고 그 순서가 `main` → `feature` 라고 단언(자리를 맞바꾼 변이가 red 여야 한다 — 존재만 세면 통과한다) | `base.ref` + `gitStatus.branch` → 라벨 파생 → `GitContextBar` |
| R-59 | AT-66 | 파일 섹션이 **기본 전부 접힘**이고 상단에 안내 한 줄이 있으며, 헤더를 누른 파일만 펼쳐진다. `모든 파일 펼치기`/`접기` 가 그 반대 방향으로 동작한다 | 렌더: 파일 2개 패치의 첫 출력에 **두 파일의 diff 줄이 모두 없고** 헤더 2개와 안내 문구는 있다고 단언. 한 파일을 `expandedFiles` 에 넣으면 **그 파일의 줄만** 나오고 다른 파일 줄은 없다고 단언(전체 펼침으로 되돌린 변이가 red). 순수: `모든 파일 펼치기` 가 현재 패치의 **전 경로**를 집합에 넣고 `모든 파일 접기` 가 **빈 집합**을 만든다고 단언. 전수: `rg 'collapsedFiles' app/src` = **0건**(반전이 절반만 되면 두 진실원이 생긴다) | 세션 상태 `expandedFiles` → `DiffReview` → `FileDiffSection` |
| R-60 | AT-67 | 비교 기준 메뉴가 **2행**(`모든 변경사항` + `커밋 ›`)이고 커밋 목록은 **서브메뉴**에 있다. `미커밋 변경` 진입점이 **없다**. `⋮` 는 **일곱 항목**이다 | 렌더: 메뉴 첫 화면의 행이 정확히 2개이고 커밋 subject 가 **첫 화면에 없다**가, 서브메뉴를 연 뒤 있다고 단언(서브메뉴를 평면으로 되돌린 변이가 첫 단언에서 red). `모든 변경사항` 행이 `<기준> 대비` 와 체크를 함께 그린다고 단언. `⋮` 항목 배열이 **일곱**이고 그 순서라고 **집합 동등**으로 단언(AT-51 과 같은 축, `새로고침` 제거 반영). 전수: `rg "'uncommitted'" app/src/renderer` = **0건** · `rg 'diffUncommittedBlock\|diffAllChanges' app/src` 의 값이 `모든 변경사항`/`All changes` 뿐 | `GitContextBar` → `ComparisonMenu` → `chatActions.setDiffComparison` |
| R-61 | AT-68 | 파일 헤더가 **한 줄**에 이름·부모 경로·`+N −M`·`↗` 를 담고, `↗` 가 그 파일을 탐색기에서 **선택해** 연다. 세션 cwd 밖 경로는 거부된다 | 렌더: 헤더 안에서 이름과 부모 경로가 **같은 행**이고(둘을 감싸는 블록 요소가 없다) `↗` 가 접근성 이름을 가진 버튼이라고 단언. 클릭이 `filesApi` 스텁을 그 **절대 경로**로 1회 부른다고 단언. 통합(main): 세션 cwd 하위 **파일** 경로가 `showItemInFolder` 를 부르고, 상위로 빠져나가는 경로(`..`)와 세션과 무관한 경로가 **거부**된다고 단언(세 케이스 각각). 디렉토리 전용 기존 호출이 그대로 동작한다고 단언(회귀) | `FileDiffSection` `↗` → `orca:files:openPath`(reveal 모드) → `shell.showItemInFolder` |
| R-62 | AT-69 | 사이드바 트리에서 고른 파일이 **먼저 펼쳐진 뒤** 본문의 그 섹션으로 이동하고, 이동은 **그 화면의 스크롤 소유자**를 대상으로 한다 | 렌더(주입): `DiffReview` 에 스크롤 소유자 ref 를 주입하고 트리 선택 시 `revealFileSection` 의 **첫 인자가 주입한 소유자와 같다**고 단언 — `null` 이나 다른 값을 넘기는 변이가 red 다(**D22 가 열어 둔 축**). 기존 두 단언(경로 인자 · `onExpandFile` 선행)은 회귀로 유지한다. 순수: `revealFileSection` 이 `CSS.escape` 된 선택자를 쓰고 소유자가 `null` 이면 `false` 를 돌려준다고 단언 | `ChangedNavigationSidebar.onPickFile` → `DiffReview.pickFile` → `revealFileSection(owner, path)` |

#### ΔV5 주의사항

- **AT-62 는 “0건” 과 “1건” 을 함께 요구한다**(AT-20·AT-32·AT-46 과 같은 축). 마운트·세션 전환의 증가 0 만 보면 **조회를 통째로 지운 구현**이 통과한다 — 같은 스위트에서 턴 종료 전이의 **1** 을 짝짓는다.
- **AT-62 의 두 함수는 서로가 서로의 oracle 이다.** 한 함수만 보면 둘을 같은 함수로 합친 변이(= 이름 조회까지 턴 종료로 묶어 컴포저 행이 사라지는 D-101 위반)가 통과한다 — 같은 입력에서 **다른 값**을 낸다고 단언한다.
- **AT-63 은 부재까지 단언한다.** “미싱크 문구가 나온다” 만 보면 두 문구를 같은 자리에 함께 그리는 구현이 통과한다 — 각 상태에서 **다른 쪽 문구가 없다**를 함께 센다.
- **AT-64 의 세 단계는 분해할 수 없다.** 이 회귀의 형태가 “가드가 있다/없다” 가 아니라 **“해제되지 않는다”** 라서, `fetch:true → false → (해제) → true` 의 세 번째 값만이 교착의 직접 부정이다. 두 번째를 빼면 중복 조회 구현이 통과하고, 세 번째를 빼면 지금의 회귀가 그대로 통과한다.
- **AT-65 는 순서를 센다.** `main` 과 `feature` 가 둘 다 출력에 있다는 단언은 자리를 맞바꾼 구현에도 참이다 — 인덱스 비교로 방향을 잠근다(0204 r4 형제 슬롯 사례와 같은 축).
- **AT-66 의 전수 0건은 단독으로 목적을 증명하지 않는다.** `collapsedFiles` 소멸은 “옛 이름이 없다”만 말한다 — 첫 출력에 diff 줄이 **없다**는 양성 단언과 짝짓는다. 허용 예외 없음: `collapsedFiles` 는 0211 ΔV4 가 만든 식별자다.
- **AT-67 의 항목 수는 일곱이다.** ΔV4 의 여덟에서 `새로고침` 이 D-099 로 빠진 결과다 — 여덟로 남겨 두면 누를 수 없는 항목이 메뉴에 남거나 D-099 가 절반만 적용된다.
- **AT-68 의 거부 케이스가 이 AC 의 보안 축이다.** “파일을 연다” 만 보면 임의 경로 오픈 벡터가 열린다 — 기존 핸들러가 디렉토리 전용으로 막던 자리를 넓히는 변경이므로 `..` 이탈과 무관 경로 **둘 다** 거부를 센다.
- **AT-69 는 ΔV4 r3 의 D22 를 닫는 자리다.** r3 는 `revealFileSection(null, path)` 로 프로덕션이 아무것도 스크롤하지 않게 해도 3,080 케이스가 전건 green 이었다 — 소유자를 **주입**해 첫 인자 자체를 단언해야 그 변이가 red 가 된다.
- **ΔV4 r3 의 non-blocking 둘도 이 라운드에서 닫는다.** D23(아이콘 버튼 셋의 접근성 이름을 맞바꿔도 green)은 AT-68 의 `↗` 를 포함해 헤더 버튼을 `data-*` 로 지목해 이름과 짝지어 단언한다. D24(§10 EP-36 ② 의 “먼저 펼친 뒤 이동” 순서가 무관측)는 **문구를 코드에 맞춘다** — React 상태 갱신이 동기 DOM 을 바꾸지 않아 순서에 프로덕션 차이가 없다(r3 검증 판정 그대로).
- 사람 실기 항목: **ΔV5 에서 3건**(① 기본 접힘 상태에서 파일 목록이 한눈에 읽히는가 ② `기준 → 현재` 라벨이 좁은 폭에서 잘리는 방식이 납득되는가 ③ 미싱크 문구가 “고장” 으로 읽히지 않는가). ΔV4 의 4건 중 ①(연속 스크롤 파일 경계)은 기본 접힘으로 **성격이 바뀌어** ΔV5 ① 이 대체하고, 나머지 3건은 유지 — 총 **6건**.

### ΔV6 — 추가 Requirements / Acceptance

| R | AT / AC | 동작 기준 | 검증 수단 — 무엇을 단언하는가 | 프로덕션 도달 경로 |
|---|---|---|---|---|
| R-63 | AT-70 | 컴포저 git 행 오른쪽 끝에 `×` 가 있고, 누르면 행이 **사라진다**. **다음 턴 종료 싱크에서 다시 선다** — 그 사이의 어떤 렌더도 되살리지 않는다 | 순수: 노출 판정이 `(닫힘=true, tick=3)` 에서 `false`, 같은 상태로 재렌더해도 `false`, `tick` 이 4로 오르면 `true` 라고 단언(**세 값을 한 케이스로** — 둘째를 빼면 닫기가 전혀 안 먹는 구현이, 셋째를 빼면 영영 안 돌아오는 구현이 통과한다). 렌더: `×` 가 접근성 이름을 가진 버튼이고 클릭 뒤 `data-surface="git-row"` 가 출력에 **없다**고 단언. 자리: `×` 가 변경량 버튼 **뒤**라고 인덱스로 단언(형제와 맞바꾼 변이가 red) | `GitRowView` `×` → `chatActions.closeGitRow` → 세션 상태 → 다음 `turn.ended` tick |
| R-64 | AT-71 | 변경 목록 조회는 **Stop hook 신호 하나**로 난다. `busy` 가 내려가도 신호가 없으면 **0회**이고, 신호가 오면 `busy` 와 무관하게 **1회**다. hook 콜백은 조회를 **기다리지 않는다** | 순수: `gitSummaryQueryReason(null,{tick:0})` = `null`, `({tick:0},{tick:0})` = `null`, `({tick:0},{tick:1})` = `'turn-end'` 세 값을 한 케이스로 단언. 렌더: `gitApi.diffSummary` 스텁으로 ① 마운트 0 ② `busy` true→false 만 **0** ③ tick 증가 **1** 이라고 단언(②가 이 AC 의 직접 부정 — 옛 계기가 남아 있으면 red). main 순수: `on-turn-end` 핸들러가 **동기적으로** `{}` 를 돌려주고(반환값이 Promise 가 아니거나 즉시 resolve) 버퍼 길이가 1 늘었다고 단언. main 통합: 어댑터가 `Stop` 매처를 **등록한다**고 `adaptHooks` 산출의 키 집합으로 단언 | claude `Stop` → `adaptHooks` 콜백 → `turnEnded` 버퍼 → `eventBatches` drain → `turn.ended` → 리듀서 tick → `useGitSnapshot` |
| R-65 | AT-72 | 목록·트리·패치에 **커밋된 변경만** 온다. 커밋하지 않은 추적 변경과 미추적 파일은 **어디에도 없다** | main 통합(임시 저장소): 기준선 커밋 → 파일 A 를 고쳐 **커밋** → 파일 B 를 고치고 **커밋하지 않음** → 파일 C 를 **미추적**으로 생성. `gitDiffSummary` 의 `files` 가 **A 하나**이고 `totals` 가 A 의 값이라고 단언(B·C 가 **없다**를 함께 센다 — 존재만 세면 범위를 안 좁힌 구현이 통과한다). `gitDiffPatch` 의 `files` 도 **A 하나**라고 단언. 전수: `rg 'readUntracked\|untrackedPatchFiles' app/src/main` = **0건**(양성 단언과 짝 — §5 방향 규칙) | `resolveDiffRange` → `diffRevArgs(base)` = `[base.oid,'HEAD']` → `git diff <base> HEAD` → 요약/패치 |
| R-66 | AT-73 | 기준선 이후 커밋이 **0이면** 본문·트리·커밋 서브메뉴가 모두 `표시할 변경 사항이 없습니다.` 를 그린다 | main 통합: 위 fixture 에서 **커밋을 만들지 않고** B·C 만 둔 저장소의 `gitDiffSummary` 가 `files: []` · `commits: []` 라고 단언. 렌더: 그 요약과 빈 패치로 본문이 그 문구 하나를 그리고 **파일 섹션이 0개**라고 단언. 카탈로그: `diffEmpty` 의 ko 값이 정확히 `표시할 변경 사항이 없습니다.` 라고 단언하고, 이 키를 읽는 자리가 **넷**이라고 전수(`rg "diffEmpty" app/src/renderer --glob '!*resources*'` = 4건) | `gitDiffSummary` → 세션 상태 → `DiffReview` 의 `sections.length === 0` 분기 |
| R-67 | AT-74 | 컨텍스트 바 라벨이 **모드별**이다 — `all` 에서 `<기준> → <현재>`, `commit` 에서 `<sha 7자> <커밋 제목>` | 순수: 라벨 파생이 `all` 에서 `{base:'main',head:'feature'}`, 같은 요약 + `commit` 에서 `{sha:'4ea4a51', subject:'feat: …'}` 를 낸다고 단언(**두 모드가 다른 모양**이라는 것이 oracle — 한 모양으로 합친 변이가 red). 렌더: 모드를 바꾸면 출력의 문자열이 실제로 바뀐다고 단언하고, `commit` 모드 출력에 `→` 가 **없다**고 단언. 회귀: AT-65 의 `all` 모드 순서 단언(`main` → `feature`)을 그대로 재실행 | `gitSnapshot.comparison` + `summary` + `gitStatus` → 라벨 파생 → `GitContextBar` |
| R-68 | AT-75 | 헤더 좌측이 **세그먼트 토글 둘**이고, 현재 선택된 쪽만 `aria-pressed="true"` 이며 활성 채움 클래스를 갖는다. 둘은 **상호배타**다 | 렌더: 사이드바 숨김에서 첫째가 `aria-pressed="true"`·둘째가 `"false"`, 열림에서 반대라고 단언(**두 상태를 한 케이스로** — 한쪽만 보면 항상 같은 값을 내는 구현이 통과한다). 둘째를 눌러 열고 첫째를 눌러 닫는 왕복이 `sidebarVisible` 을 두 번 바꾼다고 단언. 클래스: 눌린 쪽만 `bg-fill-selected` 를 갖는다고 단언(두 버튼에 같은 클래스를 준 변이가 red). 전수: `data-diff-sidebar-toggle` 이 **2건**이고 `leadingIcon="folder"` 가 이 파일에서 **0건**이라고 단언 | `GitContextBar` → `chatActions.setDiffSidebarVisible(bool)` → `gitSnapshot.sidebarVisible` |
| R-69 | AT-76 | 참조 실측표(§8 ΔV6)의 **9행**이 화면 클래스로 성립한다 — 사이드바 폭 · 헤더 제목 서체/굵기 · 파일 밴드 배경 · 선택 채움 · 커밋 카드 무테 · hover 채움 · 메뉴 라벨 톤 · 헤더 제목 톤 · 활성 채움 | 렌더 + 클래스 단언: 각 행마다 대상 요소를 `data-*` 로 지목하고 기대 토큰 클래스를 단언한다. **부정 단언을 짝짓는다** — 사이드바에 `w-[38%]` 가 없고, 헤더 제목에 `font-serif`·`font-semibold` 가 없고, 커밋 카드에 `border-t5` 가 없다. 전수: `rg 'w-\[38%\]' app/src/renderer/src/features/chat/components/rightpanel` = **0건** · `rg 'border-t5' <ChangedNavigationSidebar>` 가 구획 구분선 **1건**만 남는다 | 각 컴포넌트의 className → SSR 출력 |

#### ΔV6 주의사항

- **AT-70 의 세 값은 분해할 수 없다.** 이 요구의 형태가 “닫힌다” 가 아니라 **“닫혔다가 다음 턴에 돌아온다”** 라서, 재렌더 후에도 닫혀 있음(둘째)과 tick 증가 후 복귀(셋째)가 각각 다른 결함을 부정한다.
- **AT-71 ②가 이 AC 의 직접 부정이다.** “tick 증가 시 1회” 만 보면 **옛 `busy` 계기를 그대로 둔 채 tick 계기를 덧붙인 구현**이 통과한다 — 사용자가 없애라고 한 자리가 그것이다. `busy` 만 움직였을 때 **0** 을 함께 센다.
- **AT-71 의 “기다리지 않는다” 는 구조가 아니라 값이다.** “콜백이 `async` 가 아니다” 같은 구조 단언은 안에서 `void gitQuery()` 를 부르는 구현을 막지 못한다 — 핸들러 안에서 어떤 IPC/그git 호출도 일어나지 않았다(스텁 증가 0)와 버퍼 길이 +1 을 함께 센다.
- **AT-72 는 부재를 센다.** “A 가 있다” 만 보면 세 파일이 다 오는 지금 구현이 통과한다 — `files.map(f=>f.path)` 를 **집합 동등**으로 단언해 B·C 가 없음을 같은 단언에 담는다.
- **AT-72 의 전수 0건은 단독으로 목적을 증명하지 않는다.** `readUntracked` 소멸은 “호출이 사라졌다” 만 말한다 — 위 집합 동등(양성)과 짝짓는다. 허용 예외 없음: 그 두 함수는 0211 이 만든 식별자이고 소비처가 `git-diff.ts` 안뿐이다(전수 2건).
- **AT-74 는 두 모양이 다르다는 것을 센다.** 한 모드만 단언하면 두 모드가 같은 문자열을 내는 구현이 통과한다 — `commit` 출력에 `→` 부재를 함께 센다.
- **AT-75 는 두 버튼의 상태가 반대라는 것을 센다.** 하나만 단언하면 두 버튼이 늘 같은 값을 내는 구현(= 세그먼트가 아니라 버튼 둘)이 통과한다.
- **AT-76 은 양성·부정을 짝짓는다.** “새 토큰이 있다” 만 보면 옛 클래스가 함께 남아 뒤에 오는 쪽이 이기는 구현이 통과한다 — 옛 클래스의 부재를 같은 단언에 담는다.
- 사람 실기 항목: **ΔV6 에서 3건**(① 러스트 활성 채움이 참조의 파랑 자리에서 “선택됨” 으로 읽히는가 ② 커밋 0 세션의 문구가 “고장” 으로 읽히지 않는가 ③ 24.8% 사이드바에서 파일명이 실사용 폭에서 읽히는가). ΔV5 의 3건 중 ①(기본 접힘 가독)은 유지, ②(`기준 → 현재` 잘림)는 모드별 라벨로 **성격이 바뀌어** ΔV6 ③ 이 대체, ③(미싱크 문구)은 ΔV6 ② 와 같은 축이라 통합 — ΔV4 의 3건 + ΔV5 ① + ΔV6 3건 = 총 **7건**.
...
### ΔV4 — Delta V (기준 `V1 + ΔV1 + ΔV2 + ΔV3` @ `46047ac`)

- Delta V 판정 근거: 기준이 같은 handoff 의 연속 revision 이고 공유 브랜치 커밋 하나로 고정된다(`46047ac` = ΔV3 구현 병합 `#420` 이후의 `main`). ΔV4 는 그 위에서 **무엇을 어떤 화면 구조로 보여주는가**를 바꾸고, ΔV3 의 백엔드 축(D-062~D-064)은 그대로 상속한다.
- 변경이 시작되는 수준: **R** — 사용자가 관측하는 화면 구조·비교 기준 표기·조회 횟수가 바뀐다. 아래로 `SD`·`AR`·`MD` 가 따라온다.
- 영향 없는 pair 는 복사하지 않는다. 아래 표에 없는 노드·pair 는 그대로 유효하다.
...
| Node | 레벨 | 계약 / 본문 절 | provenance | 기준선 출처 / 대체 node |
|---|---|---|---|---|
| R-43 · R-44 | R | §7 ΔV4 비교 기준 라벨 · 기준선 ref 기록 | NEW | — |
| R-45 | R | §7 ΔV4 한 화면 연속 review | NEW | — |
| R-46 · R-47 | R | §7 ΔV4 패치 조회 1회 · 패치 계약과 상한 | NEW | — |
| R-48 | R | §7 ΔV4 양방향 문맥 확장 | NEW | — |
| R-49 · R-50 | R | §7 ΔV4 비교 모드 · 탐색 사이드바 | NEW | — |
| R-51 · R-52 | R | §7 ΔV4 `⋮` 8항목 · `↗` 열 폭 | NEW | — |
| R-53 | R | §7 ΔV4 제거 전수 | NEW | — |
| R-54 | R | §7 ΔV4 요구사항 회귀 | CHANGED | R-34·R-35(ΔV2) — 붙는 자리와 재anchor 시점만 바뀐다 |
| R-28 | R | §7 요약 여섯 값 | SUPERSEDED | → R-53 (사용자가 “전부 제거”를 골랐다) |
| R-30 | R | §7 확장이 조회를 부르지 않는다 | SUPERSEDED | → R-48 (재조회 0 은 승계, 방향 축이 붙는다) |
| R-31 | R | §7 Prev/Next 범위 | SUPERSEDED | → R-45 (peek 화면이 사라져 주어가 없다) |
| R-26 | R | §7 커밋 노드의 `+N개 더` 인라인 확장 | SUPERSEDED | → R-49 (커밋 카드에 파일 목록이 없다 — 고르면 본문 목록이 그 커밋의 파일을 전부 그린다) |
| R-38 | R | §7 본문 LRU 캐시 | SUPERSEDED | → R-46 (파일별 조회가 사라져 캐시 대상이 없다) |
| R-40 | R | §7 depth 전환 | SUPERSEDED | → R-50 (연출은 승계, 주어가 사이드바로 바뀐다) |
| R-18 · R-22 · R-23 · R-24 · R-25 · R-29 · R-33 · R-39 · R-41 | R | §7 합계·기준선·미추적·커밋 노드·범위 단일화·상한·조회 감축·primitive | INHERITED | ΔV1~ΔV3 @ `46047ac` |
| SD-10 | SD | §5 ΔV4 전이표 — 패치 수명(세대·타일 열림·폐기) | NEW | — |
| SD-11 | SD | §5 ΔV4 파생 UX — 비교 모드·접힌 집합의 세대 넘김 | NEW | — |
| SD-07 · SD-09 | SD | ΔV2 전이표 · peek 본문 수명 | SUPERSEDED | → SD-10 (두 화면과 peek 본문이 사라진다) |
| SD-03 | SD | §5 요약 조회 계기 셋 | INHERITED | ΔV1 @ `46047ac` — 패치는 별도 축이고 요약 계기를 늘리지 않는다 |
| AR-13 | AR | §10 EP-29·EP-30 `orca:git:diffPatch` 신설 + `orca:git:diffFile` 제거 | NEW | — |
| AR-14 | AR | §10 EP-28 `sessions.baseline_ref` + `managed.baseRef` 배선 | NEW | — |
| AR-07 | AR | §10 EP-14 세션 기준선 | CHANGED | ΔV2 — 한 컬럼에서 **두 컬럼**(oid + ref)이 같은 insert 로 |
| AR-12 | AR | §10 EP-25 조회 형태 | INHERITED | ΔV3 @ `46047ac` — 패치도 같은 `run()` 관문을 지난다 |
| AR-10 | AR | §10 EP-21 요구사항 전송 홉 | INHERITED | ΔV2 @ `46047ac` |
| MD-18 | MD | §11 ΔV4 통합 패치 파서(순수) | NEW | — |
| MD-19 | MD | §11 ΔV4 양방향 gap 확장(순수) | CHANGED | MD-09·MD-14(ΔV2) — 키·순서 보존은 승계, 방향 인자가 붙는다 |
| MD-20 | MD | §11 ΔV4 표시 옵션 넷의 순수 파생 | NEW | — |
| MD-21 · MD-22 | MD | §11 ΔV4 변경 파일 트리 파생 · 비교 모드 필터(순수) | NEW | — |
| MD-23 | MD | §11 ΔV4 비교 기준 라벨 4상태(순수) | CHANGED | `summaryBaseLabel`(ΔV2) — 3상태에서 4상태로 |
| MD-16 | MD | §11 본문 LRU 캐시 | SUPERSEDED | → 없음 (D-074 가 조회 대상을 없앤다) |
| MD-11 | MD | §11 anchor 10필드 · 재anchor | CHANGED | ΔV2 — 필드는 불변, 실행 시점이 peek → 패치 도착 |

#### ΔV4 Pair registry

| Pair | left ↔ right | requiredness | production path `start → edges → end` | 직접 evidence oracle | 선택적 적대 증거 | §10 강제 지점 전수 |
|---|---|---|---|---|---|---|
| VP-51 | R-43 · MD-23 ↔ AT-43 | REQUIRED | `base.ref` → 라벨 파생 → `GitContextBar` | 4상태 각각의 산출; 렌더에서 `main` 출력 + `→`·현재 브랜치 문자열 **부재** | required — “브랜치명이 나온다”는 **부재를 말하지 않는다**. 현재 브랜치를 다른 값으로 준 뒤 화살표를 되살리는 변이가 red 인지 확인한다 | EP-28 (3) |
| VP-52 | R-44 · AR-14 ↔ AT-44 | REQUIRED | `service.prepare` → `managed.baseRef` → `buildTurn` → `insertSession` → `sessions.baseline_ref` | 격리·비격리 각각 기록값 일치; 2턴 뒤 **불변**; detached 는 `null`; append-only 게이트 통과 | required — insert 값만 보면 **덮어쓰는 구현**이 통과한다. 이후 턴에서 `UPDATE` 를 넣는 변이가 “불변” 단언에서 red 인지 확인한다 | EP-28 (3) |
| VP-53 | R-45 ↔ AT-45 | REQUIRED | 세션 상태 patch → `DiffReview` → `FileDiffSection` | 두 섹션이 한 컨테이너에 순서대로; 개별 접기가 그 파일만 접음; peek·뒤로·이전/다음 전수 0건 | required — 0건 스윕은 **대체가 붙었는지** 말하지 않는다. 한 파일 접기를 전체 접기로 바꾼 변이가 “다른 파일의 줄은 남는다”에서 red 인지 확인한다 | EP-30 (4) |
| VP-54 | R-46 · SD-10 ↔ AT-46 | REQUIRED | `DiffTileContent` 마운트 ∧ patch 부재 → `gitApi.diffPatch` → 세션 상태 | 닫힘 0 · 첫 열기 1 · 왕복 증가 0 · 턴 종료 1 · 새로고침 1; 40파일에서도 1 | required — “증가 0”은 **소거에도 참**이다(AT-20·AT-32·AT-38 과 같은 축). ① 항상 재조회하는 변이(왕복 증가가 0이 아니게 되어 red) ② 조회를 통째로 지우는 변이(첫 열기 1 이 깨져 red) 둘로 양방향을 잠근다 | EP-34 (3) |
| VP-55 | R-47 · AR-13 · MD-18 ↔ AT-47 | REQUIRED | `gitDiffPatch()` → `run()` → `runGit` → 파서 | 다섯 status 산출; 상한 셋 각각; 폴백 인자 차집합(`--unified=1000000` 부재 + `--unified=3` 존재) | required — 상한 셋이 **서로를 대신하지 못한다**. 줄 상한을 지운 변이가 파일 수 단언으로 통과하는지, 파일 상한을 지운 변이가 줄 단언으로 통과하는지 각각 확인한다(둘 다 red 라야 셋이 독립이다) | EP-29 (3) · EP-31 (3) |
| VP-56 | R-48 · MD-19 ↔ AT-48 | REQUIRED | patch 줄 → `buildDiffHunks` → `expandGap(dir)` → `FileDiffSection` | `up` 은 끝쪽 + `insertedAbove===n`, `down` 은 앞쪽 + `0`; 선두·말미 gap 의 후보 방향; 키 부분수열 보존; 확장 중 조회 0 | required — 방향은 **줄 수 증가로 관측되지 않는다**. `up`·`down` 구현을 맞바꾸는 변이가 `insertedAbove` 두 값의 차이에서 red 인지 확인한다 | EP-32 (2) |
| VP-57 | R-49 · MD-22 ↔ AT-49 | REQUIRED | 세션 상태 comparison → 순수 필터 → `DiffReview` | 목록만 좁혀짐; **같은 파일 줄 배열이 두 모드에서 깊은 동등**; 모드 전환 조회 0; `commit` 멤버 전수 0건 | required — 목록 축만 보면 **diff 까지 커밋 단위로 바꾼 구현**이 통과한다. 파일 줄을 커밋 범위로 다시 계산하는 변이가 깊은 동등에서 red 인지 확인한다 | EP-35 (2) |
| VP-58 | R-50 · MD-21 · SD-11 ↔ AT-50 | REQUIRED | 세션 상태 sidebarVisible → `ChangedNavigationSidebar` → `scrollIntoView` | 기본 부재 → 토글 후 존재; 트리 3단 접기; 커밋 카드 4값; 파일 클릭이 스크롤; 연출 180ms + `motion-reduce` | required — “사이드바가 있다”는 **두 구획을 말하지 않는다**. 커밋 목록을 지운 변이가 카드 4값 단언에서 red 인지, 연출 utility 를 한쪽에만 붙인 변이가 CSS 파싱에서 red 인지 확인한다 | EP-36 (2) |
| VP-59 | R-51 · MD-20 ↔ AT-51 | REQUIRED | `DiffViewMenu` → `chatActions` → 세션 상태 → 순수 파생 | 8항목 **집합 동등**; 넷 각각의 표시 변화; 넷 각각에서 조회 0; 새로고침 요약·패치 각 1 | required — 항목을 하나씩 세면 **한 항목 삭제 변이**가 나머지로 통과한다(AT-35 와 같은 축). 항목 하나를 지운 변이와 순서를 바꾼 변이가 배열 비교에서 red 인지 확인한다 | EP-33 (4) |
| VP-60 | R-52 ↔ AT-52 | REQUIRED | `GitContextBar` → `setRightPanelColWidth` → `rightPanelColWidths` | 첫 클릭 `(col, MAX)` · 두 번째 `(col, DEFAULT)`; `aria-label` 카탈로그 해석 | not selected — 직접 행동 결과를 인자로 관측한다 | EP-36 (2) |
| VP-61 | R-53 ↔ AT-53 | REQUIRED | — (제거) | 세 스윕 0건 + 네 파일 부재 + IPC 문서 행 교체 + 인벤토리 차이 0 | required — 0건 스윕은 **방향이 반대**다(§5). AT-45·AT-46 양성 단언을 같은 스위트에서 짝짓고, 대체 없이 지우기만 한 변이가 그 둘에서 red 인지 확인한다 | EP-30 (4) |
| VP-62 | R-54 · MD-11 ↔ AT-54 | REQUIRED | `FileDiffSection` `+` → `addDiffRequirement` → `RequirementTray` → `chatApi.send` | 칩 1 + 줄 아래 본문 1; 재anchor 가 파일별로 자기 줄만; 미발견은 `located:false` 로 잔존 | required — 재anchor 를 **파일 경계 없이** 도는 구현이 한 파일 픽스처에서 통과한다. 파일 A 의 anchor 를 파일 B 본문에 붙이는 변이가 red 인지 확인한다 | EP-23 (3) |
| VP-29 · VP-30 | R-22 · R-23 · R-24 ↔ AT-22 · AT-23 · AT-24 | REGRESSION | ΔV2 경로 | `baseline_ref` 컬럼 추가 뒤에도 `baseline_oid` 기록·NULL 폴백·미추적 제외가 같은 값 | not selected — 기존 oracle 재실행 | EP-14 (5) · EP-15 (3) |
| VP-31 · VP-39 | R-25 · R-33 ↔ AT-25 · AT-33 | REGRESSION | ΔV2 경로 | 사이드바 커밋 목록이 같은 `commits[]` 를 읽고 절단·폴백 값이 그대로 | not selected — 기존 oracle 재실행 | EP-17 (6) |
| VP-35 | R-29 ↔ AT-29 | REGRESSION | ΔV2 경로 | 비교 모드가 붙은 뒤에도 파일 diff 요청에 범위 인자가 **없다** | not selected — 기존 oracle 재실행(**D-036 이 ACTIVE 임을 재확인한다**) | EP-16 (2) |
| VP-22 · VP-48 | R-18 · R-39 ↔ AT-18 · AT-39 | REGRESSION | ΔV1·ΔV3 경로 | 컴포저 합계가 그대로; **패치 호출을 포함해** 모든 읽기 조회의 인자 0번이 `--no-optional-locks`(차집합 0) | not selected — 기존 oracle 재실행 + 분모에 신규 호출 1건 추가 | EP-11 (2) · EP-25 (3) |
| VP-50 | R-41 ↔ AT-41 | REGRESSION | ΔV3 경로 | 새 컴포넌트에서도 `Icon` 사용 ≥1 · hover 행 수 == `group/` 수 == `transition-colors` 수 · 접기 컨트롤 `<button aria-expanded>` | not selected — 기존 oracle 을 **새 컴포넌트 집합**에 재실행 | EP-27 (3) |
| VP-40 · VP-41 · VP-42 · VP-45 | R-34 · R-35 · R-36 · R-37 ↔ AT-34~AT-37 | REGRESSION | ΔV2 경로 | anchor 10키 · 소비 신호 · 홉 전수 · 중화가 자리 이동 뒤에도 같다 | not selected — 기존 oracle 재실행 | EP-21 (3) · EP-22 (3) |
| VP-01~VP-08 · VP-18~VP-21 · VP-24~VP-27 | V1·ΔV1 준비 단계 · 표시 이름 · 조회 소유자 | NOT_REQUIRED | — | ΔV1 @ `46047ac` 의 기존 증거 그대로 | — | 비영향 근거: ΔV4 가 `prepare` 진행 이벤트·`CwdButton`·`gitRowView`·`useGitSnapshot` 의 **요약 축**을 한 줄도 건드리지 않는다(§18 ΔV4 목록에 없다). `useGitSnapshot` 파일은 바뀌지 않고 패치 소유자는 별도 훅이다 |
| VP-46 · VP-47 · VP-49 | R-38 · R-42 · R-40 ↔ AT-38 · AT-42 · AT-40 | NOT_REQUIRED | — | ΔV3 @ `46047ac` 의 기존 증거 그대로 | — | **왼쪽 node 가 `SUPERSEDED` 라 닫을 계약이 없다** — 본문 LRU·다섯 축 키·두 화면 연출의 주어(파일별 조회·peek 화면)가 §18 ΔV4 삭제 목록으로 사라진다. 그 pair 들이 잠그던 축은 VP-54(조회 1회, 양방향)·VP-58(사이드바 연출, 같은 180ms·`motion-reduce`)가 **다시 유도해** 받는다 |

#### ΔV4 gate 차분

| Gate | ΔV4 에서 달라지는 점 | 증거 / 명령 |
|---|---|---|
| `app` 정적 게이트 | 동일 — `app/src/**` 를 바꾼다 | `npm run lint` · `npm run typecheck` |
| `app` 테스트 | **DB 스위트가 는다** — 마이그레이션 `0020` 과 `baseline_ref` 기록이 임시 DB 통합이다 | `npm test`(DB 포함) · 순수·SSR 은 `./node_modules/.bin/vitest run <suite>` |
| 문서 인벤토리 | **바뀐다** — 채널 **이름** `orca:git:diffFile` → `orca:git:diffPatch`(총계 **82 불변** · 도메인 `git 5` 불변, 목록 문자열만 바뀐다), DB 마이그레이션 **19 → 20** | `node scripts/check-doc-inventory.mjs` 재생성 후 `--check` 가 **차이 0** |
| IPC 계약 문서 | **바뀐다** — `docs/IPC_CONTRACT.md` 의 `orca:git:diffFile` 행을 `orca:git:diffPatch` 행으로 교체하고 `orca:git:diffSummary` 의 `base.ref` 를 적는다 | 해당 문서 diff |
| i18n 카탈로그 | 컨텍스트 바·메뉴 8항목·사이드바·문맥 확장 라벨이 늘고, 요약 6값 키 5종이 **빠진다** | `resources.test.ts` |
| 마이그레이션 append-only | **신규 `0020_session_baseline_ref.sql`** — 기존 파일을 고치지 않는다 | `check-migrations-appendonly.mjs` exit 0 |
| **사람 실기** | **3 → 4** (D-060 scroll 보정 유지 + 파일 경계 가독성 · 사이드바 연출 · 형제 대비 이질감) | §19 |

---

### ΔV5 — Delta V (기준 `V1 + ΔV1 + ΔV2 + ΔV3 + ΔV4` @ `177def67`)

- Delta V 판정 근거: 기준이 같은 handoff 의 연속 revision 이고 공유 브랜치 커밋 하나로 고정된다(`177def67` = ΔV4 r3 이후 `claude/git-uphuk-panel-redesign-xybgmd` 의 tip). ΔV5 는 그 위에서 **언제 싱크하는가 · 무엇이 접혀 있는가 · 어느 문구가 나오는가**를 바꾸고, ΔV4 의 조회 1회 축(D-074 전반부·D-075~D-078)은 그대로 상속한다.
- 변경이 시작되는 수준: **R** — 사용자가 관측하는 갱신 시점·기본 상태·문구·메뉴 배치가 바뀐다. 아래로 `SD`·`AR`·`MD` 가 따라온다.
- 영향 없는 pair 는 복사하지 않는다. 아래 표에 없는 노드·pair 는 그대로 유효하다.
...
| Node | 레벨 | 계약 / 본문 절 | provenance | 기준선 출처 / 대체 node |
|---|---|---|---|---|
| R-55 | R | §7 ΔV5 싱크 계기 하나 + 이름 축 분리 | NEW | — |
| R-56 | R | §7 ΔV5 미싱크 문구 | NEW | — |
| R-57 | R | §7 ΔV5 패치 교착 부정 | NEW | — |
| R-58 | R | §7 ΔV5 `기준 → 현재` | NEW | — |
| R-59 | R | §7 ΔV5 기본 접힘 + 안내 | NEW | — |
| R-60 | R | §7 ΔV5 메뉴 2행 · 커밋 서브메뉴 · `⋮` 7항목 | NEW | — |
| R-61 | R | §7 ΔV5 파일 헤더 1줄 + `↗` | NEW | — |
| R-62 | R | §7 ΔV5 이동의 소유자 | CHANGED | R-50(ΔV4) — 두 구획·연출은 승계, **소유자 인자**가 붙는다 |
| R-43 | R | §7 ΔV4 비교 기준 라벨 하나 | SUPERSEDED | → R-58 (사용자가 화살표를 되살렸다) |
| R-46 | R | §7 ΔV4 패치 조회 1회 | CHANGED | 계기 목록에서 `새로고침` 이 빠지고 **교착 부정**이 붙는다 → R-55·R-57 |
| R-49 · R-51 | R | §7 ΔV4 비교 모드 · `⋮` 8항목 | SUPERSEDED | → R-60 (2행 + 서브메뉴 · 7항목 · `미커밋` 진입점 제거) |
| R-45 · R-47 · R-48 · R-53 · R-54 | R | §7 ΔV4 한 화면 · 패치 계약 · 문맥 확장 · 제거 전수 · 요구사항 | INHERITED | ΔV4 @ `177def67` |
| R-44 · R-18 · R-22~R-25 · R-29 · R-33 · R-39 · R-41 | R | 기준선 기록 · 합계 · 미추적 · 범위 단일화 · 상한 · 조회 형태 · primitive | INHERITED | ΔV1~ΔV4 @ `177def67` |
| SD-12 | SD | §5 ΔV5 전이표 — 미싱크 → 요약만 → 준비됨 | NEW | — |
| SD-03 | SD | §5 요약 조회 계기 셋 | SUPERSEDED | → SD-12 (계기가 하나로 줄고 이름 축이 갈린다) |
| SD-10 | SD | §5 ΔV5 패치 수명 — 무효화 주체 하나 + 가드 해제 | CHANGED | ΔV4 SD-10 — 세대당 1회는 승계, **해제 규칙**이 붙는다 |
| SD-11 | SD | §5 비교 모드·펼친 집합의 세대 넘김 | CHANGED | ΔV4 SD-11 — 집합의 **방향**이 반대가 된다 |
| AR-15 | AR | §10 EP-44 `orca:files:openPath` 의 파일 reveal 모드 | NEW | — |
| AR-13 · AR-14 · AR-12 · AR-10 · AR-07 | AR | 패치 채널 · 기준선 배선 · 조회 형태 · 요구사항 홉 · 세션 기준선 | INHERITED | ΔV2~ΔV4 @ `177def67` |
| MD-24 | MD | §11 ΔV5 계기 판정 **두 순수 함수** | CHANGED | `gitSnapshotQueryReason`(ΔV1) — 하나에서 둘로 갈리고 요약 쪽 값 집합이 준다 |
| MD-25 | MD | §11 ΔV5 패치 조회 가드 상태 기계(순수) | NEW | — |
| MD-26 | MD | §11 ΔV5 펼친 집합 파생(순수) | CHANGED | ΔV4 접힌 집합 파생 — 집합의 의미가 반전된다 |
| MD-27 | MD | §11 ΔV5 스크롤 소유자 주입 seam | NEW | — |
| MD-23 | MD | §11 비교 기준 라벨 | CHANGED | ΔV4 4상태 — **현재 브랜치 축**이 붙어 `(기준, 현재)` 쌍이 된다 |
| MD-22 | MD | §11 비교 모드 필터 | CHANGED | ΔV4 — `uncommitted` 분기가 빠져 두 갈래가 된다 |
| MD-18 · MD-19 · MD-20 · MD-21 · MD-11 | MD | 패치 파서 · gap 확장 · 표시 옵션 · 트리 파생 · anchor | INHERITED | ΔV4 @ `177def67` |

#### ΔV5 Pair registry

| Pair | left ↔ right | requiredness | production path `start → edges → end` | 직접 evidence oracle | 선택적 적대 증거 | §10 강제 지점 전수 |
|---|---|---|---|---|---|---|
| VP-63 | R-55 · SD-12 · MD-24 ↔ AT-62 | REQUIRED | `busy` 전이 → `useGitSnapshot` → `gitApi.diffSummary` → 세션 상태 | 두 순수 함수의 값 대조; 마운트 0 · 세션 전환 0 · 턴 종료 1; 네 식별자 전수 0건 | required — “증가 0”은 **소거에도 참**이다. ① 조회를 통째로 지운 변이(턴 종료 1 이 깨져 red) ② 두 함수를 하나로 합친 변이(이름 축이 턴 종료로만 돌아 `'initial'` 단언에서 red) 둘로 양방향을 잠근다 | EP-42 (3) |
| VP-64 | R-56 ↔ AT-63 | REQUIRED | 세션 상태 `gitSnapshotRequest` → `DiffTileContent` → `DiffReview` | 각 상태에서 자기 문구 존재 + **다른 문구 부재**; 두 카탈로그 키가 다름 | required — 한 문구만 세면 **두 문구를 같은 자리에 함께 그린 구현**이 통과한다. 두 키를 같은 값으로 만든 변이가 red 인지 확인한다 | EP-43 (2) |
| VP-65 | R-57 · SD-10 · MD-25 ↔ AT-64 | REQUIRED | `diffPatch` 응답 → `receiveGitPatch` → `RECEIVE_SUMMARY` → 재조회 | `fetch:true → false → (해제) → true` 세 값; `BEGIN` 의 키별 보존/소거; `patch: null` 지점 전수 **4**(5 − REFRESH) | required — 이 회귀의 형태가 **“해제되지 않는다”** 라 존재/부재 축으로는 관측되지 않는다. 해제를 지운 변이가 세 번째 값에서 red 이고, 가드를 통째로 지운 변이가 두 번째 값에서 red 인지 **둘 다** 확인한다 | EP-34 (3) · EP-43 (2) |
| VP-66 | R-58 · MD-23 ↔ AT-65 | REQUIRED | `base.ref` + `gitStatus.branch` → 라벨 파생 → `GitContextBar` | 두 값 + 구분자; `head:null` 에서 화살표 부재; 출력 **순서**가 기준 → 현재 | required — 존재만 세면 **자리를 맞바꾼 구현**이 통과한다(0204 r4 와 같은 축). 두 값을 뒤집은 변이가 인덱스 비교에서 red 인지 확인한다 | EP-28 (3) |
| VP-67 | R-59 · SD-11 · MD-26 ↔ AT-66 | REQUIRED | 세션 상태 `expandedFiles` → `DiffReview` → `FileDiffSection` | 첫 출력에 diff 줄 **0**; 한 파일만 펼침; 전체 펼침/접기의 두 방향; `collapsedFiles` 전수 0건 | required — 0건 스윕은 **방향이 반대**다(§5). 반전을 절반만 한 변이(집합 이름은 바뀌고 기본이 여전히 펼침)가 “첫 출력에 diff 줄 0” 에서 red 인지 확인한다 | EP-45 (2) |
| VP-68 | R-60 · MD-22 ↔ AT-67 | REQUIRED | `GitContextBar` → `ComparisonMenu` → `setDiffComparison` | 첫 화면 2행 + 커밋 subject 부재 → 서브메뉴 후 존재; `⋮` 7항목 집합 동등; `'uncommitted'` 전수 0건 | required — 항목을 하나씩 세면 **한 항목 삭제·순서 변경 변이**가 통과한다(AT-35·AT-51 과 같은 축). 서브메뉴를 평면으로 되돌린 변이가 “첫 화면에 subject 부재” 에서 red 인지 확인한다 | EP-33 (4) · EP-35 (2) |
| VP-69 | R-61 · AR-15 ↔ AT-68 | REQUIRED | `FileDiffSection` `↗` → `orca:files:openPath`(reveal) → `shell.showItemInFolder` | 한 행 구조; 절대 경로 1회 호출; **거부 3케이스**(`..` 이탈 · 무관 경로 · 디렉토리 회귀) | required — “파일을 연다”는 **거부를 말하지 않는다**. 화이트리스트를 지운 변이가 두 거부 케이스에서 red 인지 확인한다(보안 축이라 fail-open 을 직접 부정한다) | EP-44 (3) |
| VP-70 | R-62 · MD-27 ↔ AT-69 | REQUIRED | `onPickFile` → `DiffReview.pickFile` → `revealFileSection(owner, path)` | **첫 인자가 주입한 소유자와 같음**; 경로 인자; `onExpandFile` 선행; `owner:null` → `false` | required — **D22 가 이 축에서 열려 있다**. `revealFileSection(null, path)` 로 만든 변이가 red 인지 확인한다(r3 에서는 3,080 케이스가 전건 green 이었다) | EP-36 (2) |
| VP-53 · VP-55 · VP-56 · VP-61 · VP-62 | R-45 · R-47 · R-48 · R-53 · R-54 ↔ AT-45 · AT-47 · AT-48 · AT-53 · AT-54 | REGRESSION | ΔV4 경로 | 기본 접힘·헤더 재배치 뒤에도 한 화면 연속·패치 계약·문맥 확장·제거 전수·요구사항이 같은 값 | not selected — 기존 oracle 재실행 | EP-29 (3) · EP-30 (4) · EP-31 (3) · EP-32 (2) · EP-23 (3) |
| VP-54 | R-46 · SD-10 ↔ AT-46 | REGRESSION | ΔV4 경로 | 계기 목록에서 `새로고침` 을 뺀 뒤에도 **닫힘 0 · 첫 열기 1 · 왕복 0 · 턴 종료 1** 이 그대로 | not selected — 기존 oracle 을 **줄어든 계기 집합**으로 재실행 | EP-34 (3) |
| VP-51 · VP-52 | R-43(→R-58) · R-44 ↔ AT-43 · AT-44 | REGRESSION | ΔV4 경로 | `baseline_ref` 기록·불변은 그대로. **AT-43 의 “화살표 부재” 단언만 AT-65 가 대체**한다 | not selected — 기록 축은 기존 oracle 재실행 | EP-28 (3) |
| VP-22 · VP-48 · VP-50 | R-18 · R-39 · R-41 ↔ AT-18 · AT-39 · AT-41 | REGRESSION | ΔV1·ΔV3 경로 | 컴포저 합계; 읽기 조회 전건 `--no-optional-locks`; 새 헤더·메뉴에서도 `Icon`·`group/`·`transition-colors` 짝 | not selected — 기존 oracle 을 **새 컴포넌트 집합**에 재실행 | EP-11 (2) · EP-25 (3) · EP-27 (3) |
| VP-29~VP-31 · VP-35 · VP-39~VP-42 · VP-45 · VP-57~VP-59 | ΔV2~ΔV4 의 기준선·미추적·범위·anchor·비교 필터·메뉴 | NOT_REQUIRED | — | ΔV4 @ `177def67` 의 기존 증거 그대로 | — | 비영향 근거: ΔV5 가 `git-diff.ts`·`git-diff-parse.ts`·`handlers/git.ts`·요구사항 슬라이스를 **한 줄도 바꾸지 않는다**(§18 ΔV5 목록에 없다). VP-57·VP-59 는 왼쪽 node 가 `SUPERSEDED` 라 VP-68 이 그 축을 다시 유도해 받는다 |
| VP-01~VP-08 · VP-18~VP-21 · VP-24~VP-27 · VP-46 · VP-47 · VP-49 | 준비 단계 · 표시 이름 · ΔV3 캐시·연출 | NOT_REQUIRED | — | ΔV1~ΔV3 @ `177def67` 의 기존 증거 그대로 | — | 비영향 근거: `prepare-worktree.ts`·`CwdButton`·`activityLabel` 을 건드리지 않는다. `gitRowView` 는 **호출 인자만** 같고 함수 본문이 불변이다(D-101 이 이름 축을 그대로 두는 이유가 이 pair 들의 유지다) |

#### ΔV5 gate 차분

| Gate | ΔV5 에서 달라지는 점 | 증거 / 명령 |
|---|---|---|
| `app` 정적 게이트 | 동일 — `app/src/**` 를 바꾼다 | `npm run lint` · `npm run typecheck` |
| `app` 테스트 | **DB 스위트가 늘지 않는다** — 마이그레이션도 스키마 변경도 없다. 신규는 순수·SSR 렌더·main 통합(임시 디렉토리) | `./node_modules/.bin/vitest run <suite>`(ABI 중립) |
| 문서 인벤토리 | **채널 수 불변**(82) — `orca:files:openPath` 는 기존 채널이고 요청 스키마에 모드가 붙는다. 마이그레이션 수 불변(20) | `node scripts/check-doc-inventory.mjs` 재생성 후 `--check` 차이 **0** |
| IPC 계약 문서 | **바뀐다** — `docs/IPC_CONTRACT.md` 의 `orca:files:openPath` 행에 파일 reveal 모드와 **허용 경로 규칙**을 적는다 | 해당 문서 diff |
| i18n 카탈로그 | 신규 **3**(미싱크 문구 · 대량 diff 안내 · `↗` 접근성 이름) · 제거 **1**(`diffRefresh`) · 값 변경 **1**(`diffAllChanges` → `모든 변경사항`/`All changes`) | `resources.test.ts` |
| 마이그레이션 append-only | **신규 없음** | `check-migrations-appendonly.mjs` exit 0 |
| **사람 실기** | **4 → 6** (ΔV4 의 ①을 ΔV5 ①이 대체하고 ②③ 신설) | §19 |

---

### ΔV6 — Delta V (기준 `V1 + ΔV1 + ΔV2 + ΔV3 + ΔV4 + ΔV5` @ `628123bc`)

- Delta V 판정 근거: 기준이 같은 handoff 의 연속 revision 이고 공유 브랜치 커밋 하나로 고정된다(`628123bc` = ΔV5 구현이 squash 로 들어온 뒤의 `main` tip — `git show 628123bc:app/src/renderer/src/features/chat/components/composer/useGitSnapshot.ts | rg -c 'D-099'` = **2**). ΔV6 은 그 위에서 **무엇을 보여주는가 · 언제 싱크하는가 · 어떤 픽셀로 그리는가**를 바꾸고, ΔV5 의 상태 여섯·패치 가드·파일 헤더 축은 그대로 상속한다.
- 변경이 시작되는 수준: **R** — 사용자가 관측하는 목록의 내용·갱신 계기·컴포저 행의 수명·라벨·픽셀이 바뀐다. 아래로 `SD`·`AR`·`MD` 가 따라온다.
- 영향 없는 pair 는 복사하지 않는다. 아래 표에 없는 노드·pair 는 그대로 유효하다.
...
| Node | 레벨 | 계약 / 본문 절 | provenance | 기준선 출처 / 대체 node |
|---|---|---|---|---|
| R-63 | R | §7 ΔV6 컴포저 `×` 와 복귀 | NEW | — |
| R-64 | R | §7 ΔV6 Stop hook 싱크 | CHANGED | R-55(ΔV5) — 계기 **하나**는 승계, 그 하나의 **출처**가 `busy` 에서 hook 으로 바뀐다 |
| R-65 | R | §7 ΔV6 커밋 전용 범위 | NEW | — |
| R-66 | R | §7 ΔV6 빈 커밋 문구 | NEW | — |
| R-67 | R | §7 ΔV6 모드별 라벨 | CHANGED | R-58(ΔV5) — `all` 모양은 승계, `commit` 모양이 붙는다 |
| R-68 | R | §7 ΔV6 헤더 세그먼트 둘 | CHANGED | R-50(ΔV4 사이드바) — 두 구획은 승계, **진입 컨트롤**이 하나에서 둘로 갈린다 |
| R-69 | R | §7 ΔV6 실측표 9행 | CHANGED | R-41(ΔV3 primitive) — “house primitive 를 쓴다” 는 승계, **실측 대조**가 붙는다 |
| R-24 | R | §7 ΔV2 미추적 완전 제외 | SUPERSEDED | → R-65 (범위가 미추적에서 **미커밋 전체**로 넓어져 R-24 를 포함한다). ΔV4 D-095 가 R 노드 없이 이 축을 되돌려 V 행이 코드와 갈라져 있었다 — R-65 가 둘을 다시 맞춘다 |
| R-55 | R | §7 ΔV5 싱크 계기 하나 | CHANGED | → R-64 (이름 축 분리는 그대로, 요약 축의 계기 출처가 바뀐다) |
| R-56 · R-57 · R-59 · R-60 · R-61 · R-62 | R | ΔV5 미싱크 문구 · 패치 교착 · 기본 접힘 · 메뉴 2행 · 파일 헤더 · 이동 소유자 | INHERITED | ΔV5 @ `628123bc` |
| R-45 · R-46 · R-47 · R-48 · R-53 · R-54 | R | ΔV4 한 화면 · 패치 1회 · 패치 계약 · 문맥 확장 · 제거 전수 · 요구사항 | INHERITED | ΔV4 @ `628123bc` |
| SD-13 | SD | §5 ΔV6 싱크 계기의 전이 — hook → 이벤트 → tick → 조회 | NEW | — |
| SD-14 | SD | §5 ΔV6 컴포저 행의 수명 — 닫힘은 tick 하나로 풀린다 | NEW | — |
| SD-12 | SD | §5 ΔV5 미싱크 → 요약만 → 준비됨 | CHANGED | 상태 여섯은 그대로, **‘비어 있음’ 이 정상 상태**가 된다(커밋 0 세션) |
| AR-16 | AR | §10 EP-46 `on-turn-end` hook 등록 + `turn.ended` 이벤트 계약 | NEW | — |
| AR-17 | AR | §10 EP-47 `diffRevArgs` 가 요약·패치 **두 소비자**의 SSOT | CHANGED | AR-12(ΔV2 조회 형태) — 같은 함수의 산출 형태가 1항에서 2항이 된다 |
| AR-15 · AR-13 · AR-14 · AR-10 · AR-07 | AR | reveal 모드 · 패치 채널 · 기준선 배선 · 요구사항 홉 · 세션 기준선 | INHERITED | ΔV2~ΔV5 @ `628123bc` |
| MD-28 | MD | §11 ΔV6 계기 판정 순수 함수의 입력이 `tick` | CHANGED | MD-24(ΔV5) — 두 함수 구조는 승계, 요약 쪽 입력 타입이 바뀐다 |
| MD-29 | MD | §11 ΔV6 컴포저 행 노출 판정(순수) | CHANGED | `gitRowView`(V1) — 인자에 `닫힘 tick` 이 붙어 판정식이 늘어난다 |
| MD-30 | MD | §11 ΔV6 모드별 라벨 파생(순수) | CHANGED | MD-23(ΔV5) — 산출이 `{base,head}` 에서 판별 유니온이 된다 |
| MD-31 | MD | §11 ΔV6 실측 토큰 표(SSOT 상수) | NEW | — |
| MD-25 · MD-26 · MD-27 · MD-22 | MD | 패치 가드 · 펼침 집합 · 스크롤 소유자 · 비교 필터 | INHERITED | ΔV5 @ `628123bc` |

#### ΔV6 Pair registry

| Pair | left ↔ right | requiredness | production path `start → edges → end` | 직접 evidence oracle | 선택적 적대 증거 | §10 강제 지점 전수 |
|---|---|---|---|---|---|---|
| VP-71 | R-63 · SD-14 · MD-29 ↔ AT-70 | REQUIRED | `×` → `closeGitRow` → 세션 상태 → 다음 tick 이 해제 | 노출 판정 세 값(닫힘 → 재렌더 → tick 증가); 클릭 후 행 부재; `×` 가 변경량 버튼 뒤 | required — “닫힌다” 만 보면 **영영 안 돌아오는 구현**이 통과한다. 복귀 해제를 지운 변이가 세 번째 값에서 red 인지, `×` 와 변경량 버튼을 맞바꾼 변이가 인덱스 단언에서 red 인지 확인한다 | EP-48 (2) |
| VP-72 | R-64 · SD-13 · AR-16 · MD-28 ↔ AT-71 | REQUIRED | claude `Stop` → `adaptHooks` → `turnEnded` 버퍼 → drain → `turn.ended` → tick → `diffSummary` | 순수 세 값; 마운트 0 · **`busy` 전이만 0** · tick 증가 1; hook 이 즉시 `{}` 이고 그 안에서 조회 0; `Stop` 매처 등록 | required — 이 회귀의 형태가 **“옛 계기가 남는다”** 라 양성만으로는 관측되지 않는다. `busy` 계기를 되살린 변이가 ② 에서 red 인지, hook 등록을 지운 변이가 매처 키 단언에서 red 인지 **둘 다** 확인한다 | EP-46 (4) |
| VP-73 | R-65 · AR-17 ↔ AT-72 | REQUIRED | `resolveDiffRange` → `diffRevArgs` → `git diff <base> HEAD` → 요약·패치 | 커밋/미커밋/미추적 3파일 fixture 에서 경로 **집합 동등** = {A}; 패치도 {A}; `readUntracked` 전수 0건 | required — 0건 스윕은 **방향이 반대**다(§5). 요약만 좁히고 패치를 안 좁힌 변이(= 두 소비자 중 하나만 고침)가 패치 집합 단언에서 red 인지 확인한다 | EP-47 (3) |
| VP-74 | R-66 · SD-12 ↔ AT-73 | REQUIRED | `gitDiffSummary`(커밋 0) → 세션 상태 → `DiffReview` 비어있음 분기 | `files: []`·`commits: []`; 본문 문구 1 + 파일 섹션 0; ko 값 정확 일치; 소비처 전수 4 | not selected — 직접 행동 결과(화면 문구)를 관측하는 oracle 이고 구조 proxy 가 아니다 | EP-49 (2) |
| VP-75 | R-67 · MD-30 ↔ AT-74 | REQUIRED | `comparison` + `summary` + `gitStatus` → 라벨 파생 → `GitContextBar` | 두 모드가 **다른 모양**; `commit` 출력에 `→` 부재; `all` 모드 순서 회귀 | required — 한 모드만 세면 **두 모드가 같은 문자열을 내는 구현**이 통과한다. 판별 유니온을 한 갈래로 접은 변이가 부재 단언에서 red 인지 확인한다 | EP-50 (2) |
| VP-76 | R-68 ↔ AT-75 | REQUIRED | `GitContextBar` 세그먼트 → `setDiffSidebarVisible` → `sidebarVisible` | 두 상태에서 `aria-pressed` 가 서로 반대; 왕복이 상태를 두 번 바꿈; 눌린 쪽만 채움 클래스; 토글 2건·`folder` 0건 | required — 하나만 세면 **두 버튼이 늘 같은 값을 내는 구현**이 통과한다. 두 버튼에 같은 `pressed` 를 준 변이가 반대 단언에서 red 인지 확인한다 | EP-51 (2) |
| VP-77 | R-69 · MD-31 ↔ AT-76 | REQUIRED | 각 컴포넌트 className → SSR 출력 | 실측표 9행의 토큰 클래스 존재 + **옛 클래스 부재**; `w-[38%]` 전수 0건 | required — 존재만 세면 **옛 클래스가 함께 남은 구현**이 통과한다(뒤 규칙이 이겨 화면은 맞아 보인다). 옛 클래스를 되살린 변이가 부재 단언에서 red 인지 확인한다 | EP-27 ④ 로 편입 (기존 3지점 + **1**) |
| VP-63 | R-55(→R-64) · MD-24(→MD-28) ↔ AT-62 | REGRESSION | ΔV5 경로 | 계기 출처가 바뀐 뒤에도 **이름 축은 세 계기**(`initial`·`identity`·`turn-end`)를 유지한다 — 앱 재시작 직후 컴포저 git 행이 사라지지 않는다 | not selected — 기존 oracle 을 **바뀐 입력 타입**으로 재실행 | EP-42 (3) |
| VP-66 | R-58(→R-67) · MD-23(→MD-30) ↔ AT-65 | REGRESSION | ΔV5 경로 | `all` 모드의 `기준 → 현재` 순서가 그대로. **AT-65 의 “두 값” 단언은 `all` 모드로 범위가 좁혀진다** | not selected — 기존 oracle 을 모드 인자와 함께 재실행 | EP-28 (3) |
| VP-64 · VP-65 · VP-67 · VP-68 · VP-69 · VP-70 | R-56 · R-57 · R-59 · R-60 · R-61 · R-62 ↔ AT-63~AT-69 | REGRESSION | ΔV5 경로 | 범위가 좁아지고 라벨·헤더가 바뀐 뒤에도 미싱크 문구·패치 가드 해제·기본 접힘·메뉴 2행·파일 헤더 1줄·이동 소유자가 같은 값 | not selected — 기존 oracle 재실행 | EP-43 (2) · EP-34 (4) · EP-45 (2) · EP-33 (4) · EP-35 (2) · EP-44 (3) · EP-36 (3) |
| VP-30 | R-24 ↔ AT-24 | SUPERSEDED | — | 왼쪽 node 가 `SUPERSEDED` 다 — “미추적 파일이 패널 어디에도 없다” 를 VP-73 이 **더 넓은 술어**(커밋되지 않은 것이 어디에도 없다)로 다시 유도해 받는다 | — | EP-15 (3) 는 EP-47 (3) 이 대체 |
| VP-53 · VP-55 · VP-56 · VP-61 · VP-62 · VP-54 | ΔV4 한 화면 · 패치 계약 · 문맥 확장 · 제거 전수 · 요구사항 · 패치 1회 | REGRESSION | ΔV4 경로 | 커밋 전용 범위로 좁힌 뒤에도 한 화면 연속·패치 계약·확장·요구사항·조회 1회가 같은 값 | not selected — 기존 oracle 재실행 | EP-29 (3) · EP-30 (4) · EP-31 (3) · EP-32 (2) · EP-23 (3) · EP-34 (4) |
| VP-22 · VP-48 · VP-50 | R-18 · R-39 · R-41(→R-69) ↔ AT-18 · AT-39 · AT-41 | REGRESSION | ΔV1·ΔV3 경로 | 컴포저 합계가 **커밋된 것만**으로 바뀌어도 `totals` 한 곳에서 온다; 읽기 조회 전건 `--no-optional-locks`; 새 세그먼트·카드에서도 `Icon`·`group/`·`transition-colors` 짝 | not selected — 기존 oracle 을 **바뀐 값·새 컴포넌트**에 재실행 | EP-11 (2) · EP-25 (3) · EP-27 (4) |
| VP-01~VP-08 · VP-18~VP-21 · VP-24~VP-29 · VP-31 · VP-35 · VP-39~VP-42 · VP-45 · VP-46 · VP-47 · VP-49 · VP-57~VP-59 | 준비 단계 · 표시 이름 · ΔV3 캐시·연출 · 기준선 기록 · anchor · 상한 | NOT_REQUIRED | — | ΔV1~ΔV4 @ `628123bc` 의 기존 증거 그대로 | — | 비영향 근거: ΔV6 이 `prepare-worktree.ts`·`CwdButton`·`activityLabel`·`queries.ts`·`migrations/**`·요구사항 슬라이스를 **한 줄도 바꾸지 않는다**(§18 ΔV6 목록에 없다). `resolveDiffRange` 는 **호출 결과만** 소비처가 달리 쓰고 함수 본문이 불변이라 기준선 기록 pair 가 살아 있다 |

#### ΔV6 gate 차분

| Gate | ΔV6 에서 달라지는 점 | 증거 / 명령 |
|---|---|---|
| `app` 정적 게이트 | 동일 — `app/src/**` 를 바꾼다 | `npm run lint` · `npm run typecheck` |
| `app` 테스트 | **DB 스위트가 늘지 않는다** — 마이그레이션도 스키마 변경도 없다. 신규는 순수·SSR 렌더·main 통합(임시 저장소) | `./node_modules/.bin/vitest run <suite>`(ABI 중립) |
| 문서 인벤토리 | **채널 수 불변** — `turn.ended` 는 기존 `orca:chat:event` 의 payload variant 이고 새 채널이 아니다. 마이그레이션 수 불변 | `node scripts/check-doc-inventory.mjs` 재생성 후 `--check` 차이 **0** |
| IPC 계약 문서 | **바뀐다** — `docs/IPC_CONTRACT.md` 의 `orca:chat:event` 행에 `turn.ended` variant 를 적는다 | 해당 문서 diff |
| i18n 카탈로그 | 신규 **3**(컴포저 닫기 · 세그먼트 둘의 접근성 이름) · 값 변경 **1**(`diffEmpty`) · 제거 **1**(`diffShowFiles` 는 세그먼트 이름으로 대체) | `resources.test.ts` |
| 마이그레이션 append-only | **신규 없음** | `check-migrations-appendonly.mjs` exit 0 |
| **사람 실기** | **6 → 7** (ΔV5 ②③ 이 ΔV6 ②③ 으로 성격 전환·통합, ΔV6 ① 신설) | §19 |
...
### ΔV4 전수 조사 (2026-09-02 실측)

| 대상 | 검색 / 확인 | N | 의미 |
|---|---|---|---|
| 파일 본문 조회부 | `rg 'gitApi\.diffFile' app/src --glob '!*.test.*'` | **1** (`DiffTileContent.tsx:148`) | 파일 하나를 열 때마다 IPC 1회다 — 전부 펼치면 파일 수만큼 난다(D-074 의 근거) |
| 요약 조회부 | `rg 'gitApi\.diffSummary' app/src --glob '!*.test.*'` (주석 1건 제외) | **1** (`useGitSnapshot.ts:104`) | D-031 이 여전히 성립한다 — 패치는 **두 번째 소유자**를 만들지 않고 별도 훅으로 나눈다 |
| 화면 분기 | `rg 'data-session-changes-screen' app/src --glob '!*.test.*'` | **4** (`list` 3 · `peek` 1) | 지금 화면이 둘이다(D-073 이 하나로 만든다) |
| `uncommitted` 소비처 | `rg '\.uncommitted' app/src/renderer --glob '!*.test.*'` | **9** (`SessionChangesList` 6 · `sessionChangesData` 3 · `peekNavigation` 1 — 표시 6 / 파생 3) | 앞 둘은 D-081·D-082 로 갈리고 `peekNavigation` 은 D-073 이 지운다 |
| 기준 브랜치 이름 | `rg 'baseline_ref\|base_ref\|sourceBranch' app/src` | **0** | 라벨에 쓸 이름이 저장소에 없다 — D-070 의 마이그레이션 근거 |
| worktree 표의 컬럼 | `migrations/0018_managed_worktrees.sql` | `repo_root`·`source_cwd`·`worktree_root`·`branch`·`base_oid` **5** | `branch` 는 *새* worktree 브랜치이고 base 의 이름이 아니다 |
| 기준선 컬럼 | `migrations/0019_session_baseline.sql` | `baseline_oid` **1** | ΔV4 는 같은 표에 `baseline_ref` 를 **덧붙인다**(0020) |
| 현재 브랜치 해석 | `git-cli.ts:49 currentBranch` (`symbolic-ref --short -q HEAD`) | **1**, 비공개 | `repository.ts` 에 공개 함수가 없다 — D-072 가 쓰려면 그 레이어에 `resolveHeadRef` 를 연다 |
| 패널 최대화 개념 | `rg 'maximiz\|fullscreen\|rightPanelExpanded' app/src/renderer` | **1** (`WinControls.tsx:34` — OS 창 컨트롤) | 타일 최대화가 **0건**이다 — D-091 이 열 폭 축을 쓰는 근거 |
| 열 폭 조절 | `SET_RIGHT_PANEL_COL_WIDTH` + `PANEL_MIN/DEFAULT/MAX_WIDTH` = `280/360/640` | 액션 **1** · 상수 **3** | D-091 이 새 축 없이 이 셋을 쓴다 |
| 메뉴 primitive | `Popover.tsx` · `MenuItem.tsx` · `KebabButton.tsx` | **3** | `⋮` 와 비교 기준 `▾` 가 새 팝오버를 만들지 않는다 |
| 아이콘 이름 | `Icon.tsx` 의 `folder`·`doc`·`chevD`·`chevR`·`chevU`·`kebab` | **6/6 존재** | D-085 가 새 SVG asset 을 만들지 않는다 |
| 상대 시각 | `shared/i18n/datetime.ts:107 formatRelativeTime` (`shared/time/relative.ts` 위) | **1** | 커밋 카드의 `4시간 전` 이 새 파생을 만들지 않는다 |
| gap 확장 방향 | `diffHunks.ts:109 expandGap` — `split = gap.end - count` | **1방향** | 끝쪽만 연다(D-090 이 방향 인자를 더한다) |
| depth 연출 | `app.css:167~195` `depth-in`/`depth-out` **180ms** + `prefers-reduced-motion` | keyframe **2** · utility **2** | D-092 가 같은 utility 를 사이드바에 재사용한다 |
| `--others` 잔존 | `rg -- '--others' app/src/main` | **0** | D-035 가 유지된다 — 패치 조회도 `git diff` 라 미추적을 담지 않는다 |
| `git diff -U1000000` 출력 형태 | 임시 저장소 실기(git 2.43) — 수정·추가·삭제·rename·binary·개행없음 6케이스 | **6/6 확인** | 수정 파일이 `@@ -1,10 +1,10 @@` 뒤 **전체 10줄**을 낸다(전문맥 성립) · binary 블록에 `---`/`+++` 가 **없다** · rename 이 `rename from/to` 를 낸다 — MD-18 파서 표의 근거 |
| 영향 받는 렌더 테스트 | `diffTile.render`(5) · `sessionChangesList.render`(6) · `diffPeek.render`(2) · `diffPanelDesign`(6) · `peekNavigation`(2) · `diffPeekRequest`(3) · `DiffTileContent.bridge`(3) · `diffBodyCache`(4) · `diffFileCache`(3) · `sessionChangesData`(6) | **10파일 40케이스** | 이 중 `diffPeek`(2)·`peekNavigation`(2)·`diffBodyCache`(4)·`diffFileCache`(3)·`diffPeekRequest`(3) = **5파일 14케이스**는 대상이 사라져 **삭제**, 나머지는 새 화면으로 재작성 |

#### 수치 / 전칭 표현 검산

- “파일당 조회 1회” — 소비자가 1곳(`DiffTileContent`)이고 `useEffect` 가 `bodyKey` 마다 돈다. 파일 N 개를 전부 펼치면 N 회다. 합계 검산: 지금 `+2`(요약 `readDiff` 2회) `+N`(본문) → ΔV4 는 `+2 +1` 로 **N 과 무관**하다.
- “새 채널 순증 0” — `orca:git:diffFile` **−1** + `orca:git:diffPatch` **+1**. 인벤토리의 채널 총계는 불변이고 마이그레이션만 **+1** 이다.
- “사람 실기 4건” — ΔV2 1(D-060) + ΔV4 3(파일 경계 가독성 · 사이드바 연출 · 형제 대비). ΔV3 의 2건은 그 대상 화면이 사라져 ΔV4 의 앞 둘이 **대체**한다(순증 +1).

### ΔV5 전수 조사 (2026-09-03 실측)

| 대상 | 검색 | N | 의미 |
|---|---|---|---|
| 요약 조회 호출부 | `rg 'gitApi\.diffSummary' app/src --glob '!*.test.*'` | **1** (`useGitSnapshot.ts:126`. 같은 파일 93행은 주석) | D-031(소유자 하나)이 지금도 성립한다 — 계기를 줄이면 그 하나에서 전부 줄어든다 |
| 패치 조회 호출부 | `rg 'gitApi\.diffPatch' app/src --glob '!*.test.*'` | **1** (`useGitPatch.ts:56`) | 교착 수정의 대상이 한 자리다 |
| 제거 대상 식별자 | `rg 'refreshGitSnapshot\|REFRESH_GIT_SNAPSHOT\|refreshGeneration\|subscribeGitExternalChange\|diffRefresh' app/src --glob '!*.test.*'` | **21** | D-099 의 삭제 분모. 테스트 포함 47 |
| 접힘 집합 소비 파일 | `rg -l 'collapsedFiles\|setAllDiffFilesCollapsed\|toggleDiffFileCollapsed' app/src --glob '!*.test.*'` | **5파일** (`DiffReview`·`DiffTileContent`·`GitContextBar`·`chatReducer`·`chatStore`) | D-105 반전의 분모. 절반만 뒤집으면 기본 상태가 조용히 펼침으로 남는다 |
| 패치 무효화 지점 | `rg 'patch: null' app/src/renderer/src/features/chat/reducer/chatReducer.ts` | **5** (초기 375 · cwd 리셋 925 · REFRESH 1261 · BEGIN 키불일치 1272 · RECEIVE_SUMMARY 1291) | 넷은 `gitSnapshotRequest` 도 바꾸고 **RECEIVE_SUMMARY 하나만** 요청을 유지한 채 비운다 — 교착이 정확히 그 자리다 |
| `uncommitted` renderer | `rg 'uncommitted' app/src/renderer --glob '!*.test.*'` | **12** (`diffComparison` 3 · `GitContextBar` 4 · `ChangedNavigationSidebar` 3 · 카탈로그 2) | D-107 의 삭제 분모. `summary.uncommitted` **데이터** 소비처는 그중 **1** (`diffComparison.ts:59`) |
| 경로 열기 소비처 | `rg 'fileApi\.openPath' app/src/renderer --glob '!*.test.*'` | **1** (`CwdButton.tsx:40`, 디렉토리) | D-108 이 **둘째** 소비처를 만든다 — 파일이라 핸들러의 `isDirectory()` 게이트를 지나지 못한다 |
| 경로 화이트리스트 | `handlers/files.ts:66-72` | `isDirectory()` **필수** + (`projectsDir` 하위 ∨ `hasSessionWithCwd`) | 파일을 허용하려면 **부모 디렉토리**로 같은 판정을 돌려야 한다 — 술어를 새로 쓰지 않는다 |
| 현재 브랜치 출처 | `rg 'gitStatus' app/src/renderer --glob '!*.test.*'` | `state.gitStatus.branch` (`gitRowState.ts:57` 이 이미 읽는다) | D-104 의 우측 값이 이미 세션 상태에 있다 — 새 조회가 필요 없다 |
| 이동 소유자 seam | `DiffReview.tsx:60` `useRef<HTMLDivElement>(null)` | **내부 생성 1** | SSR 에서 `current` 가 비어 D22 가 열렸다 — D-110 이 prop 기본값으로 바꾼다 |

#### ΔV5 수치 / 전칭 표현 검산

- “요약 조회 소유자는 하나” — `rg` **1건**으로 재확인(ΔV1 D-031 이래 불변). 2건 이상이면 D-099 가 절반만 적용된다.
- “패치를 비우는 지점 중 요청을 유지하는 것은 하나” — 5건을 각각 열어 `gitSnapshotRequest` 동시 변경 여부를 확인했다(4/5 가 함께 바뀐다). 이 전칭이 거짓이면 해제 규칙만으로 교착이 닫히지 않는다.
- “`↗` 가 쓸 채널이 이미 있다” — 있으나 **디렉토리 전용**이다(`files.ts:67` 의 `isDirectory()` 던지기). “재사용하면 된다” 는 주장은 실측에서 반증됐고 그래서 AR-15 가 NEW 다.

### ΔV6 전수 조사 (2026-09-03 실측)

| 대상 | 검색 / 명령 | N | 의미 |
|---|---|---|---|
| 미커밋·미추적 조회 호출부 | `rg 'readUntracked\(' app/src/main` | **3** = 정의 1(`git-diff.ts:206`) + **호출 2**(`:268` 요약 · `:333` 패치) | D-111 이 닫아야 할 지점이 정확히 둘이다. 하나만 고치면 요약과 패치가 다른 범위를 본다 |
| 범위 인자 생성 | `rg 'diffRevArgs\(' app/src/main` | **3** = 정의 1(`git-diff.ts:145`) + **호출 2**(`:269` 요약 · `:332` 패치) | 두 소비자가 **같은 함수**를 부른다 — SSOT 가 이미 하나라 한 곳만 고치면 된다(AR-17) |
| `GitDiffSummary.uncommitted` renderer 소비 | `rg '\.uncommitted' app/src/renderer --glob '!*.test.*'` | **0** (주석 1건 제외) | ΔV5 D-107 이 이미 비웠다 — 계약 필드를 지우지 않아도 화면에 영향이 없다 |
| `diffEmpty` 소비처 | `rg "diffEmpty" app/src/renderer --glob '!*resources*'` | **4** (`DiffReview:119` 본문 · `ChangedNavigationSidebar:65` 트리 · `FileDiffSection:273` 섹션 · `GitContextBar:82` 커밋 서브메뉴) | D-113 의 문구 교체가 네 자리에 한 번에 닿는다 |
| 요약 조회 프로덕션 호출부 | `rg 'gitApi\.diffSummary' app/src/renderer --glob '!*.test.*'` | **2** = 주석 1(`useGitSnapshot.ts:87`) + **호출 1**(`:119`) | AT-71 의 “0회/1회” 분모가 하나라 스텁 한 개로 전건을 관측한다 |
| 턴 종료 계기의 현재 출처 | `rg 'sessionBusy' app/src/renderer/src/features/chat/store/chatStore.ts` | 정의 1 + 소비 다수 | `sessionBusy = inflight \|\| listening` (`:1459`). `inflight` 를 내리는 것은 terminal 이벤트라 **지금 계기는 result 도착**이다 — 사용자 지목이 정확하다 |
| 정규화 hook 어휘 | `rg "on-turn-end" app/src/main` | **2** (`hooks.ts:14` 정의 · `claude-adapt.ts:105` 매핑) | `Stop` 으로 굽는 배선이 이미 있다. ΔV6 은 **핸들러를 등록**할 뿐 어휘를 만들지 않는다 |
| 현재 등록된 hook 핸들러 | `rg 'hooks: \{ normalized' app/src/main` | **1** (`builder.ts:73`, 값이 `{}`) | 프로덕션 hook 핸들러가 **0건**이다 — `adaptHooks` 가 `{}` 를 내 `options.hooks` 가 주입되지 않는다 |
| 어댑터 내부 hook 조각 선례 | `rg 'hooks: \{ (PreToolUse\|PostToolBatch\|PostCompact)' app/src/main` | **2** (`workspace-guard.ts:152` PreToolUse · `claude-adapt.ts:182` PostToolBatch). `withPostCompactHook`(`:236`)은 기존 조각에 **덧씌우는** 세 번째 형태다 | 어댑터가 자기 hook 을 붙이는 패턴이 이미 있다 — `Stop` 도 같은 형태로 붙인다 |
| 스트림 밖 신호의 합류 선례 | `rg 'drainCompactSummaries' app/src/main/adapters/claude.ts` | **3** (정의 1 · 드레인 2) | hook 콜백이 버퍼에 적재하고 `eventBatches` 가 SDK 메시지 경계에서 비우는 형태가 이미 있다 |
| 등록된 어댑터 | `rg 'adapters\.set' app/src/main/adapters/registry.ts` | **1** (`ClaudeAdapter`) | mock 은 테스트 전용이라 hook 미지원이 프로덕션 경로를 막지 않는다 |
| `turn.` 이벤트 variant | `rg "type: 'turn\." app/src/shared/ipc.ts` | **2** (`turn.retrying`·`turn.aborted`) | 턴 **정상 종료** variant 가 없다 — `turn.ended` 를 새로 만든다 |
| terminal 이벤트 판정 | `rg "turn\.aborted" app/src/main app/src/renderer --glob '!*.test.*'` | **9** = 주석 4 + **판정·발화 5**(`chat-turn/index.ts:176` 발화 · `turn-coordinator.ts:307` · `session-runtime.ts:61` · `chatStore.ts:280` · 리듀서/스토어 `case` 2) | 판정이 전부 **열거형**이라 새 variant 가 terminal 로 오인되지 않는다. 이력 writer 는 `default` 절이 없어 미지정 type 을 무시한다(`writer.ts` 케이스 9종) |
| 사이드바 폭 | `rg 'w-\[38%\]' app/src/renderer` | **1** (`ChangedNavigationSidebar.tsx:60`) | 참조 실측 24.8% 와 어긋난 한 자리 |
| 헤더 제목 서체 | `rg 'font-serif' app/src/renderer/src/features/chat/components/rightpanel` | **7** — 이번 대상은 `GitContextBar.tsx:215`·`226` **둘**뿐이고 나머지 5(`RightPanelTile`·`SubAgentTileContent` 2·`TaskTileContent` 2)는 **형제 타일 제목**이라 손대지 않는다 | 참조의 그 자리는 제목이 아니라 **드롭다운 버튼 라벨**이다. Orca 의 버튼 타이포는 `Button` 이 이미 regular sans 로 정했고(`TEXT` 맵에 serif 없음) `font-serif font-semibold` 가 그 규칙에서 벗어난 자리였다 — D-066(형제 primitive)과 충돌하지 않는다 |
| 커밋 카드 테두리 | `rg 'border border-t5' app/src/renderer/src/features/chat/components/rightpanel/ChangedNavigationSidebar.tsx` | **1** (`:96`) | 참조 비선택 카드에는 테두리가 없다(D-119) |

#### ΔV6 참조 실측표 (k = 0.5 CSS px / device px, D-120)

캡처는 `786c94db-image.jpg`(2340×1080, 사이드바 열림·메뉴 닫힘)와 `a789b02b-image.jpg`(메뉴 열림)다. 좌표는 device px, 괄호는 CSS px 환산.

| # | 요소 | 참조 실측 | 매핑할 Orca 토큰 | 현재 값 |
|---|---|---|---|---|
| 1 | 사이드바 폭 | `x 117..632` / 패널 `117..2194` = **24.8%** | `w-[25%]` | `w-[38%]` |
| 2 | 헤더 제목 서체·굵기 | 본문과 같은 sans, regular (굵기 대비 없음) | 기본 서체 + `font-normal` | `font-serif font-semibold` |
| 3 | 헤더 제목 톤 | 최암 `rgb(81,80,76)` — 메뉴 라벨 `rgb(11,11,11)` 보다 밝다 | `text-t7` | `text-t9` |
| 4 | 메뉴 라벨 톤 | 최암 `rgb(11,11,11)` | `text-t9` | `text-ink`(=t9) ✔ 유지 |
| 5 | 파일 섹션 헤더 밴드 | 배경 `#f2f2f2`, 높이 68 (34) | `bg-bg2`(#f4f4f3) | 배경 없음 |
| 6 | 선택 채움 (사이드바 행·카드) | `#e6e6e6`, `x 135..616` `y 865..931` (높이 33.5) | `bg-fill-selected` (D-118 — 러스트 계열) | `text-accent` + 체크 아이콘 / `border-accent` |
| 7 | 비선택 커밋 카드 | 배경·테두리 **없음** (`x=560, y 932..1060` 순백) | 클래스 없음 | `border border-t5` |
| 8 | 메뉴 행 hover 채움 | `#f3f3f3`, 행 높이 52 (26) | `bg-fill-uncontained-hover` | 동일 ✔ 유지 |
| 9 | 헤더 활성 토글 채움 | `#cce2fa` 채움 + `#244d81` 글리프, `54×48` (27×24) | `bg-fill-selected` + `text-accent` (D-118) | 활성 상태 **없음** |

**참조와 의도적으로 다른 두 값**(§16 기록): 활성/체크 색 — 참조 `#cce2fa`/`#3975bd`, Orca 러스트(D-118, 사용자 결정). `+N`/`−M` 색 — 참조 `#398e4d`/`#9f3856`, Orca `#3f7a34`/`#a83c2b`(D-121).
...
### ΔV4 — AS-IS(=ΔV3 구현물) → TO-BE

```text
AS-IS  (두 화면 · 파일당 조회)
  useGitSnapshot ──diffSummary──► gitSnapshot.summary
                                        │
                              SessionChangesList (list 화면)
                                        │ 파일 클릭
                                        ▼
                                   peekTarget ──diffFile(path)──► DiffPeek (peek 화면)
                                        │                    ▲
                                        └── LRU 12항목 ───────┘   ← 파일마다 1회

TO-BE  (한 화면 · 범위당 1회)
  useGitSnapshot ──diffSummary──► gitSnapshot.summary ─┐ (컴포저 행도 읽는다)
                                                       │
  useGitPatch(타일 열림 ∧ patch 부재) ──diffPatch──► gitSnapshot.patch
                                                       │
                     ┌─────────────────────────────────┴───────────────┐
                     ▼                                                 ▼
            ChangedNavigationSidebar                              DiffReview
             ├ ChangedFileTree  (MD-21)                    FileDiffSection × N
             └ CommitList       (summary.commits)           ├ FileHeader
                                                            └ DiffHunks (MD-19 양방향)
                     비교 모드 필터(MD-22)는 두 소비자가 같은 함수를 쓴다
```

| 축 | AS-IS | TO-BE | 근거 |
|---|---|---|---|
| 화면 수 | 2 (`list` ↔ `peek`) | **1** (연속 섹션) | D-073 |
| 본문 조회 | 파일당 1회 + LRU 12 | **범위당 1회**, 파일 수 무관 | D-074·D-075·D-078 |
| 본문 표현 | `{oldValue,newValue}` 전문 + renderer `diffLines` O(n·m) | **패치 줄 배열**(main 파싱) | D-075·D-076 |
| 문맥 확장 | 한 방향, gap 끝쪽 5줄 | **양방향**, 숨은 줄 수 라벨, 20줄 | D-090 |
| 비교 기준 표시 | 상단 요약 6값 + `기준선 → 현재` | **브랜치 이름 하나** | D-069·D-081 |
| 비교 범위 선택 | 없음 (항상 세션 전체) | `모든 변경사항` · `미커밋 변경` · 커밋 — **목록만** 좁힘 | D-079·D-082 |
| 파일 탐색 | 없음 | **사이드바 트리**(기본 숨김) | D-083·D-084 |
| 표시 옵션 | 없음 | `⋮` **8항목**, 넷은 순수 파생 | D-086·D-088 |
| 상태 소유 | 요약 · peek 대상 · 커밋 확장 / 로컬 본문 캐시 | 요약 · 패치 · 비교 모드 · 접힌 집합 · 사이드바 · 표시 옵션 / 로컬 확장 상태 | D-094 |
| 기준선 | `baseline_oid` 1컬럼 | `baseline_oid` + **`baseline_ref`** 2컬럼, 같은 insert | D-070 |

### ΔV5 — AS-IS(=ΔV4 r3 구현물) → TO-BE

```text
AS-IS — 계기 다섯이 한 판정 함수를 공유하고, 무효화 주체가 둘이다
  마운트 ─┐
  세션전환 ─┤                          ┌─ gitApi.status ──▶ gitStatus
  턴 종료 ─┼─ gitSnapshotQueryReason ─┤
  창 포커스 ┤   (initial/identity/     └─ gitApi.diffSummary ─▶ summary
  ⋮새로고침 ┘    turn-end)                                        │
                                                                 ▼
        REFRESH ──▶ patch:null ──┐                    RECEIVE_SUMMARY ──▶ patch:null
                                 ├──▶ useGitPatch ──▶ diffPatch ──▶ receiveGitPatch
                                 │      (inFlight 키, .catch 에서만 해제)
                                 └── 두 무효화가 경쟁하면 가드가 잠긴다  ← 회귀

TO-BE — 계기가 축별로 갈리고, 같은 요청을 유지한 채 비우는 자리는 하나다
  마운트 ─┐
  세션전환 ─┼─ gitStatusQueryReason ──▶ gitApi.status ──▶ gitStatus   (D-101)
  턴 종료 ─┘
  턴 종료 ─── gitSummaryQueryReason ─▶ gitApi.diffSummary ─▶ summary  (D-099)
                                                              │
                                                              ▼
                                                  RECEIVE_SUMMARY ──▶ patch:null
                                                              │
                                        beginPatchQuery / settlePatchQuery  (D-103)
                                                              │
                                                   diffPatch ─┴─▶ receiveGitPatch
```

| 축 | AS-IS (ΔV4 r3) | TO-BE (ΔV5) | 근거 |
|---|---|---|---|
| 변경 목록 계기 | 다섯 | **하나**(턴 종료) | D-099 |
| 이름 계기 | 같은 다섯 | 셋(마운트·세션 변경·턴 종료) | D-101 |
| 계기 판정 함수 | 1 (`gitSnapshotQueryReason`) | **2** (`gitSummaryQueryReason` · `gitStatusQueryReason`) | MD-24 |
| 패치 무효화 주체 | 2 (REFRESH · RECEIVE_SUMMARY) | **1** (RECEIVE_SUMMARY) | D-103 |
| 패치 가드 해제 | 실패에서만 | **성공·실패·폐기 셋 다** | D-103 |
| 싱크 전 화면 | 로딩 문구 | **미싱크 문구** | D-102 |
| 파일 기본 상태 | 전부 펼침(`collapsedFiles`) | **전부 접힘**(`expandedFiles`) + 안내 | D-105 |
| 기준 라벨 | 기준 하나 | **기준 → 현재** | D-104 |
| 비교 메뉴 | 평면 3구획 | **2행 + 커밋 서브메뉴** | D-106·D-107 |
| `⋮` 항목 | 8 | **7**(`새로고침` 제거) | D-099·D-106 |
| 파일 헤더 | 2줄, `↗` 없음 | **1줄 + `↗`(탐색기 선택)** | D-108 |
| 이동 소유자 | 내부 ref (관측 불가) | **주입 가능 prop** | D-110 |

**AS-IS → TO-BE Delta 한 줄**: 계기를 줄이는 것과 교착을 고치는 것은 **다른 작업**이다 — 전자는 생산자를 지우고 후자는 소비자의 해제 규칙을 만든다. 전자만 하면 세션 키가 바뀌는 턴 종료에서 같은 경쟁이 남는다(§4 ΔV5).
...
| AR-14 / VP-51 · VP-52 | 비교 기준 이름은 **한 컬럼**이고 세션 출생 시 1회 기록한다 (**EP-28, 3지점**: ① 마이그레이션 `0020` 의 `sessions.baseline_ref` ② `PrepareWorktreeResult.managed.baseRef` 로 실어 보낸다(D-072) + 비격리는 `resolveHeadRef(sourceCwd)` ③ 라벨 파생이 `ref`·`oid`·`head`·`none` **4상태**) | `sessions.baseline_ref`(`0020`) · `PrepareWorktreeResult`(`features/worktrees/service.ts:21`~) · `summaryBaseLabel`(`sessionChangesData.ts`) | 구현자 | 마이그레이션 · prepare 반환 · 세션 출생 · 매 렌더 파생 | ①이 빠지면 이름을 담을 자리가 없다. ②가 빠지면 구현자가 렌더 시점에 `git symbolic-ref` 를 다시 부르는 역산을 발명한다 — D-007 이 이름 축에서 이미 금지한 것이다. ③이 3상태로 남으면 `ref` 가 있어도 sha 가 그려져 D-069 가 화면에 도달하지 않는다 |
| AR-13 / VP-55 | 패치는 **한 채널·한 호출**이다 (**EP-29, 3지점**: ① `orca:git:diffPatch` 계약 + zod 스키마 + preload 배선 ② main 실행부가 `git diff --unified=1000000 -M --no-color <base>` **1회** + 전용 버퍼 16 MiB ③ 실패 시 `--unified=3` **재조회 1회** + `contextLimited`) | `GitDiffPatch*`(`shared/ipc.ts`) · `protocol.ts` · `git-diff.ts` | 구현자 | 계약 정의 · 조회 구성 · 실패 분기 | ①의 preload 가 빠지면 renderer 가 부를 수 없다. ②를 파일별 호출로 쪼개면 D-074 가 없앤 축이 그대로 돌아온다. ③이 빠지면 큰 저장소에서 패널이 **통째로 빈다** — 사용자는 “변경이 없다” 로 읽는다(D-053 이 커밋 이력에서 이미 닫은 실패 모드) |
| AR-13 · R-53 / VP-53 · VP-61 | 구 본문 경로가 **네 자리 모두** 사라진다 (**EP-30, 4지점**: ① `shared/ipc.ts` 의 `gitDiffFile` 채널·`GitDiffFileRequest`·`GitDiffFileContent` ② `preload/index.ts` 배선 ③ `handlers/git.ts` 등록 ④ renderer 소비 — `DiffTileContent` 호출부 + `DiffPeek.tsx`·`diffBodyCache.ts`·`diffFileCache.ts`·`peekNavigation.ts` 파일) | `shared/ipc.ts` · `preload` · `handlers/git.ts` · `rightpanel/` | 구현자 | 제거 시 | ①만 지우면 preload 가 없는 채널을 부르고 typecheck 가 잡지 못하는 자리가 남는다(`ipcRenderer.invoke` 는 문자열이다). ④의 파일을 남기면 죽은 캐시 모듈이 카탈로그에 남아 다음 사람이 되살린다 — 0206 의 `diffTileMock` 과 같은 함정이다 |
| MD-18 / VP-55 | 패치 상한은 **셋**이고 서로를 대신하지 못한다 (**EP-31, 3지점**: ① 파일 200(`MAX_DIFF_FILES` 재사용) ② 파일당 줄 **50,000** → `kind:'too-large'` ③ 패치 전체 줄 **200,000** → 합계를 넘기는 그 파일이 `too-large`(예산은 수집한 파일만 소비하므로 뒤의 더 작은 파일은 다시 실린다)) | `git-diff-parse.ts` 상수 | 구현자 | 파싱 정규화 | ①만 있으면 파일 하나가 50만 줄일 때 IPC 페이로드가 터진다. ②만 있으면 5,000줄짜리 파일 200개(=100만 줄)가 통과한다. ③만 있으면 201번째 파일이 조용히 사라진다 — **세 축의 분모가 다르다** |
| SD-10 / VP-54 | 패치 조회는 **세대당 1회 · 타일 닫힘 0회**다 (**EP-34, 3지점**: ① 조회 조건 = 타일 마운트 ∧ `patch === null` ② 새 요약 도착이 `patch` 를 `null` 로 만든다 ③ `gitApi.diffPatch` 를 부르는 renderer 파일이 **1개**) | `hooks/useGitPatch.ts`(신규) · `chatReducer` 의 `gitSnapshot` 슬라이스 | 구현자 | 타일 마운트 · 요약 수신 · 렌더 트리 조립 | ①에서 `patch === null` 조건을 빼면 마운트마다 조회가 돌아 사용자가 지적한 증상이 그대로 돌아온다. ②가 빠지면 턴이 끝나도 낡은 diff 가 남는다 — 새로고침의 의미가 사라진다. ③이 2 이상이면 D-031 이 요약 축에서 닫은 것을 패치 축에서 다시 연다 |
| MD-19 / VP-56 | 확장은 **방향을 값으로 갖는다** (**EP-32, 2지점**: ① `expandGap(state, id, n, direction)` 이 `up` = gap 끝쪽·`insertedAbove=n`, `down` = gap 앞쪽·`insertedAbove=0` ② gap 행이 **가능한 방향만** 버튼으로 그린다 — 선두 gap 은 `up`, 말미 gap 은 `down`, 중간은 둘) | `lib/diffHunks.ts` · `FileDiffSection` | 구현자 | 순수 파생 · 렌더 | ①이 한 방향이면 제안서 §11 의 `﹀` 가 아무 데도 없다. ②가 방향을 안 가리면 파일 맨 위에서 “아래로 펼치기” 가 눌리고 아무 일도 일어나지 않는다 — 실패가 화면에서 **“아무 일도 안 일어남”** 으로 보인다 |
| MD-20 / VP-59 | 표시 옵션 넷은 **순수 파생 네 자리**다 (**EP-33, 4지점**: ① 공백 변경 숨기기 = 줄 쌍을 `unchanged` 로 접는 순수 변환 ② 변경된 단어 강조 = 공백 경계 토큰의 공통 접두/접미 제거(D-089) ③ 나란히 = 같은 줄 배열의 행 짝 파생 ④ 자동 줄 바꿈 = 본문 컨테이너 클래스) | `lib/diffDisplay.ts`(신규, 순수) | 구현자 | 렌더 파생 | ①~③ 중 하나라도 조회를 부르면 D-088 이 깨지고 D-078 의 “세대당 1회” 가 함께 깨진다. ②를 줄 전체 강조로 대신하면 메뉴 항목이 켜짐/꺼짐에서 **같은 화면**을 낸다 — 항목이 있는데 아무 일도 하지 않는다 |
| MD-22 / VP-57 | 비교 모드는 **목록만** 좁힌다 (**EP-35, 2지점**: ① 필터가 `patch.files` 를 걸러낼 뿐 요청 인자를 바꾸지 않는다 — `GitDiffPatchRequest` 에 `commit` 멤버 **0** ② 커밋 파일이 세션 패치에 없으면 `세션 기준 변경 없음` 을 그린다(D-080)) | `lib/diffComparison.ts`(신규, 순수) · `GitDiffPatchRequest`(`shared/ipc.ts`) | 구현자 | 모드 전환 · 목록 렌더 | ①이 요청을 바꾸면 사용자가 명시적으로 거절한 커밋 단위 diff 가 되살아나고 D-036 이 깨진다. ②가 빠지면 그 파일이 목록에서 조용히 빠져 사용자가 “이 커밋에 없었다” 로 읽는다 |
| R-50 · R-52 / VP-58 · VP-60 | 사이드바와 확대는 **각각 두 지점**이다 (**EP-36, 2지점**: ① 사이드바 기본 숨김 + 두 진입점(폴더 버튼 · `⋮ › 파일 표시`)이 **같은 상태**를 토글한다 ② 트리 파일 선택이 그 섹션으로 스크롤한다 — 새 화면으로 가지 않는다) | `ChangedNavigationSidebar` · `chatReducer` 의 `diffView` 슬라이스 | 구현자 | 렌더 트리 조립 · 선택 처리 | ①이 두 상태면 폴더 버튼과 메뉴 항목이 서로 다른 사이드바를 켠다 — 사용자가 메뉴로 켠 것을 버튼으로 못 끈다. ②가 화면 전환이면 D-073 이 없앤 두 번째 화면이 이름만 바꿔 돌아온다 |
...
- **ΔV4 의 선택적 필드 의미.** `GitDiffBase.ref = null` 은 “기준선 커밋은 아는데 그때의 브랜치 이름을 모른다”이고 라벨은 sha 7자로 접힌다(D-071) — `kind:'head'`(“기준선 자체를 모른다”)와 **다른 상태**다. `GitDiffPatchFile.kind` 는 `'text' | 'binary' | 'too-large'` 이고 `too-large` 는 “줄을 싣지 않았다”이지 “변경이 없다”가 아니다 — `added`/`removed` 는 그대로 실린다. `GitDiffPatch.contextLimited = true` 는 “문맥이 3줄뿐”이고 확장 버튼은 그 범위까지만 선다.
- **ΔV4 의 상호배타 상태.** 비교 모드는 `{ kind:'all' } | { kind:'uncommitted' } | { kind:'commit', sha }` 다 — `selectedCommit?: string` + `uncommittedOnly?: boolean` 같은 평면 조합은 둘 다 참인 불가능 상태를 허용한다. 표시 옵션 레이아웃은 `'inline' | 'side-by-side'` 유니온이고 `sideBySide: boolean` 로 두지 않는다(다음 모드가 붙으면 두 boolean 이 된다).
- **ΔV4 의 worst-case 출력.** 패치 페이로드 = 파일 **200** × 파일당 줄 **50,000** 이 상한이지만 **전체 줄 200,000** 이 먼저 걸린다 — 줄당 평균 80 B 로 약 **16 MB**. 그래서 전용 버퍼도 16 MiB 다(EP-29 ②). 세 상한이 없으면 `-U1000000` 은 **저장소 전체 크기**가 상한이 된다. 실패 시 `--unified=3` 재조회는 변경 줄 + 3줄 문맥이라 상한 밖이다.
- **ΔV4 의 두 저장소 쓰기 — 늘지 않는다.** `baseline_ref` 는 `baseline_oid` 와 **같은 `insertSession` 한 문장**에 실린다(EP-28 ①). 두 값을 다른 시점에 쓰면 이름과 커밋이 갈라지고, 그 사이에 죽으면 “커밋은 아는데 이름은 모르는” 행이 남는다 — 한 문장이라 그 조합이 생기지 않는다.
- **ΔV4 가 만드는 새 표면의 실패 모드.** 사이드바 파일 선택은 `scrollIntoView` 라 **대상 섹션이 접혀 있으면 눈에 띄는 변화가 없다** — 선택 시 그 파일을 먼저 펼친 뒤 스크롤한다(EP-36 ②). 그렇게 하지 않으면 클릭이 “아무 일도 안 일어남”으로 보인다.
...
| V node / pair | 계약/필드 | SSOT | 누가 | 언제 강제 | 실패 의미 |
|---|---|---|---|---|---|
| SD-12 · MD-24 / VP-63 | 계기 판정이 **두 함수**이고 요약 쪽은 `'turn-end' \| null` 만 낸다 (EP-42, **3지점**: `gitSummaryQueryReason` 정의 · `gitStatusQueryReason` 정의 · `useGitSnapshot` 의 두 effect 가 각각 자기 함수를 부르는 배선) | `useGitSnapshot.ts` 의 두 순수 함수 | 구현자 | 매 렌더 판정 | 한 함수만 고치면 요약과 이름이 같은 계기를 계속 공유한다 — 이름까지 턴에 묶이면 앱 재시작 직후 컴포저 git 행이 통째로 사라지고(D-101 위반), 반대로 요약에 `identity` 가 남으면 세션을 열 때마다 조회가 나 D-099 가 절반만 적용된다 |
| R-56 / VP-64 | 미싱크와 로딩이 **다른 문구**다 (EP-43, **2지점**: `DiffReview` 의 분기 · 카탈로그 두 키) | `DiffReview.tsx` 분기 + `i18n/resources` | 구현자 | 매 렌더 | 한 지점만 고치면 키는 둘인데 화면은 하나이거나(분기 누락) 화면은 둘인데 문구가 같다 — 다음 회귀를 사용자가 화면에서 구분하지 못한다 |
| SD-10 · MD-25 / VP-65 | 패치 가드는 **성공·실패·폐기 셋 다** 해제한다 (EP-34 ④ 로 편입, 기존 EP-34 3지점 + **1**) | `useGitPatch.ts` 의 순수 상태 기계 | 구현자 | 응답 도달마다 | 하나라도 빠지면 그 경로에서 지금의 교착이 그대로 남는다. 특히 **폐기**(세대 불일치로 리듀서가 버린 응답)에서 해제하지 않으면 화면은 영원히 로딩이다 |
| AR-15 / VP-69 | 파일 reveal 은 **부모 디렉토리**로 기존 화이트리스트를 통과해야 한다 (EP-44, **3지점**: 요청 스키마의 모드 · 핸들러의 파일/디렉토리 분기 · 화이트리스트 판정 재사용) | `OpenPathRequestSchema` + `handlers/files.ts` | 구현자 | IPC 진입 | 화이트리스트를 새로 쓰면 두 벌이 되고, 파일 분기에서 판정을 건너뛰면 **임의 경로 오픈 벡터**가 열린다 — 기존 핸들러가 디렉토리 전용으로 막던 자리다 |
| SD-11 · MD-26 / VP-67 | 펼침 집합은 **한 방향**이다 — 상태·리듀서·두 컴포넌트·전체 토글이 모두 `expandedFiles` 를 읽는다 (EP-45, **2지점**: 리듀서 액션 쌍 · 소비 컴포넌트 둘) | `chatReducer` 의 `expandedFiles` | 구현자 | 상태 갱신·매 렌더 | 반전을 절반만 하면 이름은 바뀌었는데 기본이 여전히 펼침이거나, 전체 토글 둘이 서로 반대로 동작한다 |
| MD-27 / VP-70 | 이동의 소유자는 **인자로 들어온다** (EP-36 ③ 로 편입, 기존 EP-36 2지점 + **1**) | `DiffReview` 의 ref prop | 구현자 | 트리 선택마다 | 소유자가 내부에서만 만들어지면 SSR 에서 `null` 이라 **아무것도 스크롤하지 않는 구현이 전건 green** 이다 — ΔV4 r3 의 D22 가 정확히 이것이다 |

- **ΔV5 의 선택적 필드 의미.** `OpenPathRequest` 의 모드는 `'directory' | 'reveal'` **필수 유니온**이다 — optional 로 두고 미지정을 파일 허용으로 접으면 기존 호출부가 조용히 넓어진다(fail-open). 기존 `CwdButton` 호출은 `'directory'` 를 명시한다.
- **ΔV5 의 상호배타 상태.** 본문 상태는 `미싱크 | 로딩 | 저장소아님 | 불가 | 비어있음 | 준비됨` 여섯이고 `gitSnapshotRequest === null` 이 첫째를 **단독으로** 결정한다 — `patch===null` 하나에 두 의미를 싣던 것이 회귀를 감춘 구조였다.
- **ΔV5 의 저장소 쓰기 — 늘지 않는다.** DB·파일 어느 쪽도 새로 쓰지 않는다. `shell.showItemInFolder` 는 읽기 부작용이고 실패해도 앱 상태가 남지 않는다.
- **ΔV5 가 만드는 새 표면의 실패 모드.** ① 미싱크 문구가 “고장” 으로 읽힐 수 있다 — 문구가 **다음에 무엇이 일어나는지**를 말해야 한다(“턴이 끝나면 표시됩니다”). ② 기본 접힘에서 파일이 많으면 헤더만 수백 줄이다 — 상한은 이미 `MAX_DIFF_FILES` 200 이 갖는다(D-076 승계). ③ `↗` 로 연 탐색기는 앱 밖이라 되돌아오는 경로가 없다 — 그래서 창 재포커스 계기를 없앤 D-099 와 충돌하지 않는지 확인했다: 탐색기에서 파일을 고쳐도 다음 턴 종료까지 반영되지 않는 것이 **의도된 계약**이다.
...
export type GitDiffBase =
  // ref = 세션 시작 시점의 브랜치 이름. 모르면 null 이고 라벨은 oid 로 접힌다(D-071).
...
export interface GitDiffPatchFile {
  ...
  added: number
  removed: number
  // 'too-large' = 상한을 넘겨 줄을 싣지 않았다. added/removed 는 그대로다(D-077).
...
export interface GitDiffPatch {
  ...
  // true = `--unified=3` 폴백으로 받았다. 확장이 로드된 문맥까지만 선다.
  contextLimited: boolean
  // true = 두 조회가 모두 실패했다. `files` 는 빈 배열이고 화면이 사유를 말한다.
  unavailable: boolean
}

export interface GitDiffPatchRequest { cwd: string; sessionId?: string }  // `commit` 없음(D-036·EP-35)
```

| 변경/신규 파일 | 책임 | 변경 내용 | 테스트 seam |
|---|---|---|---|
| `app/src/main/infra/db/migrations/0020_session_baseline_ref.sql` | **신규** 스키마 | `ALTER TABLE sessions ADD COLUMN baseline_ref TEXT` | `migrate.test.ts` + append-only 게이트 |
| `app/src/main/infra/db/queries.ts` | 영속 | `insertSession` 에 `baseline_ref` 열 추가(같은 문장) · `getSessionBaseline` 이 `{ oid, ref }` 를 돌려주도록 확장 | `queries.test.ts` |
| `app/src/main/infra/git/repository.ts` | 조회 | `resolveHeadRef(cwd)` 공개 — `symbolic-ref --short -q HEAD`(unborn 브랜치에서도 이름을 준다, `git-cli.ts:48` 주석 근거) | `repository.test.ts` |
| `app/src/main/features/worktrees/service.ts` | 발생 | `PrepareWorktreeResult.managed` 에 `baseRef: string \| null` — `input.baseRef ?? resolveHeadRef(sourceCwd)`, `baseOid` 와 **같은 자리**에서 결정 | `service.test.ts` |
| `app/src/main/app/chat-turn/prepare-worktree.ts` | 통과 | `sessionBaselineRef` 를 세 결과에 실어 보낸다 — managed 는 `prepared.baseRef`, passthrough(비격리)는 `resolveHeadRef(sourceCwd)`, recovered 는 `null` | `prepare-worktree.test.ts` |
| `app/src/main/app/chat-turn/{send,turn-context}.ts` · `contracts/turn.ts` | 운반 | `sessionBaselineRef` 를 `sessionBaseline` **옆자리**에 더한다 — 새 홉을 만들지 않는다 | 기존 케이스 확장 |
| `app/src/main/features/history/writer.ts` | 기록 | `insertSession` 호출에 `baselineRef: turn.sessionBaselineRef` | `writer.test.ts` |
| `app/src/main/infra/git/git-diff.ts` | 조회 | `gitDiffSummary` 의 `base` 에 `ref` 를 싣는다 · **`gitDiffPatch()` 신설**(1회 + 폴백 1회) · `gitDiffFile`·`showAt`·`isTrackedDiffPath` **제거** | `git-diff.test.ts`(인자 수집 runner) |
| `app/src/main/infra/git/git-diff-parse.ts` | 파싱 | **`parseUnifiedPatch()` 신설**(순수) + 상한 셋 상수. `MAX_DIFF_FILE_BYTES` 제거 | `git-diff-parse.test.ts` |
| `app/src/main/app/handlers/git.ts` | 등록 | `gitDiffPatch` 핸들러 등록(무해 폴백 = `unavailable:true`) · `gitDiffFile` 등록 제거 · `SessionBaselineLookup` 이 `{oid, ref}` 반환 | `handlers/git.test.ts` |
| `app/src/shared/{ipc,protocol}.ts` · `app/src/preload/index.ts` · `renderer/shared/api/ipc.ts` | 계약·배선 | 위 DTO + `GitDiffPatchRequestSchema` · `diffPatch` 배선 4자리 · `diffFile` 제거 4자리(EP-30) | `ipc-documentation.test.ts` · `git-diff-schema.test.ts` |
| `renderer/.../lib/diffHunks.ts` | 순수 | `expandGap(state, id, n, direction)` — `up` 은 끝쪽·`insertedAbove=n`, `down` 은 앞쪽·`0`. gap 행이 `canUp`/`canDown` 을 값으로 갖는다 | `diffHunks.test.ts` |
| `renderer/.../lib/diffPatchLines.ts` | **신규** 순수 | `GitDiffPatchLine[] → DiffLine[]` 한 자리 변환(`lineNo` 채움). `buildDiffLines` 는 도구 카드 전용으로 남는다 | 신규 |
| `renderer/.../lib/diffDisplay.ts` | **신규** 순수 | 공백 접기 · 단어 강조(토큰 접두/접미) · side-by-side 행 짝 — 셋 다 `DiffLine[]` 입출력(EP-33) | 신규 |
| `renderer/.../lib/changedFileTree.ts` | **신규** 순수 | 경로 배열 → 폴더/파일 계층 + 접힘 파생(MD-21) | 신규 |
| `renderer/.../rightpanel/diffComparison.ts` | **신규** 순수 | 비교 모드 → 표시 파일 목록. 커밋 파일 ∖ 세션 패치 = `세션 기준 변경 없음` 표시(MD-22·D-080) | 신규 |
| `renderer/.../hooks/useGitPatch.ts` | **신규** 조회 소유 | 타일 마운트 ∧ `patch === null` 에서만 1회. `gitApi.diffPatch` 의 **유일** 호출부(EP-34) | 스텁 주입 렌더 |
| `renderer/.../rightpanel/GitContextBar.tsx` | **신규** 헤더 | 폴더 토글 · 비교 기준 `▾`(Popover+MenuItem) · `⋮`(8항목) · `↗`(열 폭 토글). `RightPanelTile` 의 `headerContent`+`headerActions` 로 주입 | SSR 렌더 |
| `renderer/.../rightpanel/ChangedNavigationSidebar.tsx` | **신규** 탐색 | 상단 트리 + 하단 커밋 목록(`모든 변경사항`·`미커밋 변경`·커밋 카드). `animate-depth-in/out` 재사용(D-092) | SSR 렌더 |
| `renderer/.../rightpanel/DiffReview.tsx` · `FileDiffSection.tsx` | **신규** 본문 | 연속 파일 섹션 + 헤더(chevron·`Icon doc`·파일명·부모 경로·`+N −M`) + 양방향 gap + 요구사항 줄 | SSR 렌더 |
| `renderer/.../rightpanel/DiffTileContent.tsx` | 축소 | 화면 분기 제거 — 사이드바 + `DiffReview` 조립만 | 기존 파일 재작성 |
| `renderer/.../rightpanel/sessionChangesData.ts` | 축소 | `summaryBaseLabel` 4상태 · 커밋 표시 파생 유지 · `sessionChangeGroups` 제거 | `sessionChangesData.test.ts` |
| `renderer/.../reducer/chatReducer.ts` | 상태 | `gitSnapshot` 에서 `peekTarget`·`bodyCache` 제거, `patch`·`comparison`·`collapsedFiles`·`diffView` 추가(D-094) | 리듀서 순수 |
| **삭제** | — | `DiffPeek.tsx` · `SessionChangesList.tsx` · `diffBodyCache.ts` · `diffFileCache.ts` · `peekNavigation.ts` + 그 테스트 5파일 | — |
| `renderer/src/shared/i18n/resources/{ko,en}.ts` | 문구 | 컨텍스트 바·메뉴 8항목·사이드바·문맥 확장 키 추가, 요약 6값 키 5종 제거 | `resources.test.ts` |
| `docs/IPC_CONTRACT.md` · `docs/generated/inventory.md` | 문서 | `diffFile` 행 → `diffPatch` 행 교체 · `diffSummary` 의 `base.ref` 기술 · 인벤토리 재생성 | `check-doc-inventory.mjs --check` |

**`parseUnifiedPatch` 설계 (MD-18, 순수)**

입력은 `git diff --unified=1000000 -M --no-color <base>` 의 stdout 한 덩어리다. 파일 블록은 `diff --git a/<old> b/<new>` 로 시작한다.

| 케이스 | 입력 표식 (git 2.43 실측) | 산출 |
|---|---|---|
| 수정 | `--- a/p` · `+++ b/p` · `@@ -1,10 +1,10 @@` 뒤 **파일 전체** | `status:'modified'`, 줄 배열 |
| 추가 | `new file mode` · `--- /dev/null` | `status:'added'`, 전 줄 `added` |
| 삭제 | `deleted file mode` · `+++ /dev/null` | `status:'deleted'`, 전 줄 `removed` |
| rename | `rename from src.txt` · `rename to dst.txt` (+ `--- a/src` · `+++ b/dst`) | `status:'renamed'` + `oldPath` |
| binary | `Binary files /dev/null and b/x differ` — **`---`/`+++` 줄이 없다** | `kind:'binary'`, `lines: []`. 경로는 `diff --git` 헤더에서 읽는다 |
| 개행 없음 | 해당 줄 **뒤**의 `\ No newline at end of file` | 직전 줄에 병합, 새 줄로 만들지 않는다 |
| 모드만 변경 | `old mode`/`new mode` 만 있고 hunk 없음 | `status:'modified'`, `lines: []`, `added/removed = 0` |

줄번호는 `@@ -a,b +c,d @@` 의 시작값에서 전진시킨다 — `unchanged` 는 둘 다, `removed` 는 old 만, `added` 는 new 만. `-U1000000` 이라 파일마다 hunk 가 보통 **하나**지만 파서는 여러 hunk 를 전제한다(폴백 `-U3` 이 그렇다). **binary 경로는 `diff --git` 헤더에서만 얻는다** — 그 블록에 `+++` 줄이 없다(실측). 따라서 파서는 헤더의 `b/<path>` 를 먼저 잡고 `+++` 는 확인용으로만 쓴다.

**`useGitPatch` 설계 (SD-10)**

```text
useGitPatch(cwd, sessionId)        ← DiffTileContent 안에서만 호출(타일이 마운트일 때만 돈다)
  ├ patch !== null  → 아무것도 하지 않는다        (재열기 증가 0)
  └ patch === null  → gitApi.diffPatch 1회 → receiveGitPatch(request, patch)
리듀서: RECEIVE_GIT_SNAPSHOT_SUMMARY 가 patch 를 null 로 만든다   (세대 경계 = 폐기)
        REFRESH_GIT_SNAPSHOT 이 세대를 올려 요약·패치를 함께 다시 부른다
```

요청 세대 판정은 `createGitSnapshotQueryOwner` 와 같은 형태를 쓴다 — 늦은 응답이 새 세대의 상태에 닿지 않게 한다.

### ΔV5 구현 설계

**1) 계기 판정을 두 함수로 가른다** (`composer/useGitSnapshot.ts`, MD-24 · EP-42)

```ts
// 이름 축 — 지금 함수의 이름만 바꾼다. 값 집합 불변.
export function gitStatusQueryReason(
  previous: GitStatusPoint | null,
  next: GitStatusPoint
): 'initial' | 'identity' | 'turn-end' | null

// 변경 목록 축 — **턴 종료 하나**. previous 가 없으면 null 이다(마운트는 계기가 아니다).
...
- `refreshGeneration` 이 사라지므로 키는 `gitStatusTriggerKey(cwd)` · `gitSnapshotRequestKey(cwd, sessionId)` 로 준다.
- 요약 effect 의 deps 에서 `triggerKey` 를 빼지 **않는다** — 키가 바뀌면 다음 턴 종료가 새 키로 조회해야 한다. 계기 판정만 `busy` 를 본다.
- 창 포커스 effect(`subscribeGitExternalChange`)와 그 export 를 통째로 지운다.

**2) 패치 가드를 순수 상태 기계로 내린다** (`hooks/useGitPatch.ts`, MD-25 · EP-34 ④)

```ts
export interface PatchQueryGuard { inFlightKey: string | null }
export function patchQueryKey(sessionKey: string, request: GitSnapshotRequest): string
// 조회할지 + 다음 가드 상태. `patch !== null` 이면 fetch:false 로 세대당 1회를 유지한다(D-078).
export function beginPatchQuery(
  guard: PatchQueryGuard,
  input: { cwd: string | null; patch: GitDiffPatch | null; request: GitSnapshotRequest | null; sessionKey: string }
): { guard: PatchQueryGuard; fetch: boolean; key: string | null }
// 응답이 도달했다 — **성공·실패·폐기 공통**. 같은 키일 때만 비운다(늦은 응답이 새 조회를 풀지 않게).
export function settlePatchQuery(guard: PatchQueryGuard, key: string): PatchQueryGuard
```

훅은 `useRef<PatchQueryGuard>` 하나를 들고 두 함수를 부른다. `.then` 과 `.catch` **양쪽**에서 `settlePatchQuery` 를 부르고, `.then` 안에서는 dispatch **뒤에** 부른다.

**3) 리듀서** (`chatReducer.ts` · `chatStore.ts`)

- `REFRESH_GIT_SNAPSHOT` 액션·`chatActions.refreshGitSnapshot`·`gitSnapshot.refreshGeneration` 제거.
- `collapsedFiles: readonly string[]` → `expandedFiles: readonly string[]`. `TOGGLE_DIFF_FILE_COLLAPSED` → `TOGGLE_DIFF_FILE_EXPANDED`, `SET_ALL_DIFF_FILES_COLLAPSED` → `SET_ALL_DIFF_FILES_EXPANDED`(인자 `expanded: boolean` + `paths`). 전체 펼침이 경로를 채우고 전체 접힘이 비운다.
- 초기값·cwd 리셋의 `collapsedFiles: []` 는 `expandedFiles: []` 로 — **의미가 반대라 값은 같지만 뜻이 “전부 접힘” 이다**. 주석으로 그 반전을 남긴다.

**4) 본문 상태 분기** (`DiffReview.tsx`, EP-43)

`DiffReview` 가 `hasRequest: boolean` 을 props 로 받아 첫 분기로 쓴다.

```
hasRequest === false          → chat.rightpanel.diffNotSynced   (신규)
patch === null                → chat.rightpanel.diffFileLoading (기존)
!patch.isRepo / unavailable / sections 0 → 기존 셋
```

**5) 컨텍스트 바** (`sessionChangesData.ts` · `GitContextBar.tsx`, MD-23)

```ts
export interface ComparisonLabel { base: string; head: string | null }
export function summaryComparisonLabel(
  summary: GitDiffSummary | null, status: GitStatus | null, tr: (k: MessageKey) => string
): ComparisonLabel
```

`head` 는 `status.branch`(detached 면 `null`). 렌더는 `base`, `→`, `head` 세 조각을 **이 순서로** 낸다. `head === null` 이면 뒤 둘을 그리지 않는다. `summaryBaseText` 는 남긴다 — 메뉴의 `{{base}} 대비` 가 그것을 쓴다.

**6) 비교 기준 메뉴** (`GitContextBar.tsx`, D-106·D-107)

- 첫 화면 2행: `모든 변경사항`(우측에 `{{base}} 대비` + 체크) · `커밋 ›`(`aria-haspopup="menu"`).
- 커밋 서브메뉴는 기존 `Popover` 를 anchor 를 바꿔 한 번 더 쓴다 — 새 primitive 를 만들지 않는다(D-092 승계).
- `미커밋 변경` 행과 `ScopeRow` 의 `uncommitted` 항목, `DiffComparison` 의 `uncommitted` 멤버, `diffSections` 의 그 분기를 지운다. `reconcileComparison` 은 `kind !== 'commit'` 조기 반환이라 그대로 성립한다.
- `diffViewMenuItems.ts` 에서 `refresh` 항목과 `DiffViewMenuAction` 의 `{kind:'refresh'}` 를 지워 **일곱**으로 만든다.

**7) 파일 헤더 한 줄 + `↗`** (`FileDiffSection.tsx`, D-108)

- 헤더를 `items-center` 한 행으로 바꾸고 이름·부모 경로를 형제 `<span>` 으로 나란히 둔다(부모는 `truncate text-caption text-t5`).
- `↗` 는 헤더 `<button>` **안에 중첩하지 않는다** — 중첩 버튼은 무효 HTML 이다. 헤더 전체를 감싸던 `<button>` 을 `<div>` 로 바꾸고 접기 토글을 이름 영역 버튼으로, `↗` 를 형제 버튼으로 둔다.
- 절대 경로는 `state.cwd` + `section.path` 로 만든다(POSIX `/` 구분자를 그대로 넘긴다 — main 이 `resolve` 한다).

**8) main 파일 reveal** (`shared/protocol.ts` · `handlers/files.ts`, AR-15 · EP-44)

```ts
OpenPathRequestSchema = z.object({ path: z.string(), mode: z.enum(['directory', 'reveal']) })
```

`reveal` 분기: `stat` 이 **파일**인지 확인 → `dirname(path)` 로 기존 판정(`isWithinDir(projectsDir()) ∨ hasSessionWithCwd`)을 **그대로 재사용** → `shell.showItemInFolder(path)`. 판정 함수를 새로 쓰지 않는다.

**9) 이동 소유자 주입** (`DiffReview.tsx`, MD-27 · EP-36 ③ · D-110)

```ts
interface DiffReviewProps { /* … */ scrollOwnerRef?: React.RefObject<FileSectionOwner | null> }
```

기본값은 내부 `useRef` 다. `pickFile` 은 `revealFileSection(ownerRef.current, path)` 를 부르고, 테스트는 `{ current: fakeOwner }` 를 주입해 **첫 인자**를 단언한다.

### ΔV6 — AS-IS(=ΔV5 구현물) → TO-BE (2026-09-03)

```text
AS-IS ── 싱크 계기가 result 파생이고 범위가 작업 트리까지다

  SDK result ──► claudeToNormalized ──► telemetry ──► 리듀서 inflight=false
                                                        │
                                              sessionBusy: true→false
                                                        │
                                        useGitSnapshot(busy 전이) ──► diffSummary
                                                                          │
                          git diff <base>            ◄──────────────── 요약/패치
                          + ls-files --others            (base → 작업 트리 + 미추적)

TO-BE ── 싱크 계기가 Stop hook 이고 범위가 커밋된 것까지다

  claude Stop hook ──► makeTurnEndHook 콜백
        │                    │  (즉시 {} 반환 — git 을 기다리지 않는다)
        │                    └──► turnEnded 버퍼 +1
        │
  다음 SDK 메시지 경계 ──► eventBatches drain ──► `turn.ended` NormalizedEvent
                                                        │
                                          리듀서: turnEndTick += 1
                                                  gitRowClosedAtTick 해제 판정
                                                        │
                                        useGitSnapshot(tick 전이) ──► diffSummary
                                                                          │
                          git diff <base> HEAD      ◄──────────────── 요약/패치
                          (미추적 조회 없음)               (커밋된 것만)
```

#### ΔV6 AS-IS → TO-BE Delta

| 축 | AS-IS | TO-BE | 근거 |
|---|---|---|---|
| 싱크 계기 | `sessionBusy` 전이 (result 파생) | `turnEndTick` 증가 (Stop hook 파생) | D-115 |
| hook 등록 | 프로덕션 핸들러 0건 | 어댑터 내부 `Stop` 조각 1개 | D-115 |
| 비교 범위 | `git diff <base>` + `ls-files --others` | `git diff <base> HEAD` | D-111 |
| 커밋 0 세션 | 미커밋 변경이 목록에 뜬다 | `표시할 변경 사항이 없습니다.` | D-112·D-113 |
| 컴포저 행 | 항상 보인다 | `×` 로 닫고 다음 tick 에 복귀 | D-114 |
| 컨텍스트 라벨 | 항상 `기준 → 현재` | 모드별 두 모양 | D-116 |
| 사이드바 진입 | `폴더` 버튼 하나(상태 표시 없음) | 세그먼트 둘(선택된 쪽 채움) | D-117 |
| 선택 표시 | `text-accent` + 체크 / `border-accent` | 채움(`fill-selected`) | D-119 |

### ΔV6 추가 강제 지점

| V node / pair | 계약/필드 | SSOT | 누가 | 언제 강제 | 실패 의미 |
|---|---|---|---|---|---|
| AR-16 · SD-13 / VP-72 | 턴 종료 신호가 **Stop hook 에서 시작해 renderer 계기까지 닿는다** (EP-46, **4지점**: ① 어댑터의 `Stop` hook 조각 등록 ② `turnEnded` 버퍼의 drain ③ `turn.ended` NormalizedEvent 정의 ④ 리듀서의 `turnEndTick` 증가) | `claude.ts` hook 조각 + `ipc.ts` variant + `chatReducer` | 구현자 | 턴 종료마다 | 하나라도 빠지면 **화면이 영영 갱신되지 않는다** — 옛 `busy` 계기를 함께 지우기 때문에 폴백이 없다. ①만 빠지면 hook 이 등록되지 않아 조용히 0회, ④만 빠지면 이벤트는 오는데 아무도 세지 않는다 |
| R-64 / VP-72 | 옛 계기(`busy`)가 **남지 않는다** (EP-46 ⑤ 아님 — EP-42 ③ 을 갱신: `useGitSnapshot` 의 요약 effect deps 에서 `busy` 가 빠진다) | `useGitSnapshot.ts` | 구현자 | 매 렌더 판정 | 옛 계기를 남긴 채 새 계기를 덧붙이면 **턴마다 2회** 조회하고, 사용자가 없애라고 한 자리가 그대로 산다. 게이트는 초록이다(둘 다 조회를 만든다) |
| AR-17 · R-65 / VP-73 | 비교 범위는 **한 함수**가 정하고 두 소비자가 그것만 쓴다 (EP-47, **3지점**: ① `diffRevArgs` 산출이 `[base, 'HEAD']` ② 요약의 미추적 인자 제거 ③ 패치의 미추적 병합 제거) | `git-diff.ts` `diffRevArgs` | 구현자 | 조회마다 | ①만 고치면 미추적이 여전히 병합돼 목록에 남고, ②③ 만 고치면 커밋하지 않은 **추적** 변경이 남는다 — 둘은 서로 다른 결함이라 하나로 덮이지 않는다 |
| R-63 · SD-14 · MD-29 / VP-71 | 컴포저 행의 노출 판정은 **한 함수**가 하고 닫힘은 tick 으로 푼다 (EP-48, **2지점**: ① `gitRowView` 의 판정식 ② 리듀서의 `turn.ended` 처리가 닫힘 표식을 해제) | `gitRowState.ts` + `chatReducer` | 구현자 | 매 렌더 · 턴 종료마다 | ②가 빠지면 한 번 닫은 행이 **영영 돌아오지 않는다**. 호출부가 `closed && <GitRow/>` 로 거르면 판정자가 둘이 되어 0206 EP-05 가 막던 자리가 다시 열린다 |
| R-66 / VP-74 | 빈 상태 문구는 **카탈로그 키 하나**이고 소비처 넷이 같은 키를 쓴다 (EP-49, **2지점**: ① `ko`/`en` 값 ② 소비처 4곳이 새 키를 만들지 않음) | `i18n/resources` | 구현자 | 매 렌더 | 자리마다 키를 새로 만들면 “빈 커밋” 화면과 “빈 트리” 화면이 다른 말을 하고, 사용자가 지정한 문구가 일부 자리에만 뜬다 |
| R-67 · MD-30 / VP-75 | 라벨 파생은 **판별 유니온**을 낸다 (EP-50, **2지점**: ① 파생 함수의 반환 타입 ② `GitContextBar` 의 두 갈래 렌더) | `sessionChangesData.ts` | 구현자 | 매 렌더 | 반환을 `{base, head}` 한 모양으로 두고 렌더에서만 갈라면 `commit` 모드가 `sha → 브랜치` 같은 **없는 비교**를 그린다 |
| R-68 / VP-76 | 세그먼트 둘은 **하나의 상태**를 반대로 읽는다 (EP-51, **2지점**: ① 두 버튼의 `pressed` 가 `sidebarVisible` 과 `!sidebarVisible` ② 액션이 `toggle` 이 아니라 `set(boolean)`) | `GitContextBar` + `chatReducer` | 구현자 | 클릭마다 | `toggle` 을 남겨 두면 이미 선택된 세그먼트를 눌렀을 때 **반대로 넘어간다** — 세그먼트는 멱등이어야 한다 |
| R-69 · MD-31 / VP-77 | 실측 토큰은 **표 한 곳**에서 오고 옛 클래스가 함께 남지 않는다 (EP-27 ④ 로 편입, 기존 3지점 + **1**) | §8 ΔV6 실측표 | 구현자 | 매 렌더 | 새 클래스를 더하고 옛 클래스를 지우지 않으면 Tailwind 규칙 순서가 결과를 정해 **화면은 맞아 보이는데 계약이 둘**이다 — 다음 라운드에 어느 쪽을 고칠지 아무도 모른다 |

- **ΔV6 의 상호배타 상태.** 세그먼트 둘은 `sidebarVisible: boolean` **하나**가 정한다 — 두 개의 `pressed` 플래그를 따로 두면 둘 다 켜지거나 둘 다 꺼진 불가능 조합이 타입에서 허용된다.
- **ΔV6 의 선택적 필드 의미.** `turn.ended` 는 `{ type, sessionId }` **둘만** 갖는다. 페이로드를 붙이면 renderer 가 그것을 읽기 시작하고 hook 이 조회 결과를 실어야 하는데, 그것이 D-115 가 없애려는 동기 대기다.
- **ΔV6 의 저장소 쓰기 — 늘지 않는다.** DB·파일 어느 쪽도 새로 쓰지 않는다. `turnEndTick`·`gitRowClosedAtTick` 은 세션 메모리 상태이고 영속하지 않는다(D-087 과 같은 축).
- **ΔV6 이 만드는 새 표면의 실패 모드.** ① **hook 이 오지 않는 백엔드**에서는 목록이 영영 미싱크다 — 등록된 어댑터가 claude 하나라 지금은 발생하지 않지만, opencode 가 붙는 순간 그 어댑터가 `turn.ended` 를 내야 한다는 것을 `AR-16` 계약으로 적는다. ② **커밋 없이 일하는 세션**은 패널이 계속 비어 보인다 — 그 화면이 “고장” 으로 읽히지 않게 D-113 의 문구가 “표시할” 을 포함한다(있는데 못 보여주는 게 아니라 보여줄 것이 없다). ③ 컴포저 행을 닫은 직후 사용자가 변경량을 다시 보려면 **턴을 한 번 돌려야 한다** — 사용자가 그 수명을 명시적으로 골랐다(D-114).

### ΔV6 구현 설계

| 변경/신규 파일 | 책임 | 변경 내용 | 테스트 seam |
|---|---|---|---|
| `app/src/shared/ipc.ts` | 이벤트 계약 | `NormalizedEvent` 에 `{ type: 'turn.ended'; sessionId: string }` variant 추가 | 타입 |
| `app/src/main/adapters/claude-adapt.ts` | hook 조각 | `makeTurnEndHook(onTurnEnd: () => void)` 신설 — `{ hooks: { Stop: [{ hooks: [cb] }] } }`. 콜백은 `onTurnEnd()` 호출 후 **즉시** `{ continue: true }` 를 돌려준다(await 없음) | 순수 — 조각 반환값과 콜백 동작 |
| `app/src/main/adapters/claude.ts` | 배선 | `turnEnded` 카운터 버퍼 + `mergeHooks(...)` 에 `makeTurnEndHook` 추가 + `drainTurnEnded()` 를 `eventBatches` 의 메시지 경계와 종료 시 드레인(=`drainCompactSummaries` 와 같은 자리) | 통합 — fake SDK 메시지 스트림 |
| `app/src/renderer/src/features/chat/reducer/chatReducer.ts` | 세션 상태 | `turnEndTick: number` + `gitRowClosedAtTick: number \| null` 추가. `turn.ended` → tick +1. `CLOSE_GIT_ROW` → `gitRowClosedAtTick = turnEndTick` | 순수 리듀서 |
| `app/src/renderer/src/features/chat/store/chatStore.ts` | 액션 | `chatActions.closeGitRow` · `setDiffSidebarVisible(bool)` 추가. 이벤트 스위치에 `turn.ended` 분기 | 순수 |
| `.../composer/useGitSnapshot.ts` | 계기 | `gitSummaryQueryReason` 의 입력을 `{busy}` → `{tick}` 으로. **요약 effect 의 deps 에서 `busy` 제거**. 이름 축(`gitStatusQueryReason`)은 `busy` 대신 `tick` 을 셋째 계기로 쓴다 | 순수 + 렌더 |
| `.../composer/gitRowState.ts` | 노출 판정 | `gitRowView(..., closedAtTick, tick)` — `closedAtTick !== null && closedAtTick === tick` 이면 `visible:false` | 순수 |
| `.../composer/GitRow.tsx` | 컴포저 행 | 변경량 버튼 **뒤**에 `Button iconOnly leadingIcon="x" size="small"` 추가. `Notice` 와 같은 primitive·같은 자리 | SSR 렌더 |
| `app/src/main/infra/git/git-diff.ts` | 비교 범위 | `diffRevArgs` 가 `worktree-base`·`head` 에서 `[oid, 'HEAD']` 를 낸다. 요약·패치의 `readUntracked` 호출 **2건**과 `untrackedPatchFiles` 제거 | main 통합(임시 저장소) |
| `.../rightpanel/sessionChangesData.ts` | 라벨 파생 | `summaryComparisonLabel(summary, status, comparison, tr)` → `{kind:'range'; base; head} \| {kind:'commit'; sha; subject}` 판별 유니온 | 순수 |
| `.../rightpanel/GitContextBar.tsx` | 헤더 | 세그먼트 둘(`data-diff-sidebar-toggle="off"|"on"`) · 모드별 라벨 렌더 · 제목 타이포를 `text-[13px] text-t7`(serif·semibold 제거) | SSR 렌더 |
| `.../rightpanel/ChangedNavigationSidebar.tsx` | 사이드바 | 폭 `w-[25%]` · `ScopeRow`·커밋 카드의 선택 표시를 `bg-fill-selected` 채움으로 · 커밋 카드 상시 테두리 제거 | SSR 렌더 |
| `.../rightpanel/FileDiffSection.tsx` | 파일 헤더 | 헤더 행에 `bg-bg2` 밴드 부여(실측 5) | SSR 렌더 |
| `app/src/renderer/src/shared/i18n/resources/{ko,en}.ts` | 문구 | `diffEmpty` 값 교체 · `gitRow.close` · `diffFilesOff`/`diffFilesOn` 추가 · `diffShowFiles` 제거 | `resources.test.ts` |
| `docs/IPC_CONTRACT.md` | 계약 문서 | `orca:chat:event` 행에 `turn.ended` variant | 문서 diff |

#### ΔV6 구현 주의

- **`turn.ended` 는 terminal 이 아니다.** `session-runtime.ts:61`·`turn-coordinator.ts:307`·`chatStore.ts:280` 의 세 판정은 열거형이라 손대지 않는다 — 손대면 턴이 조기 종료된다.
- **이력에 쓰지 않는다.** `writer.ts` 는 `default` 절이 없어 미지정 type 을 무시한다(케이스 9종) — `turn.ended` 를 케이스에 추가하지 않는 것이 그대로 계약이다.
- **`gitStatusQueryReason` 의 셋째 계기도 `tick` 으로 옮긴다.** `busy` 를 한쪽에만 남기면 두 축이 다시 다른 계기를 쓰고, ΔV5 D-101 이 잠근 “두 함수가 다른 값을 낸다” 는 유지되지만 **출처가 갈린다**. 두 함수 모두 `tick` 을 읽되 이름 축은 `initial`·`identity` 를 계속 갖는다.
- **`toggleDiffSidebar` 는 남기지 않는다.** 세그먼트가 멱등이려면 액션이 `set(boolean)` 이어야 한다 — 토글 액션을 남기면 다음 구현자가 그것을 다시 쓴다.
...
#### ΔV4 기존 소비처 전수 (2026-09-02)

**`sessions` 표에 컬럼이 하나 는다.** 그 표를 읽는 프로덕션 지점을 전수로 확인한다.

| 소비처 | 검색 | 영향 |
|---|---|---|
| `insertSession` | `queries.ts:115`(prepare) · `:480`(run) | **변경** — 같은 문장에 `baseline_ref` 한 열을 더한다. `SessionInsert` 타입이 필수 필드를 늘리므로 호출부 누락은 typecheck 가 잡는다 |
| `getSessionBaseline` | `queries.ts:147`(prepare) · `:504`(run) | **변경** — `string \| null` 에서 `{ oid: string \| null; ref: string \| null }` 로. 유일 호출부는 `handlers/git.ts` 의 `baseOidFor` 하나다 |
| `SELECT ... FROM sessions` 그 밖 | `rg 'FROM sessions' app/src/main` | **비영향** — 목록·검색·계보 조회는 열을 **이름으로** 고르므로 새 컬럼이 늘어도 산출이 같다. `SELECT *` 는 저장소 전체에 **4건**이고 전부 다른 표다(`managed_worktrees` 2 · `turn_usage` 1 · `turn_model_usage` 1) — `sessions` 에 `SELECT *` 는 **0건** |
| `sessions` 행 → `SessionListItem` | `shared/ipc.ts` | **비영향** — 사이드바 카탈로그 DTO 에 기준선 축이 없다(추가하지 않는다) |

**IPC 채널이 하나 나가고 하나 들어온다.** 등록·배선 지점은 EP-30(제거 4자리)·EP-29 ①(신설 4자리 = 계약·zod·preload·handler)이 전수를 갖는다. 채널 이름을 문자열로 부르는 곳은 `preload/index.ts` 뿐이고(`ipcRenderer.invoke(CHANNELS.x)`) typecheck 가 잡지 못하므로 **그 파일이 EP-30 의 명시 지점**이다.

**`PrepareWorktreeResult.managed` 에 필드가 하나 는다.** 그 결과를 읽는 프로덕션 지점은 `prepare-worktree.ts:92` 한 곳이고(`rg 'kind === .managed.' app/src/main`), 타입이 필수 필드를 늘리므로 누락은 typecheck 가 잡는다 — D-051 이 `baseOid` 를 더할 때와 같은 자리다.
...
- **패치의 수명은 요약 세대**다. `RECEIVE_GIT_SNAPSHOT_SUMMARY` 가 `patch` 를 `null` 로 만들고, 타일이 열려 있으면 `useGitPatch` 가 곧바로 1회 다시 받는다. 닫혀 있으면 다음 열기까지 조회가 없다(EP-34).
- **세션·cwd 전환은 요약과 함께 접힌다.** `SET_CWD`·`NEW_CHAT`·`START_LOAD_SESSION` 이 이미 `gitSnapshot` 슬라이스를 초기화한다(`chatReducer.ts:901`~) — 그 자리에 `patch`·`comparison`·`collapsedFiles` 를 함께 넣는다. 새 초기화 지점을 만들지 않는다.
- **늦은 응답은 세대로 버린다.** 패치 요청도 요약과 같은 세대 소유자 형태를 쓴다 — 세션을 옮긴 뒤 도착한 이전 저장소의 패치가 새 화면에 닿지 않는다.
- **취소는 없다.** git 조회는 15s 타임아웃 안에서 끝나고 실패는 값(`unavailable`)으로 온다 — 새 abort 경로를 만들지 않는다.
- **표시 옵션·비교 모드는 조회 수명과 무관**하다(D-088). 전환은 전부 순수 파생이라 정리할 자원이 없다.
...
| 축 | ΔV3 (현재) | ΔV4 | 근거 |
|---|---|---|---|
| 타일을 열고 파일 N 개를 다 보는 데 드는 IPC | `1`(요약) + `N`(본문, 캐시 miss 마다) | **`1`(요약) + `1`(패치)** | D-074·D-078 — 파일 수와 무관 |
| 같은 조회의 git 프로세스 | 요약 3(좌표 캐시 후) + 본문 파일당 3~4 | 요약 3 + **패치 1**(폴백 시 2) | EP-29 ② |
| renderer CPU | 파일마다 `buildDiffLines` O(n·m) | **0** — 줄 배열이 이미 파싱돼 온다 | D-075 |
| 패치 페이로드 worst-case | — | **약 16 MB**(전체 줄 200,000 × 80 B) | §10 ΔV4 worst-case |
| 상태 메모리 | 본문 LRU 12 × 최대 2 MiB | 패치 1벌(위 상한) | D-094 — 사본이 하나다 |
| 재열기 | LRU 적중이면 0, 아니면 파일당 1 | **0**(패치가 세션 상태에 있다) | D-078 |

- **전문맥의 대가를 명시한다.** `-U1000000` 은 변경 파일의 **전체 내용**을 싣는다 — 변경 줄만 받는 `-U3` 보다 페이로드가 크다. 그 대가로 문맥 확장이 **재조회 0** 인 순수 파생으로 남고(R-30 승계), 파일마다 다시 묻는 경로가 사라진다. 상한 셋이 그 대가의 천장이다.
- **요약은 무거워지지 않는다.** 패치를 요약에 합치지 않는 이유가 이것이다 — 요약은 컴포저 git 행이 매 턴 읽으므로(D-025) 거기 전문맥을 실으면 타일을 열지 않아도 매 턴 그 비용을 낸다(D-078).
...
#### ΔV4 — 기존 결정·규칙과의 관계 (2026-09-02)

| 기존 결정 / 규칙 | 어디에 있나 | 이번 본문 | 판정 |
|---|---|---|---|
| 0206 D-013 “설정·펼치기·이동 핸들 금지” | `DiffTileHeader.tsx:9` | D-086 · D-091 | **변경** — 컨텍스트 바가 설정(`⋮`)·펼치기(파일 접기/펼치기)·확대(`↗`)를 **전부** 갖는다. 그 결정의 이유는 “대응 동작이 없는 장식 금지”였고 여기서는 셋 다 실재 동작이 있다 |
| 0206 D-010 “좌측 트리 + 커밋 목록” | `DiffTileContent.tsx` | D-083 · D-084 | **되살림** — D-037 이 지웠던 3영역의 *탐색* 축이 제안서 §8 로 돌아온다. 다만 **기본 숨김**이고 파일 항목 영역은 없다(두 구획) |
| 0206 D-017 “파일 항목은 기본 접힘” | — | D-074 | **변경** — 사용자가 “전부 펼침”을 골랐다. 그 결정의 이유(“열자마자 저장소 전체를 읽는다”)는 **패치 1회**가 대신 지킨다(D-078) |
| 0206 D-019 “View 는 props 만 읽는다” | `DiffTable.tsx:4` | §11 ΔV4 신규 컴포넌트 4건 | **유지** — `GitContextBar`·`ChangedNavigationSidebar`·`DiffReview`·`FileDiffSection` 을 View + 컨테이너로 가른다 |
| 0211 D-016 “본문 1 MiB” | 본 plan §3 | D-077 | **변경** — 본문 채널이 사라져 그 상한의 주어가 없다. 파일당 줄 50,000 이 같은 자리를 받는다 |
| ΔV1 D-031 “조회 소유자는 한 훅” | 본 plan §3 | D-078 · §10 EP-34 | **유지 + 분할** — 요약 소유자는 `useGitSnapshot` 그대로이고 패치 소유자는 `useGitPatch` **하나**다. 같은 조회에 소유자가 둘이 되지 않는다 |
| ΔV2 D-036 “diff 범위는 하나” | 본 plan §3 | D-079 · §10 EP-35 | **유지** — 사용자가 “목록만 좁히고 diff는 세션 기준 유지”를 골랐다. `commit` 인자를 되살리지 않는다 |
| ΔV2 D-035 “미추적 완전 제외” | 본 plan §3 | D-081 | **유지 + 문구 제거** — 조회·목록·수치의 제외는 그대로이고 그 사실을 알리던 화면 문구만 사라진다 |
| ΔV3 D-062~D-064 “한 호출 · 좌표 캐시 · 잠금 회피” | 본 plan §3 | D-076 · §10 EP-25 | **유지 + 확장** — 새 패치 조회도 같은 `run()` 관문을 지나 `--no-optional-locks` 를 받는다. AT-39 의 차집합 분모에 호출 1건이 는다 |
| ΔV3 D-066 “형제 primitive” | 본 plan §3 | D-085 · AT-41 | **유지 + 확대** — 대상이 두 컴포넌트에서 신규 4 + 변경 2 로 는다 |
| 마이그레이션 append-only | `scripts/check-migrations-appendonly.mjs` | D-070 | **유지** — 기존 0001~0019 를 수정하지 않고 `0020` 을 더한다 |
| `docs/IPC_CONTRACT.md` 의 `orca:git:diffFile` 행 | 그 문서 §git | D-075 | **교체 필요** — 채널이 나가고 `orca:git:diffPatch` 가 들어온다. 같은 커밋에서 고친다(§6 docs/AGENTS 규칙) |
...
| 리스크 | 완화 | 남는 것 |
|---|---|---|
| 전문맥 패치가 크다 — 변경 파일 전체 내용을 싣는다 | 상한 셋(파일 200 · 파일당 줄 50,000 · 전체 줄 200,000) + 전용 버퍼 16 MiB + `--unified=3` 폴백(EP-29·EP-31) | 상한을 넘는 파일은 `+N −M` 만 보인다 — **그 파일의 diff 를 제품에서 볼 수 없다**(현재는 1 MiB 까지 볼 수 있었다) |
| “미추적 제외” 문구가 사라진다(D-081) | 제외 정책 자체는 D-035 로 유효하고 AT-24 가 계속 잠근다 | 에이전트가 만든 새 파일이 커밋 전까지 안 보이는데 화면이 그 이유를 말하지 않는다 — **“변경 없음”과 구분되지 않는다** |
| `미커밋 변경`을 제안서에 없는 자리에 더한다(D-082) | 사이드바 범위 항목 하나라 되돌리기 비용이 낮다 | 사용자가 “첨부와 똑같이”를 엄격히 읽으면 빼야 한다 — **빼면 미커밋만 보는 범위가 제품에서 사라진다** |
| 마이그레이션 `0020` 이 되돌릴 수 없다 | 컬럼 추가라 기존 행에 `NULL` 이 들어가고 D-071 이 그 상태를 화면까지 정의한다 | 컬럼 자체는 append-only 라 제거하려면 다음 마이그레이션이 필요하다 |
| 렌더 테스트 **5파일 14케이스**를 **삭제**한다 | 대상(peek·본문 캐시)이 사라져 그 케이스가 잴 것이 없다. 각 케이스가 잠그던 축은 AT-45(연속 화면)·AT-46(조회 1회)로 **다시 유도했다**(§7 ΔV4 주의사항) | 삭제 자체가 회귀 덮개가 될 수 있다 — verify 가 “이전 라운드 red 변이 재실행”으로 확인한다 |
| ΔV3 이 미검증인 채 UI 가 교체된다 | 백엔드 축(D-062~D-064)은 그대로 살아 VP-48 이 REGRESSION 으로 닫는다. UI 축(D-065~D-067)은 VP-58·VP-50 이 새 컴포넌트에서 다시 센다 | ΔV3 UI 구현물 자체는 검증 없이 대체된다 — 그 코드가 남기는 회귀는 없다(파일이 삭제된다) |
...
**main** (12): `infra/db/migrations/0020_session_baseline_ref.sql`(신규) · `infra/db/queries.ts` · `infra/git/repository.ts` · `infra/git/git-diff.ts` · `infra/git/git-diff-parse.ts` · `features/worktrees/service.ts` · `app/chat-turn/prepare-worktree.ts` · `app/chat-turn/send.ts` · `app/chat-turn/turn-context.ts` · `contracts/turn.ts` · `features/history/writer.ts` · `app/handlers/git.ts`.

**shared / preload** (3): `shared/ipc.ts` · `shared/protocol.ts` · `preload/index.ts`.

**renderer — 신규 프로덕션** (11): `lib/diffPatchLines.ts` · `lib/diffDisplay.ts` · `lib/changedFileTree.ts` · `hooks/useGitPatch.ts` · `rightpanel/GitContextBar.tsx` · `rightpanel/ChangedNavigationSidebar.tsx` · `rightpanel/DiffReview.tsx` · `rightpanel/FileDiffSection.tsx` · `rightpanel/diffComparison.ts` · `rightpanel/diffPanelWidth.ts` · `rightpanel/diffViewMenuItems.ts`.

**renderer — 신규 테스트** (6): `rightpanel/diffComparison.test.ts` · `rightpanel/diffViewMenu.test.ts` · `rightpanel/diffPeekRemoved.test.ts` · `hooks/gitPatchQuery.test.ts` · `lib/changedFileTree.test.ts` · `lib/diffDisplay.test.ts`.

**renderer — 변경** (7): `lib/diffHunks.ts` · `rightpanel/DiffTileContent.tsx` · `rightpanel/sessionChangesData.ts` · `rightpanel/tileRegistry.ts` · `reducer/chatReducer.ts` · `store/chatStore.ts` · `shared/api/ipc.ts`.

**renderer — 삭제** (14 = 소스 7 + 테스트 7): 소스 `rightpanel/DiffPeek.tsx` · `rightpanel/DiffTileHeader.tsx` · `rightpanel/SessionChangesList.tsx` · `rightpanel/diffBodyCache.ts` · `rightpanel/diffFileCache.ts` · `rightpanel/diffRequirementBridge.ts` · `rightpanel/peekNavigation.ts`. 테스트 `rightpanel/DiffTileContent.bridge.test.ts` · `rightpanel/diffPeek.render.test.ts` · `rightpanel/diffPeekRequest.test.ts` · `rightpanel/peekNavigation.test.ts` · `rightpanel/diffBodyCache.test.ts` · `rightpanel/diffFileCache.test.ts` · `rightpanel/sessionChangesList.render.test.ts`.

**main — 삭제** (1): `infra/git/git-diff-service.test.ts`. **이 파일이 잠그던 계약은 사라지지 않았다** — 좌표 캐시·log 폴백·전용 버퍼 축은 `infra/git/git-diff.test.ts` 가 받는다(r2, D5·D6).

**i18n** (2): `shared/i18n/resources/ko.ts` · `en.ts`.
...
**main** (2): `app/handlers/files.ts`(reveal 분기) · `shared/protocol.ts`(`OpenPathRequestSchema` 에 `mode`).

**shared / preload** (2): `shared/ipc.ts`(`OpenPathRequest` 타입) · `preload/index.ts`(형 전달만).

**renderer — 변경** (9): `composer/useGitSnapshot.ts`(계기 두 함수 · 포커스 구독 삭제) · `hooks/useGitPatch.ts`(가드 상태 기계) · `reducer/chatReducer.ts`(REFRESH 삭제 · `expandedFiles` 반전) · `store/chatStore.ts`(액션 개명·삭제) · `rightpanel/DiffReview.tsx`(미싱크 분기 · 안내 문구 · 소유자 prop) · `rightpanel/DiffTileContent.tsx`(`hasRequest` 전달 · 펼침 반전) · `rightpanel/GitContextBar.tsx`(라벨 두 값 · 메뉴 2행 + 서브메뉴) · `rightpanel/ChangedNavigationSidebar.tsx`(`미커밋` 스코프 행 삭제) · `rightpanel/FileDiffSection.tsx`(헤더 1줄 + `↗`).

**renderer — 변경(작음)** (3): `rightpanel/sessionChangesData.ts`(비교 라벨 쌍 추가) · `rightpanel/diffComparison.ts`(`uncommitted` 멤버·분기 삭제) · `rightpanel/diffViewMenuItems.ts`(`refresh` 항목 삭제 → 7).

**renderer — 신규 테스트** (3): `composer/gitQueryReason.test.ts`(두 함수 대조) · `hooks/gitPatchGuard.test.ts`(교착 3단계) · `rightpanel/diffSyncState.render.test.ts`(미싱크/로딩 두 문구).

**테스트 — 변경** (6): `composer/gitSnapshotQuery.test.ts` · `store/chatStore.test.ts` · `reducer/chatReducer.plan.test.ts` · `rightpanel/diffViewMenu.test.ts` · `rightpanel/diffReviewNavigation.test.ts`(소유자 주입) · `rightpanel/diffTile.render.test.ts`.

**main — 변경 테스트** (1): `app/handlers/files.test.ts`(reveal 허용 1 + 거부 2 + 디렉토리 회귀 1).

**i18n** (2): `shared/i18n/resources/ko.ts` · `en.ts` — 신규 3 · 삭제 1 · 값 변경 1.
...
- **ΔV4 추가**: 게이트 집합이 **늘어난다**. ① 마이그레이션 `0020` 으로 append-only 게이트가 다시 적용되고 인벤토리의 `DB 마이그레이션 19 → 20` + 목록에 `0020_session_baseline_ref` 가 는다. ② 채널이 하나 나가고 하나 들어와 **총계 82·도메인 `git 5` 는 불변**이고 목록 문자열만 바뀌지만, `docs/IPC_CONTRACT.md` 의 행 교체가 필요하다 — `check-doc-inventory.mjs --check` 차이 0 이 그 짝이다. ③ DB 스위트가 는다(VP-52 가 임시 DB + 임시 저장소) — `npm test` 경로이고 ABI 미준비 환경에서는 그 red 를 기준선으로 분리 보고한다. ④ 사람 실기 **3 → 4**: ΔV3 의 ②③은 대상 화면이 사라져 ΔV4 의 새 항목이 대체하고, D-060 의 scroll 보정 1건은 그대로 남는다. 새 목록은 — ① (D-060 · AT-48) 위쪽 문맥을 펼친 뒤 보던 줄이 화면에 남는지 ② (**ΔV4** · AT-45) 파일 섹션이 연속으로 이어질 때 파일 경계가 읽히는지 ③ (**ΔV4** · D-092 · AT-50) 사이드바 열기/닫기 연출이 끊기거나 튀지 않는지 ④ (**ΔV4** · D-067 · AT-41) 새 패널을 형제 타일과 나란히 놓고 이질감이 없는지.
- **ΔV5 추가**: 게이트 집합이 **줄지도 늘지도 않는다**. ① 마이그레이션·채널 신설이 0이라 append-only 게이트는 무변경 통과이고 인벤토리는 **차이 0** 이어야 한다(채널 82 · 마이그레이션 20 불변) — 다만 `docs/IPC_CONTRACT.md` 의 `orca:files:openPath` 행에 모드와 허용 경로 규칙을 적는다. ② DB 스위트가 **늘지 않는다** — 신규 pair 8건이 순수·SSR 렌더·임시 디렉토리라 `./node_modules/.bin/vitest run` 으로 전부 판정된다(ABI 중립). ③ i18n 은 신규 3 · 삭제 1 · 값 변경 1 이라 `resources.test.ts` 의 키 대칭이 그것을 잡는다. ④ 사람 실기 **4 → 6**: ΔV4 의 ②(연속 스크롤 파일 경계)는 기본 접힘으로 성격이 바뀌어 ΔV5 ①이 대체하고, 나머지 셋은 유지된다. 새 목록은 — ① (D-060 · AT-48) 위쪽 문맥 확장 후 보던 줄이 남는지 ② (**ΔV5** · D-105 · AT-66) 전부 접힌 목록에서 파일 목록이 한눈에 읽히는지 ③ (ΔV4 · D-092 · AT-50) 사이드바 연출이 끊기지 않는지 ④ (ΔV4 · D-067 · AT-41) 형제 타일과 나란히 놓고 이질감이 없는지 ⑤ (**ΔV5** · D-104 · AT-65) `기준 → 현재` 라벨이 좁은 폭에서 잘리는 방식이 납득되는지 ⑥ (**ΔV5** · D-102 · AT-63) 미싱크 문구가 "고장" 으로 읽히지 않는지.
...
## READY self-review — ΔV4 (2026-09-02)

- [x] 여러 턴의 결정이 Decision Ledger에 보존됐다 — 신설 **26**(D-069~D-094) · `SUPERSEDED` **10**(D-030·D-037·D-039·D-042·D-043·D-046·D-056·D-061·D-065 + D-058 방향 축 확장) · `OPEN` **0**. 전부 사용자 명시 변경이다.
- [x] Product & UX Contract가 Technical Design보다 앞에 있다 — §5 ΔV4 전이표 8행과 §6 ΔV4 범위/비범위가 구현 방식 없이 완료 상태를 설명한다.
- [x] 조건절·이유절을 재해석하지 않았다 — 사용자 4답을 **원문 그대로** §2 ΔV4 표에 인용했다(“우측화살표+우측값 표시하지 말것” · “목록만 좁히고 diff는 세션 기준 유지” · “전체 조회로 1회만 실행” · “전부 제거”).
- [x] 물어야 할 제품 결정과 조사로 닫을 사실을 구분했다 — 비교 기준 라벨·커밋 모드 의미·기본 펼침·요약 행선 **4건**을 물었고, 브랜치 이름 저장 위치·아이콘 존재·패널 최대화 유무·gap 확장 방향은 **조사로 닫았다**(§8 ΔV4 전수 조사 18행).
- [x] 수치·전칭·외부 규약·앵커·기존 테스트 인용을 실측했다 — `gitApi.diffFile` **1건** · `gitApi.diffSummary` **1건**(주석 제외) · 화면 분기 **4건** · `uncommitted` 소비처 **9건** · `baseline_ref` **0건** · 패널 최대화 **0건** · 영향 테스트 **10파일 40케이스**.
- [x] 저장소 규칙을 설계 입력으로 확인했다 — append-only 가드(D-070) · doc-inventory(§7-A gate 차분) · `IPC_CONTRACT` 동시 갱신(§16) · 그룹 스코프 격리(`group/<이름>`, D-085) · main 레이어 DAG(신규 파서는 `infra/git`, 조회는 `handlers/`).
- [x] 각 AC가 행동 단언·검증 방법·프로덕션 도달 경로를 갖는다 — AT-43~AT-54 **12건** 전부 §7 ΔV4 표의 세 칸이 채워져 있다.
- [x] Delta V 를 썼고 유효 V를 재구성할 수 있다 — 기준 `V1+ΔV1+ΔV2+ΔV3` @ `46047ac`, 유효 V = 그 위에 ΔV4.
- [x] 변경 효과에 필요한 레벨을 골랐다 — R(화면 구조·표기·조회 횟수) · SD(패치 수명) · AR(신규/제거 채널 · 기준선 배선) · MD(파서·확장·표시 파생·트리·필터·라벨). 모든 `NEW`·`CHANGED` 노드에 같은 레벨 `REQUIRED` pair 가 있다(VP-51~VP-62 **12건**).
- [x] 영향받은 `INHERITED` 상위 노드를 `REGRESSION` 으로 닫았다 — VP-29·VP-30·VP-31·VP-35·VP-39·VP-22·VP-48·VP-50·VP-40~VP-42·VP-45(**7행 13 pair**). 비영향은 VP-01~VP-08·VP-18~VP-21·VP-24~VP-27 을 근거와 함께 `NOT_REQUIRED` 로 적었다.
- [x] 각 pair 가 production path·§10 강제 지점 전수·직접 oracle 을 갖는다. 적대 증거는 **10 pair** 에 선택했고 각각 선택 이유와 심을 결함을 적었다(VP-60 만 직접 행동 oracle 이라 미선택).
- [x] 현재 변경 산출물에 적용되는 gate 를 열거했다 — lint · typecheck · vitest(+DB) · doc-inventory · IPC_CONTRACT · i18n · append-only. 관련 없는 기존 실패를 새 blocking 으로 만들지 않는다.
- [x] 사람 실기로 미룬 순수 로직이 없다 — 패치 파싱·상한 절단·양방향 확장·표시 옵션 넷·트리 파생·비교 필터·라벨 4상태가 전부 순수 함수이고, 조회 횟수는 스텁 카운트, 배치는 SSR 렌더 단언이다. 남긴 4건은 전부 **레이아웃이 있어야 관측되는 시각 축**이다.
- [x] semantic 목표를 structural proxy 만으로 검증하는 AC 가 없다 — AT-53(0건 스윕)은 AT-45·AT-46 양성 단언과 짝지었고, AT-41(primitive 구조 단언)은 그 한계를 §10 EP-27 `실패 의미` 와 사람 실기 ④가 받는다.
- [x] “X가 쓰인다”의 검사 장치가 X를 지웠을 때 실패한다 — AT-46 은 **양방향**(항상 재조회 변이 · 조회 소거 변이)으로 잠근다. **자리를 말하는 불변식**인 AT-48 은 `up`↔`down` 을 맞바꾼 변이가 `insertedAbove` 두 값의 차이에서 red 다.
- [x] 정책 파라미터의 단위/범위가 명확하다 — 파일 200(개) · 파일당 줄 50,000(줄) · 전체 줄 200,000(줄) · 버퍼 16 MiB(바이트) · 확장 20(줄) · 연출 180(ms). 상호배타 상태는 discriminated union 이다(비교 모드 3분기 · `GitDiffPatchFile.kind` 3분기 · 레이아웃 2분기).
- [x] 참조 구현 대비 coverage — `parseUnifiedPatch` 가 다뤄야 할 `git diff` 출력 케이스 **7종**을 §11 표에 전수 나열했다(수정·추가·삭제·rename·binary·개행 없음·모드만 변경).
- [x] 신규 모듈/계약마다 레이어·강제 지점·테스트 seam 이 있다 — §11 ΔV4 표 **25행**이 파일마다 셋을 갖는다.
- [x] producer/consumer 가 end-to-end 로 닫혔다 — `sessions.baseline_ref` 는 prepare → turn → writer → insert → 조회 → 라벨까지, 패치는 git → 파서 → 채널 → 훅 → 상태 → 세 소비자(본문·트리·필터)까지 §10 EP-28·EP-29·EP-34 가 지점을 나열한다.
- [x] worst-case 와 one-way door 를 계산했다 — 패치 페이로드 약 **16 MB**(§10 ΔV4) · 마이그레이션 `0020` 은 append-only 라 되돌리려면 다음 마이그레이션이 필요하다(§17 ΔV4).
- [x] 본문 전체를 Ledger 와 교차검증했다. **ACTIVE 결정 ↔ AC 대조: 충돌 0** — D-069·D-071↔AT-43 · D-070·D-072↔AT-44 · D-073↔AT-45 · D-074·D-078↔AT-46 · D-075·D-076·D-077↔AT-47 · D-090↔AT-48 · D-079·D-080↔AT-49 · D-082·D-083·D-084·D-092↔AT-50 · D-086·D-087·D-088·D-089↔AT-51 · D-091↔AT-52 · D-081↔AT-53 · D-093↔AT-54 · D-085·D-094↔AT-41·AT-45. **D-036↔AT-49** 는 대체가 아니라 **재확인**이다(VP-35 REGRESSION).
- [x] 산출물 문장 규칙을 지켰다 — 판정 먼저, 주장 한 줄에 관측 하나, Part I(관측 결과)과 Part II(경로·계약)에 같은 사실을 두 번 쓰지 않았다.

**교차검증 관측 3건.**

1. `ACTIVE 결정 ↔ AC 대조: 충돌 0`(위 목록 26건 중 ACTIVE 26 전건이 AC 를 갖는다 — D-085·D-094 는 두 AC 에 걸친다).
2. `SUPERSEDED 결정에 걸려 있던 AC 재유도: 5건` — AT-28→AT-53 · AT-30→AT-48 · AT-31→AT-45 · AT-38→AT-46 · AT-40→AT-50. 재유도 없이 사라진 AC **0건**.
3. `인용 경로 실재 확인` — `Icon.tsx` 의 `folder`·`doc`(2/2) · `app.css:167~195` 의 depth keyframe 2 + utility 2 · `chatReducer.ts:374` 의 `PANEL_MIN_WIDTH=280` · `git-cli.ts:49` 의 `symbolic-ref --short -q HEAD` · `datetime.ts:107` 의 `formatRelativeTime` · `migrations/0019_session_baseline.sql` — **6/6 실재**.


## READY self-review — ΔV5 (2026-09-03)

- [x] 여러 턴의 결정이 Ledger 에 보존됐다 — 신설 **D-099~D-110(12)**, `SUPERSEDED` **7**(D-045·D-097·D-069·D-082·D-086 전부 + D-074·D-078 의 절 단위). `OPEN` **0**.
- [x] Product & UX Contract 가 Technical Design 보다 앞에 있다 — §5 ΔV5 전이표 **5행**과 §6 ΔV5 범위/비범위가 구현 방식 없이 완료 상태를 말한다.
- [x] 조건절·이유절을 재해석하지 않았다 — 사용자 6답을 **원문 그대로** §2 ΔV5 표에 인용했다(“둘 다 없앨다”·“Hook이 이니라면 현재 동작을 유지하겠다”·“모두 첨부이미지의 배치를 따르라”). 특히 D-100 은 조건절이 지정한 대안(**현재 동작 유지**)을 그대로 받았고 다른 배선으로 바꾸지 않았다.
- [x] 물어야 할 제품 결정과 조사로 닫을 사실을 구분했다 — **되물은 것 3**(기준 라벨 · 파일 기본 상태 · 계기 범위)은 전부 사용자 명시 결정과 정면 충돌하거나 화면 결과가 갈리는 축이다. **조사로 닫은 것**은 회귀 원인·소비처 수·화이트리스트 형태였고 묻지 않았다.
- [x] 수치·전칭·앵커·기존 테스트 인용을 실측했다 — `gitApi.diffSummary` **1** · `diffPatch` **1** · 제거 대상 식별자 **21**(non-test) · `patch: null` **5**(4/5 가 요청 동시 변경) · `uncommitted` renderer **12**(데이터 소비처 **1**) · `fileApi.openPath` 소비처 **1**. 인용한 테스트 `diffReviewNavigation.test.ts`·`files.test.ts`·`resources.test.ts` 실재 확인.
- [x] 저장소 규칙을 설계 입력으로 확인했다 — renderer 4-layer(신규 파일 전부 `features/chat` 안) · 보안 베이스라인(경로 화이트리스트 재사용, §10 EP-44) · doc-inventory · `IPC_CONTRACT` 동시 갱신 · Tailwind 시맨틱 토큰.
- [x] 각 AC 가 행동 단언·검증 방법·프로덕션 도달 경로를 갖는다 — AT-62~AT-69 **8건** 전부 §7 ΔV5 표의 세 칸이 채워져 있다. 유효 AC 는 **20건**(ΔV4 12 중 AT-43·AT-46·AT-49·AT-51 이 대체·변경되고 ΔV5 8 이 는다).
- [x] Delta V 를 썼고 유효 V 를 재구성할 수 있다 — 기준 `V1+ΔV1+ΔV2+ΔV3+ΔV4` @ `177def67`, 유효 V = 그 위에 ΔV5.
- [x] 변경 효과에 필요한 레벨을 골랐다 — R(갱신 시점·기본 상태·문구·배치) · SD(미싱크 상태와 패치 수명) · AR(기존 채널의 모드 확장) · MD(계기 판정·가드·집합 반전·소유자 seam). 넷 다 `NEW`/`CHANGED` 노드가 있고 같은 레벨 `REQUIRED` pair 를 갖는다.
- [x] 영향받은 `INHERITED` 상위 노드를 `REGRESSION` 으로 닫았다 — VP-53·VP-55·VP-56·VP-61·VP-62(ΔV4 표면) · VP-54(계기 축소 후 조회 수) · VP-51·VP-52(기준선 기록) · VP-22·VP-48·VP-50. 비영향은 출처·근거와 함께 `NOT_REQUIRED`.
- [x] 각 pair 가 production path·§10 강제 지점 전수·직접 oracle 을 갖는다. 적대 증거는 **8 pair 전부**에 선택했고 각각 심을 결함을 적었다 — 이번 라운드의 회귀가 “장치는 있는데 방향이 없다” 형태였기 때문이다.
- [x] 현재 변경 산출물에 적용되는 gate 를 열거했다 — lint · typecheck · vitest(비-DB) · doc-inventory · IPC_CONTRACT · i18n. 관련 없는 기존 실패(better-sqlite3 ABI 1파일)를 새 blocking 범위로 만들지 않았다.
- [x] 사람 실기로 미룬 순수 로직이 없다 — 계기 판정·가드 상태·라벨 쌍·집합 반전·메뉴 항목·경로 화이트리스트가 전부 순수 함수 또는 SSR/임시 디렉토리 통합이다. 남긴 6건은 전부 시각·가독성 축이다.
- [x] semantic 목표를 structural proxy 만으로 검증하는 AC 가 없다 — AT-62·AT-66 의 0건 스윕은 각각 “턴 종료 1”·“첫 출력 diff 줄 0” 양성 단언과 짝지었고, AT-64 는 구조가 아니라 **세 값의 순열**로 교착을 직접 부정한다.
- [x] “X가 쓰인다”의 검사 장치가 X를 지웠을 때 실패한다 — AT-62 는 **양방향**(조회 소거 · 두 함수 합침), AT-64 는 **양방향**(해제 삭제 · 가드 삭제)을 각각 심는다. **자리를 말하는 불변식**인 AT-65(`기준 → 현재`)와 AT-69(소유자 인자)는 형제와 맞바꾼 변이·`null` 로 바꾼 변이에서 red 여야 한다고 명시했다.
- [x] 정책 파라미터의 단위/범위가 명확하고 불가능 조합을 타입이 막는다 — `OpenPathRequest.mode` 는 **필수 유니온**(optional 이면 미지정이 fail-open 된다) · 본문 상태 여섯은 `gitSnapshotRequest === null` 이 첫째를 단독 결정한다.
- [x] 참조 구현 대비 coverage — 참조는 첨부 캡처 **2장**이고, 그것이 말하는 배치 축 **6**(라벨 · 메뉴 2행 · 서브메뉴 · 안내+기본 접힘 · 헤더 1줄+`↗` · 라벨 문구)을 전수 나열해 각각 D-104~D-109 에 매핑했다. 캡처가 말하지 않는 축(`↗` 의 동작)은 파생으로 표시했다.
- [x] 신규 모듈/계약마다 레이어·강제 지점·테스트 seam 이 있다 — §11 ΔV5 의 9블록이 파일·강제 지점·seam 을 갖는다.
- [x] producer/consumer 가 end-to-end 로 닫혔다 — 계기(`busy`) → 판정 두 함수 → 조회 → 리듀서 → 두 소비 표면(컴포저 행 · 우측 패널)까지, 그리고 `↗` → 스키마 → 핸들러 → shell 까지 각각 §9 도식에 그렸다.
- [x] worst-case 와 one-way door 를 계산했다 — 조회 총량이 **줄기만 한다**(계기 5 → 1). one-way door 는 `OpenPathRequest` 스키마 확장 하나이고 유니온이라 되돌리기가 한 값 삭제다.
- [x] 본문 전체를 Ledger 와 교차검증했다. **ACTIVE 결정 ↔ AC 대조: 충돌 0**.
- [x] 산출물 문장 규칙을 지켰다 — 판정 먼저, 주장 한 줄에 관측 하나, 표 한 칸 3줄, Part I/II 중복 없음.

**교차검증 관측 4건.**

1. `ACTIVE 결정 ↔ AC 대조: 충돌 0` — D-099·D-100·D-101↔AT-62 · D-102↔AT-63 · D-103↔AT-64 · D-104↔AT-65 · D-105↔AT-66 · D-106·D-107·D-109↔AT-67 · D-108↔AT-68 · D-110↔AT-69. 신설 12 전건이 AC 를 갖는다.
2. `SUPERSEDED 결정에 걸려 있던 AC 재유도: 4건` — AT-43→AT-65(라벨 축) · AT-46 의 새로고침 항→AT-62 · AT-49→AT-67 · AT-51→AT-67. 재유도 없이 사라진 AC **0**.
3. `verify/FAIL 미해결 항목 흡수: 3건` — D22(blocking)→D-110/AT-69 · D23→AT-68 주의사항 · D24→§10 문구 정정. 누락 0.
4. `인용 경로 실재 확인` — `useGitPatch.ts:53,60`(가드) · `chatReducer.ts:1261,1272,1291`(세 무효화) · `git-diff.ts:271`(history 조건) · `handlers/files.ts:66-72`(화이트리스트) · `DiffReview.tsx:60`(내부 ref) · `gitRowState.ts:57`(`status.branch`) — **6/6 실재**.
...
| **D25** | `claude.ts:406` 의 `makeTurnEndHook(…)` 호출을 지워도 **전건 green**(3,179/3,179 · baseline 동일) — 계기의 **시작점**이 무관측이다 | VP-72 / §10 EP-46 ① | fake SDK 메시지 스트림으로 `options.hooks` 에 `Stop` 매처가 실렸는지 단언한다(§11 이 이미 그 seam 을 적었다) | **BLOCKING** | **closed**(r5 · M-A1 red 4/4 — 같은 실행에서 `turn-end-hook.test.ts` 는 4/4 green) |
| **D26** | `drainTurnEnded()` 두 드레인을 지우고 잔여물(미사용 제너레이터·카운터)까지 밀어도 **green**(3,178/3,179 · 유일 실패는 무관 flaky) — 신호가 **이벤트가 되는 자리**가 무관측이다 | VP-72 / §10 EP-46 ② | 같은 fake 스트림에서 `Stop` 발화 뒤 `turn.ended` 가 배치에 실린다고 단언한다 | **BLOCKING** | **closed**(r5 · 스트림 중 M-A2 red 1/4 · 꼬리 M-A3 red 1/4 — 두 드레인이 서로 다른 케이스를 연다) |
| **D27** | `chatStore.receive` 의 `case 'turn.ended'` 를 지워도 **green**(3,179/3,179 · baseline 동일) — 이벤트가 **리듀서에 닿는 자리**가 무관측이다 | VP-72 / §10 EP-46 ④ | `chatStore` 라우팅 테스트에 `turn.ended` 케이스를 더한다(형제 `turn.aborted` 가 이미 그 자리에 있다) | **BLOCKING** | **closed**(r5 · 검증 라운드 5) — 계약은 M-A5 red 2/5 · M-A6 red 5/10 · M-A7 red 2/10. 인용 변이 M-A4 는 **등가로 확정**: `chatStore.ts:610-612` 와 `:644-645` 가 같은 인자로 같은 `dispatchTo(RECV_EVENT)` 를 한다 |
| **D28** | 헤더 활성 세그먼트가 `bg-fill-uncontained-active`(ink 14% 중립) + `text-t9` 를 그린다 — **D-118 이 지정한 값은 `bg-fill-selected`(rust-soft) + `text-accent`** 이고 사이드바 선택은 그 값을 쓴다. 같은 "선택" 문법이 둘로 갈렸다 | D-118 ACTIVE · AT-75 클래스 절 · AT-76 실측 9행 / VP-76 · VP-77 | 세그먼트에 `bg-fill-selected`/`text-accent` 를 주고 **그 클래스를 단언**한다 — `Button` 의 `pressed` 는 다른 토큰을 소유하므로 위임으로는 닫히지 않는다 | **BLOCKING** | **closed**(r4) — D-122·D-128 이 계약을 대체했고 현재 토큰은 M39 red |
| **D29** | AT-76 오라클이 **소스 텍스트 스윕**이라 "어느 요소가 그 클래스를 갖는가" 를 보지 않는다 — `w-[25%]` 를 형제 안쪽 `<div>` 로 옮겨도 green. AC 검증 수단은 "대상 요소를 `data-*` 로 지목" 이었다 | VP-77 / AT-76 검증 수단 | 렌더 후 `data-*` 요소의 className 을 단언한다 | **BLOCKING** | **closed**(r4) — D-123 대체 + 렌더 className 단언으로 전환, M26~M28 red |
...
| **D35** | 메뉴 원격 조회 **배선**이 무관측 — 훅의 `void cache.ensure()` 를 지워도(M41), 갱신 세대를 상수로 만들어도(M8) **3,324/3,324 green**. 유일한 오라클은 팩토리 단위 테스트라 메뉴가 영영 `loading` 이어도 게이트가 조용하다 | VP-102 · VP-100 / §10 EP-76 ③ · EP-74 ① | `GitIdentityMenus` 를 렌더해 `gitApi.status` 호출 수와 턴 종료·새로 고침의 갱신 계기를 단언한다 | **BLOCKING** | **open(r5 미해소)** — 훅 내부는 잠겼으나(M41 red 5/11 · M8·M8b·M8c red) **처방이 지목한 컨테이너가 그대로 green** 이다. `GitIdentityMenus:36` 의 `menuEpoch` 를 `undefined` 로 굳혀도 3,352 green + typecheck 0 error(N1b) → D46 |
| **D36** | 수동 새로 고침의 **효과**가 무관측 — `REFRESH_GIT_SNAPSHOT` 이 tick 을 안 올려도(M42), `DiffTileContent` 가 `onRefresh` 를 안 배선해도(M43) green. 메뉴 항목의 **존재**만 잠겨 있다(M17 red) | VP-99 / §10 EP-73 ①② | reducer 전이(`gitRefreshTick + 1`)와 `DiffTileContent` 의 배선을 각각 단언한다 | **BLOCKING** | **closed**(r5 · M42 red 2/5 · M43 red 1/10) |
| **D37** | composer↔diff 선택 연동 **배선**이 무관측 — `DiffTileContent.activeRequirementId` 를 `null` 로 고정해도 green(M13). 선택 테스트가 producer 를 건너뛰고 `DiffReview` 를 직접 렌더한다 | VP-97 / §10 EP-71 ③ | store 를 지나는 렌더(`DiffTileContent`)로 양쪽 활성을 단언한다 | **BLOCKING** | **closed**(r5 · M13 red 2/10 — 같은 실행에서 `diffRequirementSelection.render.test.ts` 는 green · 새 축 M13b(선택 액션 배선 제거) red 1/10) |
| **D38** | 빈 diff 안내의 **중앙 배치**가 무관측 — 중앙 클래스를 지워도 green(M5). 존재(M23)·트리 0개(M6)·토글 숨김(M7)은 잠겨 있다 | VP-103 / §10 EP-77 ① · D-153 | 렌더 후 해당 요소 className 을 단언한다(M26~M28 과 같은 방식) | **BLOCKING** | **closed**(r5 · M5 red — `data-diff-empty` 마커는 유지한 채 중앙 3토큰만 제거해 재측정) |
| **D39** | 헤더 밴드 `h-[32px]` 가 무관측 — 48px 로 바꿔도 green(M29). 파일 밴드·사이드바 240px·목록 40% 는 잠겨 있다 | VP-79 / §10 EP-53 ① · D-123 | `RightPanelTile` 헤더를 같은 render 테스트의 단언 대상에 넣는다 | **BLOCKING** | **closed**(r5 · M29 red · 새 축 M29b(두 갈래를 한 모양으로 합침) red) |
| **D40** | 작성 입력 자동 높이 `[field-sizing:content]` 가 무관측 — 지워도 green(M50) | VP-88 / §10 EP-61 ① · D-136 | 렌더 className 단언 | **BLOCKING** | **closed**(r5 · M50 red 1/45) |
...
| **D49** | 코멘트 reveal 의 **대상 컨테이너**가 무관측 — `FileDiffSection:292` 의 `scrollOwnerRef.current` 를 `null` 로 굳혀도 3,369 green(V2). 오라클 fixture 가 그 값을 항상 `null` 로 넣어 인자가 지워졌다 | VP-97 · §10 EP-71 ④ · D-147 | `scrollOwnerRef` 에 센티널 객체를 넣고 `toHaveBeenCalledExactlyOnceWith(sentinel, id)` 로 단언한다 | **BLOCKING** | **closed**(r7 · V2 red 3 · 형제 인자 V2b red 2 — fixture 의 `{ current: null }` 을 센티널로 바꿨다). **r7 검증 확인** — 2변이 재현 일치 + 검증자 축 A4(형제 ref 맞바꿈) red 3 · A5(중복 호출) red 2 |
| **D50** | 선택 **cleanup** 4자리 중 3자리가 무관측 — 삭제(V3)·수동 범위 전환(V6)·cwd 초기화(V8)가 활성 id 를 안 지워도 green. 성공 clear(V7)만 잠겨 있다 | VP-97 · §10 EP-71 ① · D-147 | 순수 reducer 3케이스를 더한다 — DOM·렌더 불필요 | **BLOCKING** | **closed**(r7 · V3·V6·V8 red). **처방 3자리 대신 계약의 cleanup 8자리 전수**를 닫았고 차집합 3자리(N-a·N-b·N-c)도 red 다. **r7 검증 확인** — 8자리를 코드에서 재열거해 일치, `resetGitReview` 두 호출부를 갈라 V8a·V8b 각각 red 1 |
| D51 | draft 를 열 때의 활성 id 정리가 무관측(V5 green) | 비귀속 — §10 EP-71 ① 이 열거하지 않은 자리 | D50 을 닫을 때 같은 파일에 함께 넣으면 값이 싸다 | NON_BLOCKING | **closed**(r7 · ⑤ V5 red 1). **r7 검증 확인** — 재현 일치 |
...
| D54 | `loopback-callback.test.ts` 가 포트 45211~45216 을 하드코딩해 컨테이너가 45214 를 물면 red 다 | 비귀속 — 변경 무관(verify r6 §8) | 포트를 런타임 탐색으로 바꾸거나 충돌 시 건너뛴다 | NEXT_HANDOFF | open — **r7 검증의 전체 스위트 31회에서 재현 0회**(이 컨테이너는 포트가 비어 있다) |
| D55 | 세션 경계 **3자리**(`NEW_CHAT`·`START_LOAD_SESSION`·`LOAD_SESSION`)의 활성 id 정리가 무관측 — 활성만 물려줘도 3,385 green(A3a·A1a·A2). `...initialChatState` 스프레드라 **명시 write 스윕에 안 잡힌다** | VP-97 인접 — AT-96 “세션 경계” 절은 목록 비움(`chatReducer.plan.test.ts:705`)·store 격리(`chatStore.test.ts:709`)·`chatStore.ts:1185` 의 새 state 적용으로 이미 닫혀 있다 | 값이 싼 reducer 케이스 3개면 닫힌다 — 남는 것은 소비처 없는 dangling id 다 | NON_BLOCKING | open(r7) |
| D56 | 구현 보고 “이번 라운드 수정의 잠금” 검산 줄이 `인용 4 · 새 oracle 6` 인데 같은 표의 갈래 칸은 인용을 **5행**으로 적었다. 총계 10 과 행별 red 는 전건 정확하다 | 비귀속 — 보고 서술 | 다음 라운드에 갈래를 표와 같게 적는다 | NON_BLOCKING | open(r7) |

## [설계 정정] ΔV4 r3 추가 범위 — 사용자 보고 3건 (라운드 유지)

> **라운드를 올리지 않는다** — 사용자가 "현재 라운드에 이어서 한다" 를 명시했다. r3 의 남은
> 파생 이슈(D22·D23·D24)는 그대로 열려 있고, 이 절은 r3 산출물 **위에** 얹힌 범위 추가다.

사용자가 화면에서 겪은 것 셋이 입력이다. 하나는 외형 요구이고 둘은 ΔV4 가 만든 표면이
**사용자에게 아무것도 말하지 않는** 상태였다.

| 보고 | 진단 | 결정 |
|---|---|---|
| 컴포저 상단 브랜치·워크트리 버튼을 테두리 하나가 감싸야 한다 | 두 칩이 각자 `outlined` 라 낱개로 흩어져 있었다 | **D-096** |
| 터미널에서 커밋했는데 컴포저 행과 우측 패널 **둘 다** 반영되지 않는다 | 조회 계기 셋이 전부 앱 **안**의 사건이다 — 앱 밖 변경에는 발화하는 계기가 없고, 유일한 수동 탈출구(우측 패널 `⋮ › 새로고침`)는 타일을 열어야 닿는다 | **D-097** |
| 로컬에 미커밋 파일이 많은데 「미커밋 변경」이 비어 있다 | `git diff <base>` 는 **추적 파일만** 본다(실측: `git status --porcelain` 은 `?? untracked.ts` 를 주는데 `git diff --raw --numstat -z <base>` 는 그 파일을 내지 않는다). ACTIVE 였던 D-035("완전 제외")의 결과이고, 함께 요구된 "미추적 제외" 문구는 **구현된 적이 없어** 제외가 설명 없는 빈 화면으로 도달했다 | **D-095** (사용자가 "+0 −0 으로 표시" 를 골랐다) |

### AC 추가

| # | 인수 기준 | 결정 |
|---|---|---|
| AT-55 | 추적 수정 1 + 미추적 1 인 저장소에서 요약의 `files` 는 **둘 다** 담고, `totals` 는 **추적 하나의 값 그대로**다. 미커밋 블록도 같은 미추적 항목을 싣는다 — 두 목록이 갈리지 않는다 | D-095 |
| AT-56 | 패치의 미추적 항목은 `status:'added'` · `added:0` · `removed:0` · `lines:[]` 다 — 이름은 목록·트리에 남고 본문은 비어 있다 | D-095 |
| AT-57 | 묶음 안 칩은 높이·반경·테두리 유틸리티를 **하나도** 갖지 않고, 묶음이 그 셋을 하나씩만 갖는다. 구분선은 `trailingDivider` 를 받은 앞 칩이 조건부로 그린다 | D-096 |
| AT-58 | 창 재포커스 구독은 `focus` **하나**를 등록하고, 그 사건이 오면 `refreshGitSnapshot` 을 부르며, 해제는 **등록한 바로 그 함수**를 뗀다 | D-097 |
| AT-59 | 커밋 0개 저장소에서 시작한 세션은 그 뒤의 터미널 커밋이 `commits` 에 서고 그 내용이 `files`·`totals` 에 남는다 | D-098 |
| AT-60 | 커밋이 있던 저장소는 **출생 시각의 커밋**이 기준이다 — 그 이전 이력은 범위 밖이고 이후 커밋만 목록에 선다 | D-098 |
| AT-61 | 기록된 `baseOid` 가 있으면 출생 시각을 보지 않는다(고정점 우선), `bornAt` 이 없으면 예전 HEAD 폴백 그대로다 | D-098 |

### §10 계약 / 강제 지점 (추가)

| # | 계약 | 언제 강제 | 실패 의미 |
|---|---|---|---|
| EP-37 | 미추적 목록은 합계에 기여하지 않는다 | `mergeDiffEntries` 의 **미추적 인자** 1지점 (호출부: 요약 `overall`·`uncommitted` 2지점) | 미추적이 합계로 새면 컴포저 행 `+N −M` 과 헤더가 커밋 가능한 변경량을 과장한다 |
| EP-38 | 미추적 조회는 요약·패치에서 **각 1회**다 | `readUntracked` 호출 2지점(`gitDiffSummary`·`gitDiffPatch`) | 범위마다 다시 부르면 목록이 조회 시점마다 갈린다 |
| EP-39 | 묶음 칩은 자기 높이·반경·테두리를 갖지 않는다 | `chipSurface` 의 `GEOMETRY.segment` 1지점 + 사용처 2지점(`CwdPanel` 의 `variant="segment"`) | 하나라도 남으면 테두리가 두 겹이 되거나 형제 outlined 칩보다 2px 커진다 |
| EP-40 | 외부 변경 계기는 `refreshGitSnapshot` 을 부른다 | `useGitSnapshot` 의 focus effect 1지점 | 두 조회만 다시 돌리면 요청 키가 같아 리듀서가 `patch` 를 보존해 우측 패널 본문만 옛 값에 머문다 |
| EP-41 | 기준선 폴백은 질의 시점 HEAD 를 읽지 않는다 | `resolveDiffRange` 1지점 + `bornAt` 운반 3지점(`getSessionBaseline` · `gitDiffSummary` · `gitDiffPatch` 호출부) | 기준선이 커밋을 따라 움직여 사용자가 커밋할수록 diff 가 빈다 — 실사용에서 관측된 증상 그 자체다 |

## [구현자 기입] 구현 보고 (ΔV4 r3 추가 범위)

- 변경 파일: 프로덕션 **5**(`infra/git/git-diff-parse.ts` · `infra/git/git-diff.ts` · `composer/chipSurface.ts` · `composer/BranchChip.tsx` · `components/CwdPanel.tsx` · `composer/useGitSnapshot.ts` — 여섯이다) · 테스트 신규 **1**(`composer/chipGroup.test.ts`) · 테스트 변경 **2**.
- 강제 지점: **EP-37 1/1 · EP-38 2/2 · EP-39 3/3 · EP-40 1/1**.
- D-035 → D-095 대체에 걸려 있던 **기존 잠금 2건을 반대 방향으로 다시 세웠다** — `git-diff.test.ts` 의 "추적 변경만 목록·합계에 남는다"·"미추적 파일은 패치에 없다" 를 지우지 않고 새 계약의 단언으로 **교체**했다(덮개 회귀 0: 미추적이 합계로 새는 변이는 여전히 red 다).
- 진단 근거는 실측이다 — 임시 저장소에서 `git diff --raw --numstat -z <base>` 가 `?? untracked.ts` 를 내지 않는 것을 직접 확인했다. "화면 배선이 끊겼다" 가 아니라 **조회가 그 파일을 본 적이 없다**.

### 설계 대비 명시적 차이

| # | 차이 | 판단 |
|---|---|---|
| Δ12 | 미추적 패치 항목의 `kind` 를 새로 만들지 않고 `'text' + lines:[]` 로 두었다 | `GitDiffPatchFile.kind` 유니온에 `'untracked'` 를 더하면 IPC 계약이 넓어지고 `IPC_CONTRACT` 동시 갱신이 따라온다. 본문이 비어 있다는 사실은 `lines:[]` 가 이미 말하고, 헤더 `+0 −0` 이 "변경 없음" 과 구분한다 |
| Δ13 | 미추적을 패치 목록의 **꼬리**에 붙이고 경로로 재정렬하지 않았다 | 정렬하면 추적 파일의 기존 순서(git 이 준 순서)가 바뀌어 본문 배치가 조용히 달라진다 — 이번 범위 밖의 변화다 |
| Δ14 | 계기를 `.git` 파일 감시가 아니라 **창 재포커스**로 잡았다 | 감시는 main 에 새 채널·수명·플랫폼 분기를 만든다. 보고된 시나리오(터미널에 갔다 돌아온다)는 재포커스가 정확히 덮고, 부족하면 `⋮ › 새로고침` 이 그대로 남아 있다 |

### 놓친 잠재 문제 + 대응

| # | 문제 | 대응 |
|---|---|---|
| Δ15 | `chipSurface` 의 `pressed` 가 **테두리 두께 없이** `border-accent` 만 얹고 있었다(`border-width:0`) — 눌린 outlined 칩은 테두리가 사라진다 | 선조치. 두께를 `GEOMETRY` 로 분리해 pressed 도 두께를 유지한다. `ariaPressed` 소비처는 워크트리 칩 하나뿐이라(전수 확인) 다른 화면에 파급이 없다 |
| Δ16 | 미추적 파일이 목록 상한(200)을 밀어낼 수 있다 | 상한을 미추적 포함해 다시 재고 `truncated` 를 세운다. 합계는 절단 **앞**에서 세므로 수치는 영향받지 않는다(미추적은 0 을 더한다) |
| Δ17 | **baseline 이 없는 세션**(migration 0019 이전 행)은 `base.kind:'head'` 로 접혀 `commits` 가 항상 비고, 터미널 커밋이 diff에서 *사라진다* | 이번 범위 밖 — 아래 파생 이슈 D25 |

## [구현자 기입] Review Signals — 사실만 (ΔV4 r3 추가 범위)

- 현재 라운드: **3 유지**(사용자 명시). Decision 신설 **3**(D-095·D-096·D-097) · `SUPERSEDED` **1**(D-035) · 새 AC **4**(AT-55~AT-58) · 새 강제 지점 **4**(EP-37~EP-40).
- 사용자 보고 셋 중 **둘은 "구현이 없다" 가 아니라 "계약이 그렇게 정해져 있었다"** 였다 — D-035 는 ACTIVE 였고 조회 계기 셋은 D-045 그대로였다. 배선 진단으로 시작해 **결정 원장에서 답을 찾은** 라운드다.
- 그중 하나는 **결정의 절반이 미구현**이었다 — D-035 는 제외와 함께 "미추적 제외 문구" 를 요구했는데 그 문구는 존재한 적이 없다(전수 0건). 제외 자체는 옳게 구현됐고 **설명만 빠져** 사용자에게 버그로 도달했다.
- 게이트: `npm run lint` **0 error**(exit 0) · `npm run typecheck` 3구성 **0줄** · `vitest run` 대상 스위트 전건 green. 전체 스위트의 red 는 Windows 임시 저장소 실기의 **5초 timeout·EBUSY** 로, 병렬도를 낮추면 사라지고 변경 파일과 import 경로가 겹치지 않는다(`features/worktrees/`·`infra/git/repository.test.ts` — 이번 diff 무관).

## [구현자 기입] 구현 보고 (ΔV4 r3 추가 범위 — 2차: 기준선 고정, D-098)

- 진단이 **한 번 틀렸다가 실측으로 바로잡혔다.** 1차에서 "터미널 커밋 미반영 = 새로고침 계기 없음(D-097)" 으로 보고했는데, dev DB 를 열어 보니 그 세션은 애초에 기준선이 없어 **새로고침을 해도 못 보는** 상태였다. 계기는 부차 원인이었고 D-098 이 주 원인이다.
- 실측 근거 3건: ① `web/movie` 세션행 `oid=NULL, ref='master'`, `created_at=08:03:05` ② 그 저장소의 유일한 커밋 `f1b46a0`, `08:04:17` — 세션 시작 **72초 뒤** ③ `git init` 직후 `rev-parse --verify -q HEAD` = exit 1 / `symbolic-ref --short -q HEAD` = `master` (NULL 과 ref 가 갈리는 자리).
- 변경 파일: 프로덕션 **3**(`infra/db/queries.ts` — `created_at` 을 baseline 조회에 포함 · `app/handlers/git.ts` — `bornAt` 운반 · `infra/git/git-diff.ts` — `resolveDiffRange` 폴백) · 테스트 변경 **3**.
- 강제 지점: **EP-41 4/4**(`resolveDiffRange` 1 + 운반 3).
- **스키마 변경 0.** 마이그레이션을 만들지 않았다 — `created_at` 은 `sessions` 에 이미 있고 조회 SELECT 에만 없었다.

### 설계 대비 명시적 차이

| # | 차이 | 판단 |
|---|---|---|
| Δ18 | 빈 저장소 기준을 새 `GitDiffBase` 종류가 아니라 **empty tree sha 값**으로 표현했다 | 실측으로 `git diff <sha>` 는 전량 추가를, `git log <sha>..HEAD` 는 루트부터의 커밋을 그대로 낸다 — `readDiff`·`readCommitHistory` 를 손대지 않고 `kind:'worktree-base'` 한 갈래로 흡수된다. IPC 계약도 그대로다 |
| Δ19 | 되짚은 기준선을 DB 에 **쓰지 않는다**(읽기 경로에서 파생) | 조회가 쓰기를 하면 읽기 전용 계약(D-064)이 깨진다. `created_at` 이 고정값이라 파생은 결정적이다 |

### 놓친 잠재 문제 + 대응

| # | 문제 | 대응 |
|---|---|---|
| Δ20 | `rev-list --before` 는 **committer 시각**을 보고 git 의 시각 해상도는 1초다 — 벽시계로 쓴 테스트가 한 번은 통과하고 한 번은 깨졌다 | 선조치. 테스트가 `GIT_COMMITTER_DATE` 로 커밋 시각을 못 박는다(`commitAt`). 프로덕션에서는 출생과 커밋이 같은 초에 걸리면 그 커밋이 기준에 포함될 수 있으나, 그 경우 손해는 커밋 1개이고 예전처럼 전부 잃지 않는다 |
| Δ21 | empty tree sha 는 **sha1 저장소 값**이다 — sha256 저장소에서는 다르다 | 보고만. 실패하면 `readDiff`·`readCommitHistory` 가 빈 값을 내 현행(고장난) 동작으로 되돌아갈 뿐 더 나빠지지 않는다. sha256 저장소는 opt-in 이고 이 저장소에는 없다 |
| Δ22 | 기능 도입(`1f9dc2d5`, 09-01) 이전 세션 **40개**는 되짚기가 `created_at` 기준이라 그 시각의 커밋을 base 로 잡는다 — 대체로 맞지만 그 사이 브랜치를 갈아탄 세션은 어긋날 수 있다 | 보고만. 옛 세션의 진짜 출발 커밋을 알 방법이 없고, 되짚기는 질의 시점 HEAD 보다 언제나 낫다(후자는 커밋을 **전부** 잃는다) |

### 게이트

- `npm run typecheck` 3구성 **0줄** · `npm run lint` **0 error / 1 warning**(기존분).
- `vitest run src/main/infra/db/ src/main/infra/git/ src/main/app/handlers/git.test.ts` — **13파일 157케이스 전건 green**. `git-diff.test.ts` 는 2회 연속 33/33 로 결정성을 확인했다(Δ20 이 만든 흔들림의 재발 여부).
- **실저장소 대조는 미수행** — 이 샌드박스의 node 프로세스는 워크스페이스 밖 경로를 보지 못한다(`stat('C:\workspace\web\movie')` → `ENOENT`). 임시 저장소 재현 4케이스가 그 자리를 대신하고, 최종 확인은 앱 실기 몫이다.

### ΔV6 영향 파일 (전수)

| 파일 | 성격 | 이유 |
|---|---|---|
| `app/src/shared/ipc.ts` | 변경 | `turn.ended` NormalizedEvent variant |
| `app/src/main/adapters/claude-adapt.ts` | 변경 | `makeTurnEndHook` 신설 |
| `app/src/main/adapters/claude.ts` | 변경 | hook 병합 + `turnEnded` 버퍼 드레인 |
| `app/src/main/infra/git/git-diff.ts` | 변경 | `diffRevArgs` 2항화 · 미추적 조회 2건 제거 |
| `app/src/renderer/src/features/chat/reducer/chatReducer.ts` | 변경 | `turnEndTick` · `gitRowClosedAtTick` · `turn.ended` |
| `app/src/renderer/src/features/chat/store/chatStore.ts` | 변경 | `closeGitRow` · `setDiffSidebarVisible` · 이벤트 분기 |
| `.../features/chat/components/composer/useGitSnapshot.ts` | 변경 | 계기 입력을 `tick` 으로 |
| `.../features/chat/components/composer/gitRowState.ts` | 변경 | 노출 판정에 닫힘 tick |
| `.../features/chat/components/composer/GitRow.tsx` | 변경 | 우측 정렬 `×` |
| `.../features/chat/components/rightpanel/sessionChangesData.ts` | 변경 | 라벨 판별 유니온 |
| `.../features/chat/components/rightpanel/GitContextBar.tsx` | 변경 | 세그먼트 둘 · 모드별 라벨 · 타이포 |
| `.../features/chat/components/rightpanel/ChangedNavigationSidebar.tsx` | 변경 | 폭 · 선택 채움 · 카드 무테 |
| `.../features/chat/components/rightpanel/FileDiffSection.tsx` | 변경 | 헤더 밴드 배경 |
| `app/src/renderer/src/shared/i18n/resources/{ko,en}.ts` | 변경 | 문구 4건 |
| `docs/IPC_CONTRACT.md` | 변경 | `turn.ended` variant 기술 |
| 신규 테스트 | 추가 | hook 조각 · 이벤트→tick · 커밋 전용 범위(main 통합) · 컴포저 닫기 · 모드별 라벨 · 세그먼트 · 실측 토큰 |

**손대지 않는 것**: `prepare-worktree.ts` · `CwdButton` · `queries.ts` · `migrations/**` · 요구사항 슬라이스 · `DiffTable` · `tokens.css`. 마지막 둘이 §7-A 의 `NOT_REQUIRED` 근거이고 D-121 의 근거다.

## READY self-review — ΔV6 (2026-09-03)

- [x] 여러 턴의 결정이 Decision Ledger 에 보존됐다 — ΔV6 이 **5건을 SUPERSEDED** 로 바꾸고(D-005 닫기 절 · D-083 폴더 버튼 절 · D-095 · D-100 · D-104 부분) 11건(D-111~D-121)을 추가했다. 전부 사용자 명시 변경이라 실패로 위장하지 않았다.
- [x] Product & UX Contract 가 Technical Design 보다 앞에 있고 구현 방식 없이 완료 상태를 설명한다 — §5 ΔV6 의 두 도면과 상태표.
- [x] 조건절·이유절을 재해석하지 않았다 — “다음 턴 종료 때 다시 뜸”·“형태만 참조, 색은 Orca 러스트”·“미커밋 변경분을 항상 목록에서 제외” 를 원문으로 인용했다.
- [x] 사용자에게 물어야 할 제품 결정과 코드로 닫을 사실을 구분했다 — 6건을 물었고(§2 ΔV6 질의 응답 행) 나머지(hook 배선 형태·이벤트 terminal 여부·소비처 전수)는 조사로 닫았다.
- [x] 수치·전칭·문서 앵커를 실측했다 — §8 ΔV6 표 15행이 전부 이번 세션 `grep` 결과이고, 정의/호출을 나눠 셌다(`readUntracked` 3 = 정의 1 + 호출 2 등).
- [x] 저장소 규칙을 설계 입력으로 확인했다 — `writer.ts` 가 `default` 절 없이 9케이스라 새 variant 를 무시한다는 것, terminal 판정 셋이 열거형이라는 것, `Button` 의 `TEXT` 맵에 serif 가 없다는 것.
- [x] 각 AC 가 행동 단언·검증 방법·프로덕션 도달 경로를 갖는다 — AT-70~AT-76 일곱.
- [x] Delta V 를 썼고 기준을 커밋으로 고정했다(`628123bc`) — 유효 V 를 `V1 + ΔV1..ΔV6` 으로 재구성할 수 있다.
- [x] 변경 효과에 필요한 레벨을 골랐고 `NEW`·`CHANGED` 노드마다 같은 레벨 `REQUIRED` pair 가 있다 — VP-71~VP-77 일곱.
- [x] 영향받은 `INHERITED` 상위 노드는 `REGRESSION`(VP-63·VP-66 등), 비영향만 `NOT_REQUIRED` 이며 근거를 적었다.
- [x] 각 pair 가 production path · §10 강제 지점 전수 · 직접 oracle 을 갖고, 적대 증거를 고른 6 pair 는 심을 결함을 적었다.
- [x] 현재 변경 산출물에 걸리는 gate 를 열거했다 — §7-A ΔV6 gate 차분 7행.
- [x] 사람 실기로 미룬 순수 로직이 없다 — 범위 판정·계기 판정·노출 판정·라벨 파생·세그먼트 상태는 전부 순수 함수 또는 임시 저장소 통합이다. 실기 3건은 색 인지·문구 인상·좁은 폭 가독뿐이다.
- [x] semantic 목표를 structural proxy 만으로 검증하는 AC 가 없다 — AT-71 의 “기다리지 않는다” 를 `async` 여부가 아니라 **핸들러 안 조회 0 + 버퍼 +1** 로 관측한다.
- [x] “X 가 쓰인다” 불변식의 장치가 X 를 지웠을 때 실패한다 — AT-72·AT-76 이 0건 스윕에 양성 단언을 짝지었고, AT-70 은 형제와 맞바꾼 변이를, AT-75 는 두 버튼을 같게 만든 변이를 red 로 잡는다.
- [x] 정책 파라미터의 단위/범위가 명확하고 불가능 조합을 타입이 막는다 — `sidebarVisible: boolean` 하나가 세그먼트 둘을 정하고, 라벨 파생이 판별 유니온이라 `sha → 브랜치` 조합이 만들어지지 않는다.
- [x] 참조 구현 대비 coverage — 참조 캡처 5장에서 실측 9행을 뽑고, **의도적으로 다른 2행**(활성/체크 색 · git 수치 색)을 §8 말미에 명시했다.
- [x] 신규 모듈/계약마다 레이어·강제 지점·테스트 seam 이 있다 — `makeTurnEndHook`(adapters, EP-46, 순수) · `turn.ended`(shared, EP-46) · 라벨 유니온(rightpanel, EP-50, 순수).
- [x] producer/consumer 가 end-to-end 로 닫혔다 — hook → 버퍼 → 이벤트 → 리듀서 → 훅 → IPC → git 까지 §9 도면이 한 줄로 잇는다.
- [x] worst-case 와 one-way door 를 계산했다 — 조회 수는 턴당 1회로 **변하지 않는다**(계기의 출처만 바뀐다). `turn.ended` variant 추가는 되돌리기 쉬운 문이고, `GitDiffSummary.uncommitted` 계약 필드를 **지우지 않는 것**이 이번 범위의 one-way door 회피다.
- [x] 본문을 Decision Ledger 와 교차검증했다 — **ACTIVE 결정 ↔ AC 대조: 충돌 0**. D-111 ↔ AT-72(범위) · D-112·D-113 ↔ AT-73(문구) · D-114 ↔ AT-70(수명) · D-115 ↔ AT-71(계기) · D-116 ↔ AT-74(라벨) · D-117 ↔ AT-75(세그먼트) · D-118·D-119·D-120 ↔ AT-76(토큰) · D-121 ↔ §8 말미(비범위로 기록, AC 없음).
- [x] 산출물 문장 규칙을 지켰다 — 판정 먼저, 주장 한 줄에 관측 하나, 표 한 칸 3줄.
