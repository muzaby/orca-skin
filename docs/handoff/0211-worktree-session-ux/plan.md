...
| 작성자 | Claude Code |
| 일자 | 2026-08-30 (V1) · 2026-08-31 (ΔV1 · ΔV2) · 2026-09-02 (ΔV3 · ΔV4) |
| 매핑 | 0209·0210 격리 기능의 사용자 대면 잔여 3건 (준비 안내 · 표시 이름 · diff 실데이터) + 라운드 1 사용자 피드백 3건 (표시 정본 소멸 · 변경량 출처 · 조회 계기) + 변경사항 패널 UI/UX 명세 (Session Git Panel) + git 우측 패널 재설계 제안서 (Git Review Surface) |
| 상태 | READY (ΔV4 — git 우측 패널 UI/UX 재설계) |
| V mode | `Baseline V` + `Delta V` |
| 기준 V | `V1` @ `0d8cf037` (ΔV1 의 기준) · `V1 + ΔV1` @ `553da6a8` (ΔV2 의 기준) · `V1 + ΔV1 + ΔV2` @ `d23c5be` (ΔV3 의 기준) · `V1 + ΔV1 + ΔV2 + ΔV3` @ `46047ac` (ΔV4 의 기준) |
| 이번 V revision | `ΔV4` |
...
| D-069 | 컨텍스트 바의 비교 기준 라벨은 **세션 시작 시점의 브랜치 이름 하나**다. `→ 현재 브랜치` 화살표와 우측 값을 그리지 않는다. | 사용자 명시 — “세션 시작시 지정한 브랜치 (당시 head) 만 출력 우측화살표+우측값 표시하지 말것”. | 사용자 명시 | ACTIVE | D-043 의 기준선 표기 축을 대체 |
...
| D-074 | 파일 섹션은 **기본 전부 펼침**이고 본문은 **패치 조회 1회**로 전부 온다. 파일당 조회를 하지 않는다. | 사용자 명시 — “전부 펼침 + 파일 1개씩 조회가 아닌 전체 조회로 1회만 실행”. 지금은 파일 하나를 열 때마다 `gitApi.diffFile` 이 돌고(실측: 프로덕션 호출부 1건, `DiffTileContent.tsx:148`) 전부 펼치면 그 수가 파일 수만큼 난다. | 사용자 명시 | ACTIVE | D-061 을 대체 |
...
| D-078 | 패치 조회 계기는 **요약 세대당 최대 1회**이고 **타일이 닫혀 있으면 0회**다. 요약(`diffSummary`)의 계기 셋(cwd/세션 변경 · 턴 종료 · 새로고침)은 그대로다. | 요약은 컴포저 git 행도 읽으므로(D-025) 무거운 패치를 거기 실으면 타일을 열지 않아도 매 턴 전문맥 diff 를 돈다. 반대로 타일 열림을 *매번* 계기로 두면 사용자가 지적한 증상이 돌아온다 — “세대당 1회 + 세션 상태 보관”이 두 축을 함께 만족한다. | D-045 승계 + D-074 파생 | ACTIVE | — |
...
| D-082 | `미커밋 변경` 은 별도 블록이 아니라 **사이드바 커밋 목록의 한 항목**이다 — `모든 변경사항` 아래, 커밋 카드 위. 고르면 `HEAD` 대비 추적 변경 파일로 목록이 좁혀진다(diff 기준은 D-079 와 같다). | 제안서 §8.2 는 `All changes` + commit card 만 그리지만 “미커밋을 없애라” 는 문장은 없다 — 레이아웃 재구성은 명시적 제거가 아니다(§4 “이동은 제거가 아니다”). D-039 의 “두 위치의 의미가 다르다” 는 사용자 결정이라 새 IA 의 범위 선택 자리로 옮겨 0 비용으로 유지한다. | D-039 승계 | ACTIVE | D-039 의 배치 축을 대체 |
...
| D-086 | `⋮` 메뉴는 **여덟 항목**이다 — 파일 표시 · 모든 파일 접기 · 모든 파일 펼치기 · 나란히 · 자동 줄 바꿈(기본 켬) · 변경된 단어 강조(기본 켬) · 공백 변경 숨기기(기본 끔) · 새로고침. Git 조작(stage·commit·push) 항목을 두지 않는다. | 제안서 §12 도면 + 캡처 5 + §18 비목표. 수동 새로고침(D-030)이 여기로 옮겨 온다 — 능력을 없애는 것이 아니라 자리를 바꾼다. | 사용자 명시 | ACTIVE | D-030 의 자리를 대체 |
