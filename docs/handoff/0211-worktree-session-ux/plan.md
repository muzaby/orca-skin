...
| R-47 | AT-47 | 패치가 파일별 diff 줄을 싣고 상한 셋이 값으로 표시되며, 조회가 실패하면 **축소 문맥으로 한 번 더** 부른다 | 통합(임시 저장소): 추가·수정·삭제·rename·binary 다섯 파일에서 각 `status` 와 줄 배열이 실제와 맞는다고 단언. 삭제 파일의 모든 줄이 `removed`, 추가 파일의 모든 줄이 `added` 라고 단언. 순수: 파일 201건 → `files.length===200 && filesTruncated`; 줄 50,001건 파일 → `kind==='too-large'` 이고 `added`/`removed` 는 **그대로**; 전체 줄 200,001 → 넘긴 파일부터 `too-large`. 통합(인자 수집 runner): 첫 호출을 **실패시키면** 두 번째 호출 인자에 `--unified=3` 이 있고 `--unified=1000000` 이 **없으며** 결과가 `contextLimited:true` 라고 단언. 둘 다 실패하면 `files` 빈 배열 + `unavailable:true` | `gitApi.diffPatch` → `orca:git:diffPatch` → `gitDiffPatch()` → `run()` → `runGit` |
...
| MD-18 / VP-55 | 패치 상한은 **셋**이고 서로를 대신하지 못한다 (**EP-31, 3지점**: ① 파일 200(`MAX_DIFF_FILES` 재사용) ② 파일당 줄 **50,000** → `kind:'too-large'` ③ 패치 전체 줄 **200,000** → 넘긴 파일부터 `too-large`) | `git-diff-parse.ts` 상수 | 구현자 | 파싱 정규화 | ①만 있으면 파일 하나가 50만 줄일 때 IPC 페이로드가 터진다. ②만 있으면 5,000줄짜리 파일 200개(=100만 줄)가 통과한다. ③만 있으면 201번째 파일이 조용히 사라진다 — **세 축의 분모가 다르다** |
...
**renderer — 신규** (9): `lib/diffPatchLines.ts` · `lib/diffDisplay.ts` · `lib/changedFileTree.ts` · `hooks/useGitPatch.ts` · `rightpanel/GitContextBar.tsx` · `rightpanel/ChangedNavigationSidebar.tsx` · `rightpanel/DiffReview.tsx` · `rightpanel/FileDiffSection.tsx` · `rightpanel/diffComparison.ts`.

**renderer — 변경** (8): `lib/diffHunks.ts` · `rightpanel/DiffTileContent.tsx` · `rightpanel/DiffTileHeader.tsx` · `rightpanel/sessionChangesData.ts` · `rightpanel/tileRegistry.ts` · `reducer/chatReducer.ts` · `store/chatStore.ts` · `shared/api/ipc.ts`.

**renderer — 삭제** (10): `rightpanel/DiffPeek.tsx` · `rightpanel/SessionChangesList.tsx` · `rightpanel/diffBodyCache.ts` · `rightpanel/diffFileCache.ts` · `rightpanel/peekNavigation.ts` + 테스트 `diffPeek.render.test.ts` · `peekNavigation.test.ts` · `diffBodyCache.test.ts` · `diffFileCache.test.ts` · `diffPeekRequest.test.ts`.
