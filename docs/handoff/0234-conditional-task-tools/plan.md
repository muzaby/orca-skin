...
# 도구 계약 실측 · 결정 갱신 (2026-09-12)

> §4 의 **가용성 전제**("네 도구가 실제 세션에 나타나는지 먼저 확인한다")를 닫는다. **나타난다.**
> 근거는 같은 CLI 계열 세션에서 도구 스키마를 직접 조회한 결과다.

## Δ1. 가용성 — 닫힘

| 검사 | 관측 | 의미 |
|---|---|---|
| `sdk.d.ts` 의 `Monitor` | **0건** | 타입 정의에 없다. 여기까지만 보고 "가용하지 않음" 으로 판정하면 오판이다 |
| CLI 런타임 도구 목록 | `Monitor`·`TaskOutput`·`TaskStop`·`TaskGet`·`TaskList` **존재** | 도구는 타입이 아니라 **런타임이 노출**한다 |
| Orca 채팅 경로의 게이팅 | `claude.ts:437` `disallowedTools: ['Bash','WebSearch']` 뿐. `allowedTools` 화이트리스트 **없음**(`:272` 의 `allowedTools: []` 는 제목 생성 전용) | 나머지 도구가 **전부 모델에 도달한다** |

## Δ2. `Monitor` 의 실제 계약 — 가정과 다르다

`Monitor` 는 기존 실행 태스크를 조회하는 도구가 **아니다**. 긴 스크립트를 새로 띄우고 그 stdout
한 줄마다 이벤트를 보내는 **감시자**다.

| 인자 | 의미 |
|---|---|
| `command` | 임의 셸 스크립트. 각 stdout 줄이 알림 1건 |
| `ws` | WebSocket URL. 각 텍스트 프레임이 알림 1건 |
| `persistent` | 참이면 **세션 수명 내내** 돈다. 거짓이면 `timeout_ms`(기본 300000 · 최대 3600000) |

- `task_type: 'local_bash'` 를 셸과 공유하는 이유가 이것이다 — 실제로 셸을 돌린다.
- 백그라운드 작업 추적 수단으로 쓰려면 **출력 파일을 tail 하는 감시**를 거는 형태가 된다. 그 읽기는
  0232 D-204(Orca 하위 경로만 rw)와 0237 의 가드 수정이 선행이다.

## Δ3. D-407 — 닫힘

| ID | 결정 | 이유/조건 | 출처 | 상태 |
|---|---|---|---|---|
| D-407 | `Monitor`·`Workflow` 를 **`RISKY_TOOLS` 에 넣는다** | 둘 다 프로세스를 띄운다. 현재 `Monitor` 는 승인 없이 임의 셸을 실행한다 | 사용자 턴 2026-09-12 | **ACTIVE** |

### 왜 지금 구멍인가

| 경로 | `disallowedTools` | `RISKY_TOOLS` 승인 카드 | 결과 |
|---|---|---|---|
| `Bash` | 차단 | — | 도달 불가 |
| `PowerShell` | 미차단 | **있음** | 사용자가 보고 승인한다 |
| `Monitor({command})` | 미차단 | **없음** | **조용히 실행된다** |

- `canUseTool` 은 `isRiskyTool(toolName)` 이 거짓이면 `{ behavior: 'allow' }` 로 떨어진다(`claude.ts:215`).
  `RISKY_TOOLS` 는 `Bash`·`PowerShell`·`Write`·`Edit`·`MultiEdit`·`NotebookEdit` 여섯이다(`risky-tools.ts:6-14`).
- `guardToolAccess` 는 헤더가 밝힌 대로 셸 명령 문자열을 판정하지 않으므로 이 경로도 보지 않는다.
- 즉 `Bash` 차단과 `PowerShell` 승인 카드로 세운 셸 정책을 `Monitor` 가 **우회한다**.
- 사용자 지시로 순서는 **실기 결함 라우팅 뒤**다. 범위는 `Monitor`·`Workflow` 두 이름으로 한정한다
  (전수 도구 조사·화이트리스트 전환은 채택하지 않았다).

## Δ4. 남은 OPEN

| ID | 상태 |
|---|---|
| D-406 | A/B 분할 — **OPEN** 유지 |
| D-404 | `TaskOutput`/`TaskStop` 미채택 유지 — **강화됨**. 도구 설명 첫 줄이 `DEPRECATED` 이고 벤더가 `Read` 를 대체 경로로 지목한다 |
