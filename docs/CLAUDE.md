...
| `FRONTEND_ARCHITECTURE.md` | **Renderer 진실 원천** — Tailwind v4 + React Context + useReducer 상태 관리, 컴포넌트 구조, 도메인 화면 카탈로그 (Chat / Projects / Engine / SkillsMcp / Tweaks), IPC 호출 패턴 | Renderer 작업할 때. UI / 상태 / 사용자 입력 / 자동완성 / Tweaks. |
...
3. **문서에 명시된 결정을 임의로 바꾸지 마라.** 예: "Provider 어휘 폐기, Backend/Adapter 로 통일" (GLOSSARY §3), "2 계층 영속성 모델" (BACKEND §6.3), "어댑터별 자격증명 저장" (BACKEND §8.4) 같은 채택 결정은 협의 결과다. 변경이 필요하면 사용자에게 먼저 확인.
