...
| `FRONTEND_ARCHITECTURE.md` | **Renderer 진실 원천** — Tailwind v4 + React Context + useReducer 상태 관리 (현재), **Zustand 전환 채택 결정 (Phase 4 진입 PR 묶음, 단일 root + sessions 슬라이스)**, 컴포넌트 구조, 도메인 화면 카탈로그 (Chat / Projects / Engine / SkillsMcp / Tweaks), IPC 호출 패턴, **DOM Architecture Specification (§3.3 — `app-frame-*` 마커 체계 + Custom titlebar `frame:false` + Grid z-stack (overlay/modal/debug 슬롯) + Sidebar resize-handle + Tile structure, Phase 3+ 적용 완료)** | Renderer 작업할 때. UI / 상태 / 사용자 입력 / 자동완성 / Tweaks / DOM 마크업 컨벤션. |
