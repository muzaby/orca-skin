...
| `FRONTEND_ARCHITECTURE.md` | **Renderer 진실 원천** — Tailwind v4 + React Context + useReducer 상태 관리 (현재), **Zustand 전환 채택 결정 (Phase 4 진입 PR 묶음, 단일 root + sessions 슬라이스)**, **Feature-based 목표 구조 (§3-2): `frame/` 완전 해체 → app/ + features/ + shared/ 분산. `pages/` = 조립만(비즈니스 로직 없음). `router` = 어느 pages/를 보여줄지. `features/<domain>/` = 비즈니스 로직. 구현 대기 (§3.A / §10 명시).**, 도메인 화면 카탈로그 (Chat / Projects / Engine / SkillsMcp / Tweaks), IPC 호출 패턴, **DOM Architecture Specification (§3.3 — `app-frame-*` 마커 체계 + Custom titlebar `frame:false` + Grid z-stack (overlay/modal/debug 슬롯) + Sidebar resize-handle + Tile structure, Phase 3+ 적용 완료)** | Renderer 작업할 때. UI / 상태 / 사용자 입력 / 자동완성 / Tweaks / DOM 마크업 컨벤션. |
...
| `IPC_CONTRACT.md` | **Main ↔ Renderer 채널 SSOT** — Phase 2 활성 11 채널 + **Phase 3+ window 3 채널 = 14 채널**, ChatEvent variant, ErrorCode, 변경 절차 | IPC 채널 추가/변경할 때. FRONTEND/BACKEND 가 이 문서를 인용. |
