...
| Tweaks (theme/density/sidebar) + electron-store 영속화 | Phase 2+ | ✅ 완료 | `shared/theme/TweakProvider` — 컨트롤 UI 는 dev 전용 `features/debug/components/DebugPanel.tsx` (구 shared/ui/TweaksPanel 부재). 테마는 white/dark 2종 |
...
| **FTS5 대화 검색 모달** | Phase 3++ | ✅ 완료 | `0003_messages_fts.sql` 마이그레이션 (가상 테이블 + 3 트리거 + 백필). `orca:search:messages` IPC (IPC_CONTRACT §2.9). `features/sessions/components/SearchModal.tsx` (150ms debounce + request id supersede + `<mark>` split-parse XSS 방어). `toFtsMatch` 가 모든 토큰에 prefix wildcard `*` 부착. |
...
| **플러그인 카탈로그 탭** | Phase 4 | ✅ 완료 | 설정 카탈로그 첫 탭 — 전체 connection row의 상태·방식 선택·인증·재인증·해제를 유지하고, plugin row는 local icon과 locale별 title/body를 표시한다. 순수 판정은 `features/skills/lib/providerRows.ts`, 표시 해석은 `features/skills/lib/pluginPresentation.ts`, 상세는 `ProviderDetail.tsx`. **추가 버튼 없음**(빌드타임 선언) |
