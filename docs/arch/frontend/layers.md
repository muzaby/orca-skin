...
└── src/
    ...
    │   ├── Sidebar.tsx              # `app-frame-sidebar` — NAV 4-항목(새 대화·프로젝트·엔진&모델·스킬&확장 기능) + collapsible/resizable + 슬롯 (sessions/footer). 마지막 항목은 modal store action. React.memo 유지
    ...
    │   ├── router.tsx               # `<Routes>` — URL path → Page. `/skills`는 카탈로그 모달을 열고 `/new`로 replace하는 딥링크 호환 route
    ...
    │   ├── SkillsPage.tsx           # `/skills` 딥링크 → extensionsModalStore open + `/new` replace
    │   ├── CapturesPage.tsx
    ...
    │   ├── skills/                  # 스킬·커넥터·플러그인 카탈로그 Modal + table→detail depth, extensionsModalStore, MCP/Skill CRUD
...
- **`shared/`** (renderer 내부): 도메인 지식 없는 재사용 자산. `ui/` = 도메인-무관 UI 원자 (구 `components/atoms/`). `hooks/` = 도메인-무관 hook — 셸 컴포넌트의 _재사용 가능한 메커니즘_ (예: 1D 드래그→숫자 `useDragResize`) 도 여기에. _도메인 특정 설정_ (sidebar 폭 경계, sessions 액션 wiring 등) 은 shared 자격 미달. `api/ipc.ts` = `window.orca.*` 타입드 래퍼. `config/` = 상수·테마 타입. **store / 도메인 hook import 금지.**
