...
세션 시작으로 실제 프로젝트가 생성되면 Main은 최초 세션 확정 이벤트에 `projectCreated` 신호를 보낸다. app의 `useProjectCatalogSync`가 프로젝트 목록과 해당 대화 목록을 조회하고, sessions feature의 `projectNavStore`가 ID별 펼침 요청을 보관한다. 목록 지연·고정 그룹 이동에도 펼침 상태가 남고, 같은 생성 신호를 다시 받아도 사용자가 접은 상태를 덮지 않는다.

프로젝트 대화 행과 프로젝트·아티팩트 목록은 도메인 없는 `CatalogListRow`를 공유한다. 프로젝트 목록의 보조 메타는 경로이며 개별 프로젝트의 대화 행에는 보조 메타를 전달하지 않는다.
