...
**③ 현재 코드 갭.** 스테이지 C2 (`2d4ab8f`) 로 `PlanApprovalCard` → `features/chat/components/ApprovalCard.tsx` 일반화 — `permission.requested` 의 `action.kind` 분기. **스테이지 C 마무리로 `tool_approval` 게이트 활성** — `ApprovalCard` 가 `pendingPlanReview`/`pendingToolApproval` 둘 다 디스패치하고, `ToolApprovalBody`(거부/세션 동안 허용/허용 3지)가 위험 도구 승인을 surface 한다. main 의 `makeCanUseTool` 이 `RISKY_TOOLS` 화이트리스트(Bash/Write/Edit/MultiEdit/NotebookEdit)만 게이트하고 안전 도구는 자동 통과(../backend/provider-runtime.md §3 ③). `AskUserQuestionCard.tsx`(명확화 질문)는 별도 유지. **잔여 갭**: `pendingApprovals` 단일 큐 미통합(도메인별 분리 저장 `pendingAsks`/`pendingPlanReview`/`pendingToolApproval` 으로 충족 — 불필요) · PermissionModeController(6종 런타임 전환) 미연동(현재 Composer `ModeMenu` 가 per-turn 2종 plan/acceptEdits 만).
...
| `skills` (`features/skills/components/customize/SkillsCustomizeView.tsx`, 구 SkillsMcpView) | 04 스킬 & 확장 기능                               | (모달)             | ✅ '스킬 & 확장 기능'         | **✅ 카탈로그 모달 활성**            | 현재 화면 위 공용 `Modal`로 열림. 좌측 스킬·커넥터·플러그인 레일 + 우측 table→detail depth. Orca builtin marker 항목만 플러그인으로 분류하고 사용자/adapter 스킬은 스킬에 유지. MCP CRUD는 커넥터 탭에서 실 연동. |
...
>
> **Sidebar nav 재구성 (Phase 3++)**: nav 노출은 4-항목 (새 대화·프로젝트·엔진 & 모델·스킬 & 확장 기능). 마지막 항목은 route 이동 없이 현재 화면 위 모달을 연다(`/skills`는 딥링크 호환용). 자동화/captures 는 _Future Scope 로 nav 미노출_.
...
- **스킬 & 확장 기능 — 커넥터**: ✅ `orca:mcp:*` CRUD + `McpStore`(safeStorage) + `handleChatSend` 가 `query().options.mcpServers`+`allowedTools` 주입 (전역). UI 용어는 커넥터지만 런타임 계약은 MCP를 유지한다. 권한 섹션 + `options.permissionMode`/`canUseTool` 는 Phase 4+ (PRD OQ9 미정).
...
- Engine 화면은 하드코딩 샘플 대신 orca.json agent 환경을 렌더하며 `adapter`/`provider`/`models` 만 표시한다. 동작하지 않는 "엔진 추가"/"+ 모델" 버튼은 mock 규약을 따른다.
