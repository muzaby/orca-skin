...
목록은 왼쪽 접기 버튼이 있는 실행 중/완료 그룹으로 나뉜다. 성공·실패·중단이 완료 그룹에 들어가며 오른쪽 휴지통은 현재 완료 카드만 지운다. 제거한 표시 상태는 세션 왕복과 패널 재열기 동안 유지하고 앱 재시작 시 복원되는 원본 이력은 삭제하지 않는다. 실행 상태가 불명확한 작업과 새로 완료된 작업은 지우지 않는다.
