...
Work Transcript의 일반 출력과 게시 아티팩트 카드는 원래 턴의 마지막에 모은다. 확정/live 본문 아래 spark/status와 완료 메타를 배치하고 그 뒤에 카드를 표시한다. 일반 출력은 `output.captured`의 도구 ID 또는 Main이 확인한 응답 경계로 연결하며 영속 artifact part로 재로드한다. 소유권 근거가 없는 과거 일반 출력은 우측 출력 목록에 유지하고 메시지 소유자를 시각으로 추측하지 않는다. 두 표면은 `ArtifactCard`를 사용하며 게시 카드의 부제는 아티팩트만 표시한다. 메뉴의 보관 폴더 열기·상단 파일 메타와 작업 성공 안내는 표시하지 않고 부분 실패·접근 오류·재시도는 유지한다.
