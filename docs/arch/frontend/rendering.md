...
Work는 랜딩에 패널을 표시하지 않는다. 첫 전송이나 처음 불러온 대화에서 작업 타일을 열고, 이후 케밥에서 켜거나 끈 선택은 세션 캐시 안에서 유지한다. 케밥의 타일 항목은 작업만 제공한다. 작업이 있으면 번호·상태·제목이 있는 세로 목록을, 없으면 빈 상태 그림을 표시한다. 질문 버튼은 기존 입력 뒤에 작업 인용을 추가하고 포커스한다. 진행 상황·출력·컨텍스트를 접거나 패널 폭을 확대·복귀할 수 있다. 메뉴·이벤트·저장 상태의 복원·최종 렌더는 같은 모드별 표시 정책을 사용한다. 세션을 이동하면 패널 본문의 로컬 접힘·선택 UI를 초기화한다.

출력은 명시 게시된 산출물이며 기존 저장·부재·삭제 동작을 재사용한다. 컨텍스트에는 사용자가 추가한 허용 폴더를 표시하며, 현재 작업 폴더도 직접 선택한 경우 목록에 남긴다. 유휴 Work 대화는 폴더 추가를 DB에 저장하고 다음 요청부터 적용하며, 실행·준비 중에는 Main에서도 거부한다([IPC 계약](../../IPC_CONTRACT.md)). 일반 생성물 자동 수집·실제 참조 추적·파일 뷰어는 현재 제공하지 않는다. Work의 계획 승인과 서브에이전트 상세는 해당 대화 영역 안에서 확인한다.
...
작업 타일은 진행 상황·출력·컨텍스트 섹션을 제공한다. 출력 접기는 카드의 파일 상태 구독을 정리하고 목록 메타 구독은 헤더 개수 갱신을 위해 유지하며, 타일 닫기·작업 상세 진입·세션 전환은 목록 구독도 해제한다. 컨텍스트 리소스 수집과 본문 미리보기·파일뷰어는 제공하지 않는다.
