...
`CanonicalBackgroundContent`는 main에서 관측한 호출·작업을 표시한다. 셸과 snapshot-only 작업도 포함하며 실행 상태·live 소속·연결·중단 확인을 각각 보여 준다. 출력은 main 소유 참조의 현재 파일과 완료 snapshot을 선택해서 읽고 텍스트로 표시한다. 중첩 Agent 상세는 부모 도구 ID를 따라 연결하며 구조화 결과와 메타를 보존한다. 이전 이력은 기존 parts 기반 표시를 사용할 수 있다. 상태 계약은 [백그라운드 작업](../backend/background-tasks.md)을 따른다.
