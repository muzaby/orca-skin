...
Coding은 계획 패널 상단에 ExitPlanMode 본문, 하단에 작업 목록·상세를 표시한다. 독립 작업 타일은 표시하지 않는다. Work는 랜딩과 기존 대화 모두 작업 타일을 상시 표시하며 케밥의 타일 항목도 작업만 제공한다. 진행 상황·출력·컨텍스트를 접거나 패널 폭을 확대·복귀할 수 있다. 메뉴·이벤트·저장 상태의 복원·최종 렌더는 같은 모드별 표시 정책을 사용한다. 세션을 이동하면 패널 본문의 로컬 접힘·선택 UI를 초기화한다.

출력은 명시 게시된 산출물이며 기존 저장·부재·삭제 동작을 재사용한다. 컨텍스트에는 사용자가 추가한 허용 폴더를 표시한다. 유휴 Work 대화는 폴더 추가를 DB에 저장하고 다음 요청부터 적용하며, 실행·준비 중에는 Main에서도 거부한다([IPC 계약](../../IPC_CONTRACT.md)). 일반 생성물 자동 수집·실제 참조 추적·파일 뷰어는 현재 제공하지 않는다. Work의 계획 승인과 서브에이전트 상세는 해당 대화 영역 안에서 확인한다.
