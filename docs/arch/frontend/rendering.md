...
`CanonicalBackgroundContent`는 main에서 관측한 호출·작업을 Agent 형태의 카드로 표시한다. 셸과 snapshot-only 작업도 포함하며 카드에 제목·종류·상태·경과 시간과 개별 중단을 표시한다. 선택 좌표는 세션과 generation/taskId이며 기존 toolUseId 선택과 분리한다.
...
셸 등의 출력은 상세에서 main 소유 참조의 현재 파일과 완료 snapshot을 읽고 텍스트로 표시한다. Agent의 출력 참조도 내부 상태에 보존하며 중첩 대화록은 부모 도구 ID로 연결한다. 이전 이력은 기존 parts 기반 표시를 사용할 수 있다. 상태 계약은 [백그라운드 작업](../backend/background-tasks.md)을 따른다.
