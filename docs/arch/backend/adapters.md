...
async function* sendMessage(sessionId, text, cwd, caps, resolvedMcp, signal) {
  try {
    const opts = {
      ...
      // req.providerSettings.sourcesSettingsFile → cwd/.claude/settings.local.json 복사
...
`adaptMcp` 는 활성 서버가 없으면 옵션 자체를 빈 객체로 반환(생략). `allowedTools` 는 `mcp__<name>__*` 와일드카드로 서버 전체 도구 자동 허용 — `canUseTool` 미도입(Phase 4 anchor) 환경에서 도구 호출 차단 방지. Provider settings 는 query 옵션으로 주입하지 않는다. `ResolvedProviderSettings.sourcesSettingsFile` 을 작업 디렉토리의 `.claude/settings.local.json` 으로 복사해 Claude Code 기본 settings source 정책에 맡긴다(해석/캐시는 `settings/provider-settings.ts`, materialize 는 `adapters/claude-settings-local.ts`).
...
- MCP 는 `options.mcpServers` 로 별도 주입 (이중 주입 없음). settings.json 은 플러그인 스펙 밖이라 query cwd 의 `.claude/settings.local.json` 으로 복사 (TRD §6.8).
