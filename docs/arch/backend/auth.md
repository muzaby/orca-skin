...
adapter patch가 생략됐거나 비어 있고 legacy 동적 레이어도 없는 턴에는 settings 채널을 건드리지
않는다. patch에 null directive만 있어도 상속 process env에서 키를 제거해야 하므로 완전한
`options.env`를 만든다. 모든 레이어를 접은 최종 `CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST` 값이 정확히
`1`이면 settings env 전체를 `options.env`로 hoist하며 adapter의 set/delete가 이 판정에도 최우선이다.
**디스크 `settings.json` 은 수정하지 않는다.**

**순서가 곧 우선순위다.** 구현은 legacy 값을 `baseEnv → appEnv → settings env → runtimeEnv →
customEnv`로 얹은 뒤 adapter patch의 문자열을 설정하고 null 키를 삭제한다. app을 settings 뒤에
얹으면 전역 폴백이 ModelProvider 전용 설정을 덮는다. injector는 legacy 최상위 환경 적응이고,
adapter patch는 활성 `SessionAdapter`가 `AgentKind`별 예약 키를 강제하는 최종 정책이다.
...
등도 같은 spawn 환경을 쓰며, legacy 순서에서 settings/app/process 값을 덮는다.

종류별 정책은 `SessionAdapter.agentSpawnEnv(agentKind)`가 소유한다. 현재 Claude Work는 Todo/Tasks와
foreground-only background-task 정책 세 키를 `1`로 강제하고 Claude Code는 같은 키를 제거한다.
generic `prepareHarnessConfig()`는 adapter id를 분기하지 않고 전달받은 patch만 최종 적용한다.
...
Claude settings 파서는 `sonnet`·`opus`·`haiku`·`fable` 네 family와
`ANTHROPIC_DEFAULT_FABLE_MODEL`을 같은 경로에서 인식한다. `availableModels`의 Fable 항목도
`modelIdentity`(모델명 + `[1m]`)를 통해 Composer와 Engine 카드에서 동일하게 식별한다.
