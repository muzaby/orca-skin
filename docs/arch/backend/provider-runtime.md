...
- **활동 스냅샷(0143·0167)**: 턴-후 루프는 transport 사실만 projector에 설정하고, `SessionActivityProjector`가 lease·pending queue·background tracker와 합쳐 `chat.activity` 전체 스냅샷을 broadcast한다. renderer는 revision 가드 뒤 `inflight ‖ listening` UX를 유지하되 전달 확인·잔여·백그라운드 작업 중 무엇을 기다리는지 StatusLine에 표시한다.
