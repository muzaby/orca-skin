...
>
> **정적 사용량 provider opt-in (0099)**: 외부 quota/usage report provider 는 기본 배포에 포함하지 않는다. 신규 설치의 활성 정적 provider 목록은 0개이며, `sources/settings/<adapter>/<provider>/settings.json` 도 bedrock/vertex/custom 등을 선생성하지 않는다. 회사별 연동은 `app/src/main/features/providers/static/modules/<name>/` 에 배포 소유 모듈을 두고 `modules/index.ts` 레지스트리에 명시 추가하는 opt-in 절차다. 이때 `ExternalUsageService`·scheduler·IPC·tracker·provider enumeration·materializer 는 `StaticUsageProviderModule[]` 만 소비하며 provider 이름 분기를 추가하지 않는다. 기존 settings 파일은 삭제하지 않지만, 모듈을 레지스트리에 다시 등록하기 전까지 외부 사용량 refresh 는 비활성이다.
