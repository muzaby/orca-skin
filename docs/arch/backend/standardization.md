...
>
> **사용량 provider opt-in (0099 → 0183)**: 외부 quota/usage report 는 기본 배포에 포함하지 않는다. 신규 설치의 활성 사용량 선언은 0개이며 `sources/settings/<adapter>/<provider>/settings.json` 도 bedrock/vertex/custom 을 선생성하지 않는다. 회사별 연동은 **`features/providers/declarations/` 의 `Provider.usage` 선언 하나**로 한다(0183 — 구 `static/modules/` 레지스트리는 제거). `ExternalUsageService`·scheduler·IPC·tracker 는 선언에서 파생된 spec 만 소비하며 provider 이름 분기를 추가하지 않는다. 기존 settings 파일은 삭제하지 않지만, 선언에 `usage` 를 다시 넣기 전까지 외부 사용량 refresh 는 비활성이다.
