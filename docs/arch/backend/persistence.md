...
- 도구는 채널의 cwd·extraDirs 또는 OS 사용자 Temp 아래의 `orcinus-orca` 앱 폴더와 그 하위에 있는 완성된 로컬 Markdown/HTML·텍스트/코드·이미지 파일을 읽어 게시 원본으로 복사한다. OS Temp 부모와 형제는 자동 게시 입력에 포함하지 않는다. Temp 경로는 `infra/config/temp-path.ts`를 따르며 게시 입력 허용에만 사용한다. 허용 확장자·언어·MIME 정본은 `features/artifacts/formats.ts`이며 UTF-8 또는 이미지 서명과 파일 크기를 검사한다. 입력 원본을 이동하거나 삭제하지 않는다. 일반 readRoots의 앱 설정/런타임 경로는 게시 입력 권한에 포함하지 않는다.
- 일반 출력은 Work에만 주입하는 `TurnExtensions.outputFiles`로 수집한다. 완성본 위치는 OS 사용자 임시 폴더 아래 `orcinus-orca` 앱 폴더 바로 아래 파일이다. 공통 `infra/config/temp-path.ts`가 OS의 `os.tmpdir()` override를 기준으로 앱 폴더를 결정한다. Code와 Work 모두 SDK의 additionalDirectories와 workspace 가드에 같은 앱 루트를 추가하며 사용자 extraDirs 기록은 바꾸지 않는다. 성공한 Write/Edit의 파일과 Stop 응답의 명시적 로컬 Markdown 파일 링크를 수집하고, 루트 바로 아래 파일만 받아 중간 하위 폴더와 다른 경로·리디렉션을 제외한다. 디렉터리 전체를 스캔하지 않는다.
- 일반 파일은 `category: file`의 메타데이터와 세션 참조를 등록하며 보관 사본을 만들지 않는다. 파일 작업은 등록한 `input_source`가 `orcinus-orca` 앱 폴더 바로 아래의 일반 파일인지 재검사하여 원본을 사용한다. 기존 DB의 고유 `relative_path` 필드는 일반 파일에서 `temporary/<파일 ID>/<파일명>` 등록 키이며 실제 읽기 경로가 아니다. 같은 세션·원본 경로·category의 최신 버전을 표시하며 동일 내용의 반복 수신은 기존 참조를 재사용한다. Office·PDF 등은 원본 다운로드를 지원하고 미리보기 지원 형식과 별개로 분류한다. 크기 제한과 DTO는 [IPC 계약](../../IPC_CONTRACT.md#26-a-산출물-게시-파일)을 따른다.
- `orcinus-orca` 앱 임시 폴더는 Code와 Work 세션이 공유하므로 모델에는 작업별로 다른 파일명을 사용하도록 안내한다. Write 수집은 입력 본문과 읽은 파일 내용까지 확인하고 불일치는 거부한다. Stop 링크는 모델이 완성본으로 명시한 현재 파일을 등록하며 파일 작성 주체를 증명하는 별도 OS 격리는 아니다. 동일 내용이면 기존 참조를 재사용하고 변경된 내용은 새 메타데이터로 등록한다. 같은 시각의 버전도 원본별 게시 시각을 단조 증가시켜 최신 순서를 보존한다. 정책 변경 전 OS Temp 부모의 일반 출력은 이관하거나 새 자동 범위에서 읽지 않으며, 새 앱 폴더 원본 소실 시 관리 사본으로 대체하지 않는다.
- 일반 출력 hook 결과는 SDK result 경계에서 telemetry 전에 `output.captured`로 합류한다. Write/Edit는 원래 tool ID의 메시지, Stop은 현재 확인된 응답 경계의 메시지에 artifact part를 영속한다. 다른 세션·없는 호출·없는 경계는 연결하지 않는다. 같은 파일의 같은 메시지 연결은 멱등이며, 이후 턴에서 명시적으로 재수집하면 그 턴에도 연결한다. 중단된 호출의 지각 결과는 버린다.
...
- Composer의 파일 선택·드래그·클립보드 이미지 첨부는 전송 정규화 단계에서 같은 `orcinus-orca` 임시 폴더 아래 고유 파일명으로 보관한다. 원래 표시 이름과 보관 경로·SHA-256을 첨부 파트에 보존하고, 이미지 썸네일과 기존 모델 본문 주입은 유지한다. 컨텍스트는 이 첨부 메타데이터를 사용하며 경로가 없는 과거 첨부는 이름만 표시한다. 출력 수집은 세션에 등록된 첨부와 경로·내용 hash가 같은 파일을 제외한다.
