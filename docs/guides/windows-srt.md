...
명령 정본은 [`app/package.json`](../../app/package.json), 검사·설치·실증 동작은
[`check-sandbox.mjs`](../../app/scripts/check-sandbox.mjs), native 전달 계약은
[`sandbox-launch-frame.mjs`](../../app/scripts/sandbox-launch-frame.mjs)와
[`main.cpp`](../../app/native/sandbox-launcher/main.cpp)다. 설계 근거는
[도입 제안](../etc/study/srt/orca-adoption-proposal.md), 단계·판정 상태는
[handoff 보드](../handoff/INDEX.md)를 확인한다(0219).
...
| 저장소 의존성 | 프로젝트 의존성을 먼저 준비한다. Node/Electron ABI 관련 설치 주의사항은 [`app/AGENTS.md`](../../app/AGENTS.md)를 따른다. |
...
[`build-sandbox-launcher.mjs`](../../app/scripts/build-sandbox-launcher.mjs)가 갖는다.
...
상류 근거와 제품 수용이 필요한 조건은 [도입 제안 §10](../etc/study/srt/orca-adoption-proposal.md#10-srt만-사용하는-조건에서-남는-실제-한계)을 따른다.
...
앱 연결은 공통 실행 lease·utility process·Claude custom spawn을 통합하는 후속 단계에서
진행한다. 계획 파일 Write, Confluence 다운로드 후 Read, resume/fork와 배포 경로는 그 단계의
회귀 검증 대상이다. OpenCode와 cowork 제품 기능은 별도 후속이며 이 절차에 포함하지 않는다.
