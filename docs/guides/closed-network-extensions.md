...
- **자동 업데이트**: 피드가 설정되지 않으면 updater 는 이미 noop 으로 저하된다(`feed-not-configured`, `app/src/main/app/updater.ts`). 사내 업데이트 서버를 쓰려면 electron-builder `publish` 설정을 사내 generic feed(`latest.yml` 호스팅)로 지정한다. 외부 GitHub Releases 피드는 폐쇄망에서 자연히 불능이다.
