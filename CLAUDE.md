...
| **범용 Provider Runtime 재설계 (설계 + 인터페이스)** | **설계 확정 / 구현 대기** — OpenCode/Claude 공통 정규화 계층을 기존 아키텍처 문서에 흡수. `docs/[docs/arch/backend/provider-runtime.md](docs/arch/backend/provider-runtime.md) (정본: NormalizedEvent·PermissionBridge·ApprovalResolution 2분기·AppCommandPolicy 3분기·PermissionModeController·SessionCapability·RevertManager·ErrorClassifier·AppMessagePart·Telemetry·AuthStore·AuditLog·DirectBackendAPI·WorkspaceManager·ConfigManager·우선순위 P0/P1) + `docs/[docs/arch/frontend/rendering.md](docs/arch/frontend/rendering.md) §1.6~6.9·§7.6 (ToolRendererRegistry·ApprovalCard 일반화·StructuredOutput·Streaming·TelemetryPanel). Claude 기준 + OpenCode 확장점만. 코드 변경 0 (rename 범위 밖, 매핑표로만). 두 SDK 미설치 → `[미확인]` 보존. |
