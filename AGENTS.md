...
- 흐름: Claude `plan.md`(READY, Delta V 확정) → Codex 구현 + 게이트 통과(`impl/IMPL_DONE`) → Claude `verify.md`(`PASS`/`FAIL`/`RETURN_TO_PLAN`). `FAIL`은 명시 pair·전역 불변식 위반이라 구현자가 고치고, `RETURN_TO_PLAN`은 필요한 Decision·AC·V 노드/pair·§10이 누락·모순된 `PLAN_GAP`이라 Claude가 규범 행을 먼저 정정한다. 공통 V 추적 규약과 상태 정본은 [`docs/handoff/AGENTS.md`](docs/handoff/AGENTS.md).
