...
| `src/main/config/{paths,mcp-file,secret-store}.ts`        | `~/.config/orca` 루트·확장 디렉토리(`skills`/`agents`/`commands`) 헬퍼 + `mcp.json` atomic R/W (정규 소스) + 비밀 저장소(env-var 이름 키잉, safeStorage) | 구현됨 |
...
| `src/main/skills/plugin-bundle.ts`                        | **확장 정규 레이어 골격 보장**(`ensureOrcaPlugin`, 멱등) — `~/.config/orca` 디렉토리 *자체*를 Claude 로컬 플러그인으로 머티리얼라이즈(`.claude-plugin/plugin.json` + `skills/`·`agents/`·`commands/`). claude-code query() 에 `plugins:[{local, path: ~/.config/orca}]`+`skills:'all'` 명시 로드 | 구현됨 |
