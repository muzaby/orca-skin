...
| **범용 Provider Runtime 재설계 (설계 + 인터페이스)** | **OpenCode/Claude 공통 정규화 계층을 기존 아키텍처 문서에 흡수.** 별도 문서 없이 `docs/BACKEND_ARCHITECTURE.md` §12 (정본 — NormalizedEvent · PermissionBridge · ApprovalResolution 2분기 · PendingApprovalStateMachine · AppCommandPolicy 3분기 · PermissionModeController · SessionCapability · RevertManager · ErrorClassifier · AppMessagePart persistence · Telemetry · AuthStore · AuditLog · ModelProviderConfig + 현행→목표 매핑표 + SDK 타입 확정 절차) + `docs/FRONTEND_ARCHITECTURE.md` §6.6~6.9·§7.6 (ToolRendererRegistry · ApprovalCard 일반화 · StructuredOutput · Streaming lifecycle · TelemetryPanel) 에 인터페이스 코드블럭으로 정의. **Claude 기준 + OpenCode 확장점만**, 각 항목 ①설명②예시③현재 코드 갭④인터페이스 4단. **코드 변경 0** (실구현 rename 범위 밖 — 매핑표로만). `[검증]`/`[미확인]` 태그 보존(두 SDK 미설치). | **설계 확정 / 구현 대기** |
| 후속    | CI 워크플로우 (`.github/workflows/`), Playwright 테스트, opencode 어댑터(변환기는 구현됨 — 어댑터/라이프사이클만 남음), `V1Captures` 실 구현 (캡처 RAW 보관 + AI 분석), 다국어 (`src/shared/i18n/ko.ts`), 세션 휴지통 30일 보존 (soft delete), 세션 메타 LRU 캐시 cap, 자동 제목 생성 (요약), `pages/RoutinesPage.tsx` placeholder + `/routines` 라우트 등록, **Phase 4 (Zustand 전환 + 멀티세션, `FRONTEND_ARCHITECTURE.md` §4.4)**. | Future Scope                |
