...
| 경로                                                      | 책임                                                                            | 현 상태                                 |
| --------------------------------------------------------- | ------------------------------------------------------------------------------- | --------------------------------------- |
| `src/main/index.ts`                                       | Electron `app` 부트, BrowserWindow (`frame: false` + macOS `titleBarStyle: 'hidden'` + `trafficLightPosition`), IpcRouter 부착, 윈도우 컨트롤 IPC 3개 직접 부착. **`whenReady` 최상단에서 `loadOperatorEnv()` (orca.env 주입) 선행** | 구현됨 (Phase 3+)                       |
...
| `src/main/runtime/`                                       | **Python 런타임 (uv 격리 인터프리터).** `paths.ts` (userData 경로 + 번들 uv 해석) · `env.ts` (`buildPyEnv` 단일 소스 + `PY_AGENT_RULES` + `UV_NATIVE_TLS` 기본 ON) · `PythonRuntime.ts` (멱등 초기화 상태기계 + `.ready` 마커 + EventEmitter status + 폐쇄망 remediation 힌트) · **`envFile.ts` (`loadOperatorEnv` — `orca.env` 3계층 주입 + `ORCA_ENV_TEMPLATE`)**. `IpcRouter.start()` 가 `ensure()` 비동기 킥, `handleChatSend` 가 `getEnv()` 를 SDK `query().options.env` 로 주입. 폐쇄망 배포는 `docs/CLOSED_NETWORK_RUNTIME.md` | 구현됨 (Phase 3++)        |
