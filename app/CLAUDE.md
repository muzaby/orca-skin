...
│   │   ├── components/PlanApprovalCard.tsx # plan 검토 시 Composer 입력창을 대체하는 승인 카드 (거부/수정…/수락)
...
| `src/main/config/{paths,mcp-file,secret-store,migrate-sources}.ts` | **sources/dist 레이아웃 헬퍼** (`sourcesDir`/`distDir(engine)`/`sourcesInstructionsDir`/`agentsMdPath`/`sourcesHooksDir`/`distSkillsDir` 등) + `mcp.json` atomic R/W (정규 소스, 이제 `sources/mcp/mcp.json`) + 비밀 저장소(env-var 이름 키잉, safeStorage) + **구 평면 레이아웃 → `sources/` 1회 이전**(`migrateConfigToSources`, 멱등, root 파라미터로 테스트 가능) | 구현됨 (표준화 §5.1) |
