...
| **2.1 F3 — ClaudeCodeAdapter 실제 구현** | **완료** — claude-code.ts `sendMessage()` 전체 구현: child_process.spawn, NDJSON 파싱, --resume 플래그, ChatEvent 정규화, 에러 처리 |
...
| `src/main/adapters/claude-code.ts` | Claude Code spawn / NDJSON / `--resume` | **완료** (`isInstalled()` + 로깅, `sendMessage()` 완성 + NDJSON 파싱, listSessions/loadSession Phase 2/3 스텁) |
