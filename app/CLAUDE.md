...
│   │   ├── components/PlanTile.tsx         # 우측 계획 타일 — plan 마크다운 + 승인/수정/거부 액션바 (ExitPlanMode 자동 오픈)
...
| **Phase 3++ (plan 우측 분할 타일)** | **계획을 우측 분할 타일로.** `ExitPlanMode` 가 제출한 `plan` 마크다운을 채팅 tile 우측의 `PlanTile`(`app-frame-tile-separator` 분리선 + `useDragResize({invert})` 280–640px)에 표시하고 **자동 오픈**(plan_review → `planTileOpen=true`). 수동 토글은 채팅 titlebar 우측 끝 `panelR` 버튼. 승인/수정 제안/거부 액션바를 인라인 카드(`PlanReviewCard` 삭제)에서 타일 푸터로 완전 이전 — 검토 종료 후에도 `planContent` 유지(읽기전용). `ChatState` 에 `planTileOpen`/`planTileWidth`/`planContent` 추가, `useChat` 에 `togglePlanTile`/`closePlanTile`/`setPlanTileWidth`. `Icon` 에 `panelR` 추가. | **완료** |
