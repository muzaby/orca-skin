...
이 디렉토리는 **Orca v1 의 실제 구현체**가 사는 곳이다. 현재는 Electron Forge `webpack-typescript` 템플릿이 깔린 *스캐폴드 상태* 이며, 본 구현은 `docs/TRD.md` 의 사양을 따른다.
...
| Electron | 42.0.1 |
| TypeScript | 4.5.4 — TRD 의 `strict + target: ES2022` 적용 시 5.x 로 갱신 필요 |
| 메인 (`src/index.ts`) | Forge 기본 BrowserWindow 만 있음. **`contextIsolation` / `nodeIntegration` / `sandbox` 명시 누락** — TRD §1.3 보안 베이스라인 미충족. |
| 렌더러 (`src/renderer.ts`) | CSS 임포트 + `console.log` |
| 프리로드 (`src/preload.ts`) | 빈 스텁 |
| 도메인 코드 | **없음** — 어댑터, IPC 라우터, 인스톨러, UI 컴포넌트 미구현 |
| `package.json` | `name: "my-app"` 등 템플릿 기본값 — 첫 도메인 PR 에서 `orca` 로 갱신 |

## 타깃 모듈 레이아웃 (TRD §1.2 그대로)

| 경로 | 책임 | 현 상태 |
|---|---|---|
| `src/main/index.ts` | Electron `app` 부트, BrowserWindow, IpcRouter 부착 | 현 `src/index.ts` 내용 일부를 옮겨야 함 |
...
| `src/renderer/index.tsx` | React 엔트리 | 미작성 (현재 `src/renderer.ts`) |
| `src/renderer/app/*` | `ChatShell`, `Composer`, `MessageList`, `Markdown`, `TweaksPanel` | 미작성 |
| `src/renderer/preload.ts` | `contextBridge.exposeInMainWorld('orca', ...)` 화이트리스트 | 현재 빈 스텁 |
...
`BrowserWindow` 생성 시 다음을 *명시*. 스캐폴드 기본값을 신뢰하지 말 것.
...
2. **위 모듈 레이아웃을 따르라.** 스캐폴드의 평면 구조(`src/index.ts`, `src/renderer.ts`)에 코드 누적 금지. `src/main/`, `src/renderer/`, `src/shared/` 로 분리한 뒤 진행.
3. **새 의존성 추가 시 사용자 확인.** TRD §2 표 밖이면 PR 설명에 사유 명시.
4. **Electron 보안 옵션은 항상 명시.** 기본값 의존 금지 (위 코드 블록).
5. **테스트 동반.** 어댑터 정규화, reducer, IPC 스키마는 단위 테스트와 함께 작성 (TRD §10).
6. **`package.json` 메타데이터는 템플릿 기본값이다.** 첫 도메인 PR 에서 `name`, `productName`, `description`, `author` 갱신.
