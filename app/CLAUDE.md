...
이 디렉토리는 **Orca v1 의 실제 구현체**가 사는 곳이다. 현재는 **Phase 3++ (로컬 SQLite SSOT + 사이드바 세션 히스토리 + DOM Architecture + 4-layer Feature 아키텍처)** 상태로, claude-code 어댑터 + 채팅 IPC + Composer 스킬 UX + `better-sqlite3` 기반 로컬 DB 영속화 + 사이드바 세션 관리 위에 **① `app-frame-*` 구조 클래스 + `data-*` 행동/상태 마커 체계, ② 4-layer 렌더러 아키텍처 (`app/` 셸 · `pages/` 조립 · `features/` 도메인 · `shared/` 공통), ③ ESLint boundaries v6 로 레이어 역방향·cross-feature import 회귀 차단** 이 누적 적용되었다. 본 구현은 `docs/TRD.md` 의 사양과 `docs/[docs/arch/frontend/layers.md](docs/arch/frontend/layers.md) 의 아키텍처 명세를 따른다. 그 위에 **에이전트 개발환경 표준화 리팩토링**(배포 A: `sources/dist` + `ExtensionDeployer` · 런타임 B: `NormalizedEvent` 단일 와이어 + `permission.requested` · 프론트 C: `ToolRendererRegistry` + `ApprovalCard` · 정리: 레거시 `ChatEvent`·`cli.*` 제거, PR #47)가 일부 구현되었다.
...
| 스타일링                          | **Tailwind CSS v4** (`@tailwindcss/vite` 플러그인, CSS-first `@theme` 설정) + **`app-frame-*` 마커 클래스 + `data-behavior` / `data-state` / `data-axis` / `data-context` / `data-platform` 속성 공존** ([docs/arch/frontend/dom-architecture.md](docs/arch/frontend/dom-architecture.md)) |
...
│   │   ├── components/ChatTile.tsx         # 채팅 tile — 헤더(panelR 토글) + transcript + <Composer /> + 우측 분할 <PlanTile />
│   │   ├── components/PlanTile.tsx         # 우측 계획 타일 — plan 마크다운 *내용 전용* (헤더 복사/닫기, ExitPlanMode 자동 오픈)
│   │   ├── components/ApprovalCard.tsx     # permission.requested 승인 게이트 — Composer 입력창 대체 (plan_review 구현 + tool_approval seam)
│   │   ├── components/AskUserQuestionCard.tsx # 명확화 질문 카드 — 질문 1개씩 + 좌우 화살표 네비게이션(1/N)
...
| `src/main/ipc/router.ts`                                  | IPC 채널 라우팅 + 입력 검증 (zod) + NormalizedEvent → DB persist (turn-local 상태 머신)| 구현됨 (Phase 3)                       |
| `src/main/adapters/claude-code.ts`                        | `@anthropic-ai/claude-agent-sdk` query() · SDKMessage → NormalizedEvent 정규화 (매핑은 `claude-map.ts` 의 `claudeToNormalized`, outbound 변환은 `claude-adapt.ts`) | 구현됨                                  |
| `src/main/adapters/claude-map.ts`                         | 순수 매퍼 `claudeToNormalized(SDKMessage, MapContext): NormalizedEvent[]` + `detectError`. SDK 메시지 → `NormalizedEvent` 직접 정규화 (구 `ChatEvent` 중간표현 제거) | 구현됨 (표준화 스테이지 B/정리)         |
| `src/main/adapters/claude-adapt.ts`                       | outbound 변환 헬퍼 (`adaptMcp`/`adaptSystemPrompt`/`adaptSkills`/`adaptHooks`). `adaptSkills` 는 `distDir('claude-code')` 플러그인 루트를 가리킴 | 구현됨 (정리)                           |
...
| `src/main/config/{paths,mcp-file,secret-store,migrate-sources}.ts` | **sources/dist 레이아웃 헬퍼** — `paths.ts` 는 *다른 모듈이 실제 참조하는* 6종만 export (`orcaConfigDir`/`sourcesDir`/`sourcesMcpDir`/`mcpJsonPath`/`distDir(engine)`/`ensureConfigDir`). **미사용 `sources*/dist*` 서브 게터는 제거됨 — 레이아웃 구성은 `deploy/deployer.ts`·`config/migrate-sources.ts` 가 root 기준 join 으로 담당** (paths.ts 주석과 일치). `mcp.json` atomic R/W (정규 소스, 이제 `sources/mcp/mcp.json`) + 비밀 저장소(env-var 이름 키잉, safeStorage) + **구 평면 레이아웃 → `sources/` 1회 이전**(`migrateConfigToSources`, 멱등, root 파라미터로 테스트 가능) | 구현됨 (표준화 §5.1) |
...
| `src/main/deploy/{deployer,conformance}.ts`              | **ExtensionDeployer** (`deploy(engine, {dryRun?})` — render(sources→engine 규약) → validate(MCP 키 이름) → dryRun?plan:backup-then-write; `dist/<engine>/.claude-plugin` 매니페스트 + skills/agents/commands/hooks **복사**; claude MCP 는 런타임 query 주입이라 검증만) + **StandardConformance** (엔진별 표준 적합도, claude 구체값). `ensureOrcaPlugin` 은 본 deployer 로 흡수(plugin-bundle.ts 삭제). claude 플러그인 루트 = `dist/claude-code/`(adaptSkills 가 가리킴) | 구현됨 (표준화 §5.2·5.3) |
...
**DOM 마커 체계 (Phase 3+ 도입).** 구조 식별을 위한 `app-frame-*` 클래스와 행동/상태 메타 (`data-behavior`, `data-state`, `data-axis`, `data-context`, `data-platform`) 는 **Tailwind 유틸과 공존하는 마커 전용**이다. 시각 스타일은 여전히 Tailwind 유틸이 진실. 새 컴포넌트 추가 시 가이드라인 (`docs/[docs/arch/frontend/dom-architecture.md](docs/arch/frontend/dom-architecture.md)) 의 트리 위치에 맞는 `app-frame-*` 클래스를 부여하고, 인터랙션이 있으면 `data-behavior` / `data-state` 도 함께 부여한다. 새 CSS 파일/규칙은 추가하지 않는다 — grid 1×1 z-stack 같은 레이아웃도 Tailwind arbitrary value 로 표현 (`grid-cols-1 grid-rows-1 [&>*]:[grid-area:1/1]`).
...
| **Phase 3+** | **DOM Architecture 일괄 적용** — `app-frame-*` 마커 클래스 + `data-behavior` / `data-state` / `data-axis` / `data-context` / `data-platform` 체계. 4종 신규 기능: ① Custom titlebar (`frame: false` + macOS `titleBarStyle: 'hidden'` + 윈도우 컨트롤 IPC 3개 + `orca.platform` sync 노출), ② Grid z-stack (`#app-frame-overlay` modal backdrop 전용 z 부호 반전 + `#app-frame-modal` focus-trap + `#app-frame-debug` floating UI 호스트 3슬롯), ③ Sidebar resize-handle (180–480px clamp, `Settings.sidebarWidth` 영속화), ④ Tile structure (`pane-host > pane-row > tile` 마크업). SSOT 는 `docs/[docs/arch/frontend/dom-architecture.md](docs/arch/frontend/dom-architecture.md). | **완료 (브랜치 `claude/charming-galileo-7lAqY`, 커밋 `45e129f` + 정정 `acf1295`)** |
...
| **Feature-based 구조 감사 + [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) 엄격 갱신 (PR #28)** | **완료 (PR #28)** — `frame/` 완전 해체 결정 (→ `app/` + `features/` + `shared/`), `pages/` = 조립만, `router` = which, App Shell §3.A 조립 규칙 신설, §3-2 목표 트리 + §10 구현 대기 행 추가. 코드 변경 없음 — 문서만. | **완료 (PR #28)** |
...
| **범용 Provider Runtime 재설계 (설계 + 인터페이스)** | **OpenCode/Claude 공통 정규화 계층을 기존 아키텍처 문서에 흡수.** 별도 문서 없이 `docs/[docs/arch/backend/provider-runtime.md](docs/arch/backend/provider-runtime.md) (정본 — NormalizedEvent · PermissionBridge · ApprovalResolution 2분기 · PendingApprovalStateMachine · AppCommandPolicy 3분기 · PermissionModeController · SessionCapability · RevertManager · ErrorClassifier · AppMessagePart persistence · Telemetry · AuthStore · AuditLog · ModelProviderConfig + 현행→목표 매핑표 + SDK 타입 확정 절차) + `docs/[docs/arch/frontend/rendering.md](docs/arch/frontend/rendering.md) §1.6~6.9·§7.6 (ToolRendererRegistry · ApprovalCard 일반화 · StructuredOutput · Streaming lifecycle · TelemetryPanel) 에 인터페이스 코드블럭으로 정의. **Claude 기준 + OpenCode 확장점만**, 각 항목 ①설명②예시③현재 코드 갭④인터페이스 4단. **코드 변경 0** (실구현 rename 범위 밖 — 매핑표로만). `[검증]`/`[미확인]` 태그 보존(두 SDK 미설치). | **설계 확정 / 구현 대기** |
...
| 후속    | CI 워크플로우 (`.github/workflows/`), Playwright 테스트, opencode 어댑터(변환기는 구현됨 — 어댑터/라이프사이클만 남음), `V1Captures` 실 구현 (캡처 RAW 보관 + AI 분석), 다국어 (`src/shared/i18n/ko.ts`), 세션 휴지통 30일 보존 (soft delete), 세션 메타 LRU 캐시 cap, 자동 제목 생성 (요약), `pages/RoutinesPage.tsx` placeholder + `/routines` 라우트 등록, **Phase 4 (Zustand 전환 + 멀티세션, [docs/arch/frontend/state.md](docs/arch/frontend/state.md) §1.4)**. | Future Scope                |
