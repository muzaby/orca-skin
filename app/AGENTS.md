...
| `src/main/contracts/`         | 공유 타입 계약 (**8**) — `turn`(`TurnContext`) · `bus-events` · `ports` · `session-state` + **진입점 2종**(`auth-method` = 인증 방식 3함수 · `internal-api` = 호출 표면) + `connector`(대상 실행) · `usage-report` · `usage-source`(0176). 0178 에서 `auth-plugin`·`connector-plugin`·`connector-template` 삭제 + 계약 동결·`apiVersion` ABI 정책 폐기 — 형태 강제는 `satisfies` 로 컴파일 타임에 한다 |
