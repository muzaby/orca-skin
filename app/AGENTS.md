...
- 비밀은 `safeStorage` 로만 봉인한다 — MCP 인증값 + provider vault(`infra/vault.ts`, 0181). raw secret 이 **나가는** 예외 **4곳**(MCP `.mcp.json` · LLM `--settings` argv · **LLM `Options.env`** · trusted stdio Plugin `Options.mcpServers.*.env`)은 `../docs/arch/backend/security.md §1.4-b` 의 경계표에 고정돼 있다 — 표 밖의 신규 노출 금지.
...
- `check-test-budgets.mjs` — **실-git 스위트의 파일 레벨 시간 예산**을 기계 강제 (CI 게이트 + `npm test`). 실제 git 을 띄우는 `*.test.ts` 는 최상위 `vi.setConfig({ testTimeout, hookTimeout })` 을 갖고, 케이스별 인라인 타임아웃을 두지 않으며(더 작은 값이 파일 예산을 조용히 이긴다), `runGit` 을 쓸 때는 프로덕션 기본 상한(10s)을 상속하지 않고 `temp-repo.testfixture` 의 `FIXTURE_GIT_TIMEOUT_MS` 를 넘기고(같은 "더 작은 캡이 이긴다" 가 프로세스 층에 있다 — git 속도는 호스트 몫이라 orca 가 정할 수 없다), 정리는 `infra/git/temp-repo.testfixture` 의 `removeTempRoots` 를 지나야 한다(끊긴 케이스가 남긴 고아 git 자식 때문에 `rm` 이 EBUSY/EPERM 으로 재실패하는 연쇄를 막는다). 순수 Node 라 ABI 무관.
