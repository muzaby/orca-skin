...
| 경로                          | 책임                                                                                          |
| ----------------------------- | --------------------------------------------------------------------------------------------- |
| `src/main/app/`               | 컴포지션 루트 — `bootstrap.ts`(부팅 배선·버스 구독 순서) · `chat-turn/`(턴 셋업 **13모듈** — 0179 에서 892줄 `handleChatSend` 를 단계로 분해. 진입점 `index.ts`(배럴) · 순서 `send.ts` · 순수 판정 `admission.ts` · 순수 조립 `turn-context.ts`·`continuation.ts` · `resolve-turn`·`runtime-entry`·`enqueue`·`turn-request`·`approval`·`post-turn`·`busy-reserve`·`turn-setup`·`deps`) + `chat-turn-continuation.ts`(자동 연속 턴, 0126) · `handlers/*`(도메인 IPC **14종** — auth·boot·cost·engine·files·log·mcp·misc·plugins·project·session·settings·skills·update) · `context.ts`(RouterContext) · `boot-report.ts`(부팅 진단, 0077) · `updater.ts`+`updater-feed.ts`(자동 업데이트 + object storage/GHE 피드, 0084~0086·0133) · `builtin-resources.ts`(번들 스킬 리소스 해석, 0078) · `auth-restore.ts`(재시작 후 인증 복원, 0170) · `usage-source.ts`(PluginHost→`UsageSourcePort` 어댑터, 0176) |
...
| `src/main/features/`          | 수직 슬라이스 (**11**) — `approvals` · `auth-platform`(인증 — 내장 방식 `methods/` + registry·transaction·레코드 스토어 + `login`/`api`/`broker` 3분해, 회사 대상은 `modules/` opt-in 레지스트리. 0178 에서 manifest·ABI·conformance 제거) · `chat`(턴 오케스트레이션) · `connectors`(인증된 내장 도구 실행 — raw credential 미접근, 호출은 `InternalApi.request({target})` 하나. UI 인스턴스 경로는 0178 에서 제거) · `extensions`(MCP·skill·deploy·seed) · `history`(persist) · `orchestration`(대화 연속성 fork/handoff, 순수 로직) · `providers`(정적 사용량 provider 는 `static/modules/` opt-in) · `scheduler`(croner 주기 실행, 0091) · `sessions`(런타임 거버넌스) · `usage` |
| `src/main/contracts/`         | 공유 타입 계약 (**8**) — `turn`(`TurnContext`) · `bus-events` · `ports` · `session-state` + **진입점 2종**(`auth-method` = 인증 방식이 채우는 3함수 · `internal-api` = **앱 안의 다른 모듈이 인증을 쓰는 API**) + `connector`(대상 실행) · `usage-report` · `usage-source`(0176). 0178 에서 `auth-plugin`·`connector-plugin`·`connector-template` 삭제 + 계약 동결·`apiVersion` ABI 정책 폐기 — 형태 강제는 `satisfies` 로 컴파일 타임에 한다 |
