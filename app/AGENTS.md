...
| `src/main/features/`          | 수직 슬라이스 (**11**) — `approvals` · `auth-platform`(인증 — provider registry·transaction·binding·broker, 회사 패키지는 `modules/` opt-in 레지스트리. 0178 에서 manifest·ABI·conformance 제거) · `chat`(턴 오케스트레이션) · `connectors`(인증된 내장 도구 실행 — raw credential 미접근. UI 인스턴스 경로는 0178 에서 제거) · `extensions`(MCP·skill·deploy·seed) · `history`(persist) · `orchestration`(대화 연속성 fork/handoff, 순수 로직) · `providers`(정적 사용량 provider 는 `static/modules/` opt-in) · `scheduler`(croner 주기 실행, 0091) · `sessions`(런타임 거버넌스) · `usage` |
| `src/main/contracts/`         | 공유 타입 계약 (**8**) — `turn`(`TurnContext`) · `bus-events` · `ports` · `session-state` + **확장점 3종**(`auth-plugin` · `connector-plugin` · `usage-report`) · `usage-source`(0176). 0178 에서 `connector-template` 삭제 + 계약 동결·`apiVersion` ABI 정책 폐기 — 형태 강제는 `satisfies` 로 컴파일 타임에 한다 |
