...
const ICONS: Record<IconName, string> = {
  ...
  sidebar:
    'M4 5.5h16a1.5 1.5 0 0 1 1.5 1.5v10a1.5 1.5 0 0 1-1.5 1.5H4a1.5 1.5 0 0 1-1.5-1.5V7A1.5 1.5 0 0 1 4 5.5zM10 5.5v13',
  ...
  fish:
    'M3 12c3-5 8-7 12-7 4 0 6 3 6 7s-2 7-6 7c-4 0-9-2-12-7zM6 12l-3-2v4l3-2zM16 11.5a.7.7 0 1 1 0 1.4.7.7 0 0 1 0-1.4z',
  dispatch:
    'M5 4h10l4 4v12a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1zM15 4v4h4M8 13h7M8 17h5M8 9h3',
  ...
  folderPlus:
    'M3 6a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6zM12 11v5M9.5 13.5h5',
  folderImp:
    'M3 6a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6zM12 16V9M9 12l3-3 3 3',
  ...
  screenshot:
    'M3 7a2 2 0 0 1 2-2h2l1.5-2h7L17 5h2a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7zM12 9a4 4 0 1 0 0 8 4 4 0 0 0 0-8z',
  ...
  rightPane:
    'M4 5.5h16a1.5 1.5 0 0 1 1.5 1.5v10a1.5 1.5 0 0 1-1.5 1.5H4a1.5 1.5 0 0 1-1.5-1.5V7A1.5 1.5 0 0 1 4 5.5zM14 5.5v13',
  ...
  cloudDown:
    'M7 18a4 4 0 1 1 .5-7.9A5 5 0 0 1 17 11a4 4 0 0 1-2 7.9M12 13v5M9.5 16l2.5 2.5L14.5 16',
  copy: 'M9 9h9v9H9zM6 6h9v3M6 6v9h3',
  thumbUp:
    'M6 11h2v10H6zM8 11l4-7a2 2 0 0 1 4 0v5h4a1 1 0 0 1 1 1l-2 8a2 2 0 0 1-2 1.5H8',
  thumbDown:
    'M6 13h2V3H6zM8 13l4 7a2 2 0 0 0 4 0v-5h4a1 1 0 0 0 1-1l-2-8a2 2 0 0 0-2-1.5H8',
  ...
  coffee:
    'M4 8h12v6a4 4 0 0 1-4 4H8a4 4 0 0 1-4-4V8zM16 9h2.5a2.5 2.5 0 0 1 0 5H16M4 4c0 1 1 1 1 2M9 4c0 1 1 1 1 2M14 4c0 1 1 1 1 2',
  ...
  question:
    'M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0zM9.5 9.5a2.5 2.5 0 1 1 3.5 2.3c-.9.4-1 1.2-1 2.2M12 17v.5',
...
export interface IconProps extends Omit<SVGProps<SVGSVGElement>, 'name' | 'color' | 'stroke'> {
  name: IconName
  size?: number
  color?: string
  stroke?: number
...
/** Claude-style 8-point asterisk used as the brand mark and as message avatar.
 *  Drawn as 4 ellipses rotated 45° apart. */
export function Sparkle({
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={style} aria-hidden="true">
      {[0, 45, 90, 135].map((deg) => (
        <ellipse key={deg} cx={c} cy={c} rx={rx} ry={ry} fill={color} transform={`rotate(${deg} ${c} ${c})`} />
      ))}
    </svg>
  )
}
