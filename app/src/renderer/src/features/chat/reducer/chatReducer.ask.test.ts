...
import type { AskQuestionRequest, ChatEvent } from '../../../../../shared/ipc'
...
describe('chatReducer — AskUserQuestion 큐', () => {
  it('ask_question 이벤트가 pendingAsks 에 push 된다', () => {
    const s = chatReducer(initialChatState, recv({ type: 'ask_question', data: REQ('r1') }))
    expect(s.pendingAsks).toHaveLength(1)
    expect(s.pendingAsks[0].requestId).toBe('r1')
  })

  it('연속 ask_question 은 큐에 누적', () => {
    let s = chatReducer(initialChatState, recv({ type: 'ask_question', data: REQ('r1') }))
    s = chatReducer(s, recv({ type: 'ask_question', data: REQ('r2') }))
  ...
  it('NEW_CHAT / CANCEL_CHAT / error 가 큐를 비운다', () => {
    const withAsk: ChatState = chatReducer(
      initialChatState,
      recv({ type: 'ask_question', data: REQ('r1') })
    )
