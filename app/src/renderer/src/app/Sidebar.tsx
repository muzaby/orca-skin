...
export interface SidebarProps {
  ...
  // 스킬·커넥터·플러그인 카탈로그는 route 화면이 아니라 현재 화면 위 모달로 연다.
    ...
    return (
      ...
      >
        ...
        <nav className="app-frame-sidebar-nav flex flex-col px-1.5 py-1">
          {NAV.map((it) => {
            const isActive = it.path === null ? extensionsOpen : it.isActive(pathname)
            return (
              <button
                key={it.path ?? 'extensions'}
                onClick={() => (it.path === null ? onOpenExtensions() : navigate(it.path))}
                aria-label={tr(it.l)}
  ...
  return (
    <>
      ...
      <div
        ...
        aria-label={tr('sidebar.resizeAria')}
      >
        <span
          aria-hidden
