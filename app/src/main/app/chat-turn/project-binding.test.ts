...
describe('first session project binding through real SQLite', () => {
  it('reuses a normalized path, keeps same names at different paths and exposes cwd in DTO', () => {
    ...
    expect(toProject(db.getProject(first)!)).toMatchObject({ cwd: 'C:\\A\\Demo' })
    db.updateProject(first, { name: 'Custom', instructions: 'Keep instructions' }, 10)
    expect(bindStartingProject(db, 'c:/a/demo', null, 'C:\\Desktop')).toBe(first)
    expect(db.getProject(first)).toMatchObject({
      name: 'Custom',
  ...
  it('keeps an explicit legacy project at its default and uses a new path after cwd selection', () => {
    ...
    expect(bindStartingProject(db, 'C:\\Desktop', 'legacy', 'C:\\Desktop')).toBe('legacy')
    const changed = bindStartingProject(db, 'C:\\Other', 'legacy', 'C:\\Desktop')
    expect(changed).not.toBe('legacy')
    expect(db.getProject('legacy')).toMatchObject({
      cwd: null,
      instructions: 'Original instructions'
    })
    expect(db.getProject(changed)).toMatchObject({ cwd: 'C:\\Other', instructions: '' })
  })
