...
// 통일한다. 어댑터가 자기 백엔드의 snake_case payload 를 OrcaHookContext 로 매핑하고, 핸들러가
// 반환한 OrcaHookDecision 들을 resolveHookDecisions() 로 1결정 병합한 뒤 자기 출력 형식으로
...
export interface OrcaHookContext {
  event: OrcaHookEvent
  sessionId: string
  cwd: string
...
export interface OrcaHookDecision {
...
export type OrcaHookHandler = (ctx: OrcaHookContext) => Promise<OrcaHookDecision> | OrcaHookDecision

export interface OrcaHookSet {
  ...
  backendSpecific?: { 'claude-code'?: unknown; opencode?: unknown }
}

// 결정 우선순위: deny > ask > allow (§6.3). 동률(미설정)은 0.
const DECISION_RANK: Record<NonNullable<OrcaHookDecision['decision']>, number> = {
  allow: 0,
  ask: 1,
  deny: 2
}
...
// 빈 배열 → {} (변경 없이 허용).
export function resolveHookDecisions(decisions: OrcaHookDecision[]): OrcaHookDecision {
  const merged: OrcaHookDecision = {}
  const contexts: string[] = []
  let decision: OrcaHookDecision['decision']
