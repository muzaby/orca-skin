...
const PROFILES: Readonly<Record<AgentKind, AgentProfile>> = {
  ...
  work: Object.freeze({
    ...
    instructions: [
      ...
      'Write final ordinary deliverables directly to the native absolute output directory provided below, using <output-directory>/<filename>. This is the user OS temporary folder (LocalAppData/Temp on Windows, typically /tmp on Linux). Keep intermediate scripts and working files in a subdirectory or the working directory, then move only the checked final deliverables to the output root.',
      "Use a distinct filename for each task in this shared output directory; do not overwrite another task's output.",
      'Composer attachments in the OS temporary folder are input sources. Keep their original files intact and write requested revisions or summaries as distinct final output files.',
