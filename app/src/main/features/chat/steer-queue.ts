...
function mergeBatch(items: SteerQueueItem[], uuid: string): SteerFlushBatch {
  return {
    uuid,
...
// 진행 중 세션에 붙는 피드백 staging 버퍼. text-only 이며, DB 영속은 커밋 시점에만 수행한다.
// 항목 수명(0060 D3·D4 — 로컬 홀드 + 게이트 flush):
//   [held]     enqueue 직후. stdin 미주입 — 취소·수정 100% 가능.
//   [flushed]  게이트 훅(flushHeld)에서 held 전체가 병합 단일 배치(batch uuid)로 stdin 주입됨.
//              취소 불가(un-push API 부재) — cancel 은 held 만 본다.
//   [consumed] CLI 가 배치를 흡수해 user echo 로 되돌린 상태(markConsumed) — drainConsumed 가
//              커밋 대상으로 회수한다. 미소비 잔여(held + 미echo flushed)는 턴을 넘어 살아남아
//              다음 chat:send 에서 이월(D2, drainForFlush).
export class SteerQueue {
  private readonly heldBySession = new Map<string, SteerQueueItem[]>()
  ...
  hasConsumed(sessionId: string): boolean {
    return (this.flushedBySession.get(sessionId) ?? []).some((batch) => batch.consumed)
  }
