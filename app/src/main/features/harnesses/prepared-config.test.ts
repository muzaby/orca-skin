...
import { prepareHarnessConfig, harnessConfigFingerprint } from './prepared-config'
...
describe('fingerprint (AC19)', () => {
  it('같은 입력은 같은 값 — 키 순서가 달라도 같다', () => {
    expect(harnessConfigFingerprint({ b: 1, a: 2 }, { Y: '1', X: '2' })).toBe(
      harnessConfigFingerprint({ a: 2, b: 1 }, { X: '2', Y: '1' })
    )
  })

  it('settings 가 같아도 env 의 token 이 바뀌면 값이 달라진다', () => {
    // 이것이 `providerSettingsChangedSinceSpawn` 만으로는 잡히지 않던 자리다.
    expect(harnessConfigFingerprint({ model: 'm' }, { TOKEN: 'a' })).not.toBe(
      harnessConfigFingerprint({ model: 'm' }, { TOKEN: 'b' })
    )
  })
  ...
  it('정상 steady state 는 같은 값을 낸다 — 상시 respawn 을 만들지 않는다', () => {
    ...
    expect(build().runtimeConfigFingerprint).toBe(build().runtimeConfigFingerprint)
  })
})
