// 사내 API 호출 (0178) — `contracts/internal-api.ts` 의 유일한 구현.
//
// 호출자가 아는 것은 **대상 이름 하나**다. 그 이름으로 인증 레코드를 찾고, 레코드 종류에 따라
// ⓐ vault 의 secret 을 presentation 대로 심거나 ⓑ 브라우저 세션의 cookie jar 로 보낸다.
// 여기 밖에서는 아무도 raw secret 을 만지지 않는다.
//
