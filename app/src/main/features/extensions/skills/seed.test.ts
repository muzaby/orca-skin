...
describe('seedBuiltinSkills', () => {
  it('최초 설치 시 manifest skill 을 sources/skills 로 복사하고 marker 를 기록한다', async () => {
    manifest('1.0.0', ['demo'])
  ...
  it('버전 업 시 builtin skill 을 덮어쓰고 새 manifest 에 없는 이전 builtin 을 prune 한다', async () => {
  ...
  it('동일 버전이면 어떤 파일도 쓰거나 삭제하지 않는다', async () => {
    manifest('1.0.0', ['demo'])
    builtinSkill('demo', '# bundled')
    await seedBuiltinSkills(builtinDir, skillsDir)
  ...
  it('manifest 가 부재하거나 손상되면 안전하게 no-op 한다', async () => {
    expect(await seedBuiltinSkills(builtinDir, skillsDir)).toEqual({
      seeded: [],
      pruned: [],
      skipped: true,
      version: null
  ...
  it('빈 builtin pipeline 은 marker 를 만들고 다음 동일 버전 호출에서 no-op 된다', async () => {
    manifest('1.0.0', [])
  ...
  it('소스 디렉토리가 없는 manifest skill 은 marker 에 관리 대상으로 기록하지 않는다', async () => {
