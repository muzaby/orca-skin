...
//   1) 파일 작업은 전용 툴(Read/Write/Edit)로 — shell 우회를 막는다. 전용 툴은
//      workspace 가드(adapters/workspace-guard.ts)가 실제로 경로 격리를 강제하므로, 파일 작업을
//      이쪽으로 몰면 격리가 자동 성립한다.
//   2) PowerShell은 경로 격리를 코드로 강제하지 않는다. 프롬프트로
//      "workspace 안으로 스코프하라"를 명시한다.
...
export const TOOLS_SECTION =
  ...
  'Reserve the PowerShell tool for operations that need a shell, such as builds, tests, package ' +
  'managers, and version control. Never use shell output to communicate with the ' +
  ...
  'Glob, Grep) are restricted to it and are denied for paths outside it. PowerShell is not path-restricted, ' +
...
export function buildSystemHeader(input: SystemHeaderInput): string {
  ...
  // PowerShell은 프롬프트로 workspace 스코프를 유도한다.
