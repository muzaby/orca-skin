// Confluence 서비스 런타임 (0160 → 0181 복원). `ctx.request` 를 fake 로 주입해 네트워크 없이
// 돌린다 — 런타임이 raw credential 을 안 보는 구조 덕에 fake 가 그대로 성립한다.
...
describe('ConfluenceConnector — start', () => {
  it('start 가 상태 코드별 health 와 message 를 낸다', async () => {
    const connector = createConfluenceRuntime(SERVER)
  ...
  it('컨텍스트 경로가 요청 path 에 붙는다', async () => {
    const connector = createConfluenceRuntime({ ...SERVER, apiBasePath: '/confluence' })
...
function ok<T>(result: ConfluenceResult): T {
...
async function search(
  ...
  connector: ReturnType<typeof createConfluenceRuntime>,
...
describe('ConfluenceConnector — search', () => {
  ...
  it('totalSize 가 없으면 한도를 채웠는지로 다음을 판단한다', async () => {
    ...
    expect(
      (await search(full.ctx, createConfluenceRuntime(SERVER), { text: 'x', limit: 2 })).nextStart
    ).toBe(2)
