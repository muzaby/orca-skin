...
export class InteractionBroker<T> {
  ...
  // (턴 취소 등) abortValue 로 resolve 하고 정리해 query() 가 멈출 수 있게 한다.
  register(requestId: string, signal: AbortSignal | undefined, abortValue: T): Promise<T> {
    return new Promise<T>((resolve) => {
      ...
      const onAbort = (): void => this.resolve(requestId, abortValue)
