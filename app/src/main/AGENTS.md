...
| **contracts**           | `src/main/contracts/` 9모듈 (`turn`·`bus-events`·`ports`·`session-state` + **동결 확장점** `auth-plugin`·`connector-plugin`·`connector-template`·`usage-report` + `usage-source`)                                                                                                            | 여러 feature 가 공유하는 **턴/버스/런타임 타입 계약**. `TurnContext`·`OrcaBusEvents`·`RuntimeLiveTurn` 등. 구현 최소. **동결 4종은 폐쇄망 확장점이라 `additive-optional-only`** — optional 멤버 *추가*만 허용하고 제거·개명·required 화는 금지한다(파괴적 변경은 `<name>-v2.ts` 병행 신설, `../../../docs/guides/closed-network-extensions.md` §1).                                                                                                                                                                         | contracts · adapters · infra · shared                  |
| **features**            | `src/main/features/<slice>/` (`chat`·`sessions`·`approvals`·`usage`·`history`·`providers`·`extensions`·`orchestration`·`scheduler`·`auth-platform`·`connectors`)                                      | 수직 슬라이스 — 턴 오케스트레이션·세션 런타임 거버넌스·승인·사용량·영속·provider(정적 사용량 provider 는 `features/providers/static/modules/` opt-in 레지스트리)·확장(MCP·skill·deploy·번들 시딩)·대화 연속성(fork/handoff)·주기 실행(croner, 0091)·인증 플랫폼(provider registry·transaction·binding·broker, 회사 패키지는 `features/auth-platform/modules/` opt-in 레지스트리, 0157)·connector 실행(인증된 내장 도구 — raw credential 미접근).                                          | **같은 slice** · contracts · adapters · infra · shared |
...
- **구체 provider/engine 리터럴**(`'claude'` 등)은 `adapters`·`features/extensions`(배포 레지스트리)·`features/providers/static/modules/`(정적 사용량 provider opt-in 레지스트리)·`features/auth-platform/{providers,modules}/`(인증 provider 구현 + opt-in 패키지 레지스트리, 0157)·컴포지션 루트(`app/bootstrap.ts`·`index.ts`) 안에만. 코어·오케스트레이션은 백엔드 중립(handoff 0016).
