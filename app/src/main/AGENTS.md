...
| **infra**               | `src/main/infra/` (`bus`·`db`·`config`·`auth`·`ipc`·`errors`·`vars`·`settings-store`·`settings-migration`·`cron`)                                                                     | DB 싱글턴·TypedBus·orca.json/secret·IPC 프리미티브(`ipc/handle`·`ipc/send`·`ipc/dto`)·에러 정규화·croner 래퍼(`cron.ts`)·인증 인프라(`auth/` — credential vault·browser session store·authenticated fetch·plugin exec·**`net-fetch.ts`**). feature/어댑터 비의존.                                                                                                                                              | infra · shared                                         |
