...
| **L1 domain/infra** | `src/main/{db,config,settings,usage,cost,lifecycle,orchestration,runtime,mcp,runtime-errors,runtime-events,capabilities,extensions,deploy,skills,ask,files,title}` | DB·설정·MCP·런타임·라이프사이클·오케스트레이션·정규화 조각 등. 어댑터/IPC 비의존. | domain(동일 레이어) · shared |
