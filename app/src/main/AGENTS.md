...
- 소비자는 `typeof fetch` 포트로 **주입받는다**(`createSender(fetchImpl)` ·
  `ExternalUsageService.fetchImpl` · `ProviderApiImpl.fetchImpl`). **기본값을 두지 않는다** —
