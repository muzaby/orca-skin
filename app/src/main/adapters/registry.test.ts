...
describe('AdapterRegistry capability 서술', () => {
  it('describeActive() 는 활성 백엔드(claude-code)의 서술자를 반환', () => {
    const reg = makeRegistry()
    const desc = reg.describeActive()
    expect(desc).not.toBeNull()
    expect(desc?.provider).toBe('claude-code')
  })

  it('describeAll() 은 등록된 모든 백엔드의 서술자를 키로 보유', () => {
    const reg = makeRegistry()
    const all = reg.describeAll()
    expect(all['claude-code'].provider).toBe('claude-code')
    expect(all['claude-code'].cancellation.sessionAbort).toBe(true)
  })
})
