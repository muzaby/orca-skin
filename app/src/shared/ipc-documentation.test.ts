...
describe('IPC contract documentation', () => {
  it('keeps the header, domain summary, and CHANNELS count at 77', () => {
    ...
    expect(header?.[1]).toBe('77')
    expect(summary?.[2]).toBe('77')

    const domainTotal = [...(summary?.[1] ?? '').matchAll(/`[^`]+` (\d+)/g)].reduce(
      (total, match) => total + Number(match[1]),
      0
    )
    expect(domainTotal).toBe(77)
    expect(Object.values(CHANNELS)).toHaveLength(77)
  })
})
