...
describe('IPC contract documentation', () => {
  it('keeps the header, domain summary, and CHANNELS count at 76', () => {
    ...
    expect(header?.[1]).toBe('76')
    expect(summary?.[2]).toBe('76')

    const domainTotal = [...(summary?.[1] ?? '').matchAll(/`[^`]+` (\d+)/g)].reduce(
      (total, match) => total + Number(match[1]),
      0
    )
    expect(domainTotal).toBe(76)
    expect(Object.values(CHANNELS)).toHaveLength(76)
  })
})
